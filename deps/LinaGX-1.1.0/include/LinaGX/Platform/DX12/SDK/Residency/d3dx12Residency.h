...
#define MAX_NUM_CONCURRENT_CMD_LISTS 32
	...
	namespace Internal
	{
		...
		public:
			...
			{
				if (pCS) { EnterCriticalSection(&pCS->CS); }
			};

			~ScopedLock()
			{
				if (pCS) { LeaveCriticalSection(&pCS->CS); }
			}
	...
	public:
		...
		ManagedObject() : pUnderlying(nullptr), Size(0), ResidencyStatus(RESIDENCY_STATUS::RESIDENT), LastGPUSyncPoint(0), LastUsedTimestamp(0)
		{
	...
	public:
		...
		ResidencySet() : CommandListIndex(InvalidIndex), MaxResidencySetSize(0), CurrentSetSize(0), ppSet(nullptr), IsOpen(false), OutOfMemory(false), pSyncManager(nullptr){};
		...
		inline bool Insert(ManagedObject* pObject)
		{
			...
			if (pObject->CommandListsUsedOn[CommandListIndex] == false)
			{
				...
				if (ppSet == nullptr || CurrentSetSize >= MaxResidencySetSize) { Realloc(); }
				if (ppSet == nullptr)
				{
					OutOfMemory = true;
					return false;
		...
		HRESULT Close()
		{
			if (IsOpen == false) { return E_INVALIDARG; }

			if (OutOfMemory == true) { return E_OUTOFMEMORY; }
	...
	namespace Internal
	{
		...
		{
			DeviceWideSyncPoint(UINT32 NumQueues, UINT64 Generation) : GenerationID(Generation), NumQueueSyncPoints(NumQueues){};
			...
			static DeviceWideSyncPoint* CreateSyncPoint(UINT32 NumQueues, UINT64 Generation)
			{
				...
				if (pAlloc && Size >= sizeof(DeviceWideSyncPoint)) { pSyncPoint = new (pAlloc) DeviceWideSyncPoint(NumQueues, Generation); }
			...
			inline bool IsCompleted()
			{
				...
				{
					if (pQueueSyncPoints[i].IsCompleted() == false) { return false; }
				}
				return true;
			}
			...
			inline void WaitForCompletion(HANDLE Event)
			{
				for (UINT32 i = 0; i < NumQueueSyncPoints; i++)
				{
					if (pQueueSyncPoints[i].IsCompleted() == false) { pQueueSyncPoints[i].WaitForCompletion(Event); }
				}
			}
		...
		{
		public:
			LRUCache() : NumResidentObjects(0), NumEvictedObjects(0), ResidentSize(0)
			{
			...
			void TrimToSyncPointInclusive(INT64 CurrentUsage, INT64 CurrentBudget, ID3D12Pageable** EvictionList, UINT32& NumObjectsToEvict, UINT64 SyncPoint)
			{
				...
				while (pResourceEntry != &ResidentObjectListHead)
				{
					...
					if (pObject->LastGPUSyncPoint > SyncPoint || CurrentUsage < CurrentBudget) { break; }
			...
			{
				if (IsListEmpty(&ResidentObjectListHead)) { return nullptr; }
		...
		public:
			ResidencyManagerInternal(SyncManager* pSyncManagerIn)
				: Device(nullptr), Device3(nullptr),
#ifdef __ID3D12DeviceDownlevel_INTERFACE_DEFINED__
				  DeviceDownlevel(nullptr),
#endif
				  AsyncThreadFence(0), CompletionEvent(INVALID_HANDLE_VALUE), AsyncThreadWorkCompletionEvent(INVALID_HANDLE_VALUE), Adapter(nullptr), AsyncWorkEvent(INVALID_HANDLE_VALUE), AsyncWorkThread(INVALID_HANDLE_VALUE), FinishAsyncWork(false),
				  cStartEvicted(false), CurrentSyncPointGeneration(0), NumQueuesSeen(0), NodeIndex(0), CurrentAsyncWorkloadHead(0), CurrentAsyncWorkloadTail(0), cMinEvictionGracePeriod(1.0f), cMaxEvictionGracePeriod(60.0f),
				  cTrimPercentageMemoryUsageThreshold(0.7f), AsyncWorkQueue(nullptr), MaxSoftwareQueueLatency(6), AsyncWorkQueueSize(7), pSyncManager(pSyncManagerIn)
			{
			...
			HRESULT Initialize(ID3D12Device* ParentDevice, UINT DeviceNodeIndex, IDXGIAdapter* ParentAdapter, UINT32 MaxLatency)
			{
				...
				if (ParentAdapter) { ParentAdapter->QueryInterface(&Adapter); }

				AsyncWorkQueueSize = MaxLatency + 1;
				AsyncWorkQueue	   = new AsyncWorkload[AsyncWorkQueueSize];

				if (AsyncWorkQueue == nullptr) { return E_OUTOFMEMORY; }
				...
				if (SUCCEEDED(hr))
				{
					...
					if (CompletionEvent == INVALID_HANDLE_VALUE) { hr = HRESULT_FROM_WIN32(GetLastError()); }
				}

				if (SUCCEEDED(hr))
				{
					AsyncThreadWorkCompletionEvent = CreateEvent(nullptr, false, false, nullptr);
					if (AsyncThreadWorkCompletionEvent == INVALID_HANDLE_VALUE) { hr = HRESULT_FROM_WIN32(GetLastError()); }
				}

				if (SUCCEEDED(hr))
				{
					AsyncWorkEvent = CreateEvent(nullptr, true, false, nullptr);
					if (AsyncWorkEvent == INVALID_HANDLE_VALUE) { hr = HRESULT_FROM_WIN32(GetLastError()); }
				}
				...
				if (SUCCEEDED(hr) && !Device3)
				{
					...
					if (AsyncWorkThread == INVALID_HANDLE_VALUE) { hr = HRESULT_FROM_WIN32(GetLastError()); }
				}
#endif
			...
			void Destroy()
			{
				...
				if (SetEvent(AsyncWorkEvent) == false) { RESIDENCY_CHECK_RESULT(HRESULT_FROM_WIN32(GetLastError())); }
				...
				{
					Internal::Fence* pObject = CONTAINING_RECORD(QueueFencesListHead.Flink, Internal::Fence, ListEntry);
				...
				{
					Internal::DeviceWideSyncPoint* pPoint = CONTAINING_RECORD(InFlightSyncPointsHead.Flink, Internal::DeviceWideSyncPoint, ListEntry);
		...
		private:
			...
			HRESULT ExecuteSubset(ID3D12CommandQueue* Queue, ID3D12CommandList** CommandLists, ResidencySet** ResidencySets, UINT32 Count)
			{
				...
				if (pMasterSet == nullptr || pMasterSet->Initialize(pSyncManager, MaxObjectsReferenced) == false) { return E_OUTOFMEMORY; }
				...
				for (UINT32 i = 0; i < Count; i++)
				{
					if (ResidencySets[i])
					{
						...
						{
							if (pMasterSet->Insert(ResidencySets[i]->ppSet[x])) { TotalSizeNeeded += ResidencySets[i]->ppSet[x]->Size; }
						}
					}
				}
				...
				if (SUCCEEDED(hr))
				{
					...
					if (SUCCEEDED(hr))
					{
						...
						if (!Device3) { AsyncThreadFence.Increment(); }
					}

					Queue->ExecuteCommandStreams(Count, CommandLists);

					if (SUCCEEDED(hr)) { hr = SignalFence(Queue, QueueFence); }
				}
				return hr;
			}
			...
			{
				AsyncWorkload() : pMasterSet(nullptr), FenceValueToSignal(0), SyncPointGeneration(0)
				{
				}
			...
			static unsigned long WINAPI AsyncThreadStart(void* pData)
			{
				...
				while (1)
				{
					...
					while (pWork)
					{
						...
						if (SetEvent(pManager->AsyncThreadWorkCompletionEvent) == false) { RESIDENCY_CHECK_RESULT(HRESULT_FROM_WIN32(GetLastError())); }
					...
					if (ResetEvent(pManager->AsyncWorkEvent) == false) { RESIDENCY_CHECK_RESULT(HRESULT_FROM_WIN32(GetLastError())); }

					if (pManager->FinishAsyncWork) { return 0; }
				}
					...
					if (NumObjectsToMakeResident)
					{
						...
						while (true)
						{
							...
							if (AvailableSpace > 0)
							{
								...
								if (Device3)
								{
									hr = Device3->EnqueueMakeResident(D3D12_RESIDENCY_FLAG_NONE, NumObjectsInBatch, &pMakeResidentList[BatchStart].pUnderlying, AsyncThreadFence.pFence, AsyncThreadFence.FenceValue + 1);
									if (SUCCEEDED(hr)) { AsyncThreadFence.Increment(); }
								}
								else { hr = Device->MakeResident(NumObjectsInBatch, &pMakeResidentList[BatchStart].pUnderlying); }
								if (SUCCEEDED(hr)) { SizeToMakeResident -= BatchSize; }
							}
							...
							if (FAILED(hr) || ObjectsMadeResident != NumObjectsToMakeResident)
							{
								...
								if (pResidentHead == nullptr || pResidentHead->LastGPUSyncPoint >= pWork->SyncPointGeneration || FirstUncompletedSyncPoint == nullptr)
								{
									...
									if (Device3)
									{
										hr = Device3->EnqueueMakeResident(D3D12_RESIDENCY_FLAG_NONE, NumObjects, &pMakeResidentList[MakeResidentIndex].pUnderlying, AsyncThreadFence.pFence, AsyncThreadFence.FenceValue + 1);
										if (SUCCEEDED(hr)) { AsyncThreadFence.Increment(); }
									}
									else { hr = Device->MakeResident(NumObjects, &pMakeResidentList[MakeResidentIndex].pUnderlying); }
									if (FAILED(hr))
									{
			...
			HRESULT EnqueueAsyncWork(ResidencySet* pMasterSet, UINT64 FenceValueToSignal, UINT64 SyncPointGeneration)
			{
				...
				if (SetEvent(AsyncWorkEvent) == false) { return HRESULT_FROM_WIN32(GetLastError()); }
			...
			{
				if (CurrentAsyncWorkloadHead == CurrentAsyncWorkloadTail) { return nullptr; }
			...
			{
				if (Adapter) { RESIDENCY_CHECK_RESULT(Adapter->QueryVideoMemoryInfo(NodeIndex, Segment, InfoOut)); }
#ifdef __ID3D12DeviceDownlevel_INTERFACE_DEFINED__
				else if (DeviceDownlevel) { RESIDENCY_CHECK_RESULT(DeviceDownlevel->QueryVideoMemoryInfo(NodeIndex, Segment, InfoOut)); }
#endif
			}
			...
			HRESULT EnqueueSyncPoint()
			{
				...
				if (pPoint == nullptr) { return E_OUTOFMEMORY; }
	...
	{
	public:
		ResidencyManager() : Manager(&SyncManager)
		{
		}
		...
		FORCEINLINE ResidencySet* CreateResidencySet()
		{
			...
			if (pSet) { pSet->Initialize(&SyncManager); }
			return pSet;
		}
