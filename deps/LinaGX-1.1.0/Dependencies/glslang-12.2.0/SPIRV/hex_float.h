...
namespace spvutils
{
	...
	// a value is Nan.
	template <typename T> struct FloatProxyTraits
	{
	...
	public:
		...
		{
			return static_cast<uint_type>(data_ ^ (uint_type(0x1) << (sizeof(T) * 8 - 1)));
		}
	...
	public:
		...
		static const uint_type fraction_represent_mask = spvutils::SetBits<uint_type, 0, num_fraction_bits + num_overflow_bits>::get;
		...
		const int_type getUnbiasedNormalizedExponent() const
		{
			...
			{ // special case if everything is 0
				return 0;
			}
		...
		void setFromSignUnbiasedExponentAndNormalizedSignificand(bool negative, int_type exponent, uint_type significand, bool round_denorm_up)
		{
			...
			if (exponent == min_exponent)
			{
				if (significand == 0 && !significand_is_zero && round_denorm_up) { significand = static_cast<uint_type>(0x1); }
			}

			uint_type new_value = 0;
			if (negative) { new_value = static_cast<uint_type>(new_value | sign_mask); }
		...
		template <typename other_T> typename other_T::uint_type getRoundedNormalizedSignificand(round_direction dir, bool* carry_bit)
		{
			typedef typename other_T::uint_type other_uint_type;
			static const int_type				num_throwaway_bits = static_cast<int_type>(num_fraction_bits) - static_cast<int_type>(other_T::num_fraction_bits);

			static const uint_type last_significant_bit = (num_throwaway_bits < 0) ? 0 : negatable_left_shift(num_throwaway_bits, 1u);
			static const uint_type first_rounded_bit	= (num_throwaway_bits < 1) ? 0 : negatable_left_shift(num_throwaway_bits - 1, 1u);
			...
			*carry_bit					= false;
			...
			// do.
			if ((significand & throwaway_mask) == 0) { return static_cast<other_uint_type>(negatable_right_shift(num_throwaway_bits, significand)); }
			...
			case kRoundToNearestEven:
				...
				if ((first_rounded_bit & significand) == 0) { break; }
			...
			if (round_away_from_zero) { return static_cast<other_uint_type>(negatable_right_shift(num_throwaway_bits, incrementSignificand(significand, last_significant_bit, carry_bit))); }
			else { return static_cast<other_uint_type>(negatable_right_shift(num_throwaway_bits, significand)); }
		}
		...
		template <typename other_T> void castTo(other_T& other, round_direction round_dir)
		{
			...
			{
				if (negate) { other.set_value(-other.value()); }
				return;
			}
			uint_type					significand			= getSignificandBits();
			bool						carried				= false;
			...
			if (exponent == min_exponent)
			{
				...
				for (uint_type check_bit = first_exponent_bit >> 1; check_bit != 0; check_bit = static_cast<uint_type>(check_bit >> 1))
				{
					exponent = static_cast<int_type>(exponent - 1);
			...
			bool is_inf = !is_nan && ((exponent + carried) > static_cast<int_type>(other_T::exponent_bias) || (significand == 0 && (getBits() & exponent_mask) == exponent_mask));

			// If we are Nan or Inf we should pass that through.
			if (is_inf)
			{
				other.set_value(BitwiseCast<typename other_T::underlying_type>(static_cast<typename other_T::uint_type>((negate ? other_T::sign_mask : 0) | other_T::exponent_mask)));
				return;
			}
			if (is_nan)
			{
				typename other_T::uint_type shifted_significand;
				shifted_significand = static_cast<typename other_T::uint_type>(negatable_left_shift(static_cast<int_type>(other_T::num_fraction_bits) - static_cast<int_type>(num_fraction_bits), significand));
				...
				other.set_value(BitwiseCast<typename other_T::underlying_type>(static_cast<typename other_T::uint_type>((negate ? other_T::sign_mask : 0) | other_T::exponent_mask | (shifted_significand == 0 ? 0x1 : shifted_significand))));
				return;
			}

			bool							   round_underflow_up = isNegative() ? round_dir == kRoundToNegativeInfinity : round_dir == kRoundToPositiveInfinity;
			...
			other.setFromSignUnbiasedExponentAndNormalizedSignificand(negate, static_cast<other_int_type>(exponent), rounded_significand, round_underflow_up);
			return;
		}
	...
	private:
		...
		static_assert(num_used_bits == Traits::num_exponent_bits + Traits::num_fraction_bits + 1, "The number of bits do not fit");
	...
	inline uint8_t get_nibble_from_character(int character)
	{
		...
		if ((p = strchr(dec, character))) { return static_cast<uint8_t>(p - dec); }
		else if ((p = strchr(lower, character))) { return static_cast<uint8_t>(p - lower + 0xa); }
		else if ((p = strchr(upper, character))) { return static_cast<uint8_t>(p - upper + 0xa); }
	...
	template <typename T, typename Traits> inline bool RejectParseDueToLeadingSign(std::istream& is, bool negate_value, HexFloat<T, Traits>& value)
	{
		if (negate_value)
		{
	...
	template <typename T, typename Traits> inline std::istream& ParseNormalFloat(std::istream& is, bool negate_value, HexFloat<T, Traits>& value)
	{
		if (RejectParseDueToLeadingSign(is, negate_value, value)) { return is; }
		T val;
		is >> val;
		if (negate_value) { val = -val; }
		value.set_value(val);
		// In the failure case, map -0.0 to 0.0.
		if (is.fail() && value.getUnsignedBits() == 0u) { value = HexFloat<T, Traits>(typename HexFloat<T, Traits>::uint_type(0)); }
		if (val.isInfinity())
		{
			// Fail the parse.  Emulate standard behaviour by setting the value to
	...
	// the error case, but only after all target platforms implement it correctly.
	// In particular, the Microsoft C++ runtime appears to be out of spec.
	template <> inline std::istream& ParseNormalFloat<FloatProxy<Float16>, HexFloatTraits<FloatProxy<Float16>>>(std::istream& is, bool negate_value, HexFloat<FloatProxy<Float16>, HexFloatTraits<FloatProxy<Float16>>>& value)
	{
	...
	template <typename T, typename Traits> std::istream& operator>>(std::istream& is, HexFloat<T, Traits>& value)
	{
		...
		if (next_char != '-' && next_char != '0') { return ParseNormalFloat(is, negate_value, value); }
		...
		else { return ParseNormalFloat(is, negate_value, value); }
		...
		while (!seen_p && !seen_dot)
		{
			...
			else if (next_char == 'p') { seen_p = true; }
			else if (::isxdigit(next_char))
			{
				...
				for (int i = 0; i < 4; ++i, number <<= 1)
				{
					...
					if (bits_written)
					{
						...
						fraction = static_cast<uint_type>(fraction | static_cast<uint_type>(write_bit << (HF::top_bit_left_shift - fraction_index++)));
						exponent = static_cast<int_type>(exponent + 1);
					}
		...
		while (seen_dot && !seen_p)
		{
			...
			else if (::isxdigit(next_char))
			{
				...
				for (int i = 0; i < 4; ++i, number <<= 1)
				{
					...
					else { fraction = static_cast<uint_type>(fraction | static_cast<uint_type>(write_bit << (HF::top_bit_left_shift - fraction_index++))); }
				}
			}
			else
			{
		...
		uint_type output_bits = static_cast<uint_type>(static_cast<uint_type>(negate_value ? 1 : 0) << HF::top_bit_left_shift);
		output_bits |= fraction;

		uint_type shifted_exponent = static_cast<uint_type>(static_cast<uint_type>(exponent << HF::exponent_left_shift) & HF::exponent_mask);
