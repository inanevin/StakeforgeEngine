...
{
	return ((alignment & PIXEventsStringAlignmentWriteMask) << PIXEventsStringAlignmentBitShift) | ((copyChunkSize & PIXEventsStringCopyChunkSizeWriteMask) << PIXEventsStringCopyChunkSizeBitShift) |
		   (((UINT64)isANSI & PIXEventsStringIsANSIWriteMask) << PIXEventsStringIsANSIBitShift) | (((UINT64)isShortcut & PIXEventsStringIsShortcutWriteMask) << PIXEventsStringIsShortcutBitShift);
}
...
// this is due to printf not ignoring correctly the upper bits of unsigned long long for a char format specifier
template <> inline void PIXCopyEventArgument<unsigned char>(_Out_writes_to_ptr_(limit) UINT64*& destination, _In_ const UINT64* limit, unsigned char argument)
{
	if (destination < limit)
	{
		*destination = static_cast<UINT64>(argument);
		...
		*destination++ = PIXEncodeStringInfo(0, 8, TRUE, FALSE);
		...
		while (destination < limit)
		{
			...
			*destination++ = qword;
			...
			if (!((qword & 0xFF00000000000000) && (qword & 0xFF000000000000) && (qword & 0xFF0000000000) && (qword & 0xFF00000000) && (qword & 0xFF000000) && (qword & 0xFF0000) && (qword & 0xFF00) && (qword & 0xFF))) { break; }
		}
	}
	else
				...
				*destination++ = PIXEncodeStringInfo(0, 16, TRUE, FALSE);
				...
				if (PIXIsPointerAligned<16>(destination))
				{
					while (destination < limit)
					{
						...
						destination += 2;
						if (_mm_movemask_epi8(res)) break;
						argument += 16;
					}
				}
				else
		...
		*destination++ = PIXEncodeStringInfo(0, 8, FALSE, FALSE);
		...
		while (destination < limit)
		{
			...
			*destination++ = qword;
			...
			if (!((qword & 0xFFFF000000000000) && (qword & 0xFFFF00000000) && (qword & 0xFFFF0000) && (qword & 0xFFFF))) { break; }
		}
	}
	else
#endif // PIX_ENABLE_BLOCK_ARGUMENT_COPY
