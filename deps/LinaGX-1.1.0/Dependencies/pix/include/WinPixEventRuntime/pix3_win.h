...
#define PIXERRORCHECK(value)                                                                                                                                                                                                                                       \
	do                                                                                                                                                                                                                                                             \
	{                                                                                                                                                                                                                                                              \
		if (FAILED(value)) return nullptr;                                                                                                                                                                                                                         \
	} while (0)
...
namespace PixImpl
{
	...
	__forceinline HMODULE PIXLoadLatestCapturerLibrary(wchar_t const* capturerDllName, DWORD flags)
	{
		...
		if (PixImpl::GetModuleHandleExW(0, capturerDllName, &libHandle)) { return libHandle; }
...
__forceinline HRESULT WINAPI PIXSetTargetWindow(HWND hwnd)
{
	...
	if (fn == nullptr) { return HRESULT_FROM_WIN32(GetLastError()); }
...
extern "C" __forceinline HRESULT WINAPI PIXEndCapture(BOOL discard)
{
	...
	if (timingFn != NULL) { return timingFn(discard); }
...
__forceinline HRESULT WINAPI PIXForceD3D11On12()
{
	...
	if (fn == NULL) { return HRESULT_FROM_WIN32(GetLastError()); }
