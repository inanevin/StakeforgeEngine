...
namespace SPIRV_CROSS_NAMESPACE
{
	...
	protected:
		...
		{
			if (variable_remap_callback) variable_remap_callback(type, var_name, type_name);
		}
		...
		{
			BufferAccessHandler(const Compiler& compiler_, SmallVector<BufferRange>& ranges_, uint32_t id_) : compiler(compiler_), ranges(ranges_), id(id_)
			{
			}
		...
		{
			InterfaceVariableAccessHandler(const Compiler& compiler_, std::unordered_set<VariableID>& variables_) : compiler(compiler_), variables(variables_)
			{
			}
		...
		{
			DummySamplerForCombinedImageHandler(Compiler& compiler_) : compiler(compiler_)
			{
			}
			bool handle(spv::Op opcode, const uint32_t* args, uint32_t length) override;
		...
		{
			CombinedImageSamplerDrefHandler(Compiler& compiler_) : compiler(compiler_)
			{
			}
			bool handle(spv::Op opcode, const uint32_t* args, uint32_t length) override;
		...
		{
			InterlockedResourceAccessHandler(Compiler& compiler_, uint32_t entry_point_id) : compiler(compiler_)
			{
		...
		{
			InterlockedResourceAccessPrepassHandler(Compiler& compiler_, uint32_t entry_point_id) : compiler(compiler_)
			{
				call_stack.push_back(entry_point_id);
			}
