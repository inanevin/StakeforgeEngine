...
int main(int argc, char** argv)
{
	...
	if (buffer.empty()) return EXIT_FAILURE;
	...
	if (spvc_compiler_msl_is_resource_used(compiler, SpvExecutionModelFragment, 2, 4)) return EXIT_FAILURE;

	// Should not be marked as used.
	if (spvc_compiler_msl_is_resource_used(compiler, SpvExecutionModelFragment, 2, 5)) return EXIT_FAILURE;

	// Should be marked, as a sanity check.
	if (!spvc_compiler_msl_is_resource_used(compiler, SpvExecutionModelFragment, 1, 2)) return EXIT_FAILURE;
	if (!spvc_compiler_msl_is_resource_used(compiler, SpvExecutionModelFragment, 1, 3)) return EXIT_FAILURE;
	if (!spvc_compiler_msl_is_resource_used(compiler, SpvExecutionModelFragment, 2, 2)) return EXIT_FAILURE;
	if (!spvc_compiler_msl_is_resource_used(compiler, SpvExecutionModelFragment, 2, 3)) return EXIT_FAILURE;
