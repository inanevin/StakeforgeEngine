...
static void dump_resource_list(spvc_compiler compiler, spvc_resources resources, spvc_resource_type type, const char* tag)
{
	...
	for (i = 0; i < count; i++)
	{
		...
		printf("  Set: %u, Binding: %u\n", spvc_compiler_get_decoration(compiler, list[i].id, SpvDecorationDescriptorSet), spvc_compiler_get_decoration(compiler, list[i].id, SpvDecorationBinding));
	}
}
...
int main(int argc, char** argv)
{
	...
	if (read_file(argv[1], &buffer, &word_count) < 0) return 1;
...
#define NUM_EXTS 2
	const char* expected_exts[NUM_EXTS] = {"EXT_first", "EXT_second"};
