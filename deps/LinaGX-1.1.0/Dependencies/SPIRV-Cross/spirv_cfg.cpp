...
namespace SPIRV_CROSS_NAMESPACE
{
	...
	bool CFG::post_order_visit(uint32_t block_id)
	{
		...
		if (block.merge == SPIRBlock::MergeLoop && post_order_visit(block.merge_block)) add_branch(block_id, block.merge_block);

		// First visit our branch targets.
		switch (block.terminator)
		{
		case SPIRBlock::Direct:
			if (post_order_visit(block.next_block)) add_branch(block_id, block.next_block);
			break;

		case SPIRBlock::Select:
			if (post_order_visit(block.true_block)) add_branch(block_id, block.true_block);
			if (post_order_visit(block.false_block)) add_branch(block_id, block.false_block);
			break;
		...
		case SPIRBlock::MultiSelect: {
			...
			{
				if (post_order_visit(target.block)) add_branch(block_id, target.block);
			}
			if (block.default_block && post_order_visit(block.default_block)) add_branch(block_id, block.default_block);
			break;
		}
		default:
			break;
		...
		if (block.merge == SPIRBlock::MergeSelection && post_order_visit(block.next_block))
		{
			...
			if (pred_itr != end(preceding_edges))
			{
				...
				if (succ_itr != end(succeeding_edges)) num_succeeding_edges = succ_itr->second.size();
				...
				if (block.terminator == SPIRBlock::MultiSelect && num_succeeding_edges == 1)
				{
					...
					if (!pred.empty()) add_branch(block_id, block.next_block);
				}
				else
				{
					if (pred.size() == 1 && *pred.begin() != block_id) add_branch(block_id, block.next_block);
				}
			}
			else
			{
	...
	void CFG::add_branch(uint32_t from, uint32_t to)
	{
		const auto add_unique = [](SmallVector<uint32_t>& l, uint32_t value) {
			...
			if (itr == end(l)) l.push_back(value);
		};
	...
	uint32_t CFG::find_loop_dominator(uint32_t block_id) const
	{
		while (block_id != SPIRBlock::NoDominator)
		{
			...
			if (itr == end(preceding_edges)) return SPIRBlock::NoDominator;
			if (itr->second.empty()) return SPIRBlock::NoDominator;
			...
			if (pred_block_id == SPIRBlock::NoDominator) pred_block_id = itr->second.front();
			...
			{
				auto& block = compiler.get<SPIRBlock>(block_id);
				if (block.merge == SPIRBlock::MergeLoop) return block_id;
			}
		}
	...
	bool CFG::node_terminates_control_flow_in_sub_graph(BlockID from, BlockID to) const
	{
		...
		if (from_block.merge == SPIRBlock::MergeLoop) ignore_block_id = from_block.merge_block;

		while (to != from)
		{
			auto pred_itr = preceding_edges.find(to);
			if (pred_itr == end(preceding_edges)) return false;
			...
			bool merges_to_nothing = dom.merge == SPIRBlock::MergeNone || (dom.merge == SPIRBlock::MergeSelection && dom.next_block && compiler.get<SPIRBlock>(dom.next_block).terminator == SPIRBlock::Unreachable) ||
			...
			if ((dom.merge == SPIRBlock::MergeSelection && dom.next_block == to) || (dom.merge == SPIRBlock::MergeLoop && dom.merge_block == to) || (dom.terminator == SPIRBlock::Direct && dom.next_block == to) ||
				(dom.terminator == SPIRBlock::Select && dom.true_block == to && false_path_ignore) || (dom.terminator == SPIRBlock::Select && dom.false_block == to && true_path_ignore))
			{
	...
	void DominatorBuilder::add_block(uint32_t block)
	{
		...
		if (block != dominator) dominator = cfg.find_common_dominator(block, dominator);
	}
	...
	void DominatorBuilder::lift_continue_block_dominator()
	{
		...
		{
		case SPIRBlock::Direct:
			if (cfg.get_visit_order(block.next_block) > post_order) back_edge_dominator = true;
			break;

		case SPIRBlock::Select:
			if (cfg.get_visit_order(block.true_block) > post_order) back_edge_dominator = true;
			if (cfg.get_visit_order(block.false_block) > post_order) back_edge_dominator = true;
			break;
		...
		case SPIRBlock::MultiSelect: {
			...
			{
				if (cfg.get_visit_order(target.block) > post_order) back_edge_dominator = true;
			}
			if (block.default_block && cfg.get_visit_order(block.default_block) > post_order) back_edge_dominator = true;
			break;
		}
		...
		if (back_edge_dominator) dominator = cfg.get_function().entry_block;
	}
} // namespace SPIRV_CROSS_NAMESPACE
