...
namespace SPIRV_CROSS_NAMESPACE
{
	...
	public:
		...
		inline void merge_and(const Bitset& other)
		{
			...
			for (auto& v : higher)
				if (other.higher.count(v) != 0) tmp_set.insert(v);
		...
		inline bool operator==(const Bitset& other) const
		{
			...
			if (higher.size() != other.higher.size()) return false;

			for (auto& v : higher)
				if (other.higher.count(v) == 0) return false;
		...
		template <typename Op> void for_each_bit(const Op& op) const
		{
			...
			{
				if (lower & (1ull << i)) op(i);
			}

			if (higher.empty()) return;
	...
	inline std::string merge(const SmallVector<std::string>& list, const char* between = ", ")
	{
		...
		{
			stream << elem;
			if (&elem != &list.back()) stream << between;
		}
		return stream.str();
	}
...
#pragma warning(pop)
	...
	public:
		...
		template <Types U> TypedID(const TypedID<U>& other)
		{
			*this = other;
		}
		...
		template <Types U> operator TypedID<U>() const
		{
...
#define SPIRV_CROSS_DECLARE_CLONE(T)                                                                                                                                                                                                                               \
	...
	struct SPIRConstant : IVariant
	{
		...
		inline bool constant_is_null() const
		{
			...
			if (!subconstants.empty()) return false;

			for (uint32_t col = 0; col < columns(); col++)
				for (uint32_t row = 0; row < vector_size(); row++)
					if (scalar_u64(col, row) != 0) return false;
		...
		SPIRConstant(TypeID constant_type_, const uint32_t* elements, uint32_t num_elements, bool specialized) : constant_type(constant_type_), specialization(specialized)
		{
		...
		SPIRConstant(TypeID constant_type_, const SPIRConstant* const* vector_elements, uint32_t num_elements, bool specialized) : constant_type(constant_type_), specialization(specialized)
		{
			...
			if (matrix)
			{
				...
				for (uint32_t i = 0; i < num_elements; i++)
				{
					...
					if (vector_elements[i]->specialization) m.id[i] = vector_elements[i]->self;
				}
			}
			else
			{
				...
				for (uint32_t i = 0; i < num_elements; i++)
				{
					...
					if (vector_elements[i]->specialization) m.c[0].id[i] = vector_elements[i]->self;
				}
			}
		}
	...
	public:
		...
		~Variant()
		{
			if (holder) group->pools[type]->deallocate_opaque(holder);
		}
...
// #define SPIRV_CROSS_COPY_CONSTRUCTOR_SANITIZE
		...
		void set(IVariant* val, Types new_type)
		{
			...
			{
				if (val) group->pools[new_type]->deallocate_opaque(val);
		...
		{
			if (!holder) SPIRV_CROSS_THROW("nullptr");
			if (static_cast<Types>(T::type) != type) SPIRV_CROSS_THROW("Bad cast");
	...
	{
		return type.basetype == SPIRType::SByte || type.basetype == SPIRType::UByte || type.basetype == SPIRType::Short || type.basetype == SPIRType::UShort || type.basetype == SPIRType::Int || type.basetype == SPIRType::UInt ||
