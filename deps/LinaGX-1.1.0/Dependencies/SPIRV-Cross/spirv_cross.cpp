...
bool Compiler::block_is_pure(const SPIRBlock& block)
{
	...
	if (block.terminator == SPIRBlock::Kill || block.terminator == SPIRBlock::TerminateRay || block.terminator == SPIRBlock::IgnoreIntersection || block.terminator == SPIRBlock::EmitMeshTasks) return false;
	...
	for (auto& i : block.ops)
	{
		...
		case OpFunctionCall: {
			uint32_t func = ops[2];
			if (!function_is_pure(get<SPIRFunction>(func))) return false;
			break;
		}
		...
		case OpStore: {
			...
			if (type.storage != StorageClassFunction) return false;
			break;
		}
...
string Compiler::to_name(uint32_t id, bool allow_alias) const
{
	if (allow_alias && ir.ids[id].get_type() == TypeType)
	{
		...
		if (type.type_alias)
		{
			...
			if (!has_extended_decoration(type.type_alias, SPIRVCrossDecorationBufferBlockRepacked)) return to_name(type.type_alias);
		}
	}
	...
	for (auto& i : block.ops)
	{
		...
		case OpImageRead: {
			...
			if (var && var->storage != StorageClassFunction)
			{
				...
				if (type.basetype != SPIRType::Image && type.image.dim != DimSubpassData) var->dependees.push_back(id);
			}
			break;
		}
...
SPIRVariable* Compiler::maybe_get_backing_variable(uint32_t chain)
{
	...
	if (!var)
	{
		...
		if (cexpr) var = maybe_get<SPIRVariable>(cexpr->loaded_from);

		auto* access_chain = maybe_get<SPIRAccessChain>(chain);
		if (access_chain) var = maybe_get<SPIRVariable>(access_chain->loaded_from);
	}
...
void Compiler::register_read(uint32_t expr, uint32_t chain, bool forwarded)
{
	...
	if (var)
	{
		...
		if (forwarded && !is_immutable(var->self)) var->dependees.push_back(e.self);
		...
		if (var && var->parameter) var->parameter->read_count++;
	}
}
...
void Compiler::register_write(uint32_t chain)
{
	...
	if (!var)
	{
		...
		if (expr && expr->loaded_from) var = maybe_get<SPIRVariable>(expr->loaded_from);

		auto* access_chain = maybe_get<SPIRAccessChain>(chain);
		if (access_chain && access_chain->loaded_from) var = maybe_get<SPIRVariable>(access_chain->loaded_from);
	}
...
{
	if ((is_builtin_variable(var) && !include_builtins) || var.remapped_variable) return true;

	// Combined image samplers are always considered active as they are "magic" variables.
	if (find_if(begin(combined_image_samplers), end(combined_image_samplers), [&var](const CombinedImageSampler& samp) { return samp.combined_id == var.self; }) != end(combined_image_samplers)) { return false; }
	...
	if (ir.get_spirv_version() >= 0x10400 && var.storage != spv::StorageClassGeneric && var.storage != spv::StorageClassFunction && !interface_variable_exists_in_entry_point(var.self)) { return true; }

	return check_active_interface_variables && storage_class_is_interface(var.storage) && active_interface_variables.find(var.self) == end(active_interface_variables);
}
...
bool Compiler::is_member_builtin(const SPIRType& type, uint32_t index, BuiltIn* builtin) const
{
	...
	if (type_meta)
	{
		...
		{
			if (builtin) *builtin = memb[index].builtin_type;
			return true;
		}
	}
...
{
	return is_physical_pointer(type) && get_pointee_type(type).self == type.parent_type && (has_decoration(type.self, DecorationBlock) || has_decoration(type.self, DecorationBufferBlock));
}
...
bool Compiler::InterfaceVariableAccessHandler::handle(Op opcode, const uint32_t* args, uint32_t length)
{
	...
	case OpFunctionCall: {
		...
		for (uint32_t i = 0; i < count; i++)
		{
			...
			if (var && storage_class_is_interface(var->storage)) variables.insert(args[i]);
		}
		break;
	}
	...
	case OpCopyMemory: {
		...
		if (var && storage_class_is_interface(var->storage)) variables.insert(args[0]);

		var = compiler.maybe_get<SPIRVariable>(args[1]);
		if (var && storage_class_is_interface(var->storage)) variables.insert(args[1]);
		break;
	}
	...
	case OpExtInst: {
		...
		case SPIRExtension::GLSL: {
			...
			case GLSLstd450InterpolateAtOffset: {
				...
				if (var && storage_class_is_interface(var->storage)) variables.insert(args[4]);
				break;
			}
			...
			case GLSLstd450Fract: {
				...
				if (var && storage_class_is_interface(var->storage)) variables.insert(args[5]);
				break;
			}
	...
	if (variable)
	{
		...
		if (var && storage_class_is_interface(var->storage)) variables.insert(variable);
	}
	return true;
}
...
unordered_set<VariableID> Compiler::get_active_interface_variables() const
{
	...
	ir.for_each_typed_id<SPIRVariable>([&](uint32_t, const SPIRVariable& var) {
		...
		if (var.initializer != ID(0) || get_execution_model() != ExecutionModelFragment) variables.insert(var.self);
	});

	// If we needed to create one, we'll need it.
	if (dummy_sampler_id) variables.insert(dummy_sampler_id);
...
ShaderResources Compiler::get_shader_resources(const unordered_set<VariableID>* active_variables) const
{
	...
	ir.for_each_typed_id<SPIRVariable>([&](uint32_t, const SPIRVariable& var) {
		...
		if (var.storage == StorageClassFunction || !type.pointer) return;

		if (active_variables && active_variables->find(var.self) == end(*active_variables)) return;
		...
		{
			if (var.storage == StorageClassInput || var.storage == StorageClassOutput) active_in_entry_point = interface_variable_exists_in_entry_point(var.self);
		}
		else
			active_in_entry_point = interface_variable_exists_in_entry_point(var.self);

		if (!active_in_entry_point) return;
		...
		if (is_builtin)
		{
			...
			{
				bool strip_array = !has_decoration(var.self, DecorationPatch) && (get_execution_model() == ExecutionModelTessellationControl || (get_execution_model() == ExecutionModelTessellationEvaluation && var.storage == StorageClassInput));
		...
		{
			if (has_decoration(type.self, DecorationBlock)) { res.stage_inputs.push_back({var.self, var.basetype, type.self, get_remapped_declared_block_name(var.self, false)}); }
			else
				res.stage_inputs.push_back({var.self, var.basetype, type.self, get_name(var.self)});
		}
		// Subpass inputs
		else if (var.storage == StorageClassUniformConstant && type.image.dim == DimSubpassData) { res.subpass_inputs.push_back({var.self, var.basetype, type.self, get_name(var.self)}); }
		// Outputs
		else if (var.storage == StorageClassOutput)
		{
			if (has_decoration(type.self, DecorationBlock)) { res.stage_outputs.push_back({var.self, var.basetype, type.self, get_remapped_declared_block_name(var.self, false)}); }
			else
				res.stage_outputs.push_back({var.self, var.basetype, type.self, get_name(var.self)});
		}
		// UBOs
		else if (type.storage == StorageClassUniform && has_decoration(type.self, DecorationBlock)) { res.uniform_buffers.push_back({var.self, var.basetype, type.self, get_remapped_declared_block_name(var.self, false)}); }
		// Old way to declare SSBOs.
		else if (type.storage == StorageClassUniform && has_decoration(type.self, DecorationBufferBlock)) { res.storage_buffers.push_back({var.self, var.basetype, type.self, get_remapped_declared_block_name(var.self, ssbo_instance_name)}); }
		// Modern way to declare SSBOs.
		else if (type.storage == StorageClassStorageBuffer) { res.storage_buffers.push_back({var.self, var.basetype, type.self, get_remapped_declared_block_name(var.self, ssbo_instance_name)}); }
		...
		else if (type.storage == StorageClassShaderRecordBufferKHR) { res.shader_record_buffers.push_back({var.self, var.basetype, type.self, get_remapped_declared_block_name(var.self, ssbo_instance_name)}); }
		// Atomic counters
		else if (type.storage == StorageClassAtomicCounter) { res.atomic_counters.push_back({var.self, var.basetype, type.self, get_name(var.self)}); }
		else if (type.storage == StorageClassUniformConstant)
		{
			if (type.basetype == SPIRType::Image)
			{
				// Images
				if (type.image.sampled == 2) { res.storage_images.push_back({var.self, var.basetype, type.self, get_name(var.self)}); }
				// Separate images
				else if (type.image.sampled == 1) { res.separate_images.push_back({var.self, var.basetype, type.self, get_name(var.self)}); }
			}
			// Separate samplers
			else if (type.basetype == SPIRType::Sampler) { res.separate_samplers.push_back({var.self, var.basetype, type.self, get_name(var.self)}); }
			// Textures
			else if (type.basetype == SPIRType::SampledImage) { res.sampled_images.push_back({var.self, var.basetype, type.self, get_name(var.self)}); }
			// Acceleration structures
			else if (type.basetype == SPIRType::AccelerationStructure) { res.acceleration_structures.push_back({var.self, var.basetype, type.self, get_name(var.self)}); }
			else { res.gl_plain_uniforms.push_back({var.self, var.basetype, type.self, get_name(var.self)}); }
		}
	});
...
{
	if (type.basetype != SPIRType::Struct) return false;
...
{
	if (type_is_top_level_block(type)) return true;

	if (type.basetype == SPIRType::Struct)
	{
		// Block-like types may have Offset decorations.
		for (uint32_t i = 0; i < uint32_t(type.member_types.size()); i++)
			if (has_member_decoration(type.self, i, DecorationOffset)) return true;
	}
...
void Compiler::parse_fixup()
{
	...
	for (auto id_ : ir.ids_for_constant_or_variable)
	{
		...
		else if (id.get_type() == TypeVariable)
		{
			...
			if (var.storage == StorageClassPrivate || var.storage == StorageClassWorkgroup || var.storage == StorageClassTaskPayloadWorkgroupEXT || var.storage == StorageClassOutput) { global_variables.push_back(var.self); }
			if (variable_storage_is_aliased(var)) aliased_variables.push_back(var.self);
		}
	}
}
...
void Compiler::update_name_cache(unordered_set<string>& cache_primary, const unordered_set<string>& cache_secondary, string& name)
{
	...
	const auto find_name = [&](const string& n) -> bool {
		if (cache_primary.find(n) != end(cache_primary)) return true;

		if (&cache_primary != &cache_secondary)
			if (cache_secondary.find(n) != end(cache_secondary)) return true;
...
{
	if (var.phi_variable || var.storage == spv::StorageClass::StorageClassAtomicCounter) return var.basetype;
...
SPIRType& Compiler::get_variable_element_type(const SPIRVariable& var)
{
	...
	if (is_array(*type)) type = &get<SPIRType>(type->parent_type);
	return *type;
}
...
void Compiler::set_member_decoration_string(TypeID id, uint32_t index, spv::Decoration decoration, const std::string& argument)
{
...
uint32_t Compiler::get_extended_decoration(uint32_t id, ExtendedDecorations decoration) const
{
	...
	if (!dec.extended.flags.get(decoration)) return get_default_extended_decoration(decoration);
...
uint32_t Compiler::get_extended_member_decoration(uint32_t type, uint32_t index, ExtendedDecorations decoration) const
{
	...
	if (index >= m->members.size()) return 0;
...
bool Compiler::has_extended_member_decoration(uint32_t type, uint32_t index, ExtendedDecorations decoration) const
{
	...
	if (index >= m->members.size()) return false;
...
bool Compiler::get_binary_offset_for_decoration(VariableID id, spv::Decoration decoration, uint32_t& word_offset) const
{
	...
	if (itr == end(word_offsets)) return false;
...
{
	if (block.terminator != SPIRBlock::Direct) return false;

	auto& child = get<SPIRBlock>(block.next_block);

	// If this block participates in PHI, the block isn't really noop.
	for (auto& phi : block.phi_variables)
		if (phi.parent == block.self || phi.parent == child.self) return false;

	for (auto& phi : child.phi_variables)
		if (phi.parent == block.self) return false;
	...
	for (auto& i : block.ops)
	{
		...
		case OpExtInst: {
			...
			bool ext_is_nonsemantic_only = ext == SPIRExtension::NonSemanticShaderDebugInfo || ext == SPIRExtension::SPV_debug_info || ext == SPIRExtension::NonSemanticGeneric;

			if (!ext_is_nonsemantic_only) return false;
...
{
	// Tried and failed.
	if (block.disable_block_optimization || block.complex_continue) return false;
	...
	else if (method == SPIRBlock::MergeToDirectForLoop)
	{
		...
		if (ret)
		{
			...
			if (merge)
				for (auto& phi : merge->phi_variables)
					if (phi.parent == block.self || phi.parent == child.false_block) return false;
		}
...
{
	if (!execution_is_branchless(from, to)) return false;

	auto* start = &from;
	for (;;)
	{
		if (start->self == to.self) return true;

		if (!block_is_noop(*start)) return false;
...
SPIRBlock::ContinueBlockType Compiler::continue_block_type(const SPIRBlock& block) const
{
	...
	if (block.complex_continue) return SPIRBlock::ComplexLoop;
	...
	if (block.merge == SPIRBlock::MergeLoop) return SPIRBlock::WhileLoop;
	...
	else
	{
		...
		if (flush_phi_to_false || flush_phi_to_true) return SPIRBlock::ComplexLoop;

		bool positive_do_while = block.true_block == dominator.self && (block.false_block == dominator.merge_block || (false_block && merge_block && execution_is_noop(*false_block, *merge_block)));

		bool negative_do_while = block.false_block == dominator.self && (block.true_block == dominator.merge_block || (true_block && merge_block && execution_is_noop(*true_block, *merge_block)));

		if (block.merge == SPIRBlock::MergeNone && block.terminator == SPIRBlock::Select && (positive_do_while || negative_do_while)) { return SPIRBlock::DoWhileLoop; }
		else
			return SPIRBlock::ComplexLoop;
	}
}
...
const SmallVector<SPIRBlock::Case>& Compiler::get_case_list(const SPIRBlock& block) const
{
	...
	else
	{
		...
		if (search == ir.load_type_width.end()) { SPIRV_CROSS_THROW("Use of undeclared variable on a switch statement."); }

		width = search->second;
	}

	if (width > 32) return block.cases_64bit;
...
bool Compiler::traverse_all_reachable_opcodes(const SPIRBlock& block, OpcodeHandler& handler) const
{
	...
	for (auto& i : block.ops)
	{
		...
		if (!handler.handle(op, ops, i.length)) return false;
		...
		if (op == OpFunctionCall)
		{
			...
			{
				if (!handler.begin_function_scope(ops, i.length)) return false;
				if (!traverse_all_reachable_opcodes(get<SPIRFunction>(ops[2]), handler)) return false;
				if (!handler.end_function_scope(ops, i.length)) return false;
	...
	if (!handler.handle_terminator(block)) return false;
...
{
	for (auto block : func.blocks)
		if (!traverse_all_reachable_opcodes(get<SPIRBlock>(block), handler)) return false;
...
{
	if (type.member_types.empty()) SPIRV_CROSS_THROW("Declared struct in block cannot be empty.");
...
#undef binary_spec_op_cast
	...
	case OpUMod: {
		...
		if (b == 0) SPIRV_CROSS_THROW("Undefined behavior in UMod, b == 0.\n");
		value = a % b;
		break;
	}
	...
	case OpSRem: {
		...
		if (b == 0) SPIRV_CROSS_THROW("Undefined behavior in SRem, b == 0.\n");
		value = a % b;
		break;
	}
	...
	case OpSMod: {
		auto a = int32_t(eval_u32(spec.arguments[0]));
		auto b = int32_t(eval_u32(spec.arguments[1]));
		if (b == 0) SPIRV_CROSS_THROW("Undefined behavior in SMod, b == 0.\n");
		auto v = a % b;

		// Makes sure we match the sign of b, not a.
		if ((b < 0 && v > 0) || (b > 0 && v < 0)) v += b;
		value = v;
		break;
	}
	...
	case OpUDiv: {
		uint32_t a = eval_u32(spec.arguments[0]);
		uint32_t b = eval_u32(spec.arguments[1]);
		if (b == 0) SPIRV_CROSS_THROW("Undefined behavior in UDiv, b == 0.\n");
		value = a / b;
		break;
	}
	...
	case OpSDiv: {
		auto a = int32_t(eval_u32(spec.arguments[0]));
		auto b = int32_t(eval_u32(spec.arguments[1]));
		if (b == 0) SPIRV_CROSS_THROW("Undefined behavior in SDiv, b == 0.\n");
		value = a / b;
		break;
	}
...
{
	if (struct_type.member_types.empty()) SPIRV_CROSS_THROW("Declared struct in block cannot be empty.");
	...
	if (type.pointer && type.storage == StorageClassPhysicalStorageBuffer)
	{
		...
		if (type.pointer_depth > get<SPIRType>(type.parent_type).pointer_depth) return 8;
	}
	...
	else if (type.basetype == SPIRType::Struct) { return get_declared_struct_size(type); }
	else
	{
...
{
	if (opcode != OpAccessChain && opcode != OpInBoundsAccessChain && opcode != OpPtrAccessChain) return true;

	bool ptr_chain = (opcode == OpPtrAccessChain);

	// Invalid SPIR-V.
	if (length < (ptr_chain ? 5u : 4u)) return false;

	if (args[2] != id) return true;
	...
	if (seen.find(index) != end(seen)) return true;
	seen.insert(index);
	...
	if (index + 1 < type.member_types.size()) { range = compiler.type_struct_member_offset(type, index + 1) - offset; }
	else
	{
...
bool Compiler::types_are_logically_equivalent(const SPIRType& a, const SPIRType& b) const
{
	...
	if (array_count && memcmp(a.array.data(), b.array.data(), array_count * sizeof(uint32_t)) != 0) return false;

	if (a.basetype == SPIRType::Image || a.basetype == SPIRType::SampledImage)
	{
		if (memcmp(&a.image, &b.image, sizeof(SPIRType::Image)) != 0) return false;
	}
	...
	{
		if (!types_are_logically_equivalent(get<SPIRType>(a.member_types[i]), get<SPIRType>(b.member_types[i]))) return false;
	}
...
bool Compiler::is_vertex_like_shader() const
{
	...
	return model == ExecutionModelVertex || model == ExecutionModelGeometry || model == ExecutionModelTessellationControl || model == ExecutionModelTessellationEvaluation;
}
...
void Compiler::add_implied_read_expression(SPIRExpression& e, uint32_t source)
{
	...
	if (itr == end(e.implied_read_expressions)) e.implied_read_expressions.push_back(source);
}
...
void Compiler::add_active_interface_variable(uint32_t var_id)
{
	...
	if (ir.get_spirv_version() >= 0x10400)
	{
		...
		if (find(begin(vars), end(vars), VariableID(var_id)) == end(vars)) vars.push_back(var_id);
	}
}
...
void Compiler::inherit_expression_dependencies(uint32_t dst, uint32_t source_expression)
{
	...
	if (forwarded_temporaries.find(dst) == end(forwarded_temporaries) || forced_temporaries.find(dst) != end(forced_temporaries)) { return; }
...
{
	auto itr = find_if(begin(ir.entry_points), end(ir.entry_points), [&](const std::pair<uint32_t, SPIREntryPoint>& entry) -> bool { return entry.second.orig_name == name; });

	if (itr == end(ir.entry_points)) SPIRV_CROSS_THROW("Entry point does not exist.");
...
{
	auto itr = find_if(begin(ir.entry_points), end(ir.entry_points), [&](const std::pair<uint32_t, SPIREntryPoint>& entry) -> bool { return entry.second.orig_name == name && entry.second.model == model; });
...
bool Compiler::interface_variable_exists_in_entry_point(uint32_t id) const
{
	...
	if (ir.get_spirv_version() < 0x10400)
	{
		if (var.storage != StorageClassInput && var.storage != StorageClassOutput && var.storage != StorageClassUniformConstant) SPIRV_CROSS_THROW("Only Input, Output variables and Uniform constants are part of a shader linking interface.");
		...
		if (ir.entry_points.size() <= 1) return true;
	}
...
uint32_t Compiler::CombinedImageSamplerHandler::remap_parameter(uint32_t id)
{
	...
	if (parameter_remapping.empty()) return id;
...
bool Compiler::CombinedImageSamplerHandler::end_function_scope(const uint32_t* args, uint32_t length)
{
	...
	if (caller.do_combined_parameters)
	{
		for (auto& param : params)
		{
			...
			if (i) image_id = i->self;
			if (s) sampler_id = s->self;
...
void Compiler::CombinedImageSamplerHandler::register_combined_image_sampler(SPIRFunction& caller, VariableID combined_module_id, VariableID image_id, VariableID sampler_id, bool depth)
{
	...
	if (param.global_image && param.global_sampler) return;
	...
	auto itr = find_if(begin(caller.combined_parameters), end(caller.combined_parameters), [&param](const SPIRFunction::CombinedImageSamplerParameter& p) {
		return param.image_id == p.image_id && param.sampler_id == p.sampler_id && param.global_image == p.global_image && param.global_sampler == p.global_sampler;
	});
	...
	if (itr == end(caller.combined_parameters))
	{
		...
		bool relaxed_precision =
			compiler.has_decoration(sampler_id, DecorationRelaxedPrecision) || compiler.has_decoration(image_id, DecorationRelaxedPrecision) || (combined_module_id && compiler.has_decoration(combined_module_id, DecorationRelaxedPrecision));

		if (relaxed_precision) compiler.set_decoration(combined_id, DecorationRelaxedPrecision);
...
bool Compiler::DummySamplerForCombinedImageHandler::handle(Op opcode, const uint32_t* args, uint32_t length)
{
	...
	case OpImageQuerySamples: {
		...
		if (var)
		{
			...
			if (type.basetype == SPIRType::Image && type.image.sampled == 1 && type.image.dim != DimBuffer) need_dummy_sampler = true;
		}
...
bool Compiler::CombinedImageSamplerHandler::handle(Op opcode, const uint32_t* args, uint32_t length)
{
	...
	case OpLoad: {
		...
		if (!separate_image && !separate_sampler) return true;
	...
	if (!functions.empty())
	{
		...
		if (callee.do_combined_parameters)
		{
			...
			if (sampler) sampler_id = sampler->self;
	...
	auto itr = find_if(begin(compiler.combined_image_samplers), end(compiler.combined_image_samplers), [image_id, sampler_id](const CombinedImageSampler& combined) { return combined.image_id == image_id && combined.sampler_id == sampler_id; });
	...
	if (itr == end(compiler.combined_image_samplers))
	{
		...
		// If any of OpSampledImage, underlying image or sampler are marked, inherit the decoration.
		bool relaxed_precision = (sampler_id && compiler.has_decoration(sampler_id, DecorationRelaxedPrecision)) || (image_id && compiler.has_decoration(image_id, DecorationRelaxedPrecision)) ||
...
SmallVector<SpecializationConstant> Compiler::get_specialization_constants() const
{
	...
	ir.for_each_typed_id<SPIRConstant>([&](uint32_t, const SPIRConstant& c) {
		if (c.specialization && has_decoration(c.self, DecorationSpecId)) spec_consts.push_back({c.self, get_decoration(c.self, DecorationSpecId)});
	});
	return spec_consts;
}
...
static bool exists_unaccessed_path_to_return(const CFG& cfg, uint32_t block, const unordered_set<uint32_t>& blocks, unordered_set<uint32_t>& visit_cache)
{
	...
	if (blocks.find(block) != end(blocks)) return false;

	// We are at the end of the CFG.
	if (cfg.get_succeeding_edges(block).empty()) return true;
	...
	for (auto& succ : cfg.get_succeeding_edges(block))
	{
		...
		{
			if (exists_unaccessed_path_to_return(cfg, succ, blocks, visit_cache)) return true;
...
void Compiler::analyze_parameter_preservation(SPIRFunction& entry, const CFG& cfg, const unordered_map<uint32_t, unordered_set<uint32_t>>& variable_to_blocks, const unordered_map<uint32_t, unordered_set<uint32_t>>& complete_write_blocks)
{
	for (auto& arg : entry.arguments)
	{
		...
		if (!potential_preserve) continue;
		...
		if (exists_unaccessed_path_to_return(cfg, entry.entry_block, itr->second, visit_cache)) arg.read_count++;
	}
}

Compiler::AnalyzeVariableScopeAccessHandler::AnalyzeVariableScopeAccessHandler(Compiler& compiler_, SPIRFunction& entry_) : compiler(compiler_), entry(entry_)
{
}
...
void Compiler::AnalyzeVariableScopeAccessHandler::set_current_block(const SPIRBlock& block)
{
	...
	case SPIRBlock::MultiSelect: {
		...
		if (block.default_block) test_phi(block.default_block);
		break;
	}
...
{
	if (id >= compiler.get_current_id_bound()) return false;
...
bool Compiler::AnalyzeVariableScopeAccessHandler::handle_terminator(const SPIRBlock& block)
{
	...
	case SPIRBlock::Return:
		if (block.return_value) notify_variable_access(block.return_value, block.self);
		break;
...
bool Compiler::AnalyzeVariableScopeAccessHandler::handle(spv::Op op, const uint32_t* args, uint32_t length)
{
	...
	if (compiler.instruction_to_result_type(result_type, result_id, op, args, length))
	{
		...
		if (op == OpConvertUToAccelerationStructureKHR)
		{
			...
			if (itr != result_id_to_type.end()) result_type = itr->second;
		}
	...
	case OpCopyMemory: {
		...
		if (var) accessed_variables_to_block[var->self].insert(current_block->self);
		break;
	}
	...
	case OpCopyObject: {
		...
		auto& type = compiler.get<SPIRType>(result_type);
		if (type.pointer) result_id_to_type[result_id] = type.parent_type;
		...
		notify_variable_access(args[1], current_block->self);
		if (access_chain_expressions.count(args[2])) access_chain_expressions.insert(args[1]);
	...
	case OpLoad: {
		...
		if (compiler.type_is_opaque_value(type)) rvalue_forward_children[args[1]].insert(args[2]);
		break;
	}
	...
	case OpFunctionCall: {
		...
		if (compiler.get_type(args[0]).basetype != SPIRType::Void) notify_variable_access(args[1], current_block->self);
	...
	case OpImageWrite:
		for (uint32_t i = 0; i < length; i++)
		{
			...
			if (i != 3) notify_variable_access(args[i], current_block->self);
		}
		break;
	...
	case OpImageSparseRead:
		for (uint32_t i = 1; i < length; i++)
		{
			...
			if (i != 4) notify_variable_access(args[i], current_block->self);
		}
		break;
	...
	case OpImageSparseDrefGather:
		for (uint32_t i = 1; i < length; i++)
		{
			...
			if (i != 5) notify_variable_access(args[i], current_block->self);
		}
		break;
...
Compiler::StaticExpressionAccessHandler::StaticExpressionAccessHandler(Compiler& compiler_, uint32_t variable_id_) : compiler(compiler_), variable_id(variable_id_)
{
}
...
void Compiler::find_function_local_luts(SPIRFunction& entry, const AnalyzeVariableScopeAccessHandler& handler, bool single_function)
{
	...
	for (auto& accessed_var : handler.accessed_variables_to_block)
	{
		...
		if (!var.is_written_to) { var.is_written_to = handler.complete_write_variables_to_block.count(var.self) != 0 || handler.partial_write_variables_to_block.count(var.self) != 0; }
		...
		else
		{
			...
			if (handler.partial_write_variables_to_block.count(var.self) != 0) continue;

			auto itr = handler.complete_write_variables_to_block.find(var.self);

			// No writes?
			if (itr == end(handler.complete_write_variables_to_block)) continue;

			// We write to the variable in more than one block.
			auto& write_blocks = itr->second;
			if (write_blocks.size() != 1) continue;
			...
			if (write_blocks.count(dominator) == 0) continue;
			...
			if (static_expression_handler.write_count != 1 || static_expression_handler.static_expression == 0) continue;

			// Is it a constant expression?
			if (ir.ids[static_expression_handler.static_expression].get_type() != TypeConstant) continue;
...
void Compiler::analyze_variable_scope(SPIRFunction& entry, AnalyzeVariableScopeAccessHandler& handler)
{
	...
	for (auto& var : handler.accessed_variables_to_block)
	{
		...
		if (find(begin(entry.local_variables), end(entry.local_variables), VariableID(var.first)) == end(entry.local_variables)) continue;
		...
		if (dominating_block && potential_continue_block != 0 && potential_continue_block != ~0u)
		{
			...
			if (merge_candidate != 0 && cfg.is_reachable(merge_candidate))
			{
				...
				if (!cfg.is_reachable(potential_continue_block) || cfg.get_visit_order(merge_candidate) > cfg.get_visit_order(potential_continue_block)) { potential_continue_block = 0; }
			}
		}

		if (potential_continue_block != 0 && potential_continue_block != ~0u) potential_loop_variables[var.first] = potential_continue_block;
	...
	for (auto& var : handler.accessed_temporaries_to_block)
	{
		...
		if (type_is_opaque_value(type)) continue;
	...
	for (auto& loop_variable : potential_loop_variables)
	{
		...
		if (block == BlockID(~(0u)) || block == BlockID(0)) continue;

		// Dead code.
		if (dominator == ID(0)) continue;
		...
		// Find the loop header for this block if we are a continue block.
		{
			...
			if (itr != end(ir.continue_block_to_loop_header)) { header = itr->second; }
		...
		{
			if (blocks.count(dominator) != 0) has_accessed_variable = true;
		...
		if (!static_loop_init || !has_accessed_variable) continue;
		...
		cfg.walk_from(seen_blocks, header_block.merge_block, [&](uint32_t walk_block) -> bool {
			...
			if (blocks.find(walk_block) != end(blocks)) static_loop_init = false;
			return true;
		});

		if (!static_loop_init) continue;
...
bool Compiler::may_read_undefined_variable_in_block(const SPIRBlock& block, uint32_t var)
{
	for (auto& op : block.ops)
	{
		...
		case OpPtrAccessChain:
			...
			if (ops[2] == var) return true;
			break;

		case OpSelect:
			// Variable pointers.
			// We might read before writing.
			if (ops[3] == var || ops[4] == var) return true;
			break;
...
bool Compiler::get_common_basic_type(const SPIRType& type, SPIRType::BaseType& base_type)
{
	if (type.basetype == SPIRType::Struct)
	{
		...
		for (auto& member_type : type.member_types)
		{
			...
			if (!get_common_basic_type(get<SPIRType>(member_type), member_base)) return false;
...
void Compiler::ActiveBuiltinHandler::handle_builtin(const SPIRType& type, BuiltIn builtin, const Bitset& decoration_flags)
{
	...
	{
		if (!type.array_size_literal[0]) SPIRV_CROSS_THROW("Array size for ClipDistance must be a literal.");
		uint32_t array_size = type.array[0];
		if (array_size == 0) SPIRV_CROSS_THROW("Array size for ClipDistance must not be unsized.");
		compiler.clip_distance_count = array_size;
	}
	else if (builtin == BuiltInCullDistance)
	{
		if (!type.array_size_literal[0]) SPIRV_CROSS_THROW("Array size for CullDistance must be a literal.");
		uint32_t array_size = type.array[0];
		if (array_size == 0) SPIRV_CROSS_THROW("Array size for CullDistance must not be unsized.");
		compiler.cull_distance_count = array_size;
	}
	else if (builtin == BuiltInPosition)
	{
		if (decoration_flags.get(DecorationInvariant)) compiler.position_invariant = true;
	}
}
...
bool Compiler::ActiveBuiltinHandler::handle(spv::Op opcode, const uint32_t* args, uint32_t length)
{
	...
	case OpPtrAccessChain: {
		...
		for (uint32_t i = 0; i < count; i++)
		{
			...
			if (opcode == OpPtrAccessChain && i == 0) continue;

			// Arrays
			if (!type->array.empty()) { type = &compiler.get<SPIRType>(type->parent_type); }
			// Structs
...
void Compiler::update_active_builtins()
{
	...
	ir.for_each_typed_id<SPIRVariable>([&](uint32_t, const SPIRVariable& var) {
		...
		// Also, make sure we preserve output variables which are only initialized, but never accessed by any code.
		if (var.initializer != ID(0)) handler.add_if_builtin_or_block(var.self);
	});
}
...
void Compiler::analyze_image_and_sampler_usage()
{
	...
	for (auto& combined : combined_image_samplers)
		if (comparison_ids.count(combined.sampler_id)) comparison_ids.insert(combined.combined_id);
}
...
void Compiler::build_function_control_flow_graphs_and_analyze()
{
	...
	for (auto& f : function_cfgs)
	{
		...
		for (auto block : func.blocks)
		{
			...
			if (b.loop_variables.size() < 2) continue;
	...
	if (!single_function)
	{
		for (auto& id : global_variables)
		{
			...
			if (is_array(type) && var.storage == StorageClassPrivate && var.initializer && !var.is_written_to && ir.ids[var.initializer].get_type() == TypeConstant)
			{
...
void Compiler::CombinedImageSamplerUsageHandler::add_dependency(uint32_t dst, uint32_t src)
{
	...
	if (comparison_ids.count(src)) comparison_ids.insert(dst);
}
...
bool Compiler::CombinedImageSamplerUsageHandler::handle(Op opcode, const uint32_t* args, uint32_t length)
{
	...
	case OpLoad: {
		...
		if (type.image.dim == DimSubpassData)
		{
			...
			if (type.image.ms) need_subpass_input_ms = true;
		}

		// If we load a SampledImage and it will be used with Dref, propagate the state up.
		if (dref_combined_samplers.count(args[1]) != 0) add_hierarchy_to_comparison_ids(args[1]);
		break;
	}
...
void Compiler::make_constant_null(uint32_t id, uint32_t type)
{
	...
	else if (!constant_type.array.empty())
	{
		...
		if (!constant_type.array_size_literal.back()) SPIRV_CROSS_THROW("Array size of OpConstantNull must be a literal.");
...
std::string Compiler::get_remapped_declared_block_name(uint32_t id, bool fallback_prefer_instance_name) const
{
	...
	if (itr != end(declared_block_names)) { return itr->second; }
	else
	{
		auto& var = get<SPIRVariable>(id);

		if (fallback_prefer_instance_name) { return to_name(var.self); }
		else
		{
			auto& type		 = get<SPIRType>(var.basetype);
			auto* type_meta	 = ir.find_meta(type.self);
...
bool Compiler::reflection_ssbo_instance_name_is_significant() const
{
	...
	ir.for_each_typed_id<SPIRVariable>([&](uint32_t, const SPIRVariable& var) {
		auto& type = this->get<SPIRType>(var.basetype);
		if (!type.pointer || var.storage == StorageClassFunction) return;
...
bool Compiler::instruction_to_result_type(uint32_t& result_type, uint32_t& result_id, spv::Op op, const uint32_t* args, uint32_t length)
{
	if (length < 2) return false;
...
Bitset Compiler::combined_decoration_for_member(const SPIRType& type, uint32_t index) const
{
	...
	if (type_meta)
	{
		...
		if (index >= members.size()) return flags;
		...
		for (uint32_t i = 0; i < member_childs.size(); i++)
		{
			...
			if (!child_member_type.pointer) flags.merge_or(combined_decoration_for_member(member_type, i));
		}
	}
...
Compiler::PhysicalStorageBufferPointerHandler::PhysicalStorageBufferPointerHandler(Compiler& compiler_) : compiler(compiler_)
{
}
...
void Compiler::PhysicalStorageBufferPointerHandler::mark_aligned_access(uint32_t id, const uint32_t* args, uint32_t length)
{
	...
	if (length && (mask & MemoryAccessAlignedMask) != 0)
	{
		...
		if (meta && alignment > meta->alignment) meta->alignment = alignment;
	}
}
...
void Compiler::PhysicalStorageBufferPointerHandler::setup_meta_chain(uint32_t type_id, uint32_t var_id)
{
	if (type_is_bda_block_entry(type_id))
	{
		...
		if (!compiler.is_physical_pointer_to_buffer_block(type)) non_block_types.insert(type_id);

		if (meta.alignment == 0) meta.alignment = get_minimum_scalar_alignment(compiler.get_pointee_type(type));
	}
}
...
bool Compiler::PhysicalStorageBufferPointerHandler::handle(Op op, const uint32_t* args, uint32_t length)
{
	...
	case OpCopyObject: {
		...
		if (itr != access_chain_to_physical_block.end()) access_chain_to_physical_block[args[1]] = itr->second;
		break;
	}

	case OpLoad: {
		setup_meta_chain(args[0], args[1]);
		if (length >= 4) mark_aligned_access(args[2], args + 3, length - 3);
		break;
	}

	case OpStore: {
		if (length >= 3) mark_aligned_access(args[0], args + 2, length - 2);
		break;
	}
...
void Compiler::analyze_non_block_pointer_types()
{
	...
	ir.for_each_typed_id<SPIRType>([&](uint32_t id, SPIRType& type) {
		...
		if (type.self == id && (has_decoration(type.self, DecorationBlock) || has_decoration(type.self, DecorationBufferBlock))) { handler.analyze_non_block_types_from_block(type); }
	});
...
bool Compiler::InterlockedResourceAccessPrepassHandler::handle(Op op, const uint32_t*, uint32_t)
{
	if (op == OpBeginInvocationInterlockEXT || op == OpEndInvocationInterlockEXT)
	{
		...
		else
		{
			...
			if (!outside_control_flow) control_flow_interlock = true;
		}
	}
	return true;
}
...
bool Compiler::InterlockedResourceAccessHandler::begin_function_scope(const uint32_t* args, uint32_t length)
{
	...
	if (args[2] == interlock_function_id) call_stack_is_interlocked = true;
...
{
	if (call_stack.back() == interlock_function_id) call_stack_is_interlocked = false;
...
{
	if ((use_critical_section && in_crit_sec) || (control_flow_interlock && call_stack_is_interlocked) || split_function_case) { compiler.interlocked_resources.insert(id); }
}
...
bool Compiler::InterlockedResourceAccessHandler::handle(Op opcode, const uint32_t* args, uint32_t length)
{
	...
	case OpLoad: {
		...
		case StorageClassUniform:
			...
			if (!compiler.has_decoration(compiler.get<SPIRType>(var->basetype).self, DecorationBufferBlock)) break;
			// fallthrough
		case StorageClassStorageBuffer:
	...
	case OpAtomicStore: {
		...
		uint32_t ptr = args[0];
		auto*	 var = compiler.maybe_get_backing_variable(ptr);
		if (var && (var->storage == StorageClassUniform || var->storage == StorageClassUniformConstant || var->storage == StorageClassStorageBuffer)) { access_potential_resource(var->self); }
	...
	case OpCopyMemory: {
		...
		if (dst_var && (dst_var->storage == StorageClassUniform || dst_var->storage == StorageClassStorageBuffer)) access_potential_resource(dst_var->self);

		if (src_var)
		{
			if (src_var->storage != StorageClassUniform && src_var->storage != StorageClassStorageBuffer) break;

			if (src_var->storage == StorageClassUniform && !compiler.has_decoration(compiler.get<SPIRType>(src_var->basetype).self, DecorationBufferBlock)) { break; }
...
{
	if (get_execution_model() == ExecutionModelFragment && (get_entry_point().flags.get(ExecutionModePixelInterlockOrderedEXT) || get_entry_point().flags.get(ExecutionModePixelInterlockUnorderedEXT) ||
...
bool Compiler::check_internal_recursion(const SPIRType& type, std::unordered_set<uint32_t>& checked_ids)
{
	...
	if (checked_ids.count(type.self)) return true;
...
bool Compiler::type_is_array_of_pointers(const SPIRType& type) const
{
	...
	if (!type.parent_type) return false;
...
bool Compiler::flush_phi_required(BlockID from, BlockID to) const
{
	...
	for (auto& phi : child.phi_variables)
		if (phi.parent == from) return true;
	return false;
}
