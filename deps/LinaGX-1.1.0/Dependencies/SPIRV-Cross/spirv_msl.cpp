...
void CompilerMSL::add_msl_shader_input(const MSLShaderInterfaceVariable& si)
{
	...
	if (si.builtin != BuiltInMax && !inputs_by_builtin.count(si.builtin)) inputs_by_builtin[si.builtin] = si;
}
...
void CompilerMSL::add_msl_shader_output(const MSLShaderInterfaceVariable& so)
{
	...
	if (so.builtin != BuiltInMax && !outputs_by_builtin.count(so.builtin)) outputs_by_builtin[so.builtin] = so;
}
...
{
	if (desc_set < kMaxArgumentBuffers) argument_buffer_discrete_mask |= 1u << desc_set;
}
...
uint32_t CompilerMSL::get_resource_array_size(const SPIRType& type, uint32_t id) const
{
	...
	StageSetBinding tuple = {get_entry_point().model, get_decoration(id, DecorationDescriptorSet), get_decoration(id, DecorationBinding)};
	auto			itr	  = resource_bindings.find(tuple);
...
void CompilerMSL::build_implicit_builtins()
{
	...
	bool need_subgroup_mask = active_input_builtins.get(BuiltInSubgroupEqMask) || active_input_builtins.get(BuiltInSubgroupGeMask) || active_input_builtins.get(BuiltInSubgroupGtMask) || active_input_builtins.get(BuiltInSubgroupLeMask) ||
							  active_input_builtins.get(BuiltInSubgroupLtMask);
	bool need_subgroup_ge_mask		 = !msl_options.is_ios() && (active_input_builtins.get(BuiltInSubgroupGeMask) || active_input_builtins.get(BuiltInSubgroupGtMask));
	bool need_multiview				 = get_execution_model() == ExecutionModelVertex && !msl_options.view_index_from_device_index && msl_options.multiview_layered_rendering && (msl_options.multiview || active_input_builtins.get(BuiltInViewIndex));
	...
	bool need_vertex_base_params	 = need_grid_params && (active_input_builtins.get(BuiltInVertexId) || active_input_builtins.get(BuiltInVertexIndex) || active_input_builtins.get(BuiltInBaseVertex) || active_input_builtins.get(BuiltInInstanceId) ||
	...
	if (need_subpass_input || need_sample_pos || need_subgroup_mask || need_vertex_params || need_tesc_params || need_tese_params || need_multiview || need_dispatch_base || need_vertex_base_params || need_grid_params || needs_sample_id ||
		needs_subgroup_invocation_id || needs_subgroup_size || needs_helper_invocation || has_additional_fixed_sample_mask() || need_local_invocation_index || need_workgroup_size)
	{
		...
		ir.for_each_typed_id<SPIRVariable>([&](uint32_t, SPIRVariable& var) {
			if (var.storage != StorageClassInput && var.storage != StorageClassOutput) return;
			if (!interface_variable_exists_in_entry_point(var.self)) return;
			if (!has_decoration(var.self, DecorationBuiltIn)) return;
			...
			if (var.storage != StorageClassInput) return;
			...
			if (need_subpass_input && (!msl_options.use_framebuffer_fetch_subpasses))
			{
				...
				case BuiltInFragCoord:
					...
					break;
				case BuiltInLayer:
					if (!msl_options.arrayed_subpass_input || msl_options.multiview) break;
					mark_implicit_builtin(StorageClassInput, BuiltInLayer, var.self);
					builtin_layer_id = var.self;
					has_layer		 = true;
					break;
				case BuiltInViewIndex:
					if (!msl_options.multiview) break;
			...
			if (need_tese_params && builtin == BuiltInTessLevelOuter) { tess_level_outer_var_id = var.self; }

			if (need_tese_params && builtin == BuiltInTessLevelInner) { tess_level_inner_var_id = var.self; }
			...
			// was used.
			if (need_dispatch_base && workgroup_id_type == 0 && (builtin == BuiltInWorkgroupId || builtin == BuiltInGlobalInvocationId)) workgroup_id_type = var.basetype;
		});

		// Use Metal's native frame-buffer fetch API for subpass inputs.
		if ((!has_frag_coord || (msl_options.multiview && !has_view_idx) || (msl_options.arrayed_subpass_input && !msl_options.multiview && !has_layer)) && (!msl_options.use_framebuffer_fetch_subpasses) && need_subpass_input)
		{
			if (!has_frag_coord)
			{
		...
		if ((need_vertex_params && (!has_vertex_idx || !has_base_vertex || !has_instance_idx || !has_base_instance)) || (need_multiview && (!has_instance_idx || !has_base_instance || !has_view_idx)))
		{
		...
		if ((need_tesc_params && (msl_options.multi_patch_workgroup || !has_invocation_id || !has_primitive_id)) || (need_tese_params && !has_primitive_id) || need_grid_params)
		{
			uint32_t type_ptr_id = ir.increase_bound_by(1);
		...
		{
			if (workgroup_id_type == 0) workgroup_id_type = build_extended_vector_type(get_uint_type_id(), 3);
			uint32_t var_id;
	...
	bool need_position = (get_execution_model() == ExecutionModelVertex || is_tese_shader()) && !capture_output_to_buffer && !get_is_rasterization_disabled() && !active_output_builtins.get(BuiltInPosition);
	...
	if (need_position)
	{
		...
		ir.for_each_typed_id<SPIRVariable>([&](uint32_t, SPIRVariable& var) {
			if (var.storage == StorageClassOutput && interface_variable_exists_in_entry_point(var.self))
			{
				...
				if (has_decoration(var.self, DecorationBuiltIn) && get_decoration(var.self, DecorationBuiltIn) == BuiltInPosition) active_output_builtins.set(BuiltInPosition);
				...
				if (var_type.basetype == SPIRType::Struct)
				{
					...
					for (uint32_t mbr_idx = 0; mbr_idx < mbr_cnt; mbr_idx++)
					{
						...
						if (is_builtin && builtin == BuiltInPosition) active_output_builtins.set(BuiltInPosition);
					}
				}
			}
		});
...
void CompilerMSL::mark_implicit_builtin(StorageClass storage, BuiltIn builtin, uint32_t id)
{
	...
	if (find(begin(var), end(var), VariableID(id)) == end(var)) var.push_back(id);
}
...
{
	if (is_tessellating_triangles()) return "MTLTriangleTessellationFactorsHalf";
...
{
	if (uint_type_id != 0) return uint_type_id;
...
void CompilerMSL::emit_entry_point_declarations()
{
	...
	for (auto& samp : constexpr_samplers_by_id)
	{
		...
		if (type.basetype == SPIRType::Sampler) add_resource_name(samp.first);

		SmallVector<string> args;
		auto&				s = samp.second;

		if (s.coord != MSL_SAMPLER_COORD_NORMALIZED) args.push_back("coord::pixel");

		if (s.min_filter == s.mag_filter)
		{
			if (s.min_filter != MSL_SAMPLER_FILTER_NEAREST) args.push_back("filter::linear");
		}
		else
		{
			if (s.min_filter != MSL_SAMPLER_FILTER_NEAREST) args.push_back("min_filter::linear");
			if (s.mag_filter != MSL_SAMPLER_FILTER_NEAREST) args.push_back("mag_filter::linear");
		}
		...
		{
			if (s.s_address != MSL_SAMPLER_ADDRESS_CLAMP_TO_EDGE) args.push_back(create_sampler_address("", s.s_address));
		}
		else
		{
			if (s.s_address != MSL_SAMPLER_ADDRESS_CLAMP_TO_EDGE) args.push_back(create_sampler_address("s_", s.s_address));
			if (s.t_address != MSL_SAMPLER_ADDRESS_CLAMP_TO_EDGE) args.push_back(create_sampler_address("t_", s.t_address));
			if (s.r_address != MSL_SAMPLER_ADDRESS_CLAMP_TO_EDGE) args.push_back(create_sampler_address("r_", s.r_address));
		}
		...
		if (s.s_address == MSL_SAMPLER_ADDRESS_CLAMP_TO_BORDER || s.t_address == MSL_SAMPLER_ADDRESS_CLAMP_TO_BORDER || s.r_address == MSL_SAMPLER_ADDRESS_CLAMP_TO_BORDER)
		{
		...
		if (s.anisotropy_enable) args.push_back(join("max_anisotropy(", s.max_anisotropy, ")"));
		if (s.lod_clamp_enable) { args.push_back(join("lod_clamp(", format_float(s.lod_clamp_min), ", ", format_float(s.lod_clamp_max), ")")); }
	...
	for (auto& dynamic_buffer : buffers_requiring_dynamic_offset)
	{
		...
		if (!type.array.empty())
		{
			...
			else
			{
				...
				statement(get_argument_address_space(var), " ", type_to_glsl(type), "* ", to_restrict(var_id, true), name, type_to_array_glsl(type), " =");

				uint32_t dim = uint32_t(type.array.size());
				uint32_t j	 = 0;
				for (SmallVector<uint32_t> indices(type.array.size()); indices[type.array.size() - 1] < to_array_size_literal(type); j++)
				{
					while (dim > 0)
					{
						begin_scope();
					...
					statement("(",
							  get_argument_address_space(var),
							  " ",
							  type_to_glsl(type),
							  "* ",
							  to_restrict(var_id, false),
							  ")((",
							  get_argument_address_space(var),
							  " char* ",
							  to_restrict(var_id, false),
							  ...
							  arrays,
							  " + ",
							  to_name(dynamic_offsets_buffer_id),
							  "[",
							  base_index + j,
							  "]),");
	...
	for (SPIRVariable* arg : entry_point_bindings)
	{
		...
		{
			if (msl_options.argument_buffers_tier < Options::ArgumentBuffersTier::Tier2) { SPIRV_CROSS_THROW("Unsized array of descriptors requires argument buffer tier 2"); }
			...
			case SPIRType::Struct:
				statement("spvDescriptorArray<", get_argument_address_space(var), " ", type_to_glsl(buffer_type), "*> ", name, " {", resource_name, "};");
				break;
			default:
				break;
			}
		...
		else if (!type.array.empty() && type.basetype == SPIRType::Struct)
		{
			...
			statement(get_argument_address_space(var), " ", type_to_glsl(buffer_type), "* ", to_restrict(var.self, true), name, "[] =");
			begin_scope();
	...
	if (has_runtime_array_declaration) statement_no_indent("");
	...
	for (auto& var_pair : buffer_aliases_argument)
	{
		...
		if (type.array.empty()) { statement(addr_space, " auto& ", to_restrict(var_id, true), to_name(var_id), " = (", addr_space, " ", type_to_glsl(type), "&)", ir.meta[alias_id].decoration.qualified_alias, ";"); }
		else
		{
			...
			is_using_builtin_array = true;
			statement(desc_addr_space, " auto& ", to_restrict(var_id, true), to_name(var_id), " = (", addr_space, " ", type_to_glsl(type), "* ", desc_addr_space, " (&)", type_to_array_glsl(type), ")", ir.meta[alias_id].decoration.qualified_alias, ";");
			is_using_builtin_array = false;
		}
	}
...
string CompilerMSL::compile()
{
	...
	if (needs_manual_helper_invocation_updates() && (active_input_builtins.get(BuiltInHelperInvocation) || needs_helper_invocation))
	{
		string discard_expr		= join(builtin_to_glsl(BuiltInHelperInvocation, StorageClassInput), " = true, discard_fragment()");
	...
	if (msl_options.force_active_argument_buffer_resources) activate_argument_buffer_resources();

	if (swizzle_buffer_id) add_active_interface_variable(swizzle_buffer_id);
	if (buffer_size_buffer_id) add_active_interface_variable(buffer_size_buffer_id);
	if (view_mask_buffer_id) add_active_interface_variable(view_mask_buffer_id);
	if (dynamic_offsets_buffer_id) add_active_interface_variable(dynamic_offsets_buffer_id);
	if (builtin_layer_id) add_active_interface_variable(builtin_layer_id);
	if (builtin_dispatch_base_id && !msl_options.supports_msl_version(1, 2)) add_active_interface_variable(builtin_dispatch_base_id);
	if (builtin_sample_mask_id) add_active_interface_variable(builtin_sample_mask_id);
	...
	if (is_tese_shader()) patch_stage_in_var_id = add_interface_block(StorageClassInput, true);

	if (is_tesc_shader()) stage_out_ptr_var_id = add_interface_block_pointer(stage_out_var_id, StorageClassOutput);
	if (is_tessellation_shader()) stage_in_ptr_var_id = add_interface_block_pointer(stage_in_var_id, StorageClassInput);

	// Metal vertex functions that define no output must disable rasterization and return void.
	if (!stage_out_var_id) is_rasterization_disabled = true;
	...
	{
		if (!msl_options.supports_msl_version(2, 0)) SPIRV_CROSS_THROW("Argument buffers can only be used with MSL 2.0 and up.");
...
void CompilerMSL::preprocess_op_codes()
{
	...
	if ((preproc.uses_buffer_write && !msl_options.supports_msl_version(2, 1)) || (preproc.uses_image_write && !msl_options.supports_msl_version(2, 2))) is_rasterization_disabled = true;
	...
	if (preproc.needs_subgroup_invocation_id) needs_subgroup_invocation_id = true;
	if (preproc.needs_subgroup_size) needs_subgroup_size = true;
	...
	if (preproc.needs_sample_id || msl_options.force_sample_rate_shading || (is_sample_rate() && (active_input_builtins.get(BuiltInFragCoord) || (need_subpass_input_ms && !msl_options.use_framebuffer_fetch_subpasses)))) needs_sample_id = true;
	if (preproc.needs_helper_invocation) needs_helper_invocation = true;
	...
	ir.for_each_typed_id<SPIRBlock>([&preproc](uint32_t, SPIRBlock& block) {
		if (block.terminator == SPIRBlock::Kill) preproc.uses_discard = true;
	});
	...
	if (msl_options.check_discarded_frag_stores && preproc.uses_discard && (preproc.uses_buffer_write || preproc.uses_image_write))
	{
...
void CompilerMSL::localize_global_variables()
{
	...
	while (iter != global_variables.end())
	{
		...
		{
			if (!variable_is_lut(var)) entry_func.add_local_variable(v_id);
...
void CompilerMSL::extract_global_variables_from_functions()
{
	...
	ir.for_each_typed_id<SPIRVariable>([&](uint32_t, SPIRVariable& var) {
		...
		if (var.storage == StorageClassInput && has_decoration(var.self, DecorationBuiltIn))
		{
			...
			if (bi_type == BuiltInHelperInvocation && !needs_manual_helper_invocation_updates()) return;
		...
		if (var.storage == StorageClassInput || var.storage == StorageClassOutput || var.storage == StorageClassUniform || var.storage == StorageClassUniformConstant || var.storage == StorageClassPushConstant || var.storage == StorageClassStorageBuffer)
		{
	...
	for (auto& var : entry_func.local_variables)
		if (get<SPIRVariable>(var).storage != StorageClassFunction) global_var_ids.insert(var);
...
void CompilerMSL::extract_global_variables_from_function(uint32_t func_id, std::set<uint32_t>& added_arg_ids, unordered_set<uint32_t>& global_var_ids, unordered_set<uint32_t>& processed_func_ids)
{
	...
	for (auto block : func.blocks)
	{
		...
		for (auto& i : b.ops)
		{
			...
			case OpArrayLength: {
				...
				if (global_var_ids.find(base_id) != global_var_ids.end()) added_arg_ids.insert(base_id);
			...
			case OpFunctionCall: {
				...
				for (uint32_t arg_idx = 3; arg_idx < i.length; arg_idx++)
				{
					...
					if (global_var_ids.find(arg_id) != global_var_ids.end()) added_arg_ids.insert(arg_id);
				}
			...
			case OpStore: {
				...
				if (global_var_ids.find(rvalue_id) != global_var_ids.end()) added_arg_ids.insert(rvalue_id);

				if (needs_frag_discard_checks()) added_arg_ids.insert(builtin_helper_invocation_id);
			...
			case OpImageWrite: {
				...
				if (op == OpAtomicStore || op == OpImageWrite)
					...
					ptr = ops[2];
				if (global_var_ids.find(ptr) != global_var_ids.end()) added_arg_ids.insert(ptr);
				break;
			}
			...
			case OpImageTexelPointer: {
				...
				if (var)
				{
					if (atomic_image_vars_emulated.count(var->self) && !get<SPIRType>(var->basetype).array.empty()) { SPIRV_CROSS_THROW("Cannot emulate array of storage images with atomics. Use MSL 3.1 for native support."); }
			...
			case OpDemoteToHelperInvocation:
				if (needs_manual_helper_invocation_updates() && (active_input_builtins.get(BuiltInHelperInvocation) || needs_helper_invocation)) added_arg_ids.insert(builtin_helper_invocation_id);
				break;

			case OpIsHelperInvocationEXT:
				if (needs_manual_helper_invocation_updates()) added_arg_ids.insert(builtin_helper_invocation_id);
				break;
			...
			if (needs_manual_helper_invocation_updates() && b.terminator == SPIRBlock::Kill && (active_input_builtins.get(BuiltInHelperInvocation) || needs_helper_invocation)) added_arg_ids.insert(builtin_helper_invocation_id);
	...
	if (func_id != ir.default_entry_point)
	{
		...
		for (uint32_t arg_id : added_arg_ids)
		{
			...
			bool is_control_point_storage		  = !is_patch && ((is_tessellation_shader() && var.storage == StorageClassInput) || (is_tesc_shader() && var.storage == StorageClassOutput));
			bool is_patch_block_storage			  = is_patch && is_block && var.storage == StorageClassOutput;
			bool is_builtin						  = is_builtin_variable(var);
			bool variable_is_stage_io			  = !is_builtin || bi_type == BuiltInPosition || bi_type == BuiltInPointSize || bi_type == BuiltInClipDistance || bi_type == BuiltInCullDistance || p_type->basetype == SPIRType::Struct;
			...
			if (is_redirected_to_global_stage_io && var.storage == StorageClassOutput) is_redirected_to_global_stage_io = !is_stage_output_variable_masked(var);
			...
			if (is_redirected_to_global_stage_io)
			{
				...
				if (is_tese_shader() && msl_options.raw_buffer_tese_input && var.storage == StorageClassInput) set_decoration(next_id, DecorationNonWritable);
			}
...
void CompilerMSL::mark_packable_structs()
{
	ir.for_each_typed_id<SPIRVariable>([&](uint32_t, SPIRVariable& var) {
		if (var.storage != StorageClassFunction && !is_hidden_variable(var))
		{
			...
			if (type.pointer && (type.storage == StorageClassUniform || type.storage == StorageClassUniformConstant || type.storage == StorageClassPushConstant || type.storage == StorageClassStorageBuffer) &&
		...
		{
			auto* type = &this->get<SPIRType>(var.basetype);
			if (type->basetype == SPIRType::Struct) mark_as_workgroup_struct(*type);
		}
	});
	...
	ir.for_each_typed_id<SPIRType>([&](uint32_t, SPIRType& type) {
		if (type.basetype == SPIRType::Struct && type.pointer && type.storage == StorageClassPhysicalStorageBuffer) mark_as_packable(type);
	});
}
...
void CompilerMSL::mark_location_as_used_by_shader(uint32_t location, const SPIRType& type, StorageClass storage, bool fallback)
{
	...
	case StorageClassInput:
		for (uint32_t i = 0; i < count; i++)
		{
			...
			if (fallback) location_inputs_in_use_fallback.insert(location + i);
		}
		break;
	case StorageClassOutput:
		for (uint32_t i = 0; i < count; i++)
		{
			...
			if (fallback) location_outputs_in_use_fallback.insert(location + i);
		}
		break;
	default:
		return;
...
uint32_t CompilerMSL::build_extended_vector_type(uint32_t type_id, uint32_t components, SPIRType::BaseType basetype)
{
	...
	if (basetype != SPIRType::Unknown) type->basetype = basetype;
...
uint32_t CompilerMSL::build_msl_interpolant_type(uint32_t type_id, bool is_noperspective)
{
	...
	if (is_noperspective) set_decoration(new_type_id, DecorationNoPerspective);
	return new_type_id;
}

bool CompilerMSL::add_component_variable_to_interface_block(spv::StorageClass storage, const std::string& ib_var_ref, SPIRVariable& var, const SPIRType& type, InterfaceBlockMeta& meta)
{
	...
	if (has_decoration(var.self, DecorationLocation))
	{
		...
		if (location_meta_itr != end(meta.location_meta)) location_meta = &location_meta_itr->second;
	}

	// Check if we need to pad fragment output to match a certain number of components.
	if (location_meta)
	{
		bool pad_fragment_output = has_decoration(var.self, DecorationLocation) && msl_options.pad_fragment_output_components && get_entry_point().model == ExecutionModelFragment && storage == StorageClassOutput;
		...
		if (var.storage == StorageClassInput)
		{
			entry_func.fixup_hooks_in.push_back([=, &type, &var]() {
				if (!type.array.empty())
				{
					...
					{
						statement(to_name(var.self), "[", loc_off, "]", " = ", ib_var_ref, ".m_location_", location + loc_off, vector_swizzle(type_components, start_component), ";");
					}
				}
		...
		else
		{
			entry_func.fixup_hooks_out.push_back([=, &type, &var]() {
				if (!type.array.empty())
				{
					...
					for (uint32_t loc_off = 0; loc_off < array_size; loc_off++)
					{
						statement(ib_var_ref, ".m_location_", location + loc_off, vector_swizzle(type_components, start_component), " = ", to_name(var.self), "[", loc_off, "];");
					}
				}
				else { statement(ib_var_ref, ".m_location_", location, vector_swizzle(type_components, start_component), " = ", to_name(var.self), ";"); }
			});
		}
		return true;
	}
...
void CompilerMSL::add_plain_variable_to_interface_block(StorageClass storage, const string& ib_var_ref, SPIRType& ib_type, SPIRVariable& var, InterfaceBlockMeta& meta)
{
	bool	is_builtin		 = is_builtin_variable(var);
	...
	if (meta.strip_array && is_array(get<SPIRType>(type_id))) type_id = get<SPIRType>(type_id).parent_type;
	...
	if (add_component_variable_to_interface_block(storage, ib_var_ref, var, type, meta)) return;
	...
	if (padded_output || padded_input)
	{
		...
		if (padded_output)
		{
			entry_func.fixup_hooks_out.push_back([=, &var]() { statement(qual_var_name, vector_swizzle(type_components, start_component), " = ", to_name(var.self), ";"); });
		}
		else
		{
			entry_func.fixup_hooks_in.push_back([=, &var]() { statement(to_name(var.self), " = ", qual_var_name, vector_swizzle(type_components, start_component), ";"); });
		}
	}
		...
		else
		{
			if (meta.strip_array)
			{
				entry_func.fixup_hooks_in.push_back([=, &var]() {
					...
					statement(to_expression(stage_out_ptr_var_id), "[", invocation, "].", to_member_name(ib_type, index), " = ", to_expression(var.initializer), "[", invocation, "];");
				});
			}
			else
			{
	...
	if (get_decoration_bitset(var.self).get(DecorationLocation))
	{
		...
		if (comp) set_member_decoration(ib_type.self, ib_mbr_idx, DecorationComponent, comp);
	...
	if (is_builtin)
	{
		...
		if (builtin == BuiltInPosition && storage == StorageClassOutput) qual_pos_var_name = qual_var_name;
	}

	// Copy interpolation decorations if needed
	if (storage != StorageClassInput || !pull_model_inputs.count(var.self))
	{
		if (is_flat) set_member_decoration(ib_type.self, ib_mbr_idx, DecorationFlat);
		if (is_noperspective) set_member_decoration(ib_type.self, ib_mbr_idx, DecorationNoPerspective);
		if (is_centroid) set_member_decoration(ib_type.self, ib_mbr_idx, DecorationCentroid);
		if (is_sample) set_member_decoration(ib_type.self, ib_mbr_idx, DecorationSample);
	}

	set_extended_member_decoration(ib_type.self, ib_mbr_idx, SPIRVCrossDecorationInterfaceOrigID, var.self);
}

void CompilerMSL::add_composite_variable_to_interface_block(StorageClass storage, const string& ib_var_ref, SPIRType& ib_type, SPIRVariable& var, InterfaceBlockMeta& meta)
{
	auto&	 entry_func = get<SPIRFunction>(ir.default_entry_point);
	auto&	 var_type	= meta.strip_array ? get_variable_element_type(var) : get_variable_data_type(var);
	uint32_t elem_cnt	= 0;

	if (add_component_variable_to_interface_block(storage, ib_var_ref, var, var_type, meta)) return;

	if (is_matrix(var_type))
	{
		if (is_array(var_type)) SPIRV_CROSS_THROW("MSL cannot emit arrays-of-matrices in input and output variables.");

		elem_cnt = var_type.columns;
	}
	else if (is_array(var_type))
	{
		if (var_type.array.size() != 1) SPIRV_CROSS_THROW("MSL cannot emit arrays-of-arrays in input and output variables.");
	...
	if (usable_type->pointer) usable_type = &get<SPIRType>(usable_type->parent_type);
	...
	if (is_builtin) set_name(var.self, builtin_to_glsl(builtin, StorageClassFunction));
	...
	if (storage == StorageClassOutput && is_builtin && builtin == BuiltInClipDistance)
	{
		...
		if (!msl_options.enable_clip_distance_user_varying) return;
	}
	else if (!meta.strip_array)
	{
	...
	for (uint32_t i = 0; i < elem_cnt; i++)
	{
		...
		// Check if we need to pad fragment output to match a certain number of components.
		if (get_decoration_bitset(var.self).get(DecorationLocation) && msl_options.pad_fragment_output_components && get_entry_point().model == ExecutionModelFragment && storage == StorageClassOutput)
		{
		...
		if (!meta.strip_array)
		{
			...
			case StorageClassInput:
				entry_func.fixup_hooks_in.push_back([=, &var]() {
					...
					else { statement(to_name(var.self), "[", i, "] = ", ib_var_ref, ".", mbr_name, ";"); }
				});
				break;
																   ...
																   const Bitset&	   interpolation_qual)
{
	...
	BuiltIn builtin			 = BuiltInMax;
	bool	is_builtin		 = is_member_builtin(var_type, mbr_idx, &builtin);
	bool	is_flat			 = interpolation_qual.get(DecorationFlat) || has_member_decoration(var_type.self, mbr_idx, DecorationFlat) || has_decoration(var.self, DecorationFlat);
	bool	is_noperspective = interpolation_qual.get(DecorationNoPerspective) || has_member_decoration(var_type.self, mbr_idx, DecorationNoPerspective) || has_decoration(var.self, DecorationNoPerspective);
	bool	is_centroid		 = interpolation_qual.get(DecorationCentroid) || has_member_decoration(var_type.self, mbr_idx, DecorationCentroid) || has_decoration(var.self, DecorationCentroid);
	bool	is_sample		 = interpolation_qual.get(DecorationSample) || has_member_decoration(var_type.self, mbr_idx, DecorationSample) || has_decoration(var.self, DecorationSample);

	Bitset inherited_qual;
	if (is_flat) inherited_qual.set(DecorationFlat);
	if (is_noperspective) inherited_qual.set(DecorationNoPerspective);
	if (is_centroid) inherited_qual.set(DecorationCentroid);
	if (is_sample) inherited_qual.set(DecorationSample);
	...
	{
		if (is_array(mbr_type)) SPIRV_CROSS_THROW("MSL cannot emit arrays-of-matrices in input and output variables.");
	...
	{
		if (mbr_type.array.size() != 1) SPIRV_CROSS_THROW("MSL cannot emit arrays-of-arrays in input and output variables.");
	...
	if (mbr_type.basetype == SPIRType::Struct)
	{
		for (uint32_t i = 0; i < elem_cnt; i++)
		{
			...
			{
				add_composite_member_variable_to_interface_block(storage, ib_var_ref, ib_type, var, mbr_type, sub_mbr_idx, meta, mbr_name, var_chain, location, var_mbr_idx, inherited_qual);
	...
	for (uint32_t i = 0; i < elem_cnt; i++)
	{
		...
		if (has_member_decoration(var_type.self, mbr_idx, DecorationComponent)) SPIRV_CROSS_THROW("DecorationComponent on matrices and arrays is not supported.");
...
void CompilerMSL::add_plain_member_variable_to_interface_block(
	StorageClass storage, const string& ib_var_ref, SPIRType& ib_type, SPIRVariable& var, SPIRType& var_type, uint32_t mbr_idx, InterfaceBlockMeta& meta, const string& mbr_name_qual, const string& var_chain_qual, uint32_t& location, uint32_t& var_mbr_idx)
{
	auto& entry_func = get<SPIRFunction>(ir.default_entry_point);
	...
	else if (!meta.strip_array && meta.allow_local_declaration)
	{
		...
		{
		case StorageClassInput:
			entry_func.fixup_hooks_in.push_back([=]() { statement(var_chain, " = ", qual_var_name, ";"); });
			break;
	...
	if (!flatten_stage_out && var.storage == StorageClassOutput && var.initializer != ID(0) && (c = maybe_get<SPIRConstant>(var.initializer)))
	{
		if (meta.strip_array)
		{
			entry_func.fixup_hooks_in.push_back([=, &var]() {
				...
				statement(to_expression(stage_out_ptr_var_id), "[", invocation, "].", to_member_name(ib_type, index), " = ", constant_chain, ".", to_member_name(type, mbr_idx), ";");
			});
		}
		else
		{
			entry_func.fixup_hooks_in.push_back([=]() { statement(qual_var_name, " = ", constant_expression(this->get<SPIRConstant>(c->subconstants[mbr_idx])), ";"); });
		}
	}
...
void CompilerMSL::add_tess_level_input(const std::string& base_ref, const std::string& mbr_name, SPIRVariable& var)
{
	...
	if (builtin == BuiltInTessLevelOuter)
	{
		entry_func.fixup_hooks_in.push_back([=]() {
			...
			if (!triangles) statement(var_name, "[3] = ", base_ref, ".", mbr_name, "[3];");
		});
	}
	else
	{
...
void CompilerMSL::emit_local_masked_variable(const SPIRVariable& masked_var, bool strip_array)
{
	...
	if (threadgroup_storage && msl_options.multi_patch_workgroup)
	{
		...
		entry_func.fixup_hooks_in.push_back([this, &masked_var]() {
			...
			statement("threadgroup ", type_to_glsl(type), " ", "spvStorage", to_name(masked_var.self), "[", max_num_instances, "]", type_to_array_glsl(type), ";");
			...
			statement("threadgroup auto ", "&", to_name(masked_var.self), " = spvStorage", to_name(masked_var.self), "[", "(", to_expression(builtin_invocation_id_id), ".x / ", get_entry_point().output_vertices, ") % ", max_num_instances, "];");
		});
	}
	else { entry_func.add_local_variable(masked_var.self); }

	if (!threadgroup_storage) { vars_needing_early_declaration.push_back(masked_var.self); }
	else if (masked_var.initializer)
	{
		...
		{
			entry_func.fixup_hooks_in.push_back([this, &masked_var, initializer]() { statement(to_expression(masked_var.self), " = ", to_expression(initializer), ";"); });
		}
	}
}

void CompilerMSL::add_variable_to_interface_block(StorageClass storage, const string& ib_var_ref, SPIRType& ib_type, SPIRVariable& var, InterfaceBlockMeta& meta)
{
	auto& entry_func = get<SPIRFunction>(ir.default_entry_point);
	...
	if (storage == StorageClassInput && has_decoration(var.self, DecorationPerVertexKHR)) SPIRV_CROSS_THROW("PerVertexKHR decoration is not supported in MSL.");
		...
		else
		{
				...
				for (uint32_t mbr_idx = 0; mbr_idx < uint32_t(var_type.member_types.size()); mbr_idx++)
				{
					...
					if (storage == StorageClassOutput && is_stage_output_block_member_masked(var, mbr_idx, meta.strip_array))
					{
						...
						if (is_builtin && !meta.strip_array)
						{
							...
							if (var.initializer)
								if (auto* c = maybe_get<SPIRConstant>(var.initializer)) initializer = c->subconstants[mbr_idx];
					...
					else if (!is_builtin || has_active_builtin(builtin, storage))
					{
						...
						if (builtin == BuiltInClipDistance || builtin == BuiltInCullDistance) is_builtin = false;
						...
						{
							add_composite_member_variable_to_interface_block(storage, ib_var_ref, ib_type, var, var_type, mbr_idx, meta, mbr_name_qual, var_chain_qual, location, var_mbr_idx, {});
						}
						else { add_plain_member_variable_to_interface_block(storage, ib_var_ref, ib_type, var, var_type, mbr_idx, meta, mbr_name_qual, var_chain_qual, location, var_mbr_idx); }
					}
					var_mbr_idx++;
				}
			}
			...
			if (masked_block && !needs_local_declaration && (!is_builtin_variable(var) || is_tesc_shader()))
			{
				if (is_builtin_variable(var))
				{
					...
					for (uint32_t mbr_idx = 0; mbr_idx < uint32_t(var_type.member_types.size()); mbr_idx++)
					{
						set_member_name(var_type.self, mbr_idx, builtin_to_glsl(BuiltIn(get_member_decoration(var_type.self, mbr_idx, DecorationBuiltIn)), StorageClassOutput));
					}
	...
	else if (is_tese_shader() && storage == StorageClassInput && !meta.strip_array && is_builtin && (builtin == BuiltInTessLevelOuter || builtin == BuiltInTessLevelInner)) { add_tess_level_input_to_interface_block(ib_var_ref, ib_type, var); }
	else if (var_type.basetype == SPIRType::Boolean || var_type.basetype == SPIRType::Char || type_is_integral(var_type) || type_is_floating_point(var_type))
	{
		if (!is_builtin || has_active_builtin(builtin, storage))
		{
			...
			if ((!is_builtin || attribute_load_store) && storage_is_stage_io && is_composite_type) { add_composite_variable_to_interface_block(storage, ib_var_ref, ib_type, var, meta); }
			else { add_plain_variable_to_interface_block(storage, ib_var_ref, ib_type, var, meta); }
		}
	}
}
...
void CompilerMSL::fix_up_interface_member_indices(StorageClass storage, uint32_t ib_type_id)
{
	...
	if (!is_tesc_shader() && !(is_tese_shader() && storage == StorageClassInput) && !(get_execution_model() == ExecutionModelFragment && storage == StorageClassInput && !pull_model_inputs.empty())) return;
	...
	for (uint32_t i = 0; i < mbr_cnt; i++)
	{
		...
		if (type.basetype == SPIRType::Struct && (flatten_composites || is_block)) mbr_idx = get_extended_member_decoration(ib_type_id, i, SPIRVCrossDecorationInterfaceMemberIndex);
		...
		if (mbr_idx != uint32_t(-1))
		{
			...
			if (!has_extended_member_decoration(var_id, mbr_idx, SPIRVCrossDecorationInterfaceMemberIndex)) set_extended_member_decoration(var_id, mbr_idx, SPIRVCrossDecorationInterfaceMemberIndex, i);
		}
		else
		{
			// Only set the lowest InterfaceMemberIndex for each variable.
			// IB struct members will be emitted in-order w.r.t. interface member index.
			if (!has_extended_decoration(var_id, SPIRVCrossDecorationInterfaceMemberIndex)) set_extended_decoration(var_id, SPIRVCrossDecorationInterfaceMemberIndex, i);
		}
	}
}
...
uint32_t CompilerMSL::add_interface_block(StorageClass storage, bool patch)
{
	...
	bool pack_components = (storage == StorageClassInput && get_execution_model() == ExecutionModelVertex) || (storage == StorageClassOutput && get_execution_model() == ExecutionModelFragment) ||
						   (storage == StorageClassOutput && get_execution_model() == ExecutionModelVertex && capture_output_to_buffer);

	ir.for_each_typed_id<SPIRVariable>([&](uint32_t var_id, SPIRVariable& var) {
		if (var.storage != storage) return;
		...
		if (is_builtin && !is_block)
		{
			...
			builtin_is_gl_in_out = bi_type == BuiltInPosition || bi_type == BuiltInPointSize || bi_type == BuiltInClipDistance || bi_type == BuiltInCullDistance;
		}

		if (is_builtin && is_block) builtin_is_gl_in_out = true;

		uint32_t location = get_decoration(var_id, DecorationLocation);

		bool builtin_is_stage_in_out = builtin_is_gl_in_out || bi_type == BuiltInLayer || bi_type == BuiltInViewportIndex || bi_type == BuiltInBaryCoordKHR || bi_type == BuiltInBaryCoordNoPerspKHR || bi_type == BuiltInFragDepth ||
									   bi_type == BuiltInFragStencilRefEXT || bi_type == BuiltInSampleMask;

		// These builtins are part of the stage in/out structs.
		bool is_interface_block_builtin = builtin_is_stage_in_out || (is_tese_shader() && !msl_options.raw_buffer_tese_input && (bi_type == BuiltInTessLevelOuter || bi_type == BuiltInTessLevelInner));
		...
		if (is_builtin && is_active)
		{
			...
			else { is_active = has_active_builtin(bi_type, storage); }
		}
		...
		if (bi_type == BuiltInClipDistance || bi_type == BuiltInCullDistance) hidden = false;
		...
		{
			if (has_seen_barycentric) SPIRV_CROSS_THROW("Cannot declare both BaryCoordNV and BaryCoordNoPerspNV in same shader in MSL.");
			...
			if (!is_builtin)
			{
				...
				if (component != 0)
				{
					...
					else if (pack_components)
					{
						...
						if (!type.array.empty()) array_size = to_array_size_literal(type);
		...
		if (is_tese_shader() && msl_options.raw_buffer_tese_input && patch && storage == StorageClassInput && (bi_type == BuiltInTessLevelOuter || bi_type == BuiltInTessLevelInner))
		{
	...
	if (vars.empty() && !(!msl_options.raw_buffer_tese_input && storage == StorageClassInput && patch && stage_in_var_id)) return 0;
	...
	case StorageClassInput:
		...
		case ExecutionModelTessellationControl:
			...
			entry_func.fixup_hooks_in.push_back([=]() {
				...
				if (msl_options.multi_patch_workgroup)
				{
					...
					// not the TC invocation ID.
					statement("device ",
							  to_name(ir.default_entry_point),
							  "_",
							  ib_var_ref,
							  "* gl_in = &",
		...
		case ExecutionModelTessellationEvaluation:
			if (!msl_options.raw_buffer_tese_input) break;
			if (patch)
			{
				entry_func.fixup_hooks_in.push_back([=]() { statement("const device ", to_name(ir.default_entry_point), "_", ib_var_ref, "& ", ib_var_ref, " = ", patch_input_buffer_var_name, "[", to_expression(builtin_primitive_id_id), "];"); });
			}
			else
			{
				entry_func.fixup_hooks_in.push_back(
					[=]() { statement("const device ", to_name(ir.default_entry_point), "_", ib_var_ref, "* gl_in = &", input_buffer_var_name, "[", to_expression(builtin_primitive_id_id), " * ", get_entry_point().output_vertices, "];"); });
			}
			break;
		default:
			break;
	...
	case StorageClassOutput: {
		...
		if (!capture_output_to_buffer)
		{
			...
			for (auto& blk_id : entry_func.blocks)
			{
				...
				if (blk.terminator == SPIRBlock::Return || (blk.terminator == SPIRBlock::Kill && blk_id == entry_func.blocks.back())) blk.return_value = rtn_id;
			}
		...
		else
		{
			...
			case ExecutionModelTessellationControl:
				if (msl_options.multi_patch_workgroup)
				{
					...
					{
						entry_func.fixup_hooks_in.push_back([=]() {
							statement("device ", to_name(ir.default_entry_point), "_", ib_var_ref, "& ", ib_var_ref, " = ", patch_output_buffer_var_name, "[", to_expression(builtin_invocation_id_id), ".x / ", get_entry_point().output_vertices, "];");
						});
					}
					else
					{
				...
				else
				{
					if (patch)
					{
						entry_func.fixup_hooks_in.push_back([=]() { statement("device ", to_name(ir.default_entry_point), "_", ib_var_ref, "& ", ib_var_ref, " = ", patch_output_buffer_var_name, "[", to_expression(builtin_primitive_id_id), "];"); });
					}
					else
					{
						entry_func.fixup_hooks_in.push_back(
							[=]() { statement("device ", to_name(ir.default_entry_point), "_", ib_var_ref, "* gl_out = &", output_buffer_var_name, "[", to_expression(builtin_primitive_id_id), " * ", get_entry_point().output_vertices, "];"); });
					}
				}
				break;
			default:
	...
	if (((is_tesc_shader() && msl_options.multi_patch_workgroup) || (is_tese_shader() && msl_options.raw_buffer_tese_input)) && storage == StorageClassInput)
	{
		...
		{
			if (location_inputs_in_use.count(input.first.location) != 0) continue;

			if (patch != (input.second.rate == MSL_SHADER_VARIABLE_RATE_PER_PATCH)) continue;

			// Tessellation levels have their own struct, so there's no need to add them here.
			if (input.second.builtin == BuiltInTessLevelOuter || input.second.builtin == BuiltInTessLevelInner) continue;
			...
			if (input.first.component) set_decoration(var_id, DecorationComponent, input.first.component);
	...
	if (capture_output_to_buffer && storage == StorageClassOutput)
	{
		...
		{
			if (location_outputs_in_use.count(output.first.location) != 0) continue;
			...
			if (output.first.component) set_decoration(var_id, DecorationComponent, output.first.component);
	...
	for (auto& loc : meta.location_meta)
	{
		...
		if (location_meta.flat) set_member_decoration(ib_type.self, ib_mbr_idx, DecorationFlat);
		if (location_meta.noperspective) set_member_decoration(ib_type.self, ib_mbr_idx, DecorationNoPerspective);
		if (location_meta.centroid) set_member_decoration(ib_type.self, ib_mbr_idx, DecorationCentroid);
		if (location_meta.sample) set_member_decoration(ib_type.self, ib_mbr_idx, DecorationSample);
	}
	...
	if (storage == StorageClassInput) set_decoration(ib_var_id, DecorationNonWritable);
...
uint32_t CompilerMSL::add_interface_block_pointer(uint32_t ib_var_id, StorageClass storage)
{
	...
	if (is_tesc_shader() || (is_tese_shader() && msl_options.raw_buffer_tese_input))
	{
		...
		ib_ptr_type.storage = storage == StorageClassInput ? ((is_tesc_shader() && msl_options.multi_patch_workgroup) || (is_tese_shader() && msl_options.raw_buffer_tese_input) ? StorageClassStorageBuffer : StorageClassWorkgroup) : StorageClassStorageBuffer;
		...
		if (storage == StorageClassInput) set_decoration(ib_ptr_var_id, DecorationNonWritable);
	}
	else
	{
...
uint32_t CompilerMSL::ensure_correct_builtin_type(uint32_t type_id, BuiltIn builtin)
{
	...
	if ((builtin == BuiltInSampleMask && is_array(pointee_type)) || ((builtin == BuiltInLayer || builtin == BuiltInViewportIndex || builtin == BuiltInFragStencilRefEXT) && pointee_type.basetype != SPIRType::UInt))
	{
		...
		if (!is_pointer(type)) return base_type_id;
...
uint32_t CompilerMSL::ensure_correct_input_type(uint32_t type_id, uint32_t location, uint32_t component, uint32_t num_components, bool strip_array)
{
	...
	if (type.basetype == SPIRType::Struct || type.array.size() > max_array_dimensions) return type_id;
	...
	if (num_components == 0) num_components = p_va->second.vecsize;
	...
	default:
		if (num_components > type.vecsize) type_id = build_extended_vector_type(type_id, num_components);
		break;
	}
...
{
	// Handle possible recursion when a struct contains a pointer to its own type nested somewhere.
	if (has_extended_decoration(type.self, SPIRVCrossDecorationPhysicalTypePacked)) return;
...
void CompilerMSL::mark_scalar_layout_structs(const SPIRType& type)
{
	...
	for (uint32_t i = 0; i < mbr_cnt; i++)
	{
		...
		if (mbr_type.basetype == SPIRType::Struct && !(mbr_type.pointer && mbr_type.storage == StorageClassPhysicalStorageBuffer))
		{
			...
			if (has_extended_decoration(struct_type->self, SPIRVCrossDecorationPhysicalTypePacked)) continue;
			...
			if (!mbr_type.array.empty())
			{
				...
				if (get_declared_struct_size_msl(*struct_type) > array_stride) struct_is_too_large = true;
			}

			if (struct_is_misaligned || struct_is_too_large) mark_struct_members_packed(*struct_type);
			...
			if (struct_needs_explicit_padding)
			{
				...
				if (array_stride < msl_size) { SPIRV_CROSS_THROW("Cannot express an array stride smaller than size of struct type."); }
				else
				{
					if (has_extended_decoration(struct_type->self, SPIRVCrossDecorationPaddingTarget))
					{
						if (array_stride != get_extended_decoration(struct_type->self, SPIRVCrossDecorationPaddingTarget)) SPIRV_CROSS_THROW("A struct is used with different array strides. Cannot express this in MSL.");
					}
					else
...
void CompilerMSL::align_struct(SPIRType& ib_type, unordered_set<uint32_t>& aligned_structs)
{
	...
	if (aligned_structs.count(ib_type_id)) return;
	...
	for (uint32_t mbr_idx = 0; mbr_idx < mbr_cnt; mbr_idx++)
	{
		...
		if (mbr_type.basetype == SPIRType::Struct) align_struct(mbr_type, aligned_structs);
	}
	...
	for (uint32_t mbr_idx = 0; mbr_idx < mbr_cnt; mbr_idx++)
	{
		...
		if (mbr_idx + 1 < mbr_cnt) msl_offset = aligned_msl_offset + get_declared_struct_member_size_msl(ib_type, mbr_idx);
	}
}
...
bool CompilerMSL::validate_member_packing_rules_msl(const SPIRType& type, uint32_t index) const
{
	...
	if (index + 1 < type.member_types.size())
	{
		...
		if (msl_mbr_size > maximum_size) return false;
	}
	...
	if (!mbr_type.array.empty())
	{
		...
		if (!relax_array_stride)
		{
			...
			if (spirv_array_stride != msl_array_stride) return false;
		}
	}
	...
	if (is_matrix(mbr_type))
	{
		...
		if (spirv_matrix_stride != msl_matrix_stride) return false;
	}

	// Now, we check alignment.
	uint32_t msl_alignment = get_declared_struct_member_alignment_msl(type, index);
	if ((spirv_offset % msl_alignment) != 0) return false;
...
{
	if (validate_member_packing_rules_msl(ib_type, index)) return;
	...
	if (mbr_type.basetype == SPIRType::Struct && !is_buff_ptr) SPIRV_CROSS_THROW("Cannot perform any repacking for structs when it is used as a member of another struct.");

	// Perform remapping here.
	// There is nothing to be gained by using packed scalars, so don't attempt it.
	if (!is_scalar(ib_type)) set_extended_member_decoration(ib_type.self, index, SPIRVCrossDecorationPhysicalTypePacked);
	...
	else if (is_matrix(mbr_type))
	{
		...
		if (elems_per_stride == 8)
		{
			if (mbr_type.basetype != SPIRType::Half) SPIRV_CROSS_THROW("Unexpected type in std140 wide matrix stride resolve.");
			add_spv_func_and_recompile(SPVFuncImplPaddedStd140);
		}
	...
	if (is_array(type))
	{
		...
		{
			if (!type.array_size_literal.back()) SPIRV_CROSS_THROW("Cannot apply scalar layout workaround with spec constant array size.");
			type.array.back() -= 1;
		}
		else
		{
	...
	else if (is_matrix(type))
	{
		...
		if (!row_major)
		{
			...
			if (type.columns > 2) { type.columns--; }
		...
		else
		{
			...
			if (type.vecsize > 2) { type.vecsize--; }
	...
	if (!validate_member_packing_rules_msl(ib_type, index)) SPIRV_CROSS_THROW("Found a buffer packing case which we cannot represent in MSL.");
}
...
void CompilerMSL::emit_store_statement(uint32_t lhs_expression, uint32_t rhs_expression)
{
	...
	if (has_decoration(lhs_expression, DecorationBuiltIn) && BuiltIn(get_decoration(lhs_expression, DecorationBuiltIn)) == BuiltInSampleMask && is_array(type))
	{
	...
	else if (!lhs_remapped_type && !lhs_packed_type)
	{
		...
		else if (lhs_e && lhs_e->need_transpose)
		{
			...
			for (uint32_t c = 0; c < type.vecsize; c++)
			{
				...
				if (column_index != string::npos) { statement(lhs_expr.insert(column_index, join('[', c, ']')), " = ", to_extract_component_expression(rhs_expression, c), ";"); }
			}
			lhs_e->need_transpose = true;
			register_write(lhs_expression);
		}
	...
	else
	{
		...
		if (is_matrix(type))
		{
			...
			if (rhs_transpose) rhs_e->need_transpose = false;
			...
			if (transpose)
			{
				...
				if (physical_type.columns != type.columns) cast_expr = join("(", cast_addr_space, " ", packed_pfx, type_to_glsl(write_type), "&)");
				...
				if (rhs_transpose)
				{
					...
					{
						statement(cast_expr, to_enclosed_expression(lhs_expression), "[", i, "]", " = ", to_unpacked_row_major_matrix_expression(rhs_expression), "[", i, "];");
					}
				}
				else
				{
					...
					for (uint32_t i = 0; i < type.vecsize; i++)
					{
						...
						for (uint32_t j = 0; j < vector_type.vecsize; j++)
						{
							...
							if (j + 1 < vector_type.vecsize) rhs_row += ", ";
						}
						rhs_row += ")";
			...
			else
			{
				...
				if (physical_type.vecsize != type.vecsize) cast_expr = join("(", cast_addr_space, " ", packed_pfx, type_to_glsl(write_type), "&)");
				...
				else
				{
					...
					for (uint32_t i = 0; i < type.columns; i++)
					{
						statement(cast_expr, to_enclosed_expression(lhs_expression), "[", i, "]", " = ", to_enclosed_unpacked_expression(rhs_expression), "[", i, "];");
					}
				}
			}

			// We're dealing with transpose manually.
			if (rhs_transpose) rhs_e->need_transpose = true;
		}
		else if (transpose)
		{
			lhs_e->need_transpose = false;
			...
			for (uint32_t c = 0; c < type.vecsize; c++)
			{
				...
				if (end_dot_index != string::npos && end_dot_index > end_column_index) lhs_expr.resize(end_dot_index);

				if (column_index != string::npos) { statement("((", cast_addr_space, " ", type_to_glsl(write_type), "*)&", lhs_expr.insert(column_index, join('[', c, ']', ")")), " = ", to_extract_component_expression(rhs_expression, c), ";"); }
			}
		...
		else if ((is_matrix(physical_type) || is_array(physical_type)) && physical_type.vecsize <= 4 && physical_type.vecsize > type.vecsize)
		{
			...
			if (!optimize_read_modify_write(expression_type(rhs_expression), lhs, rhs)) statement(lhs, " = ", rhs, ";");
		}
		else if (!is_matrix(type))
		{
			string lhs = to_dereferenced_expression(lhs_expression);
...
string CompilerMSL::unpack_expression_type(string expr_str, const SPIRType& type, uint32_t physical_type_id, bool packed, bool row_major)
{
	...
	if (physical_type_id == 0 && !packed) return expr_str;

	const SPIRType* physical_type = nullptr;
	if (physical_type_id) physical_type = &get<SPIRType>(physical_type_id);
	...
	if (physical_type && is_vector(*physical_type) && is_array(*physical_type) && !uses_std140_wrapper && physical_type->vecsize > type.vecsize && !expression_ends_with(expr_str, swizzle_lut[type.vecsize - 1]))
	{
	...
	else if (is_matrix(type))
	{
		...
		if (!physical_type) physical_type = &type;

		uint32_t vecsize = type.vecsize;
		uint32_t columns = type.columns;
		if (row_major) swap(vecsize, columns);
		...
		if (physical_vecsize != vecsize) load_swiz = swizzle_lut[vecsize - 1];
	...
	else { return join(type_to_glsl(type), "(", expr_str, ")"); }
}
...
void CompilerMSL::emit_header()
{
	...
	if (suppress_missing_prototypes) statement("#pragma clang diagnostic ignored \"-Wmissing-prototypes\"");
	if (suppress_incompatible_pointer_types_discard_qualifiers) statement("#pragma clang diagnostic ignored \"-Wincompatible-pointer-types-discards-qualifiers\"");

	// Disable warning about missing braces for array<T> template to make arrays a value type
	if (spv_function_implementations.count(SPVFuncImplUnsafeArray) != 0) statement("#pragma clang diagnostic ignored \"-Wmissing-braces\"");

	for (auto& pragma : pragma_lines)
		statement(pragma);

	if (!pragma_lines.empty() || suppress_missing_prototypes) statement("");
		...
		case SPVFuncImplStorageMatrix:
			...
			for (size_t method_idx = 0; method_idx < sizeof(address_spaces) / sizeof(address_spaces[0]); ++method_idx)
			{
				...
				for (size_t param_idx = 0; param_idx < sizeof(address_spaces) / sizeof(address_spaces[0]); ++param_idx)
				{
					...
					if (param_idx != method_idx && param_idx >= 4) statement("#endif");
					statement("");
				}
				...
				if (method_idx >= 4) statement("#endif");
				statement("");
			}
			end_scope_decl();
			statement("");
...
{
	if (spv_function_implementations.count(SPVFuncImplArrayCopyMultidim)) spv_function_implementations.insert(SPVFuncImplArrayCopy);
	...
	if (spv_function_implementations.count(SPVFuncImplDynamicImageSampler))
	{
		...
		if (!msl_options.supports_msl_version(2)) SPIRV_CROSS_THROW("spvDynamicImageSampler requires default-constructible texture objects, which require MSL 2.0.");
		spv_function_implementations.insert(SPVFuncImplForwardArgs);
		spv_function_implementations.insert(SPVFuncImplTextureSwizzle);
		if (msl_options.swizzle_texture_samples) spv_function_implementations.insert(SPVFuncImplGatherSwizzle);
	...
	for (uint32_t i = SPVFuncImplChromaReconstructNearest2Plane; i <= SPVFuncImplChromaReconstructLinear420XMidpointYMidpoint3Plane; i++)
		if (spv_function_implementations.count(static_cast<SPVFuncImpl>(i))) spv_function_implementations.insert(SPVFuncImplForwardArgs);

	if (spv_function_implementations.count(SPVFuncImplTextureSwizzle) || spv_function_implementations.count(SPVFuncImplGatherSwizzle) || spv_function_implementations.count(SPVFuncImplGatherCompareSwizzle))
	{
		spv_function_implementations.insert(SPVFuncImplForwardArgs);
	...
	for (const auto& spv_func : spv_function_implementations)
	{
		...
		case SPVFuncImplArrayCopyMultidim: {
			...
			for (uint32_t variant = 0; variant < 12; variant++)
			{
				...
				statement("inline void spvArrayCopy", function_name_tags[variant], "(", dst_address_space[variant], " T (&dst)", dim, ", ", src_address_space[variant], " T (&src)", dim, ")");
				begin_scope();
		...
		case SPVFuncImplImage2DAtomicCoords: {
			if (msl_options.supports_msl_version(1, 2))
			{
				...
				statement("constant uint spvLinearTextureAlignment = ", "is_function_constant_defined(spvLinearTextureAlignmentOverride) ? ", "spvLinearTextureAlignmentOverride : ", msl_options.r32ui_linear_texture_alignment, ";");
			}
			else
			{
				statement("// The required alignment of a linear texture of R32Uint format.");
			...
			statement("#define spvImage2DAtomicCoord(tc, tex) (((((tex).get_width() + ", " spvLinearTextureAlignment / 4 - 1) & ~(", " spvLinearTextureAlignment / 4 - 1)) * (tc).y) + (tc).x)");
			statement("");
			break;
		}
		...
		case SPVFuncImplSubgroupBallot:
			...
			begin_scope();
			if (msl_options.use_quadgroup_operation()) { statement("return uint4((quad_vote::vote_t)quad_ballot(value), 0, 0, 0);"); }
		...
		case SPVFuncImplSubgroupBallotFindLSB:
			...
			begin_scope();
			if (msl_options.is_ios()) { statement("uint4 mask = uint4(extract_bits(0xFFFFFFFF, 0, gl_SubgroupSize), uint3(0));"); }
			else
			{
		...
		case SPVFuncImplSubgroupBallotBitCount:
			...
			begin_scope();
			if (msl_options.is_ios()) { statement("uint4 mask = uint4(extract_bits(0xFFFFFFFF, 0, gl_SubgroupInvocationID + 1), uint3(0));"); }
			else
			{
			...
			begin_scope();
			if (msl_options.is_ios()) { statement("uint4 mask = uint4(extract_bits(0xFFFFFFFF, 0, gl_SubgroupInvocationID), uint2(0));"); }
			else
			{
		...
		case SPVFuncImplDynamicImageSampler:
			...
			statement("return spvChromaReconstructNearest(plane0, plane1, samp, coord, spvForward<LodOptions>(options)...);");
			...
			begin_scope();
			statement("vec<T, 4> s = spvTextureSwizzle(do_sample(coord, spvForward<LodOptions>(options)...), swizzle);");
...
static string inject_top_level_storage_qualifier(const string& expr, const string& qualifier)
{
	...
	else { return join(expr.substr(0, last_significant + 1), " ", qualifier, expr.substr(last_significant + 1, string::npos)); }
}
...
void CompilerMSL::emit_specialization_constants_and_structs()
{
	...
	ir.for_each_typed_id<SPIRType>([&](uint32_t type_id, const SPIRType& type) {
		if (type.basetype == SPIRType::Struct && has_extended_decoration(type_id, SPIRVCrossDecorationBufferBlockRepacked)) mark_scalar_layout_structs(type);
	});
	...
	ir.for_each_typed_id<SPIRConstant>([&](uint32_t, SPIRConstant& c) {
		auto& type = this->get<SPIRType>(c.constant_type);
		if (is_array(type) && has_decoration(type.self, DecorationBlock) && is_builtin_type(type)) builtin_block_type_is_required = true;
	});
	...
	for (auto& id_ : ir.ids_for_constant_undef_or_type)
	{
		...
		if (id.get_type() == TypeConstant)
		{
			...
			if (c.self == workgroup_size_id)
			{
				...
				statement("constant uint3 ", builtin_to_glsl(BuiltInWorkgroupSize, StorageClassWorkgroup), " [[maybe_unused]] = ", constant_expression(get<SPIRConstant>(workgroup_size_id)), ";");
				emitted = true;
			}
			else if (c.specialization)
			{
				...
				if (msl_options.supports_msl_version(1, 2) && has_decoration(c.self, DecorationSpecId) && !c.is_used_as_array_length)
				{
					...
					if (!unique_func_constants.count(constant_id)) unique_func_constants.insert(make_pair(constant_id, c.self));
					...
					if (unique_func_constants[constant_id] == c.self) statement("constant ", sc_type_name, " ", sc_tmp_name, " [[function_constant(", constant_id, ")]];");
					statement("constant ", sc_type_name, " ", sc_name, " = is_function_constant_defined(", sc_tmp_name, ") ? ", bitcast_expression(type, sc_tmp_type, sc_tmp_name), " : ", constant_expression(c), ";");
				}
		...
		else if (id.get_type() == TypeType)
		{
			...
			if (stage_out_var_id && get_stage_out_struct_type().self == type_id) is_declarable_struct = false;
			if (patch_stage_out_var_id && get_patch_stage_out_struct_type().self == type_id) is_declarable_struct = false;
			if (stage_in_var_id && get_stage_in_struct_type().self == type_id) is_declarable_struct = false;
			if (patch_stage_in_var_id && get_patch_stage_in_struct_type().self == type_id) is_declarable_struct = false;

			// Special case. Declare builtin struct anyways if we need to emit a threadgroup version of it.
			if (stage_out_masked_builtin_type_id == type_id) is_declarable_struct = true;
			...
			if (is_declarable_struct && declared_structs.count(type_id) == 0)
			{
				...
				if (has_extended_decoration(type_id, SPIRVCrossDecorationBufferBlockRepacked)) align_struct(type, aligned_structs);
		...
		else if (id.get_type() == TypeUndef)
		{
			...
			if (type.basetype == SPIRType::Void) return;
...
string CompilerMSL::to_ptr_expression(uint32_t id, bool register_expression_read)
{
	...
	if (!should_dereference(id)) expr = address_of_expression(expr);
	return expr;
}
...
{
	bool forward = should_forward(op0) && should_forward(op1);
	emit_op(result_type, result_id, join("(isunordered(", to_enclosed_unpacked_expression(op0), ", ", to_enclosed_unpacked_expression(op1), ") || ", to_enclosed_unpacked_expression(op0), " ", op, " ", to_enclosed_unpacked_expression(op1), ")"), forward);
...
bool CompilerMSL::emit_tessellation_io_load(uint32_t result_type_id, uint32_t id, uint32_t ptr)
{
	...
	if (ptr_type.storage != StorageClassInput && ptr_type.storage != StorageClassOutput) return false;
	if (ptr_type.storage == StorageClassOutput && is_tese_shader()) return false;

	if (has_decoration(ptr, DecorationPatch)) return false;
	...
	if (ptr_is_io_variable && is_array(result_type)) flat_data_type = true;

	if (!flat_data_type) return false;
	...
	if (!flattened_io)
	{
		...
		if (interface_index == uint32_t(-1)) return false;
		...
		for (uint32_t i = 0; i < num_control_points; i++)
		{
			...
			if (i + 1 < num_control_points) expr += ", ";
		}
		expr += " })";
	}
	else if (result_type.array.size() > 2) { SPIRV_CROSS_THROW("Cannot load tessellation IO variables with more than 2 dimensions."); }
	else if (result_type.array.size() == 2)
	{
		if (!ptr_is_io_variable) SPIRV_CROSS_THROW("Loading an array-of-array must be loaded directly from an IO variable.");
		if (interface_index == uint32_t(-1)) SPIRV_CROSS_THROW("Interface index is unknown. Cannot continue.");
		if (result_type.basetype == SPIRType::Struct || is_matrix(result_type)) SPIRV_CROSS_THROW("Cannot load array-of-array of composite type in tessellation IO.");
		...
		for (uint32_t i = 0; i < num_control_points; i++)
		{
			...
			for (uint32_t j = 0; j < array_size; j++, interface_index++)
			{
				...
				AccessChainMeta meta;
				expr += access_chain_internal(stage_in_ptr_var_id, indices, 2, ACCESS_CHAIN_INDEX_IS_LITERAL_BIT | ACCESS_CHAIN_PTR_CHAIN_BIT, &meta);
				if (!is_matrix(sub_type) && sub_type.basetype != SPIRType::Struct && expr_type.vecsize > sub_type.vecsize) expr += vector_swizzle(sub_type.vecsize, 0);

				if (j + 1 < array_size) expr += ", ";
			}
			expr += " })";
			if (i + 1 < num_control_points) expr += ", ";
		}
	...
	else if (result_type.basetype == SPIRType::Struct)
	{
		...
		if (is_array_of_struct && !ptr_is_io_variable) SPIRV_CROSS_THROW("Loading array of struct from IO variable must come directly from IO variable.");
		...
		for (uint32_t i = 0; i < num_control_points; i++)
		{
			...
			for (uint32_t j = 0; j < uint32_t(struct_type.member_types.size()); j++)
			{
				...
				if (var) { interface_index = get_extended_member_decoration(var->self, j, SPIRVCrossDecorationInterfaceMemberIndex); }
				...
				if (is_matrix(mbr_type) && ptr_type.storage == StorageClassInput)
				{
					...
					for (uint32_t k = 0; k < mbr_type.columns; k++, interface_index++)
					{
						...
						if (expr_mbr_type.vecsize > mbr_type.vecsize) expr += vector_swizzle(mbr_type.vecsize, 0);

						if (k + 1 < mbr_type.columns) expr += ", ";
					}
					expr += ")";
				}
				else if (is_array(mbr_type))
				...
				if (j + 1 < struct_type.member_types.size()) expr += ", ";
			}
			expr += " }";
			if (i + 1 < num_control_points) expr += ", ";
		}
		if (is_array_of_struct) expr += " })";
	}
	else if (is_matrix(result_type))
	{
		bool is_array_of_matrix = is_array(result_type);
		if (is_array_of_matrix && !ptr_is_io_variable) SPIRV_CROSS_THROW("Loading array of matrix from IO variable must come directly from IO variable.");
		if (interface_index == uint32_t(-1)) SPIRV_CROSS_THROW("Interface index is unknown. Cannot continue.");
		...
		if (is_array_of_matrix)
		{
			...
			for (uint32_t i = 0; i < num_control_points; i++)
			{
				...
				for (uint32_t j = 0; j < result_type.columns; j++, interface_index++)
				{
					...
					AccessChainMeta meta;
					expr += access_chain_internal(stage_in_ptr_var_id, indices, 2, ACCESS_CHAIN_INDEX_IS_LITERAL_BIT | ACCESS_CHAIN_PTR_CHAIN_BIT, &meta);
					if (expr_type.vecsize > result_type.vecsize) expr += vector_swizzle(result_type.vecsize, 0);
					if (j + 1 < result_type.columns) expr += ", ";
				}
				expr += ")";
				if (i + 1 < num_control_points) expr += ", ";
			}
		...
		else
		{
			...
			for (uint32_t i = 0; i < result_type.columns; i++, interface_index++)
			{
				expr += to_expression(ptr) + "." + to_member_name(iface_type, interface_index);
				if (expr_type.vecsize > result_type.vecsize) expr += vector_swizzle(result_type.vecsize, 0);
				if (i + 1 < result_type.columns) expr += ", ";
			}
			expr += ")";
		}
	}
...
bool CompilerMSL::emit_tessellation_access_chain(const uint32_t* ops, uint32_t length)
{
	...
	if (var)
	{
		...
		if (patch && (!is_block || is_arrayed || var->storage == StorageClassInput)) flat_data = false;
	...
	if (var && flat_data)
	{
		...
		if (builtin_variable && !is_block) bi_type = BuiltIn(get_decoration(var->self, DecorationBuiltIn));

		variable_is_flat = !builtin_variable || is_block || bi_type == BuiltInPosition || bi_type == BuiltInPointSize || bi_type == BuiltInClipDistance || bi_type == BuiltInCullDistance;
	}
	...
	if (variable_is_flat)
	{
		...
		{
			auto& ptr_type = expression_type(ptr);
			if (!is_array(ptr_type) && !is_matrix(ptr_type) && ptr_type.basetype != SPIRType::Struct) further_access_chain_is_trivial = true;
		}
		...
		if (!further_access_chain_is_trivial && (flatten_composites || is_block))
		{
			...
			{
				if (!is_array(*type) && !is_matrix(*type) && type->basetype != SPIRType::Struct) break;
				...
				if (type->basetype == SPIRType::Struct && var->storage == StorageClassOutput)
				{
					...
					for (uint32_t j = 0; j < mbr_idx; j++)
						if (!is_stage_output_block_member_masked(*var, j, true)) index++;
				}
				else
					index += c->scalar();
			...
			bool  defer_access_chain  = flatten_composites && (is_matrix(result_pointee_type) || is_array(result_pointee_type) || result_pointee_type.basetype == SPIRType::Struct);
			...
			if (!defer_access_chain)
			{
				...
				if (i < length) indices.insert(indices.end(), ops + i, ops + length);
			}
			else
			{
		...
		else
		{
			...
			bool  split_access_chain_formulation = flatten_composites && ptr_expr && ptr_expr->implied_read_expressions.size() == 2 && !further_access_chain_is_trivial;

			if (split_access_chain_formulation) { e = join(to_expression(ptr), access_chain_internal(stage_var_id, indices.data(), uint32_t(indices.size()), ACCESS_CHAIN_CHAIN_ONLY_BIT, &meta)); }
			else { e = access_chain_internal(ptr, indices.data(), uint32_t(indices.size()), 0, &meta); }
		}
		...
		if (has_extended_decoration(ops[2], SPIRVCrossDecorationTessIOOriginalInputTypeID)) expr_type = &get<SPIRType>(get_extended_decoration(ops[2], SPIRVCrossDecorationTessIOOriginalInputTypeID));
		...
		if (!is_array(*expr_type) && !is_matrix(*expr_type) && expr_type->basetype != SPIRType::Struct && expr_type->vecsize > result_ptr_type.vecsize) e += vector_swizzle(result_ptr_type.vecsize, 0);
		...
		if (meta.storage_is_packed) set_extended_decoration(ops[1], SPIRVCrossDecorationPhysicalTypePacked);
		if (meta.storage_physical_type != 0) set_extended_decoration(ops[1], SPIRVCrossDecorationPhysicalTypeID, meta.storage_physical_type);
		if (meta.storage_is_invariant) set_decoration(ops[1], DecorationInvariant);
		...
		if (expr.expression_dependencies.empty()) forwarded_temporaries.erase(ops[1]);
...
{
	if (!is_tessellating_triangles()) return false;
...
bool CompilerMSL::prepare_access_chain_for_scalar_access(std::string& expr, const SPIRType& type, spv::StorageClass storage, bool& is_packed)
{
...
bool CompilerMSL::access_chain_needs_stage_io_builtin_translation(uint32_t base)
{
	...
	if (!var || !is_tessellation_shader()) return true;
...
{
	auto* var = maybe_get_backing_variable(ops[2]);
	if (!var || !pull_model_inputs.count(var->self)) return;
	// Get the base index.
	uint32_t interface_index;
	auto&	 var_type	 = get_variable_data_type(*var);
	auto&	 result_type = get<SPIRType>(ops[0]);
	auto*	 type		 = &var_type;
	if (has_extended_decoration(ops[2], SPIRVCrossDecorationInterfaceMemberIndex)) { interface_index = get_extended_decoration(ops[2], SPIRVCrossDecorationInterfaceMemberIndex); }
	else
	{
	...
	for (uint32_t i = 3; i < length; ++i)
	{
		...
		if (!has_extended_decoration(ops[2], SPIRVCrossDecorationInterfaceMemberIndex) && i - 3 == var_type.array.size()) continue;
...
void CompilerMSL::check_physical_type_cast(std::string& expr, const SPIRType* type, uint32_t physical_type)
{
	...
	{
		if (p_physical_type->vecsize > 1) expr += ".x";
...
#define MSL_UNORD_BOP(op)		emit_binary_unord_op(ops[0], ops[1], ops[2], ops[3], #op)
	...
	case OpLoad: {
		...
		if (is_tessellation_shader())
		{
			if (!emit_tessellation_io_load(ops[0], id, ptr)) CompilerGLSL::emit_instruction(instruction);
		}
		else
		{
			// Sample mask input for Metal is not an array
			if (BuiltIn(get_decoration(ptr, DecorationBuiltIn)) == BuiltInSampleMask) set_decoration(id, DecorationBuiltIn, BuiltInSampleMask);
...
#define MSL_AFMIO(op) MSL_AFMO_IMPL(op, 1, true)
	...
	case OpImageTexelPointer: {
		...
		if (var && atomic_image_vars_emulated.count(var->self))
		{
			...
			if (type.image.dim == Dim2D) { coord = join("spvImage2DAtomicCoord(", coord, ", ", to_expression(ops[2]), ")"); }
	...
	case OpImageWrite: {
		...
		string expr;
		if (needs_frag_discard_checks()) expr = join("(", builtin_to_glsl(BuiltInHelperInvocation, StorageClassInput), " ? ((void)0) : ");
		expr += join(to_expression(img_id), ".write(", remap_swizzle(store_type, texel_type.vecsize, to_expression(texel_id)), ", ", CompilerMSL::to_function_args(args, &forward), ")");
		if (needs_frag_discard_checks()) expr += ")";
		statement(expr, ";");

		if (p_var && variable_storage_is_aliased(*p_var)) flush_all_aliased_variables();
	...
	case OpImageQuerySizeLod: {
		...
		if (img_type.basetype != SPIRType::Image) SPIRV_CROSS_THROW("Invalid type for OpImageQuerySize.");
		...
		if (opcode == OpImageQuerySizeLod)
		{
			...
			if (decl_lod != "0") lod = decl_lod;
		}

		string expr = type_to_glsl(rslt_type) + "(";
		expr += img_exp + ".get_width(" + lod + ")";

		if (img_dim == Dim2D || img_dim == DimCube || img_dim == Dim3D) expr += ", " + img_exp + ".get_height(" + lod + ")";

		if (img_dim == Dim3D) expr += ", " + img_exp + ".get_depth(" + lod + ")";

		if (img_is_array)
		{
			expr += ", " + img_exp + ".get_array_size()";
			if (img_dim == DimCube && msl_options.emulate_cube_array) expr += " / 6";
		}
	...
	case OpImageQueryLod: {
		if (!msl_options.supports_msl_version(2, 2)) SPIRV_CROSS_THROW("ImageQueryLod is only supported on MSL 2.2 and up.");
		uint32_t result_type = ops[0];
		uint32_t id			 = ops[1];
		...
		statement(to_expression(id), ".x = ", image_expr, ".calculate_clamped_lod(", sampler_expr, ", ", to_expression(coord_id), ");");
		statement(to_expression(id), ".y = ", image_expr, ".calculate_unclamped_lod(", sampler_expr, ", ", to_expression(coord_id), ");");
...
#define MSL_ImgQry(qrytype)                                                                                                                                                                                                                                        \
	...
	case OpPtrAccessChain:
		if (is_tessellation_shader())
		{
			if (!emit_tessellation_access_chain(ops, instruction.length)) CompilerGLSL::emit_instruction(instruction);
		}
		else
			CompilerGLSL::emit_instruction(instruction);
	...
	case OpStore: {
		...
		if (is_out_of_bounds_tessellation_level(ops[0])) break;
		...
		if (!maybe_emit_array_assignment(ops[0], ops[1])) CompilerGLSL::emit_instruction(instruction);
		if (needs_frag_discard_checks() && (type.storage == StorageClassStorageBuffer || type.storage == StorageClassUniform)) end_scope();
		break;
	}
	...
	case OpControlBarrier:
		...
		if (previous_instruction_opcode != OpMemoryBarrier) emit_barrier(ops[0], ops[1], ops[2]);
		break;
	...
	case OpMatrixTimesVector: {
		...
		if (e && e->need_transpose)
		{
			...
			if (opcode == OpMatrixTimesVector) { expr = join("spvFMulVectorMatrix(", to_enclosed_unpacked_expression(ops[3]), ", ", to_unpacked_row_major_matrix_expression(ops[2]), ")"); }
			else { expr = join("spvFMulMatrixVector(", to_unpacked_row_major_matrix_expression(ops[3]), ", ", to_enclosed_unpacked_expression(ops[2]), ")"); }
	...
	case OpMatrixTimesMatrix: {
		...
		if (a && b && a->need_transpose && b->need_transpose)
		{
			...
			auto expr = join("spvFMulMatrixMatrix(", enclose_expression(to_unpacked_row_major_matrix_expression(ops[3])), ", ", enclose_expression(to_unpacked_row_major_matrix_expression(ops[2])), ")");
	...
	case OpISubBorrow: {
		...
		{
			statement(to_expression(result_id), ".", to_member_name(type, 0), " = ", to_enclosed_unpacked_expression(op0), " + ", to_enclosed_unpacked_expression(op1), ";");
			statement(to_expression(result_id),
					  ".",
					  to_member_name(type, 1),
					  " = select(",
					  type_to_glsl(res_type),
					  "(1), ",
					  type_to_glsl(res_type),
					  "(0), ",
					  to_unpacked_expression(result_id),
					  ".",
					  to_member_name(type, 0),
					  " >= max(",
		...
		{
			statement(to_expression(result_id), ".", to_member_name(type, 0), " = ", to_enclosed_unpacked_expression(op0), " - ", to_enclosed_unpacked_expression(op1), ";");
			statement(to_expression(result_id), ".", to_member_name(type, 1), " = select(", type_to_glsl(res_type), "(1), ", type_to_glsl(res_type), "(0), ", to_enclosed_unpacked_expression(op0), " >= ", to_enclosed_unpacked_expression(op1), ");");
		}
		break;
	}
	...
	case OpDemoteToHelperInvocationEXT:
		if (!msl_options.supports_msl_version(2, 3)) SPIRV_CROSS_THROW("discard_fragment() does not formally have demote semantics until MSL 2.3.");
		CompilerGLSL::emit_instruction(instruction);
		break;
	...
	case OpIsHelperInvocationEXT:
		...
		emit_op(ops[0], ops[1], needs_manual_helper_invocation_updates() ? builtin_to_glsl(BuiltInHelperInvocation, StorageClassInput) : "simd_is_helper_thread()", false);
		break;

	case OpBeginInvocationInterlockEXT:
	case OpEndInvocationInterlockEXT:
		if (!msl_options.supports_msl_version(2, 0)) SPIRV_CROSS_THROW("Raster order groups require MSL 2.0.");
...
#undef MSL_RAY_QUERY_OP_INNER2
	...
	case OpSUDotAccSat: {
		...
		string exp = join(type_to_glsl(type), "(addsat(reduce_add(", type_to_glsl(input_type1), "(", vec1input, ") * ", type_to_glsl(input_type2), "(", vec2input, ")), ", bitcast_expression(pre_saturate_type, acc), "))");
...
{
	if (sparse) SPIRV_CROSS_THROW("Sparse feedback not yet supported in MSL.");
...
{
	if (get_execution_model() != ExecutionModelGLCompute && !is_tesc_shader()) return;
	...
	if (msl_options.supports_msl_version(1, 2))
	{
		...
		if (is_tesc_shader() || (mem_sem & (MemorySemanticsUniformMemoryMask | MemorySemanticsCrossWorkgroupMemoryMask))) mem_flags += "mem_flags::mem_device";
		...
		if (mem_flags.empty()) mem_flags = "mem_flags::mem_none";

		bar_stmt += mem_flags;
	}
	else
	{
		if ((mem_sem & (MemorySemanticsUniformMemoryMask | MemorySemanticsCrossWorkgroupMemoryMask)) && (mem_sem & (MemorySemanticsSubgroupMemoryMask | MemorySemanticsWorkgroupMemoryMask)))
...
bool CompilerMSL::emit_array_copy(const char* expr, uint32_t lhs_id, uint32_t rhs_id, StorageClass lhs_storage, StorageClass rhs_storage)
{
	...
	else
	{
		...
		if (lhs_var) flush_variable_declaration(lhs_var->self);
		...
		if (ir.ids[rhs_id].get_type() == TypeConstant) { is_constant = true; }
		else if (var && var->remapped_variable && var->statically_assigned && ir.ids[var->static_expression].get_type() == TypeConstant) { is_constant = true; }
		else if (rhs_storage == StorageClassUniform || rhs_storage == StorageClassUniformConstant) { is_constant = true; }
		...
		if (lhs_is_array_template && rhs_is_array_template && !msl_options.force_native_arrays) statement("spvArrayCopy", tag, "(", lhs, ".elements, ", to_expression(rhs_id), ".elements);");
...
bool CompilerMSL::maybe_emit_array_assignment(uint32_t id_lhs, uint32_t id_rhs)
{
	...
	if (!is_array(get_pointee_type(type))) return false;

	auto* var = maybe_get<SPIRVariable>(id_lhs);

	// Is this a remapped, static constant? Don't do anything.
	if (var && var->remapped_variable && var->statically_assigned) return true;
	...
	if (!emit_array_copy(nullptr, id_lhs, id_rhs, lhs_storage, rhs_storage)) return false;
...
void CompilerMSL::emit_atomic_func_op(uint32_t result_type, uint32_t result_id, const char* op, Op opcode, uint32_t mem_order_1, uint32_t mem_order_2, bool has_mem_order_2, uint32_t obj, uint32_t op1, bool op1_is_pointer, bool op1_is_literal, uint32_t op2)
{
	string exp;
	...
	if (type.width == 64) SPIRV_CROSS_THROW("MSL currently does not support 64-bit atomics.");
	...
	if (check_discard)
	{
		if (is_atomic_compare_exchange_strong)
		{
			...
			emit_uninitialized_temporary_expression(result_type, result_id);
			if (vec4_temporary_id) emit_uninitialized_temporary_expression(vec4_temporary_id + 1, vec4_temporary_id);
			statement("if (!", builtin_to_glsl(BuiltInHelperInvocation, StorageClassInput), ")");
			begin_scope();
		}
		else
	...
	if (use_native_image_atomic)
	{
		...
		if (split_index != string::npos)
		{
			...
			if ((opcode == OpAtomicUMax || opcode == OpAtomicUMin || opcode == OpAtomicSMax || opcode == OpAtomicSMin) && type.basetype != expected_type)
			{
	...
	else
	{
		...
		if (ptr_type.storage == StorageClassImage)
		{
			...
			if (decoration_flags_signal_volatile(flags)) exp += "volatile ";
			exp += "device";
		}
		else if (var && ptr_type.storage != StorageClassPhysicalStorageBuffer) { exp += get_argument_address_space(*var); }
		else
		{
	...
	if (is_atomic_compare_exchange_strong)
	{
		...
		if (vec4_temporary_id) statement(to_expression(result_id), " = ", scalar_expression, ";");
	...
	else
	{
		...
		if (op2) exp += ", " + to_expression(op2);

		if (!use_native_image_atomic)
		{
			exp += string(", ") + get_memory_order(mem_order_1);
			if (has_mem_order_2) exp += string(", ") + get_memory_order(mem_order_2);
		}

		exp += ")";

		// For some particular reason, atomics return vec4 in Metal ...
		if (use_native_image_atomic) exp += ".x";
		...
		if (expected_type != type.basetype) exp = bitcast_expression(type, expected_type, exp);
...
void CompilerMSL::emit_glsl_op(uint32_t result_type, uint32_t id, uint32_t eop, const uint32_t* args, uint32_t count)
{
	...
	case GLSLstd450InterpolateAtCentroid: {
		...
		emit_op(result_type, id, join(to_name(stage_in_var_id), ".", to_member_name(get_stage_in_struct_type(), interface_index), ".interpolate_at_centroid()", component), should_forward(args[0]));
		break;
	}
	...
	case GLSLstd450InterpolateAtSample: {
		...
		emit_op(result_type, id, join(to_name(stage_in_var_id), ".", to_member_name(get_stage_in_struct_type(), interface_index), ".interpolate_at_sample(", to_expression(args[1]), ")", component), should_forward(args[0]) && should_forward(args[1]));
		break;
	}
	...
	case GLSLstd450InterpolateAtOffset: {
		...
		emit_op(result_type, id, join(to_name(stage_in_var_id), ".", to_member_name(get_stage_in_struct_type(), interface_index), ".interpolate_at_offset(", to_expression(args[1]), " + 0.4375)", component), should_forward(args[0]) && should_forward(args[1]));
		break;
	}
	...
	case GLSLstd450Distance:
		...
		if (expression_type(args[0]).vecsize == 1)
		{
			...
			emit_op(result_type, id, join("abs(", to_enclosed_unpacked_expression(args[0]), " - ", to_enclosed_unpacked_expression(args[1]), ")"), should_forward(args[0]) && should_forward(args[1]));
			inherit_expression_dependencies(id, args[0]);
			inherit_expression_dependencies(id, args[1]);
		}
		else
	...
	case GLSLstd450Frexp: {
		...
		if (type.storage == StorageClassOutput && capture_output_to_buffer) is_thread_storage = false;
		...
		if (!is_thread_storage || (ptr && ptr->access_chain && is_scalar(expression_type(args[1]))))
		{
			...
			if (!tmp_id) tmp_id = ir.increase_bound_by(1);
...
void CompilerMSL::emit_spv_amd_shader_trinary_minmax_op(uint32_t result_type, uint32_t id, uint32_t eop, const uint32_t* args, uint32_t count)
{
	...
	if (!msl_options.supports_msl_version(2, 1)) SPIRV_CROSS_THROW("Trinary min/max functions require MSL 2.1.");
...
{
	if (func.self != ir.default_entry_point) add_function_overload(func);
	...
	if (!processing_entry_point) statement(force_inline);
	...
	if (processing_entry_point)
	{
		...
		for (auto var_id : vars_needing_early_declaration)
		{
			...
			if (!initializer) initializer = ir.increase_bound_by(1);

			// Do not override proper initializers.
			if (ir.ids[initializer].get_type() == TypeNone || ir.ids[initializer].get_type() == TypeExpression) set<SPIRExpression>(ed_var.initializer, "{}", ed_var.basetype, true);
		}
	}
	...
	for (auto& arg : func.arguments)
	{
		...
		if (var)
		{
			...
			if (arg.alias_global_variable && var->basevariable) name_id = var->basevariable;
		...
		if (arg_type.basetype == SPIRType::SampledImage && !is_dynamic_img_sampler)
		{
			...
			if (auto* constexpr_sampler = find_constexpr_sampler(name_id))
				if (constexpr_sampler->ycbcr_conversion_enable) planes = constexpr_sampler->planes;
			...
			{
				if (arg_type.array.empty() || (var ? is_var_runtime_size_array(*var) : is_runtime_size_array(arg_type))) { decl += join(", ", sampler_type(arg_type, arg.id, false), " ", to_sampler_expression(name_id)); }
				else
				{
		...
		if (&arg != &func.arguments.back()) decl += ", ";
	}
...
string CompilerMSL::to_function_name(const TextureFunctionNameArguments& args)
{
	...
	// in the swizzle case.
	if (msl_options.swizzle_texture_samples && args.base.is_gather && !is_dynamic_img_sampler && (!constexpr_sampler || !constexpr_sampler->ycbcr_conversion_enable))
	{
	...
	{
		if (constexpr_sampler->planes != 2 && constexpr_sampler->planes != 3) SPIRV_CROSS_THROW("Unhandled number of color image planes!");
	...
	else
	{
		...
		if (args.has_dref) fname += "_compare";
	}
...
string CompilerMSL::to_function_args(const TextureFunctionArguments& args, bool* p_forward)
{
	...
	case Dim1D:
		if (coord_type.vecsize > 1) tex_coords = enclose_expression(tex_coords) + ".x";
	...
	case DimBuffer:
		...
		if (msl_options.texture_buffer_native) { tex_coords = "uint(" + round_fp_tex_coords(tex_coords, coord_is_fp) + ")"; }
		else
		{
			...
			if (args.base.is_fetch)
			{
				if (msl_options.texel_buffer_texture_width > 0) { tex_coords = "spvTexelBufferCoord(" + round_fp_tex_coords(tex_coords, coord_is_fp) + ")"; }
				else { tex_coords = "spvTexelBufferCoord(" + round_fp_tex_coords(tex_coords, coord_is_fp) + ", " + to_expression(img) + ")"; }
			}
		}
	...
	case Dim2D:
		if (coord_type.vecsize > 2) tex_coords = enclose_expression(tex_coords) + ".xy";
	...
	case Dim3D:
		if (coord_type.vecsize > 3) tex_coords = enclose_expression(tex_coords) + ".xyz";
	...
	case DimCube:
		...
		if (sampling_type_needs_f32_conversion(coord_type)) tex_coords = convert_to_f32(tex_coords, 3);
	...
	if (imgtype.image.dim == DimCube && imgtype.image.arrayed && msl_options.emulate_cube_array)
	{
		...
		if (is_cube_fetch)
			...
			farg_str += ", uint(spvCubemapTo2DArrayFace(" + tex_coords + ").z) + (uint(" + round_fp_tex_coords(to_extract_component_expression(args.coord, alt_coord_component), coord_is_fp) + ") * 6u)";
	...
	else
	{
		...
		{
			// Special case for cube arrays, face and layer are packed in one dimension.
			if (imgtype.image.dim == DimCube && args.base.is_fetch) { farg_str += ", uint(" + to_extract_component_expression(args.coord, 2) + ") / 6u"; }
			else
			{
	...
	if (args.dref)
	{
		...
		string dref_expr;
		if (args.base.is_proj)
			dref_expr = join(to_enclosed_expression(args.dref), " / ", to_extract_component_expression(args.coord, alt_coord_component));
		else
			dref_expr = to_expression(args.dref);

		if (sampling_type_needs_f32_conversion(dref_type)) dref_expr = convert_to_f32(dref_expr, 1);
		...
		if (msl_options.is_macos() && bias)
		{
			...
			if (expression_is_constant_null(bias)) { bias = 0; }
			else if (!msl_options.supports_msl_version(2, 3))
			{
	...
	if (lod && (imgtype.image.dim != Dim1D || msl_options.texture_1D_as_2D))
	{
		...
		if (args.base.is_fetch) { farg_str += ", " + to_expression(lod); }
		else if (msl_options.sample_dref_lod_array_as_grad && args.dref && imgtype.image.arrayed)
		{
			...
			if (auto* combined = maybe_get<SPIRCombinedImageSampler>(img)) base_img = combined->image;
			switch (imgtype.image.dim)
			{
			case Dim1D:
				grad_opt = "gradient2d";
			...
			farg_str += join(", ", grad_opt, "(", grad_coord, "exp2(", to_expression(lod), " - 0.5) / ", extent, ", exp2(", to_expression(lod), " - 0.5) / ", extent, ")");
		}
		else { farg_str += ", level(" + to_expression(lod) + ")"; }
	}
	else if (args.base.is_fetch && !lod && (imgtype.image.dim != Dim1D || msl_options.texture_1D_as_2D) && imgtype.image.dim != DimBuffer && !imgtype.image.ms && imgtype.image.sampled != 2)
	{
	...
	if ((grad_x || grad_y) && (imgtype.image.dim != Dim1D || msl_options.texture_1D_as_2D))
	{
		...
		case Dim3D:
			...
			break;
		case DimCube:
			if (imgtype.image.arrayed && msl_options.emulate_cube_array) { grad_opt = "gradient2d"; }
	...
	if (args.min_lod)
	{
		if (!msl_options.supports_msl_version(2, 2)) SPIRV_CROSS_THROW("min_lod_clamp() is only supported in MSL 2.2+ and up.");
	...
	if (!offset_expr.empty())
	{
		...
		{
		case Dim1D:
			if (!msl_options.texture_1D_as_2D) break;
			if (offset_type->vecsize > 1) offset_expr = enclose_expression(offset_expr) + ".x";

			farg_str += join(", int2(", offset_expr, ", 0)");
			break;

		case Dim2D:
			if (offset_type->vecsize > 2) offset_expr = enclose_expression(offset_expr) + ".xy";

			farg_str += ", " + offset_expr;
			break;

		case Dim3D:
			if (offset_type->vecsize > 3) offset_expr = enclose_expression(offset_expr) + ".xyz";
	...
	if (args.component)
	{
		...
		if (imgtype.image.dim == Dim2D && offset_expr.empty()) farg_str += ", int2(0)";
		...
		if (!msl_options.swizzle_texture_samples || is_dynamic_img_sampler)
		{
			...
			{
				if (const auto* img_var = maybe_get_backing_variable(combined->image)) image_var = img_var->self;
			}
			else if (const auto* var = maybe_get_backing_variable(img)) { image_var = var->self; }

			if (image_var == 0 || !is_depth_image(expression_type(image_var), image_var)) farg_str += ", " + to_component_argument(args.component);
		}
	}
...
string CompilerMSL::to_component_argument(uint32_t id)
{
	...
	default:
		SPIRV_CROSS_THROW("The value (" + to_string(component_index) + ") of OpConstant ID " + to_string(id) + " is not a valid Component index, which must be one of 0, 1, 2, or 3.");
	}
}
...
string CompilerMSL::to_texture_op(const Instruction& i, bool sparse, bool* forward, SmallVector<uint32_t>& inherited_expressions)
{
	...
	if (constexpr_sampler && constexpr_sampler->ycbcr_conversion_enable && !is_dynamic_img_sampler)
	{
		...
		if (constexpr_sampler->ycbcr_model != MSL_SAMPLER_YCBCR_MODEL_CONVERSION_RGB_IDENTITY)
		{
			...
			if (constexpr_sampler->ycbcr_model != MSL_SAMPLER_YCBCR_MODEL_CONVERSION_YCBCR_IDENTITY) expr += ")";
		}
	}
	else
	{
...
string CompilerMSL::to_func_call_arg(const SPIRFunction::Parameter& arg, uint32_t id)
{
	...
	if (is_dynamic_img_sampler && !arg_is_dynamic_img_sampler) arg_str = join("spvDynamicImageSampler<", type_to_glsl(get<SPIRType>(type.image.type)), ">(");
	...
	if (var) var_id = var->basevariable;
	...
	if (!arg_is_dynamic_img_sampler)
	{
		...
		if (type.basetype == SPIRType::SampledImage)
		{
			...
			if (constexpr_sampler && constexpr_sampler->ycbcr_conversion_enable)
			{
				...
				if (!arg.alias_global_variable) add_spv_func_and_recompile(SPVFuncImplDynamicImageSampler);
			}
			for (uint32_t i = 1; i < planes; i++)
			...
			if (type.image.dim != DimBuffer) arg_str += ", " + to_sampler_expression(var_id ? var_id : id);
			...
			if (is_dynamic_img_sampler && constexpr_sampler && constexpr_sampler->ycbcr_conversion_enable)
			{
				...
				if (constexpr_sampler->chroma_filter != MSL_SAMPLER_FILTER_NEAREST) samp_args.push_back("spvChromaFilter::linear");

				if (constexpr_sampler->x_chroma_offset != MSL_CHROMA_LOCATION_COSITED_EVEN) samp_args.push_back("spvXChromaLocation::midpoint");
				if (constexpr_sampler->y_chroma_offset != MSL_CHROMA_LOCATION_COSITED_EVEN) samp_args.push_back("spvYChromaLocation::midpoint");
				switch (constexpr_sampler->ycbcr_model)
				{
				case MSL_SAMPLER_YCBCR_MODEL_CONVERSION_RGB_IDENTITY:
					// Default
				...
				if (constexpr_sampler->ycbcr_range != MSL_SAMPLER_YCBCR_RANGE_ITU_FULL) samp_args.push_back("spvYCbCrRange::itu_narrow");
		...
		if (is_dynamic_img_sampler && constexpr_sampler && constexpr_sampler->ycbcr_conversion_enable)
			arg_str += join(", (uint(",
							create_swizzle(constexpr_sampler->swizzle[3]),
							") << 24) | (uint(",
							create_swizzle(constexpr_sampler->swizzle[2]),
							") << 16) | (uint(",
							create_swizzle(constexpr_sampler->swizzle[1]),
							") << 8) | uint(",
		...
		if (buffer_requires_array_length(var_id)) arg_str += ", " + to_buffer_size_expression(var_id ? var_id : id);

		if (is_dynamic_img_sampler) arg_str += ")";
	}

	// Emulate texture2D atomic operations
	auto* backing_var = maybe_get_backing_variable(var_id);
	if (backing_var && atomic_image_vars_emulated.count(backing_var->self)) { arg_str += ", " + to_expression(var_id) + "_atomic"; }
...
string CompilerMSL::to_sampler_expression(uint32_t id)
{
	...
	uint32_t samp_id = 0;
	if (combined) samp_id = combined->sampler;
...
string CompilerMSL::to_buffer_size_expression(uint32_t id)
{
	...
	if (expr.size() >= 3 && expr[0] == '(' && expr[1] == '*') expr = address_of_expression(expr);
	...
	else
	{
		...
		if (auto var = maybe_get_backing_variable(id))
		{
			if (is_var_runtime_size_array(*var))
			{
				if (!msl_options.runtime_array_rich_descriptor) SPIRV_CROSS_THROW("OpArrayLength requires rich descriptor format");

				auto last_pos = array_expr.find_last_of(']');
				if (last_pos != std::string::npos) return buffer_expr + ".length(" + array_expr.substr(1, last_pos - 1) + ")";
			}
		}
...
{
	if (!has_decoration(type.self, DecorationBlock)) return false;

	for (uint32_t i = 0; i < type.member_types.size(); i++)
	{
		if (!has_member_decoration(type.self, i, DecorationPatch)) return false;
	}
...
{
	if (!is_matrix(exp_type)) { return CompilerGLSL::convert_row_major_matrix(std::move(exp_str), exp_type, physical_type_id, is_packed, relaxed); }
	else
	{
		strip_enclosed_expression(exp_str);
		if (physical_type_id != 0 || is_packed) exp_str = unpack_expression_type(exp_str, exp_type, physical_type_id, is_packed, true);
...
void CompilerMSL::emit_fixup()
{
	...
	{
		if (options.vertex.fixup_clipspace) statement(qual_pos_var_name, ".z = (", qual_pos_var_name, ".z + ", qual_pos_var_name, ".w) * 0.5;       // Adjust clip-space for Metal");

		if (options.vertex.flip_vert_y) statement(qual_pos_var_name, ".y = -(", qual_pos_var_name, ".y);", "    // Invert Y-axis for Metal");
	}
}
...
string CompilerMSL::to_struct_member(const SPIRType& type, uint32_t member_type_id, uint32_t index, const string& qualifier)
{
	...
	if (member_is_remapped_physical_type(type, index)) member_type_id = get_extended_member_decoration(type.self, index, SPIRVCrossDecorationPhysicalTypeID);
	...
	// Allow Metal to use the array<T> template to make arrays a value type
	uint32_t orig_id = 0;
	if (has_extended_member_decoration(type.self, index, SPIRVCrossDecorationInterfaceOrigID)) orig_id = get_extended_member_decoration(type.self, index, SPIRVCrossDecorationInterfaceOrigID);

	bool row_major = false;
	if (is_matrix(physical_type)) row_major = has_member_decoration(type.self, index, DecorationRowMajor);
	...
	if (member_is_packed_physical_type(type, index))
	{
		...
		if (physical_type.basetype == SPIRType::Struct) { SPIRV_CROSS_THROW("Cannot emit a packed struct currently."); }
	...
	if (physical_type.basetype == SPIRType::Image && physical_type.image.sampled == 2 && msl_options.is_ios() && msl_options.argument_buffers_tier <= Options::ArgumentBuffersTier::Tier1 && !has_decoration(orig_id, DecorationNonWritable))
	{
	...
	if (physical_type.basetype != SPIRType::Image && physical_type.basetype != SPIRType::Sampler && physical_type.basetype != SPIRType::SampledImage)
	{
		...
		if (is_ib_in_out && is_member_builtin(type, index, &builtin)) is_using_builtin_array = true;
	...
	if (orig_id)
	{
		...
		if (is_pointer(*data_type)) data_type = &get_pointee_type(*data_type);
	...
	if (declared_type->vecsize > 4)
	{
		...
		if (is_matrix(orig_type) && row_major) swap(orig_type.vecsize, orig_type.columns);
		orig_type.columns = 1;
	...
	const char* overlapping_binding_tag = has_extended_member_decoration(type.self, index, SPIRVCrossDecorationOverlappingBinding) ? "// Overlapping binding: " : "";

	auto result = join(overlapping_binding_tag, pack_pfx, decl_type, " ", qualifier, to_member_name(type, index), member_attribute_qualifier(type, index), array_type, ";");
...
string CompilerMSL::member_attribute_qualifier(const SPIRType& type, uint32_t index)
{
	...
	if (has_extended_member_decoration(type.self, index, SPIRVCrossDecorationResourceIndexPrimary))
	{
		...
		if (interlocked_resources.count(get_extended_member_decoration(type.self, index, SPIRVCrossDecorationInterfaceOrigID))) quals += ", raster_order_group(0)";
		quals += "]]";
		return quals;
	}
	...
	if (execution.model == ExecutionModelVertex && type.storage == StorageClassInput)
	{
		if (is_builtin)
		{
			...
			case BuiltInInstanceIndex:
			case BuiltInBaseInstance:
				if (msl_options.vertex_for_tessellation) return "";
		...
		if (locn != k_unknown_location) return string(" [[attribute(") + convert_to_string(locn) + ")]]";
	}

	// Vertex and tessellation evaluation function outputs
	if (((execution.model == ExecutionModelVertex && !msl_options.vertex_for_tessellation) || is_tese_shader()) && type.storage == StorageClassOutput)
	{
		if (is_builtin)
		{
			switch (builtin)
			...
			case BuiltInViewportIndex:
				if (!msl_options.supports_msl_version(2, 0)) SPIRV_CROSS_THROW("ViewportIndex requires Metal 2.0.");
				/* fallthrough */
		...
		if (!loc_qual.empty()) return join(" [[", loc_qual, "]]");
	}
	...
	if (is_tesc_shader() && type.storage == StorageClassInput)
	{
		if (is_builtin)
		{
			...
			case BuiltInPrimitiveId:
				if (msl_options.multi_patch_workgroup) return "";
				return string(" [[") + builtin_qualifier(builtin) + "]]" + (mbr_type.array.empty() ? "" : " ");
			...
			case BuiltInSubgroupSize:			   // FIXME: Should work in any stage
				if (msl_options.emulate_subgroups) return "";
				return string(" [[") + builtin_qualifier(builtin) + "]]" + (mbr_type.array.empty() ? "" : " ");
	...
	if (is_tese_shader() && type.storage == StorageClassInput)
	{
		...
		if (msl_options.raw_buffer_tese_input) return "";

		// The special control point array must not be marked with an attribute.
		if (get_type(type.member_types[index]).basetype == SPIRType::ControlPointArray) return "";
	...
	if (execution.model == ExecutionModelFragment && type.storage == StorageClassInput)
	{
		...
		if (is_builtin)
		{
			...
			{
			case BuiltInViewIndex:
				if (!msl_options.multiview || !msl_options.multiview_layered_rendering) break;
				/* fallthrough */
		...
		{
			if (has_member_decoration(type.self, index, DecorationFlat) || has_member_decoration(type.self, index, DecorationCentroid) || has_member_decoration(type.self, index, DecorationSample) ||
	...
	if (execution.model == ExecutionModelFragment && type.storage == StorageClassOutput)
	{
		if (is_builtin)
		{
			...
			case BuiltInFragStencilRefEXT:
				...
				if (!msl_options.enable_frag_stencil_ref_builtin) return "";
				if (!msl_options.supports_msl_version(2, 1)) SPIRV_CROSS_THROW("Stencil export only supported in MSL 2.1 and up.");
				return string(" [[") + builtin_qualifier(builtin) + "]]";

			case BuiltInFragDepth:
				// Ditto FragDepth.
				if (!msl_options.enable_frag_depth_builtin) return "";
				/* fallthrough */
			case BuiltInSampleMask:
				return string(" [[") + builtin_qualifier(builtin) + "]]";
		...
		if (locn != k_unknown_location && !(msl_options.enable_frag_output_mask & (1 << locn))) return "";
	...
	if (execution.model == ExecutionModelGLCompute && type.storage == StorageClassInput)
	{
		if (is_builtin)
		{
			...
			case BuiltInSubgroupLocalInvocationId: // FIXME: Should work in any stage
			case BuiltInSubgroupSize:			   // FIXME: Should work in any stage
				if (msl_options.emulate_subgroups) break;
				/* fallthrough */
...
uint32_t CompilerMSL::get_or_allocate_builtin_input_member_location(spv::BuiltIn builtin, uint32_t type_id, uint32_t index, uint32_t* comp)
{
	uint32_t loc = get_member_location(type_id, index, comp);
	if (loc != k_unknown_location) return loc;

	if (comp) *comp = k_unknown_component;
	...
	const auto location_range_in_use = [this](uint32_t location, uint32_t location_count) -> bool {
		for (uint32_t i = 0; i < location_count; i++)
			if (location_inputs_in_use.count(location + i) != 0) return true;
		return false;
	};
...
uint32_t CompilerMSL::get_or_allocate_builtin_output_member_location(spv::BuiltIn builtin, uint32_t type_id, uint32_t index, uint32_t* comp)
{
	uint32_t loc = get_member_location(type_id, index, comp);
	if (loc != k_unknown_location) return loc;
	loc = 0;
	...
	const auto location_range_in_use = [this](uint32_t location, uint32_t location_count) -> bool {
		for (uint32_t i = 0; i < location_count; i++)
			if (location_outputs_in_use.count(location + i) != 0) return true;
		return false;
	};
...
string CompilerMSL::func_type_decl(SPIRType& type)
{
	...
	if (!processing_entry_point) return return_type;

	// If an outgoing interface block has been defined, and it should be returned, override the entry point return type
	bool ep_should_return_output = !get_is_rasterization_disabled();
	if (stage_out_var_id && ep_should_return_output) return_type = type_to_glsl(get_stage_out_struct_type()) + type_to_array_glsl(type);
	...
	{
	case ExecutionModelVertex:
		if (msl_options.vertex_for_tessellation && !msl_options.supports_msl_version(1, 2)) SPIRV_CROSS_THROW("Tessellation requires Metal 1.2.");
		entry_type = msl_options.vertex_for_tessellation ? "kernel" : "vertex";
		break;
	case ExecutionModelTessellationEvaluation:
		if (!msl_options.supports_msl_version(1, 2)) SPIRV_CROSS_THROW("Tessellation requires Metal 1.2.");
		if (execution.flags.get(ExecutionModeIsolines)) SPIRV_CROSS_THROW("Metal does not support isoline tessellation.");
...
string CompilerMSL::get_type_address_space(const SPIRType& type, uint32_t id, bool argument)
{
	...
	case StorageClassPhysicalStorageBuffer: {
		...
		bool readonly = false;
		if (!var || has_decoration(type.self, DecorationBlock)) readonly = flags.get(DecorationNonWritable);
	...
	case StorageClassPushConstant:
		...
		else if (!argument) { addr_space = "constant"; }
	...
	case StorageClassInput:
		if (is_tesc_shader() && var && var->basevariable == stage_in_ptr_var_id) addr_space = msl_options.multi_patch_workgroup ? "const device" : "threadgroup";
		...
		if (is_tese_shader() && msl_options.raw_buffer_tese_input && var)
		{
			...
			if (is_stage_in || (is_patch_stage_in && !is_tess_level)) addr_space = "const device";
		}
		if (get_execution_model() == ExecutionModelFragment && var && var->basevariable == stage_in_var_id) addr_space = "thread";
		break;
	...
	case StorageClassOutput:
		if (capture_output_to_buffer)
		{
			...
			if (!addr_space) addr_space = "device";
		}
		break;
...
string CompilerMSL::entry_point_arg_stage_in()
{
	...
	if ((is_tesc_shader() && msl_options.multi_patch_workgroup) || (is_tese_shader() && msl_options.raw_buffer_tese_input)) return decl;
...
bool CompilerMSL::is_direct_input_builtin(BuiltIn bi_type)
{
	...
	case BuiltInSubgroupSize:
		if (msl_options.fixed_subgroup_size != 0) return false;
		/* fallthrough */
...
bool CompilerMSL::is_sample_rate() const
{
	...
	return get_execution_model() == ExecutionModelFragment && (msl_options.force_sample_rate_shading || std::find(caps.begin(), caps.end(), CapabilitySampleRateShading) != caps.end() || (msl_options.use_framebuffer_fetch_subpasses && need_subpass_input_ms));
}
...
void CompilerMSL::entry_point_args_builtin(string& ep_args)
{
	...
	ir.for_each_typed_id<SPIRVariable>([&](uint32_t var_id, SPIRVariable& var) {
		...
		if (is_builtin_variable(var) && get_variable_data_type(var).basetype != SPIRType::Struct && get_variable_data_type(var).basetype != SPIRType::ControlPointArray)
		{
			...
			if (!active_input_builtins.get(bi_type) || !interface_variable_exists_in_entry_point(var_id)) return;
			...
			if (is_direct_input_builtin(bi_type))
			{
				...
				{
					if (!msl_options.supports_msl_version(2)) SPIRV_CROSS_THROW("Post-depth coverage requires MSL 2.0.");
					if (msl_options.is_macos() && !msl_options.supports_msl_version(2, 3)) SPIRV_CROSS_THROW("Post-depth coverage on Mac requires MSL 2.3.");
	...
	// Handle HLSL-style 0-based vertex/instance index.
	if (needs_base_vertex_arg == TriState::Yes) ep_args += built_in_func_arg(BuiltInBaseVertex, !ep_args.empty());

	if (needs_base_instance_arg == TriState::Yes) ep_args += built_in_func_arg(BuiltInBaseInstance, !ep_args.empty());
	...
	if (capture_output_to_buffer)
	{
		...
		{
			if (!ep_args.empty()) ep_args += ", ";
			ep_args += join("device ", type_to_glsl(get_stage_out_struct_type()), "* ", output_buffer_var_name, " [[buffer(", msl_options.shader_output_buffer_index, ")]]");
		}
		...
		if (get_execution_model() == ExecutionModelVertex && msl_options.vertex_for_tessellation && (active_input_builtins.get(BuiltInVertexIndex) || active_input_builtins.get(BuiltInVertexId)) && msl_options.vertex_index_type != Options::IndexType::None)
		{
		...
		if (is_tesc_shader())
		{
			...
			{
				if (!ep_args.empty()) ep_args += ", ";
				ep_args += join("device ", type_to_glsl(get_patch_stage_out_struct_type()), "* ", patch_output_buffer_var_name, " [[buffer(", convert_to_string(msl_options.shader_patch_output_buffer_index), ")]]");
			}
			if (!ep_args.empty()) ep_args += ", ";
			ep_args += join("device ", get_tess_factor_struct_name(), "* ", tess_factor_buffer_var_name, " [[buffer(", convert_to_string(msl_options.shader_tess_factor_buffer_index), ")]]");
			...
			ir.for_each_typed_id<SPIRVariable>([&](uint32_t, SPIRVariable& var) {
				if (!has_decoration(var.self, DecorationBuiltIn) || var.storage != StorageClassOutput || !var.initializer) return;
			...
			if (inner_factor_initializer_id && (c = maybe_get<SPIRConstant>(inner_factor_initializer_id)))
			{
				...
				if (is_tessellating_triangles())
				{
					entry_func.fixup_hooks_in.push_back([=]() { statement(builtin_to_glsl(BuiltInTessLevelInner, StorageClassOutput), " = ", "half(", to_expression(c->subconstants[0]), ");"); });
				}
				else
				{
					entry_func.fixup_hooks_in.push_back([=]() {
			...
			{
				if (!ep_args.empty()) ep_args += ", ";
				if (msl_options.multi_patch_workgroup) { ep_args += join("device ", type_to_glsl(get_stage_in_struct_type()), "* ", input_buffer_var_name, " [[buffer(", convert_to_string(msl_options.shader_input_buffer_index), ")]]"); }
				else { ep_args += join("threadgroup ", type_to_glsl(get_stage_in_struct_type()), "* ", input_wg_var_name, " [[threadgroup(", convert_to_string(msl_options.shader_input_wg_index), ")]]"); }
			}
		}
	}
	...
	if (is_tese_shader() && msl_options.raw_buffer_tese_input)
	{
		...
		{
			if (!ep_args.empty()) ep_args += ", ";
			ep_args += join("const device ", type_to_glsl(get_patch_stage_in_struct_type()), "* ", patch_input_buffer_var_name, " [[buffer(", convert_to_string(msl_options.shader_patch_input_buffer_index), ")]]");
		}

		if (tess_level_inner_var_id || tess_level_outer_var_id)
		{
			if (!ep_args.empty()) ep_args += ", ";
			ep_args += join("const device ", get_tess_factor_struct_name(), "* ", tess_factor_buffer_var_name, " [[buffer(", convert_to_string(msl_options.shader_tess_factor_buffer_index), ")]]");
		}

		if (stage_in_var_id)
		{
			if (!ep_args.empty()) ep_args += ", ";
			ep_args += join("const device ", type_to_glsl(get_stage_in_struct_type()), "* ", input_buffer_var_name, " [[buffer(", convert_to_string(msl_options.shader_input_buffer_index), ")]]");
		}
	}
}
...
const MSLConstexprSampler* CompilerMSL::find_constexpr_sampler(uint32_t id) const
{
	// Try by ID.
	{
		...
		if (itr != end(constexpr_samplers_by_id)) return &itr->second;
	}
	...
	// Try by binding.
	{
		...
		if (itr != end(constexpr_samplers_by_binding)) return &itr->second;
	}
...
void CompilerMSL::entry_point_args_discrete_descriptors(string& ep_args)
{
	...
	ir.for_each_typed_id<SPIRVariable>([&](uint32_t var_id, SPIRVariable& var) {
		if ((var.storage == StorageClassUniform || var.storage == StorageClassUniformConstant || var.storage == StorageClassPushConstant || var.storage == StorageClassStorageBuffer) && !is_hidden_variable(var))
		{
			auto&	 type	  = get_variable_data_type(var);
			uint32_t desc_set = get_decoration(var_id, DecorationDescriptorSet);
			...
			if (var.storage == StorageClassUniform || var.storage == StorageClassStorageBuffer)
			{
				...
				{
					if (get_decoration(resource.var->self, DecorationDescriptorSet) == get_decoration(var_id, DecorationDescriptorSet) && get_decoration(resource.var->self, DecorationBinding) == get_decoration(var_id, DecorationBinding) &&
						resource.basetype == SPIRType::Struct && type.basetype == SPIRType::Struct && (resource.var->storage == StorageClassUniform || resource.var->storage == StorageClassStorageBuffer))
					{
						...
						if (interlocked_resources.count(var_id)) interlocked_resources.insert(resource.var->self);
						break;
					}
				}
			}
			...
			if (atomic_image_vars_emulated.count(var.self)) { secondary_index = get_metal_resource_index(var, SPIRType::AtomicCounter, 0); }
			...
			if (type.basetype == SPIRType::SampledImage)
			{
				...
				if (constexpr_sampler && constexpr_sampler->ycbcr_conversion_enable) plane_count = constexpr_sampler->planes;

				entry_point_bindings.push_back(&var);
				for (uint32_t i = 0; i < plane_count; i++)
					resources.push_back({&var, discrete_descriptor_alias, to_name(var_id), SPIRType::Image, get_metal_resource_index(var, SPIRType::Image, i), i, secondary_index});

				if (type.image.dim != DimBuffer && !constexpr_sampler) { resources.push_back({&var, discrete_descriptor_alias, to_sampler_expression(var_id), SPIRType::Sampler, get_metal_resource_index(var, SPIRType::Sampler), 0, 0}); }
			}
			else if (!constexpr_sampler)
			{
				...
				if (!discrete_descriptor_alias) resource_index = get_metal_resource_index(var, type.basetype);
	...
	stable_sort(resources.begin(), resources.end(), [](const Resource& lhs, const Resource& rhs) { return tie(lhs.basetype, lhs.index) < tie(rhs.basetype, rhs.index); });
	...
	for (auto& r : resources)
	{
		...
		case SPIRType::Struct: {
			...
			if (r.discrete_descriptor_alias)
			{
				...
				{
					auto primary_name = join("spvBufferAliasSet", get_decoration(var_id, DecorationDescriptorSet), "Binding", get_decoration(var_id, DecorationBinding));

					// Declare the primary alias as void*
					if (!ep_args.empty()) ep_args += ", ";
					ep_args += get_argument_address_space(var) + " void* " + primary_name;
					ep_args += " [[buffer(" + convert_to_string(r.index) + ")";
					if (interlocked_resources.count(var_id)) ep_args += ", raster_order_group(0)";
					ep_args += "]]";
				}
			...
			{
				if (type.array.size() > 1) SPIRV_CROSS_THROW("Arrays of arrays of buffers are not supported.");
				...
				if (is_var_runtime_size_array(var))
				{
					...
					if ((var.storage == spv::StorageClassStorageBuffer || ssbo) && msl_options.runtime_array_rich_descriptor)
					{
						add_spv_func_and_recompile(SPVFuncImplVariableSizedDescriptor);
						ep_args += "const device spvBufferDescriptor<" + get_argument_address_space(var) + " " + type_to_glsl(type) + "*>* ";
					}
					else { ep_args += "const device spvDescriptor<" + get_argument_address_space(var) + " " + type_to_glsl(type) + "*>* "; }
				...
				else
				{
					...
					{
						if (!ep_args.empty()) ep_args += ", ";
						ep_args += get_argument_address_space(var) + " " + type_to_glsl(type) + "* " + to_restrict(var_id, true) + r.name + "_" + convert_to_string(i);
		...
		case SPIRType::Image: {
			...
			if (!type_is_msl_framebuffer_fetch(basetype))
			{
				...
				if (r.plane > 0) ep_args += join(plane_name_suffix, r.plane);
			...
			{
				if (msl_options.is_macos() && !msl_options.supports_msl_version(2, 3)) SPIRV_CROSS_THROW("Framebuffer fetch on Mac is not supported before MSL 2.3.");
				ep_args += image_type_glsl(type, var_id, false) + " " + r.name;
		...
		case SPIRType::AccelerationStructure: {
			if (is_var_runtime_size_array(var))
			{
				...
				if (!ep_args.empty()) ep_args += ", ";
				ep_args += "const device spvDescriptor<" + type_to_glsl(parent_type) + ">* " + to_restrict(var_id, true) + r.name + "_";
				ep_args += " [[buffer(" + convert_to_string(r.index) + ")]]";
			}
			else
			{
...
void CompilerMSL::fix_up_shader_inputs_outputs()
{
			...
			if (msl_options.swizzle_texture_samples && has_sampled_images && is_sampled_image_type(type))
			{
				entry_func.fixup_hooks_in.push_back([this, &type, &var, var_id]() {
					...
					else
					{
						...
						statement("constant uint", is_array_type ? "* " : "& ", to_swizzle_expression(var_id), is_array_type ? " = &" : " = ", to_name(swizzle_buffer_id), "[", convert_to_string(get_metal_resource_index(var, SPIRType::Image)), "];");
					}
				});
			}
		}
		else if ((var.storage == StorageClassStorageBuffer || (var.storage == StorageClassUniform && ssbo)) && !is_hidden_variable(var))
		{
			if (buffer_requires_array_length(var.self))
			{
				entry_func.fixup_hooks_in.push_back([this, &type, &var, var_id]() {
					...
					{
						// If we have an array of images, we need to be able to index into it, so take a pointer instead.
						statement("constant uint", is_array_type ? "* " : "& ", to_buffer_size_expression(var_id), is_array_type ? " = &" : " = ", to_name(buffer_size_buffer_id), "[", convert_to_string(get_metal_resource_index(var, type.basetype)), "];");
					}
				});
			}
		}

		if (msl_options.replace_recursive_inputs && type_contains_recursion(type) && (var.storage == StorageClassUniform || var.storage == StorageClassUniformConstant || var.storage == StorageClassPushConstant || var.storage == StorageClassStorageBuffer))
		{
	...
	ir.for_each_typed_id<SPIRVariable>([this, &entry_func](uint32_t, SPIRVariable& var) {
		...
		if (var.storage == StorageClassInput && is_builtin_variable(var) && active_input_builtins.get(bi_type))
		{
			...
			{
			case BuiltInSamplePosition:
				entry_func.fixup_hooks_in.push_back([=]() { statement(builtin_type_decl(bi_type), " ", to_expression(var_id), " = get_sample_position(", to_expression(builtin_sample_id_id), ");"); });
				break;
			case BuiltInFragCoord:
				if (is_sample_rate())
				{
					entry_func.fixup_hooks_in.push_back([=]() { statement(to_expression(var_id), ".xy += get_sample_position(", to_expression(builtin_sample_id_id), ") - 0.5;"); });
				}
				break;
			case BuiltInInvocationId:
				// This is direct-mapped without multi-patch workgroups.
				if (!is_tesc_shader() || !msl_options.multi_patch_workgroup) break;

				entry_func.fixup_hooks_in.push_back([=]() { statement(builtin_type_decl(bi_type), " ", to_expression(var_id), " = ", to_expression(builtin_invocation_id_id), ".x % ", this->get_entry_point().output_vertices, ";"); });
				break;
			case BuiltInPrimitiveId:
				...
				entry_func.fixup_hooks_in.push_back(
					[=]() { statement(builtin_type_decl(bi_type), " ", to_expression(var_id), " = min(", to_expression(builtin_invocation_id_id), ".x / ", this->get_entry_point().output_vertices, ", spvIndirectParams[1] - 1);"); });
				break;
			case BuiltInPatchVertices:
				if (is_tese_shader())
				{
					if (msl_options.raw_buffer_tese_input)
					{
						entry_func.fixup_hooks_in.push_back([=]() { statement(builtin_type_decl(bi_type), " ", to_expression(var_id), " = ", get_entry_point().output_vertices, ";"); });
					}
					else
					{
						entry_func.fixup_hooks_in.push_back([=]() { statement(builtin_type_decl(bi_type), " ", to_expression(var_id), " = ", to_expression(patch_stage_in_var_id), ".gl_in.size();"); });
					}
				}
				else
				{
					entry_func.fixup_hooks_in.push_back([=]() { statement(builtin_type_decl(bi_type), " ", to_expression(var_id), " = spvIndirectParams[0];"); });
				}
				break;
			case BuiltInTessCoord:
			...
			case BuiltInSubgroupId:
				...
				entry_func.fixup_hooks_in.push_back([=]() { statement(builtin_type_decl(bi_type), " ", to_expression(var_id), " = ", to_expression(builtin_local_invocation_index_id), ";"); });
				break;
			case BuiltInNumSubgroups:
				if (!msl_options.emulate_subgroups) break;
			...
			case BuiltInSubgroupSize:
				...
				{
					entry_func.fixup_hooks_in.push_back([=]() { statement(builtin_type_decl(bi_type), " ", to_expression(var_id), " = ", msl_options.fixed_subgroup_size, ";"); });
				}
				break;
			case BuiltInSubgroupEqMask:
				if (msl_options.is_ios() && !msl_options.supports_msl_version(2, 2)) SPIRV_CROSS_THROW("Subgroup ballot functionality requires Metal 2.2 on iOS.");
				if (!msl_options.supports_msl_version(2, 1)) SPIRV_CROSS_THROW("Subgroup ballot functionality requires Metal 2.1.");
				entry_func.fixup_hooks_in.push_back([=]() {
					if (msl_options.is_ios()) { statement(builtin_type_decl(bi_type), " ", to_expression(var_id), " = ", "uint4(1 << ", to_expression(builtin_subgroup_invocation_id_id), ", uint3(0));"); }
					else
					{
						statement(builtin_type_decl(bi_type),
								  " ",
			...
			case BuiltInSubgroupGeMask:
				if (msl_options.is_ios() && !msl_options.supports_msl_version(2, 2)) SPIRV_CROSS_THROW("Subgroup ballot functionality requires Metal 2.2 on iOS.");
				if (!msl_options.supports_msl_version(2, 1)) SPIRV_CROSS_THROW("Subgroup ballot functionality requires Metal 2.1.");
				if (msl_options.fixed_subgroup_size != 0) add_spv_func_and_recompile(SPVFuncImplSubgroupBallot);
				entry_func.fixup_hooks_in.push_back([=]() {
			...
			case BuiltInSubgroupLeMask:
				...
				add_spv_func_and_recompile(SPVFuncImplSubgroupBallot);
				entry_func.fixup_hooks_in.push_back([=]() {
					if (msl_options.is_ios()) { statement(builtin_type_decl(bi_type), " ", to_expression(var_id), " = uint4(extract_bits(0xFFFFFFFF, 0, ", to_expression(builtin_subgroup_invocation_id_id), " + 1), uint3(0));"); }
					else
					{
						statement(builtin_type_decl(bi_type),
								  " ",
			...
			case BuiltInSubgroupLtMask:
				...
				add_spv_func_and_recompile(SPVFuncImplSubgroupBallot);
				entry_func.fixup_hooks_in.push_back([=]() {
					if (msl_options.is_ios()) { statement(builtin_type_decl(bi_type), " ", to_expression(var_id), " = uint4(extract_bits(0xFFFFFFFF, 0, ", to_expression(builtin_subgroup_invocation_id_id), "), uint3(0));"); }
					else
					{
						statement(builtin_type_decl(bi_type),
								  " ",
			...
			case BuiltInViewIndex:
				...
				else if (msl_options.view_index_from_device_index)
				{
					...
					entry_func.fixup_hooks_in.push_back([=]() { statement("const ", builtin_type_decl(bi_type), " ", to_expression(var_id), " = ", msl_options.device_index, ";"); });
				...
				else if (!msl_options.multiview_layered_rendering)
				{
					...
					entry_func.fixup_hooks_in.push_back([=]() { statement("const ", builtin_type_decl(bi_type), " ", to_expression(var_id), " = ", to_expression(view_mask_buffer_id), "[0];"); });
				}
				...
				else if (get_execution_model() == ExecutionModelVertex)
				{
					...
					entry_func.fixup_hooks_in.push_back([=]() {
						statement(builtin_type_decl(bi_type),
								  ...
								  to_expression(var_id),
								  " = ",
								  to_expression(view_mask_buffer_id),
								  "[0] + (",
								  to_expression(builtin_instance_idx_id),
								  " - ",
								  to_expression(builtin_base_instance_id),
					...
					entry_func.fixup_hooks_out.push_back([=]() { statement(to_expression(builtin_layer_id), " = ", to_expression(var_id), " - ", to_expression(view_mask_buffer_id), "[0];"); });
				}
				break;
			case BuiltInDeviceIndex:
				...
				break;
			case BuiltInWorkgroupId:
				if (!msl_options.dispatch_base || !active_input_builtins.get(BuiltInWorkgroupId)) break;
				...
				entry_func.fixup_hooks_in.push_back([=]() { statement(to_expression(var_id), " += ", to_dereferenced_expression(builtin_dispatch_base_id), ";"); });
				break;
			case BuiltInGlobalInvocationId:
				if (!msl_options.dispatch_base || !active_input_builtins.get(BuiltInGlobalInvocationId)) break;
				...
				entry_func.fixup_hooks_in.push_back([=]() {
					...
					if (workgroup_size_id)
						...
						statement(to_expression(var_id), " += ", to_dereferenced_expression(builtin_dispatch_base_id), " * uint3(", execution.workgroup_size.x, ", ", execution.workgroup_size.y, ", ", execution.workgroup_size.z, ");");
				});
				break;
			case BuiltInVertexId:
			case BuiltInVertexIndex:
				// This is direct-mapped normally.
				if (!msl_options.vertex_for_tessellation) break;
				...
				entry_func.fixup_hooks_in.push_back([=]() {
					...
					{
					case Options::IndexType::None:
						statement(builtin_type_decl(bi_type), " ", to_expression(var_id), " = ", to_expression(builtin_invocation_id_id), ".x + ", to_expression(builtin_dispatch_base_id), ".x;");
						break;
					case Options::IndexType::UInt16:
					case Options::IndexType::UInt32:
						statement(builtin_type_decl(bi_type), " ", to_expression(var_id), " = ", index_buffer_var_name, "[", to_expression(builtin_invocation_id_id), ".x] + ", to_expression(builtin_dispatch_base_id), ".x;");
						break;
					}
					builtin_declaration = false;
				});
			...
			case BuiltInBaseVertex:
				...
				entry_func.fixup_hooks_in.push_back([=]() { statement(builtin_type_decl(bi_type), " ", to_expression(var_id), " = ", to_expression(builtin_dispatch_base_id), ".x;"); });
				break;
			case BuiltInInstanceId:
			case BuiltInInstanceIndex:
				// This is direct-mapped normally.
				...
				entry_func.fixup_hooks_in.push_back([=]() {
					builtin_declaration = true;
					statement(builtin_type_decl(bi_type), " ", to_expression(var_id), " = ", to_expression(builtin_invocation_id_id), ".y + ", to_expression(builtin_dispatch_base_id), ".y;");
					builtin_declaration = false;
				});
				break;
			case BuiltInBaseInstance:
				...
				entry_func.fixup_hooks_in.push_back([=]() { statement(builtin_type_decl(bi_type), " ", to_expression(var_id), " = ", to_expression(builtin_dispatch_base_id), ".y;"); });
				break;
			default:
				break;
			}
		}
		else if (var.storage == StorageClassOutput && get_execution_model() == ExecutionModelFragment && is_builtin_variable(var) && active_output_builtins.get(bi_type) && bi_type == BuiltInSampleMask && has_additional_fixed_sample_mask())
		{
			...
			entry_func.fixup_hooks_out.push_back([=]() { statement(to_expression(builtin_sample_mask_id), op_str, additional_fixed_sample_mask_str(), ";"); });
		}
	});
}
...
uint32_t CompilerMSL::get_metal_resource_index(SPIRVariable& var, SPIRType::BaseType basetype, uint32_t plane)
{
	...
	if (plane == 1) resource_decoration = SPIRVCrossDecorationResourceIndexTertiary;
	if (plane == 2) resource_decoration = SPIRVCrossDecorationResourceIndexQuaternary;
	...
	if (has_extended_decoration(var.self, resource_decoration)) return get_extended_decoration(var.self, resource_decoration);
	...
	else if (msl_options.enable_decoration_binding)
	{
		...
		if (has_decoration(var.self, DecorationBinding))
		{
			...
			if (var_binding < 0x80000000u) return var_binding;
		}
	}
	...
	if (var.storage != StorageClassPushConstant) allocate_argument_buffer_ids = descriptor_set_is_argument_buffer(var_desc_set);
...
string CompilerMSL::argument_decl(const SPIRFunction::Parameter& arg)
{
	...
	bool is_dynamic_img_sampler = !arg.alias_global_variable && type.basetype == SPIRType::SampledImage && type.image.dim == Dim2D && type_is_floating_point(get<SPIRType>(type.image.type)) && spv_function_implementations.count(SPVFuncImplDynamicImageSampler);
	...
	else if (builtin)
	{
		...
		if (storage == StorageClassInput && (builtin_type == BuiltInTessLevelInner || builtin_type == BuiltInTessLevelOuter)) { is_using_builtin_array = false; }
		else if (builtin_type != BuiltInClipDistance && builtin_type != BuiltInCullDistance) { is_using_builtin_array = true; }

		if (storage == StorageClassOutput && variable_storage_requires_stage_io(storage) && !is_stage_output_builtin_masked(builtin_type)) is_using_builtin_array = true;
	...
	else
	{
		...
		if (is_pointer(type))
		{
			...
			if (*cv_qualifier != '\0') decl += join(" ", cv_qualifier);
		}
		else { decl = join(cv_qualifier, type_to_glsl(type, arg.id)); }
	}

	if (!builtin && !is_pointer(var_type) && (type_storage == StorageClassFunction || type_storage == StorageClassGeneric))
	{
		...
		{
			if (!address_space.empty()) decl = join(address_space, " ", decl);
			decl += " ";
			decl += to_expression(name_id);
		}
	}
	else if (is_array(type) && !type_is_image)
	{
		...
		else if (is_var_runtime_size_array(var)) { decl += " " + to_expression(name_id); }
		else
		{
	...
	{
		if (is_var_runtime_size_array(var)) { decl = address_space + " " + decl + " " + to_expression(name_id); }
		else if (type.array.empty())
		{
...
string CompilerMSL::to_name(uint32_t id, bool allow_alias) const
{
	if (current_function && (current_function->self == ir.default_entry_point))
	{
		...
		if (m && !m->decoration.qualified_alias_explicit_override && !m->decoration.qualified_alias.empty()) return m->decoration.qualified_alias;
	}
...
string CompilerMSL::append_member_name(const string& qualifier, const SPIRType& type, uint32_t index)
{
	...
	BuiltIn builtin = BuiltInMax;
	if (is_member_builtin(type, index, &builtin)) return builtin_to_glsl(builtin, type.storage);
...
void CompilerMSL::replace_illegal_names()
{
	...
	ir.for_each_typed_id<SPIRVariable>([&](uint32_t self, SPIRVariable&) {
		...
		if (keywords.find(dec.alias) != end(keywords)) dec.alias += "0";
	});
	...
	ir.for_each_typed_id<SPIRFunction>([&](uint32_t self, SPIRFunction&) {
		...
		auto& dec = meta->decoration;
		if (illegal_func_names.find(dec.alias) != end(illegal_func_names)) dec.alias += "0";
	});
	...
	ir.for_each_typed_id<SPIRType>([&](uint32_t self, SPIRType&) {
		...
		for (auto& mbr_dec : meta->members)
			if (keywords.find(mbr_dec.alias) != end(keywords)) mbr_dec.alias += "0";
	});
...
void CompilerMSL::replace_illegal_entry_point_names()
{
	...
	for (auto& entry : ir.entry_points)
	{
		...
		if (illegal_func_names.find(ep_name) != end(illegal_func_names)) ep_name += "0";
...
string CompilerMSL::to_qualifiers_glsl(uint32_t id)
{
	...
	if (type.storage == StorageClassWorkgroup || (var && variable_decl_is_remapped_storage(*var, StorageClassWorkgroup))) quals += "threadgroup ";
...
string CompilerMSL::type_to_glsl(const SPIRType& type, uint32_t id, bool member)
{
	...
	if (is_pointer(type) || type_is_array_of_pointers(type))
	{
		...
		else
		{
			...
			if (is_physical_pointer(type)) is_using_builtin_array = false;
	...
	case SPIRType::Interpolant:
		return join("interpolant<", type_to_glsl(get<SPIRType>(type.parent_type), id), ", interpolation::", has_decoration(type.self, DecorationNoPerspective) ? "no_perspective" : "perspective", ">");

	// Scalars
	case SPIRType::Boolean: {
		auto* var = maybe_get_backing_variable(id);
		if (var && var->basevariable) var = &get<SPIRVariable>(var->basevariable);
	...
	case SPIRType::UInt:
		...
		break;
	case SPIRType::Int64:
		if (!msl_options.supports_msl_version(2, 2)) SPIRV_CROSS_THROW("64-bit integers are only supported in MSL 2.2 and above.");
		type_name = "long";
		break;
	case SPIRType::UInt64:
		if (!msl_options.supports_msl_version(2, 2)) SPIRV_CROSS_THROW("64-bit integers are only supported in MSL 2.2 and above.");
	...
	// Vector or Matrix?
	if (type.vecsize > 1) type_name += to_string(type.vecsize);

	if (type.array.empty() || using_builtin_array()) { return type_name; }
	else
	{
		// Allow Metal to use the array<T> template to make arrays a value type
...
{
	if (variable.storage == storage) return true;
	...
	if (storage == StorageClassWorkgroup)
	{
		...
		if (is_tesc_shader() && variable.storage == StorageClassOutput && has_decoration(get<SPIRType>(variable.basetype).self, DecorationBlock)) { return true; }
	...
	else if (storage == StorageClassStorageBuffer)
	{
		...
		if (is_tese_shader() && is_builtin_variable(variable) && (builtin == BuiltInTessCoord || builtin == BuiltInPrimitiveId)) return false;
		...
		return (variable.storage == StorageClassOutput || variable.storage == StorageClassInput) && !variable_storage_requires_stage_io(variable.storage) && (variable.storage != StorageClassOutput || !is_stage_output_variable_masked(variable));
	}
	else { return false; }
}
...
std::string CompilerMSL::sampler_type(const SPIRType& type, uint32_t id, bool member)
{
	...
	if (!type.array.empty())
	{
		if (!msl_options.supports_msl_version(2)) SPIRV_CROSS_THROW("MSL 2.0 or greater is required for arrays of samplers.");

		if (type.array.size() > 1) SPIRV_CROSS_THROW("Arrays of arrays of samplers are not supported in MSL.");
		...
		if (array_size == 0)
		{
			...
			if (member) descriptor_wrapper = "spvDescriptor";
			return join(descriptor_wrapper, "<", sampler_type(parent, id, false), ">", processing_entry_point ? "*" : "");
		}
		else { return join("array<", sampler_type(parent, id, false), ", ", array_size, ">"); }
	}
	else
		return "sampler";
}
...
string CompilerMSL::image_type_glsl(const SPIRType& type, uint32_t id, bool member)
{
	...
	if (!type.array.empty())
	{
		...
		if (type.array.size() > 1) SPIRV_CROSS_THROW("Arrays of arrays of textures are not supported in MSL.");
		...
		if (array_size == 0)
		{
			...
			if (member)
			{
				...
				if (type.basetype == SPIRType::SampledImage) SPIRV_CROSS_THROW("Argument buffer runtime array currently not supported for combined image sampler.");
			}
			return join(descriptor_wrapper, "<", image_type_glsl(parent, id, false), ">", processing_entry_point ? "*" : "");
		}
		else { return join("array<", image_type_glsl(parent, id, false), ", ", array_size, ">"); }
	}
	...
	if (is_depth_image(type, id))
	{
		...
		case Dim2D:
			...
			{
				if (!msl_options.supports_msl_version(2, 1)) SPIRV_CROSS_THROW("Multisampled array textures are supported from 2.1.");
	...
	else
	{
		...
		{
		case DimBuffer:
			if (img_type.ms || img_type.arrayed) SPIRV_CROSS_THROW("Cannot use texel buffers with multisampling or array layers.");

			if (msl_options.texture_buffer_native)
			{
				if (!msl_options.supports_msl_version(2, 1)) SPIRV_CROSS_THROW("Native texture_buffer type is only supported in MSL 2.1.");
	...
	if (type.basetype == SPIRType::Image && type.image.sampled == 2 && type.image.dim != DimSubpassData)
	{
		...
		default: {
			...
			if (p_var && p_var->basevariable) p_var = maybe_get<SPIRVariable>(p_var->basevariable);
			if (p_var && !has_decoration(p_var->self, DecorationNonWritable))
			{
				img_type_name += ", access::";

				if (!has_decoration(p_var->self, DecorationNonReadable)) img_type_name += "read_";
...
void CompilerMSL::emit_subgroup_op(const Instruction& i)
{
	...
	if (msl_options.emulate_subgroups)
	{
		...
		if (op != OpGroupNonUniformElect) SPIRV_CROSS_THROW("Subgroup emulation does not support operations other than Elect.");
	...
	if (!msl_options.supports_msl_version(2)) SPIRV_CROSS_THROW("Subgroups are only supported in Metal 2.0 and up.");
	...
	if (msl_options.is_ios() && (!msl_options.supports_msl_version(2, 3) || !msl_options.ios_use_simdgroup_functions))
	{
		...
		case OpGroupNonUniformBroadcastFirst:
			if (!msl_options.supports_msl_version(2, 2)) SPIRV_CROSS_THROW("BroadcastFirst on iOS requires Metal 2.2 and up.");
			break;
		case OpGroupNonUniformElect:
			if (!msl_options.supports_msl_version(2, 2)) SPIRV_CROSS_THROW("Elect on iOS requires Metal 2.2 and up.");
			break;
		...
		case OpSubgroupAnyKHR:
		case OpSubgroupAllEqualKHR:
			if (!msl_options.supports_msl_version(2, 2)) SPIRV_CROSS_THROW("Ballot ops on iOS requires Metal 2.2 and up.");
			break;
	...
	if (scope != ScopeSubgroup) SPIRV_CROSS_THROW("Only subgroup scope is supported.");
...
{
	if (out_type.basetype == in_type.basetype) return "";
...
string CompilerMSL::builtin_to_glsl(BuiltIn builtin, StorageClass storage)
{
	...
	case BuiltInVertexId:
		...
		if (msl_options.enable_base_index_zero && msl_options.supports_msl_version(1, 1) && (msl_options.ios_support_base_vertex_instance || msl_options.is_macos()))
		{
			if (builtin_declaration)
			{
				if (needs_base_vertex_arg != TriState::No) needs_base_vertex_arg = TriState::Yes;
				return "gl_VertexID";
			}
			else
			{
	...
	case BuiltInInstanceId:
		...
		if (msl_options.enable_base_index_zero && msl_options.supports_msl_version(1, 1) && (msl_options.ios_support_base_vertex_instance || msl_options.is_macos()))
		{
			if (builtin_declaration)
			{
				if (needs_base_instance_arg != TriState::No) needs_base_instance_arg = TriState::Yes;
	...
	case BuiltInBaseVertex:
		...
		else { SPIRV_CROSS_THROW("BaseVertex requires Metal 1.1 and Mac or Apple A9+ hardware."); }
	case BuiltInBaseInstance:
		if (msl_options.supports_msl_version(1, 1) && (msl_options.ios_support_base_vertex_instance || msl_options.is_macos()))
		{
		...
		else { SPIRV_CROSS_THROW("BaseInstance requires Metal 1.1 and Mac or Apple A9+ hardware."); }
	case BuiltInDrawIndex:
		SPIRV_CROSS_THROW("DrawIndex is not supported in MSL.");
	...
	case BuiltInFragDepth:
	case BuiltInFragStencilRefEXT:
		if ((builtin == BuiltInFragDepth && !msl_options.enable_frag_depth_builtin) || (builtin == BuiltInFragStencilRefEXT && !msl_options.enable_frag_stencil_ref_builtin)) break;
		/* fallthrough */
	case BuiltInPosition:
	case BuiltInPointSize:
	case BuiltInClipDistance:
	case BuiltInCullDistance:
	case BuiltInLayer:
		if (is_tesc_shader()) break;
		if (storage != StorageClassInput && current_function && (current_function->self == ir.default_entry_point) && !is_stage_output_builtin_masked(builtin)) return stage_out_var_name + "." + CompilerGLSL::builtin_to_glsl(builtin, storage);
		break;

	case BuiltInSampleMask:
		if (storage == StorageClassInput && current_function && (current_function->self == ir.default_entry_point) && (has_additional_fixed_sample_mask() || needs_sample_id))
		{
			string samp_mask_in;
			samp_mask_in += "(" + CompilerGLSL::builtin_to_glsl(builtin, storage);
			if (has_additional_fixed_sample_mask()) samp_mask_in += " & " + additional_fixed_sample_mask_str();
			if (needs_sample_id) samp_mask_in += " & (1 << gl_SampleID)";
			samp_mask_in += ")";
			return samp_mask_in;
		}
		if (storage != StorageClassInput && current_function && (current_function->self == ir.default_entry_point) && !is_stage_output_builtin_masked(builtin)) return stage_out_var_name + "." + CompilerGLSL::builtin_to_glsl(builtin, storage);
	...
	case BuiltInBaryCoordKHR:
	case BuiltInBaryCoordNoPerspKHR:
		if (storage == StorageClassInput && current_function && (current_function->self == ir.default_entry_point)) return stage_in_var_name + "." + CompilerGLSL::builtin_to_glsl(builtin, storage);
		break;

	case BuiltInTessLevelOuter:
		if (is_tesc_shader() && storage != StorageClassInput && current_function && (current_function->self == ir.default_entry_point)) { return join(tess_factor_buffer_var_name, "[", to_expression(builtin_primitive_id_id), "].edgeTessellationFactor"); }
		break;

	case BuiltInTessLevelInner:
		if (is_tesc_shader() && storage != StorageClassInput && current_function && (current_function->self == ir.default_entry_point)) { return join(tess_factor_buffer_var_name, "[", to_expression(builtin_primitive_id_id), "].insideTessellationFactor"); }
		break;

	case BuiltInHelperInvocation:
		if (needs_manual_helper_invocation_updates()) break;
		if (msl_options.is_ios() && !msl_options.supports_msl_version(2, 3))
			SPIRV_CROSS_THROW("simd_is_helper_thread() requires version 2.3 on iOS.");
		else if (msl_options.is_macos() && !msl_options.supports_msl_version(2, 1))
			SPIRV_CROSS_THROW("simd_is_helper_thread() requires version 2.1 on macOS.");
...
string CompilerMSL::builtin_qualifier(BuiltIn builtin)
{
	...
	case BuiltInPosition:
		...
		{
			if (!msl_options.supports_msl_version(2, 1)) SPIRV_CROSS_THROW("Invariant position is only supported on MSL 2.1 and up.");
	...
	case BuiltInViewIndex:
		if (execution.model != ExecutionModelFragment) SPIRV_CROSS_THROW("ViewIndex is handled specially outside fragment shaders.");
	...
	case BuiltInSubgroupSize:
		...
		{
			if (!msl_options.supports_msl_version(2, 2)) SPIRV_CROSS_THROW("threads_per_simdgroup requires Metal 2.2 in fragment shaders.");
	...
	case BuiltInNumSubgroups:
		...
		if (!msl_options.supports_msl_version(2)) SPIRV_CROSS_THROW("Subgroup builtins require Metal 2.0.");
	...
	case BuiltInSubgroupLocalInvocationId:
		...
		if (execution.model == ExecutionModelFragment)
		{
			if (!msl_options.supports_msl_version(2, 2)) SPIRV_CROSS_THROW("thread_index_in_simdgroup requires Metal 2.2 in fragment shaders.");
			return "thread_index_in_simdgroup";
		}
		else if (execution.model == ExecutionModelKernel || execution.model == ExecutionModelGLCompute || execution.model == ExecutionModelTessellationControl || (execution.model == ExecutionModelVertex && msl_options.vertex_for_tessellation))
		{
			// We are generating a Metal kernel function.
			if (!msl_options.supports_msl_version(2)) SPIRV_CROSS_THROW("Subgroup builtins in kernel functions require Metal 2.0.");
...
string CompilerMSL::builtin_type_decl(BuiltIn builtin, uint32_t id)
{
	...
	// Tess. control function out
	case BuiltInTessLevelInner:
		if (is_tese_shader()) return (msl_options.raw_buffer_tese_input || is_tessellating_triangles()) ? "float" : "float2";
		return "half";
	case BuiltInTessLevelOuter:
		if (is_tese_shader()) return (msl_options.raw_buffer_tese_input || is_tessellating_triangles()) ? "float" : "float4";
		return "half";
...
SPIRType CompilerMSL::get_presumed_input_type(const SPIRType& ib_type, uint32_t index) const
{
	...
	if (p_va != end(inputs_by_location) && p_va->second.vecsize > type.vecsize) type.vecsize = p_va->second.vecsize;
...
{
	return get_declared_type_array_stride_msl(get_physical_member_type(type, index), member_is_packed_physical_type(type, index), has_member_decoration(type.self, index, DecorationRowMajor));
}

uint32_t CompilerMSL::get_declared_input_array_stride_msl(const SPIRType& type, uint32_t index) const
{
	return get_declared_type_array_stride_msl(get_presumed_input_type(type, index), false, has_member_decoration(type.self, index, DecorationRowMajor));
}
...
{
	return get_declared_type_matrix_stride_msl(get_physical_member_type(type, index), member_is_packed_physical_type(type, index), has_member_decoration(type.self, index, DecorationRowMajor));
}

uint32_t CompilerMSL::get_declared_input_matrix_stride_msl(const SPIRType& type, uint32_t index) const
{
	return get_declared_type_matrix_stride_msl(get_presumed_input_type(type, index), false, has_member_decoration(type.self, index, DecorationRowMajor));
}
...
uint32_t CompilerMSL::get_declared_struct_size_msl(const SPIRType& struct_type, bool ignore_alignment, bool ignore_padding) const
{
	...
	if (!ignore_padding && has_extended_decoration(struct_type.self, SPIRVCrossDecorationPaddingTarget)) return get_extended_decoration(struct_type.self, SPIRVCrossDecorationPaddingTarget);
...
uint32_t CompilerMSL::get_declared_type_size_msl(const SPIRType& type, bool is_packed, bool row_major) const
{
	...
	default: {
		...
		if (type.basetype == SPIRType::Struct) return get_declared_struct_size_msl(type);

		if (is_packed) { return type.vecsize * type.columns * (type.width / 8); }
		else
		{
			...
			if (row_major && columns > 1) swap(vecsize, columns);
...
{
	return get_declared_type_size_msl(get_physical_member_type(type, index), member_is_packed_physical_type(type, index), has_member_decoration(type.self, index, DecorationRowMajor));
}

uint32_t CompilerMSL::get_declared_input_size_msl(const SPIRType& type, uint32_t index) const
{
	return get_declared_type_size_msl(get_presumed_input_type(type, index), false, has_member_decoration(type.self, index, DecorationRowMajor));
}
...
uint32_t CompilerMSL::get_declared_type_alignment_msl(const SPIRType& type, bool is_packed, bool row_major) const
{
	...
	if (type.pointer && type.storage == StorageClassPhysicalStorageBuffer) return 8 * (type.vecsize == 3 ? 4 : type.vecsize);
	...
	default: {
		if (type.basetype == SPIRType::Int64 && !msl_options.supports_msl_version(2, 3)) SPIRV_CROSS_THROW("long types in buffers are only supported in MSL 2.3 and above.");
		if (type.basetype == SPIRType::UInt64 && !msl_options.supports_msl_version(2, 3)) SPIRV_CROSS_THROW("ulong types in buffers are only supported in MSL 2.3 and above.");
...
{
	return get_declared_type_alignment_msl(get_physical_member_type(type, index), member_is_packed_physical_type(type, index), has_member_decoration(type.self, index, DecorationRowMajor));
}

uint32_t CompilerMSL::get_declared_input_alignment_msl(const SPIRType& type, uint32_t index) const
{
	return get_declared_type_alignment_msl(get_presumed_input_type(type, index), false, has_member_decoration(type.self, index, DecorationRowMajor));
}
...
bool CompilerMSL::SampledImageScanner::handle(spv::Op opcode, const uint32_t* args, uint32_t length)
{
	...
	case OpSampledImage: {
		...
		if ((type.basetype != SPIRType::Image && type.basetype != SPIRType::SampledImage) || type.image.sampled != 1) return true;
	...
	case OpImageDrefGather:
		compiler.has_sampled_images		  = compiler.has_sampled_images || compiler.is_sampled_image_type(compiler.expression_type(args[2]));
...
bool CompilerMSL::OpCodePreprocessor::handle(Op opcode, const uint32_t* args, uint32_t length)
{
	...
	case OpAtomicLoad: {
		uses_atomics = true;
		auto it		 = image_pointers_emulated.find(args[2]);
		if (it != image_pointers_emulated.end()) { compiler.atomic_image_vars_emulated.insert(it->second); }
		break;
	}
	...
	case OpArrayLength: {
		...
		if (var != nullptr)
		{
			if (!compiler.is_var_runtime_size_array(*var)) compiler.buffers_requiring_array_length.insert(var->self);
		}
		break;
	}
	...
	case OpExtInst: {
		...
		if (compiler.get<SPIRExtension>(extension_set).ext == SPIRExtension::GLSL)
		{
			...
			case GLSLstd450InterpolateAtSample:
			case GLSLstd450InterpolateAtOffset: {
				if (!compiler.msl_options.supports_msl_version(2, 3)) SPIRV_CROSS_THROW("Pull-model interpolation requires MSL 2.3.");
				...
				if (var)
				{
					...
					if (compiler.has_decoration(var->self, DecorationSample)) { needs_sample_id = true; }
	...
	case OpIsHelperInvocationEXT:
		if (compiler.needs_manual_helper_invocation_updates()) needs_helper_invocation = true;
		break;
	...
	if (compiler.instruction_to_result_type(result_type, result_id, opcode, args, length)) result_types[result_id] = result_type;
...
void CompilerMSL::OpCodePreprocessor::check_resource_write(uint32_t var_id)
{
	...
	if (sc == StorageClassUniform || sc == StorageClassStorageBuffer) uses_buffer_write = true;
}
...
CompilerMSL::SPVFuncImpl CompilerMSL::OpCodePreprocessor::get_spv_func_impl(Op opcode, const uint32_t* args)
{
	...
	case OpFAdd:
	case OpFSub:
		if (compiler.msl_options.invariant_float_math || compiler.has_decoration(args[1], DecorationNoContraction)) { return opcode == OpFAdd ? SPVFuncImplFAdd : SPVFuncImplFSub; }
		break;
	...
	case OpVectorTimesMatrix:
	case OpMatrixTimesMatrix:
		if (compiler.msl_options.invariant_float_math || compiler.has_decoration(args[1], DecorationNoContraction)) { return SPVFuncImplFMul; }
		break;
	...
	case OpAtomicStore: {
		...
		if (it != image_pointers_emulated.end())
		{
			...
			if (tid && compiler.get<SPIRType>(tid).image.dim == Dim2D) return SPVFuncImplImage2DAtomicCoords;
		}
		break;
	}
	...
	case OpImageWrite: {
		...
		if (tid && compiler.get<SPIRType>(tid).image.dim == DimBuffer && !compiler.msl_options.texture_buffer_native) return SPVFuncImplTexelBufferCoords;
		break;
	}
	...
	case OpExtInst: {
		...
		if (compiler.get<SPIRExtension>(extension_set).ext == SPIRExtension::GLSL)
		{
			...
			case GLSLstd450Reflect: {
				...
				if (type.vecsize == 1) return SPVFuncImplReflectScalar;
				break;
			}
			case GLSLstd450Refract: {
				auto& type = compiler.get<SPIRType>(args[0]);
				if (type.vecsize == 1) return SPVFuncImplRefractScalar;
				break;
			}
			case GLSLstd450FaceForward: {
				auto& type = compiler.get<SPIRType>(args[0]);
				if (type.vecsize == 1) return SPVFuncImplFaceForwardScalar;
				break;
			}
			case GLSLstd450MatrixInverse: {
...
void CompilerMSL::remap_constexpr_sampler(VariableID id, const MSLConstexprSampler& sampler)
{
	...
	if (type.basetype != SPIRType::SampledImage && type.basetype != SPIRType::Sampler) SPIRV_CROSS_THROW("Can only remap SampledImage and Sampler type.");
	if (!type.array.empty()) SPIRV_CROSS_THROW("Can not remap array of samplers.");
...
void CompilerMSL::cast_from_variable_load(uint32_t source_id, std::string& expr, const SPIRType& expr_type)
{
	...
	bool rewrite_boolean_load = expr_type.basetype == SPIRType::Boolean && (var && (var->storage == StorageClassWorkgroup || var_type->basetype == SPIRType::Struct));
	...
	if (!msl_options.supports_msl_version(3, 0) && var && (var->storage == StorageClassWorkgroup || (var_type->basetype == SPIRType::Struct && has_extended_decoration(var_type->self, SPIRVCrossDecorationWorkgroupStruct) && !is_packed)) &&
		expr_type.columns > 1)
	{
		SPIRType matrix_type = *phys_type;
		if (source_expr && source_expr->need_transpose) swap(matrix_type.vecsize, matrix_type.columns);
	...
	if (!has_decoration(source_id, DecorationBuiltIn))
	{
		...
		if (var && var->storage == StorageClassInput)
		{
			...
			if (base_type.basetype != SPIRType::Struct && expr_type.basetype != base_type.basetype) expr = join(type_to_glsl(expr_type), "(", expr, ")");
		}
		return;
	}
	...
	else if (expected_type != expr_type.basetype)
	{
		if (is_array(expr_type) && (builtin == BuiltInTessLevelInner || builtin == BuiltInTessLevelOuter))
		{
			...
			for (uint32_t i = 0; i < array_size; i++)
			{
				...
				if (i + 1 < array_size) wrap_expr += ", ";
			}

			if (is_tessellating_triangles()) wrap_expr += ", 0.0";
...
void CompilerMSL::cast_to_variable_store(uint32_t target_id, std::string& expr, const SPIRType& expr_type)
{
	...
	bool rewrite_boolean_store = expr_type.basetype == SPIRType::Boolean && (var && (var->storage == StorageClassWorkgroup || var_type->basetype == SPIRType::Struct));

	// Type fixups for workgroup variables or struct members if they are booleans.
	if (rewrite_boolean_store)
	{
		if (is_array(expr_type)) { expr = to_rerolled_array_expression(*var_type, expr, expr_type); }
		else
		{
	...
	{
		SPIRType matrix_type = *phys_type;
		if (target_expr && target_expr->need_transpose) swap(matrix_type.vecsize, matrix_type.columns);
		expr = join("spvStorage_", type_to_glsl(matrix_type), "(", expr, ")");
	}

	// Only interested in standalone builtin variables.
	if (!has_decoration(target_id, DecorationBuiltIn)) return;
...
string CompilerMSL::to_initializer_expression(const SPIRVariable& var)
{
	...
	if (type.array.empty() && type.basetype != SPIRType::Struct && init_type.vecsize > type.vecsize) expr = enclose_expression(expr + vector_swizzle(type.vecsize, 0));
	return expr;
}
...
{
	if (!msl_options.argument_buffers) return false;
	if (desc_set >= kMaxArgumentBuffers) return false;
...
bool CompilerMSL::is_supported_argument_buffer_type(const SPIRType& type) const
{
	...
	bool is_supported_type = !(type.basetype == SPIRType::Image && type.image.sampled == 2 && msl_options.is_ios() && msl_options.argument_buffers_tier <= Options::ArgumentBuffersTier::Tier1);
...
void CompilerMSL::emit_argument_buffer_aliased_descriptor(const SPIRVariable& aliased_var, const SPIRVariable& base_var)
{
	...
	if (var_meta) var_meta->decoration.qualified_alias_explicit_override = false;
	auto unqualified_name = to_name(base_var.self, false);
	if (var_meta) var_meta->decoration.qualified_alias_explicit_override = old_explicit_qualifier;
	...
	if (aliased_var.storage == StorageClassUniformConstant)
	{
		if (is_var_runtime_size_array(aliased_var))
		{
			...
			name = join("reinterpret_cast<", descriptor_storage, " ", type_to_glsl(get_variable_data_type(aliased_var), aliased_var.self, true), ">(&", unqualified_name, ")");
		}
		else { name = join("reinterpret_cast<", descriptor_storage, " ", type_to_glsl(get_variable_data_type(aliased_var), aliased_var.self, true), " &>(", unqualified_name, ");"); }
	}
	else
	{
		// Buffer types.
		...
		if (is_var_runtime_size_array(aliased_var)) { name = join("reinterpret_cast<", type_to_glsl(var_type, aliased_var.self, true), " ", descriptor_storage, " *>(&", unqualified_name, ")"); }
		else { name = join(needs_post_cast_deref ? "*" : "", "reinterpret_cast<", type_to_glsl(var_type, aliased_var.self, true), " ", descriptor_storage, " ", ref_type, ">(", unqualified_name, ");"); }

		if (needs_post_cast_deref) descriptor_storage = get_type_address_space(var_type, aliased_var.self, false);
...
void CompilerMSL::analyze_argument_buffers()
{
	...
	ir.for_each_typed_id<SPIRVariable>([&](uint32_t self, SPIRVariable& var) {
		if ((var.storage == StorageClassUniform || var.storage == StorageClassUniformConstant || var.storage == StorageClassStorageBuffer) && !is_hidden_variable(var))
		{
			uint32_t desc_set = get_decoration(self, DecorationDescriptorSet);
			// Ignore if it's part of a push descriptor set.
			if (!descriptor_set_is_argument_buffer(desc_set)) return;

			uint32_t var_id = var.self;
			auto&	 type	= get_variable_data_type(var);

			if (desc_set >= kMaxArgumentBuffers) SPIRV_CROSS_THROW("Descriptor set index is out of range.");
			...
			if (type.basetype == SPIRType::SampledImage)
			{
				...
				for (uint32_t i = 0; i < plane_count; i++)
				{
					...
					resources_in_set[desc_set].push_back({&var, to_name(var_id), SPIRType::Image, image_resource_index, i, 0});
				}
				...
				if (type.image.dim != DimBuffer && !constexpr_sampler)
				{
					...
					resources_in_set[desc_set].push_back({&var, to_sampler_expression(var_id), SPIRType::Sampler, sampler_resource_index, 0, 0});
				}
			}
			else if (inline_uniform_blocks.count(SetBindingPair{desc_set, binding})) { inline_block_vars.push_back(var_id); }
			else if (!constexpr_sampler && is_supported_argument_buffer_type(type))
			{
				...
				resources_in_set[desc_set].push_back({&var, to_name(var_id), type.basetype, resource_index, 0, 0});

				// Emulate texture2D atomic operations
				if (atomic_image_vars_emulated.count(var.self))
				{
					uint32_t buffer_resource_index = get_metal_resource_index(var, SPIRType::AtomicCounter, 0);
					resources_in_set[desc_set].push_back({&var, to_name(var_id) + "_atomic", SPIRType::Struct, buffer_resource_index, 0, 0});
				}
			}
	...
	if (needs_swizzle_buffer_def || needs_buffer_sizes)
	{
		...
		{
			if (!set_needs_swizzle_buffer[desc_set] && !set_needs_buffer_sizes[desc_set]) continue;
			...
			if (set_needs_swizzle_buffer[desc_set])
			{
				...
				resources_in_set[desc_set].push_back({&var, to_name(var_id), SPIRType::UInt, get_metal_resource_index(var, SPIRType::UInt), 0, 0});
			}
	...
	for (uint32_t var_id : inline_block_vars)
	{
		...
		uint32_t desc_set = get_decoration(var_id, DecorationDescriptorSet);
		add_resource_name(var_id);
		resources_in_set[desc_set].push_back({&var, to_name(var_id), SPIRType::Struct, get_metal_resource_index(var, SPIRType::Struct), 0, 0});
	}
	...
	for (uint32_t desc_set = 0; desc_set < kMaxArgumentBuffers; desc_set++)
	{
		...
		stable_sort(begin(resources), end(resources), [&](const Resource& lhs, const Resource& rhs) -> bool { return tie(lhs.index, lhs.basetype) < tie(rhs.index, rhs.basetype); });
		...
		for (auto& resource : resources)
		{
			...
			if (is_var_runtime_size_array(var) && (argument_buffer_device_storage_mask & (1u << desc_set)) == 0) SPIRV_CROSS_THROW("Runtime sized variables must be in device storage argument buffers.");
			...
			if (resource.plane > 0) mbr_name += join(plane_name_suffix, resource.plane);
			...
			else
			{
				...
				if (resource.basetype == SPIRType::Image || resource.basetype == SPIRType::Sampler || resource.basetype == SPIRType::SampledImage)
				{
					...
					{
						if (!msl_options.supports_msl_version(3, 0)) SPIRV_CROSS_THROW("Full mutable aliasing of argument buffer descriptors only works on Metal 3+.");

						auto& entry_func = get<SPIRFunction>(ir.default_entry_point);
						entry_func.fixup_hooks_in.push_back([this, resource]() { emit_argument_buffer_aliased_descriptor(*resource.var, this->get<SPIRVariable>(resource.overlapping_var_id)); });
					}
					else if (resource.plane == 0) { set_qualified_name(var.self, join(to_name(buffer_variable_id), ".", mbr_name)); }
				}
				...
				else
				{
					...
					if (has_extended_decoration(var.self, SPIRVCrossDecorationOverlappingBinding))
					{
						...
						if (get<SPIRVariable>(resource.overlapping_var_id).storage == StorageClassUniformConstant && !msl_options.supports_msl_version(3, 0)) { SPIRV_CROSS_THROW("Full mutable aliasing of argument buffer descriptors only works on Metal 3+."); }
			...
			if (has_extended_decoration(var.self, SPIRVCrossDecorationOverlappingBinding)) set_extended_member_decoration(buffer_type.self, member_index, SPIRVCrossDecorationOverlappingBinding);
			member_index++;
		}
	}
}
...
MSLResourceBinding& CompilerMSL::get_argument_buffer_resource(uint32_t desc_set, uint32_t arg_idx)
{
	...
	if (arg_itr != end(resource_arg_buff_idx_to_binding_number))
	{
		...
		if (bind_itr != end(resource_bindings)) return bind_itr->second.first;
	}
...
void CompilerMSL::activate_argument_buffer_resources()
{
	...
	ir.for_each_typed_id<SPIRVariable>([&](uint32_t self, const SPIRVariable&) {
		if (!has_decoration(self, DecorationDescriptorSet)) return;

		uint32_t desc_set = get_decoration(self, DecorationDescriptorSet);
		if (descriptor_set_is_argument_buffer(desc_set)) add_active_interface_variable(self);
	});
}
