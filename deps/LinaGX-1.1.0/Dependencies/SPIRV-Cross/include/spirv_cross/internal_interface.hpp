...
namespace internal
{
	...
	template <typename T, unsigned U> struct Interface<T[U]>
	{
		enum
		{
			ArraySize = U,
	...
	template <typename T> struct PointerInterface
	{
		enum
		{
			ArraySize = 1,
	...
	template <typename T> struct DereferenceAdaptor
	{
	...
	template <typename T, unsigned U> struct PointerInterface<T[U]>
	{
		enum
		{
			ArraySize = U,
...
struct spirv_cross_shader
{
	...
	template <typename U> void register_builtin(spirv_cross_builtin builtin, const U& value)
	{
	...
	template <typename U> void register_resource(const internal::Resource<U>& value, unsigned set, unsigned binding)
	{
	...
	template <typename U> void register_stage_input(const internal::StageInput<U>& value, unsigned location)
	{
	...
	template <typename U> void register_stage_output(const internal::StageOutput<U>& value, unsigned location)
	{
	...
	template <typename U> void register_uniform_constant(const internal::UniformConstant<U>& value, unsigned location)
	{
	...
	template <typename U> void register_push_constant(const internal::PushConstant<U>& value)
	{
...
{
	template <typename T> struct BaseShader : spirv_cross_shader
	{
		void invoke()
		{
...
#define gl_FragCoord __res->gl_FragCoord.get()
	...
	template <typename T, typename Res> struct FragmentShader : BaseShader<FragmentShader<T, Res>>
	{
		inline void main()
		{
			impl.main();
...
#define gl_Position __res->gl_Position.get()
	...
	template <typename T, typename Res> struct VertexShader : BaseShader<VertexShader<T, Res>>
	{
		inline void main()
		{
			impl.main();
	...
	template <typename T, typename Res> struct TessEvaluationShader : BaseShader<TessEvaluationShader<T, Res>>
	{
		inline void main()
		{
			impl.main();
	...
	template <typename T, typename Res> struct TessControlShader : BaseShader<TessControlShader<T, Res>>
	{
		inline void main()
		{
			impl.main();
	...
	template <typename T, typename Res> struct GeometryShader : BaseShader<GeometryShader<T, Res>>
	{
		inline void main()
		{
			impl.main();
...
#define gl_GlobalInvocationID __priv_res.gl_GlobalInvocationID__
	...
	template <typename T, typename Res, unsigned WorkGroupX, unsigned WorkGroupY, unsigned WorkGroupZ> struct ComputeShader : BaseShader<ComputeShader<T, Res, WorkGroupX, WorkGroupY, WorkGroupZ>>
	{
		inline void main()
		{
			...
			for (unsigned z = 0; z < WorkGroupZ; z++)
				for (unsigned y = 0; y < WorkGroupY; y++)
					for (unsigned x = 0; x < WorkGroupX; x++)
						impl[z][y][x].__priv_res.gl_GlobalInvocationID__ = glm::uvec3(WorkGroupX, WorkGroupY, WorkGroupZ) * resources.gl_WorkGroupID__.get() + glm::uvec3(x, y, z);

			group.run();
			group.wait();
		}

		ComputeShader() : group(&impl[0][0][0])
		{
			resources.init(*this);
	...
	// Atomics
	template <typename T> inline T atomicAdd(T& v, T a)
	{
