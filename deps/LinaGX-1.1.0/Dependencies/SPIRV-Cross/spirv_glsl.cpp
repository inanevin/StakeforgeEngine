...
void CompilerGLSL::init()
{
	...
	if (conv && conv->decimal_point) current_locale_radix_character = *conv->decimal_point;
	...
	if (decimal_point && *decimal_point != '\0') current_locale_radix_character = *decimal_point;
#endif
}
...
void CompilerGLSL::reset(uint32_t iteration_count)
{
	...
	if (iteration_count >= options.force_recompile_max_debug_iterations && !is_force_recompile_forward_progress) SPIRV_CROSS_THROW("Maximum compilation loops detected and no forward progress was made. Must be a SPIRV-Cross bug!");
...
void CompilerGLSL::remap_pls_variables()
{
	for (auto& input : pls_inputs)
	{
		...
		if (var.storage != StorageClassInput && !input_is_target) SPIRV_CROSS_THROW("Can only use in and target variables for PLS inputs.");
	...
	for (auto& output : pls_outputs)
	{
		...
		if (var.storage != StorageClassOutput) SPIRV_CROSS_THROW("Can only use out variables for PLS outputs.");
		var.remapped_variable = true;
	}
}
...
{
	return std::find_if(begin(inout_color_attachments), end(inout_color_attachments), [&](const std::pair<uint32_t, bool>& elem) { return elem.first == location; }) != end(inout_color_attachments);
}

bool CompilerGLSL::location_is_non_coherent_framebuffer_fetch(uint32_t location) const
{
	return std::find_if(begin(inout_color_attachments), end(inout_color_attachments), [&](const std::pair<uint32_t, bool>& elem) { return elem.first == location && !elem.second; }) != end(inout_color_attachments);
}
...
void CompilerGLSL::find_static_extensions()
{
	ir.for_each_typed_id<SPIRType>([&](uint32_t, const SPIRType& type) {
		...
		{
			if (options.es) SPIRV_CROSS_THROW("FP64 not supported in ES profile.");
			if (!options.es && options.version < 400) require_extension_internal("GL_ARB_gpu_shader_fp64");
		}
		...
		else if (type.basetype == SPIRType::Half)
		{
			...
			if (options.vulkan_semantics) require_extension_internal("GL_EXT_shader_16bit_storage");
		}
		else if (type.basetype == SPIRType::SByte || type.basetype == SPIRType::UByte)
		{
			...
			if (options.vulkan_semantics) require_extension_internal("GL_EXT_shader_8bit_storage");
		}
	...
	case ExecutionModelGLCompute:
		if (!options.es && options.version < 430) require_extension_internal("GL_ARB_compute_shader");
		if (options.es && options.version < 310) SPIRV_CROSS_THROW("At least ESSL 3.10 required for compute shaders.");
		break;

	case ExecutionModelGeometry:
		if (options.es && options.version < 320) require_extension_internal("GL_EXT_geometry_shader");
		if (!options.es && options.version < 150) require_extension_internal("GL_ARB_geometry_shader4");
		...
		if (execution.flags.get(ExecutionModeInvocations) && execution.invocations != 1)
		{
			...
			if (!options.es && options.version < 400) require_extension_internal("GL_ARB_gpu_shader5");
		}
		break;

	case ExecutionModelTessellationEvaluation:
	case ExecutionModelTessellationControl:
		if (options.es && options.version < 320) require_extension_internal("GL_EXT_tessellation_shader");
		if (!options.es && options.version < 400) require_extension_internal("GL_ARB_tessellation_shader");
		break;
	...
	case ExecutionModelCallableKHR:
		...
		if (options.es || options.version < 460) SPIRV_CROSS_THROW("Ray tracing shaders require non-es profile with version 460 or above.");
		if (!options.vulkan_semantics) SPIRV_CROSS_THROW("Ray tracing requires Vulkan semantics.");
	...
	case ExecutionModelTaskEXT:
		if (options.es || options.version < 450) SPIRV_CROSS_THROW("Mesh shaders require GLSL 450 or above.");
		if (!options.vulkan_semantics) SPIRV_CROSS_THROW("Mesh shaders require Vulkan semantics.");
	...
	{
		if (execution.model != ExecutionModelFragment) SPIRV_CROSS_THROW("Can only use GL_EXT_shader_pixel_local_storage in fragment shaders.");
		require_extension_internal("GL_EXT_shader_pixel_local_storage");
	}

	if (!inout_color_attachments.empty())
	{
		if (execution.model != ExecutionModelFragment) SPIRV_CROSS_THROW("Can only use GL_EXT_shader_framebuffer_fetch in fragment shaders.");
		if (options.vulkan_semantics) SPIRV_CROSS_THROW("Cannot use EXT_shader_framebuffer_fetch in Vulkan GLSL.");
		...
		if (has_coherent) require_extension_internal("GL_EXT_shader_framebuffer_fetch");
		if (has_incoherent) require_extension_internal("GL_EXT_shader_framebuffer_fetch_non_coherent");
	}
	...
	{
		if (!options.vulkan_semantics) SPIRV_CROSS_THROW("GL_EXT_buffer_reference is only supported in Vulkan GLSL.");
	...
	else if (ir.addressing_model != AddressingModelLogical) { SPIRV_CROSS_THROW("Only Logical and PhysicalStorageBuffer64EXT addressing models are supported."); }
	...
	for (auto& cap : ir.declared_capabilities)
	{
		...
		case CapabilityRuntimeDescriptorArrayEXT:
			if (!options.vulkan_semantics) SPIRV_CROSS_THROW("GL_EXT_nonuniform_qualifier is only supported in Vulkan GLSL.");
			require_extension_internal("GL_EXT_nonuniform_qualifier");
			break;
		...
		case CapabilityMultiView:
			...
			else
			{
				...
				if (options.ovr_multiview_view_count == 0) SPIRV_CROSS_THROW("ovr_multiview_view_count must be non-zero when using GL_OVR_multiview2.");
				if (get_execution_model() != ExecutionModelVertex) SPIRV_CROSS_THROW("OVR_multiview2 can only be used with Vertex shaders.");
			}
			break;

		case CapabilityRayQueryKHR:
			if (options.es || options.version < 460 || !options.vulkan_semantics) SPIRV_CROSS_THROW("RayQuery requires Vulkan GLSL 460.");
	...
	{
		if (options.vulkan_semantics) SPIRV_CROSS_THROW("OVR_multiview2 cannot be used with Vulkan semantics.");
		if (get_execution_model() != ExecutionModelVertex) SPIRV_CROSS_THROW("OVR_multiview2 can only be used with Vertex shaders.");
		require_extension_internal("GL_OVR_multiview2");
	}
	...
	for (auto& ext : ir.declared_extensions)
		if (ext == "SPV_NV_fragment_shader_barycentric") barycentric_is_nv = true;
}
...
void CompilerGLSL::ray_tracing_khr_fixup_locations()
{
	...
	ir.for_each_typed_id<SPIRVariable>([&](uint32_t, SPIRVariable& var) {
		...
		if (var.storage != StorageClassRayPayloadKHR && var.storage != StorageClassCallableDataKHR && var.storage != StorageClassIncomingRayPayloadKHR && var.storage != StorageClassIncomingCallableDataKHR) return;
		if (is_hidden_variable(var)) return;
...
string CompilerGLSL::compile()
{
	...
	if (is_legacy_es()) backend.support_case_fallthrough = false;
	...
	if (!inout_color_attachments.empty()) emit_inout_fragment_outputs_copy_to_subpass_inputs();
	...
	if (ir.addressing_model == AddressingModelPhysicalStorageBuffer64EXT) analyze_non_block_pointer_types();
	...
	do
	{
		...
		if (required_polyfills != 0) emit_polyfills(required_polyfills, false);
		if (options.es && required_polyfills_relaxed != 0) emit_polyfills(required_polyfills_relaxed, true);
...
void CompilerGLSL::build_workgroup_size(SmallVector<string>& arguments, const SpecializationConstant& wg_x, const SpecializationConstant& wg_y, const SpecializationConstant& wg_z)
{
	auto& execution			= get_entry_point();
...
void CompilerGLSL::request_subgroup_feature(ShaderSubgroupSupportHelper::Feature feature)
{
	...
	{
		if (!shader_subgroup_supporter.is_feature_requested(feature)) force_recompile();
...
void CompilerGLSL::emit_header()
{
	...
	if (!options.es && options.version < 420)
	{
		...
		if (execution.flags.get(ExecutionModeEarlyFragmentTests)) require_extension_internal("GL_ARB_shader_image_load_store");
	}

	// Needed for: layout(post_depth_coverage) in;
	if (execution.flags.get(ExecutionModePostDepthCoverage)) require_extension_internal("GL_ARB_post_depth_coverage");

	// Needed for: layout({pixel,sample}_interlock_[un]ordered) in;
	bool interlock_used =
		execution.flags.get(ExecutionModePixelInterlockOrderedEXT) || execution.flags.get(ExecutionModePixelInterlockUnorderedEXT) || execution.flags.get(ExecutionModeSampleInterlockOrderedEXT) || execution.flags.get(ExecutionModeSampleInterlockUnorderedEXT);

	if (interlock_used)
	{
		if (options.es)
		{
			if (options.version < 310) SPIRV_CROSS_THROW("At least ESSL 3.10 required for fragment shader interlock.");
			require_extension_internal("GL_NV_fragment_shader_interlock");
		}
		else
		{
			if (options.version < 420) require_extension_internal("GL_ARB_shader_image_load_store");
	...
	if (!options.vulkan_semantics)
	{
		...
		for (uint32_t feature_index = 0; feature_index < Supp::FeatureCount; feature_index++)
		{
			...
			if (!shader_subgroup_supporter.is_feature_requested(feature)) continue;
	...
	case ExecutionModelVertex:
		if (options.ovr_multiview_view_count) inputs.push_back(join("num_views = ", options.ovr_multiview_view_count));
		break;
	case ExecutionModelGeometry:
		if ((execution.flags.get(ExecutionModeInvocations)) && execution.invocations != 1) inputs.push_back(join("invocations = ", execution.invocations));
		if (execution.flags.get(ExecutionModeInputPoints)) inputs.push_back("points");
		if (execution.flags.get(ExecutionModeInputLines)) inputs.push_back("lines");
		if (execution.flags.get(ExecutionModeInputLinesAdjacency)) inputs.push_back("lines_adjacency");
		if (execution.flags.get(ExecutionModeTriangles)) inputs.push_back("triangles");
		if (execution.flags.get(ExecutionModeInputTrianglesAdjacency)) inputs.push_back("triangles_adjacency");
		...
		if (!execution.geometry_passthrough)
		{
			...
			if (execution.flags.get(ExecutionModeOutputTriangleStrip)) outputs.push_back("triangle_strip");
			if (execution.flags.get(ExecutionModeOutputPoints)) outputs.push_back("points");
			if (execution.flags.get(ExecutionModeOutputLineStrip)) outputs.push_back("line_strip");
		}
		break;

	case ExecutionModelTessellationControl:
		if (execution.flags.get(ExecutionModeOutputVertices)) outputs.push_back(join("vertices = ", execution.output_vertices));
		break;

	case ExecutionModelTessellationEvaluation:
		if (execution.flags.get(ExecutionModeQuads)) inputs.push_back("quads");
		if (execution.flags.get(ExecutionModeTriangles)) inputs.push_back("triangles");
		if (execution.flags.get(ExecutionModeIsolines)) inputs.push_back("isolines");
		if (execution.flags.get(ExecutionModePointMode)) inputs.push_back("point_mode");

		if (!execution.flags.get(ExecutionModeIsolines))
		{
			if (execution.flags.get(ExecutionModeVertexOrderCw)) inputs.push_back("cw");
			if (execution.flags.get(ExecutionModeVertexOrderCcw)) inputs.push_back("ccw");
		}

		if (execution.flags.get(ExecutionModeSpacingFractionalEven)) inputs.push_back("fractional_even_spacing");
		if (execution.flags.get(ExecutionModeSpacingFractionalOdd)) inputs.push_back("fractional_odd_spacing");
		if (execution.flags.get(ExecutionModeSpacingEqual)) inputs.push_back("equal_spacing");
		break;
	...
	case ExecutionModelMeshEXT: {
		if (execution.workgroup_size.constant != 0 || execution.flags.get(ExecutionModeLocalSizeId))
		{
			...
			if (options.vulkan_semantics || ((wg_x.id == ConstantID(0)) && (wg_y.id == ConstantID(0)) && (wg_z.id == ConstantID(0)))) build_workgroup_size(inputs, wg_x, wg_y, wg_z);
		}
		else
		{
	...
	case ExecutionModelFragment:
		...
		if (execution.flags.get(ExecutionModeEarlyFragmentTests)) inputs.push_back("early_fragment_tests");
		if (execution.flags.get(ExecutionModePostDepthCoverage)) inputs.push_back("post_depth_coverage");

		if (interlock_used) statement("#if defined(GL_ARB_fragment_shader_interlock)");
	...
	for (auto& cap : ir.declared_capabilities)
		if (cap == CapabilityRayTraversalPrimitiveCullingKHR) statement("layout(primitive_culling);");

	if (!inputs.empty()) statement("layout(", merge(inputs), ") in;");
	if (!outputs.empty()) statement("layout(", merge(outputs), ") out;");
...
void CompilerGLSL::emit_struct(SPIRType& type)
{
	...
	if (type.type_alias != TypeID(0) && !has_extended_decoration(type.type_alias, SPIRVCrossDecorationBufferBlockRepacked)) return;
	...
	if (has_extended_decoration(type.self, SPIRVCrossDecorationPaddingTarget)) emit_struct_padding_target(type);
...
string CompilerGLSL::to_interpolation_qualifiers(const Bitset& flags)
{
	...
	//     res += "smooth ";
	if (flags.get(DecorationFlat)) res += "flat ";
	if (flags.get(DecorationNoPerspective))
	{
		if (options.es)
		{
			if (options.version < 300) SPIRV_CROSS_THROW("noperspective requires ESSL 300.");
	...
	if (flags.get(DecorationCentroid)) res += "centroid ";
	if (flags.get(DecorationPatch)) res += "patch ";
	...
	if (flags.get(DecorationInvariant) && (options.es || options.version >= 120)) res += "invariant ";
...
string CompilerGLSL::layout_for_member(const SPIRType& type, uint32_t index)
{
	...
	if (index >= memb.size()) return "";
	auto& dec = memb[index];

	SmallVector<string> attr;

	if (has_member_decoration(type.self, index, DecorationPassthroughNV)) attr.push_back("passthrough");
	...
	if (flags.get(DecorationRowMajor)) attr.push_back("row_major");
	// We don't emit any global layouts, so column_major is default.
	// if (flags & (1ull << DecorationColMajor))
	//    attr.push_back("column_major");

	if (dec.decoration_flags.get(DecorationLocation) && can_use_io_location(type.storage, true)) attr.push_back(join("location = ", dec.location));
...
{
	if (options.es && is_desktop_only_format(format)) SPIRV_CROSS_THROW("Attempting to use image format not supported in ES profile.");
...
uint32_t CompilerGLSL::type_to_packed_alignment(const SPIRType& type, const Bitset& flags, BufferPackingStandard packing)
{
	...
	{
		if (!type.pointer) SPIRV_CROSS_THROW("Types in PhysicalStorageBufferEXT must be pointers.");
	...
	else if (is_array(type))
	{
		...
		if (packing_is_vec4_padded(packing)) minimum_alignment = 16;
	...
	if (type.basetype == SPIRType::Struct)
	{
		...
		if (packing_is_vec4_padded(packing)) alignment = max<uint32_t>(alignment, 16u);
	...
	else
	{
		...
		if (packing_is_scalar(packing)) return base_alignment;
		...
		if (type.columns == 1 && packing_is_hlsl(packing)) return base_alignment;

		// From 7.6.2.2 in GL 4.5 core spec.
		// Rule 1
		if (type.vecsize == 1 && type.columns == 1) return base_alignment;

		// Rule 2
		if ((type.vecsize == 2 || type.vecsize == 4) && type.columns == 1) return type.vecsize * base_alignment;

		// Rule 3
		if (type.vecsize == 3 && type.columns == 1) return 4 * base_alignment;
...
uint32_t CompilerGLSL::type_to_packed_size(const SPIRType& type, const Bitset& flags, BufferPackingStandard packing)
{
	...
	else if (is_array(type))
	{
		...
		if (packing_is_hlsl(packing) && type.basetype != SPIRType::Struct) packed_size -= (4 - type.vecsize) * (type.width / 8);
	...
	else
	{
		...
		if (packing_is_scalar(packing)) { size = type.vecsize * type.columns * base_alignment; }
		else
		{
			if (type.columns == 1) size = type.vecsize * base_alignment;
			...
			// so that it is possible to pack other vectors into the last element.
			if (packing_is_hlsl(packing) && type.columns > 1) size -= (4 - type.vecsize) * (type.width / 8);
		}
	}
...
bool CompilerGLSL::buffer_is_packing_standard(const SPIRType& type, BufferPackingStandard packing, uint32_t* failed_validation_index, uint32_t start_offset, uint32_t end_offset)
{
	...
	for (uint32_t i = 0; i < type.member_types.size(); i++)
	{
		...
		if (!member_can_be_unsized || packing_is_hlsl(packing)) packed_size = type_to_packed_size(memb_type, member_flags, packing);
		...
		if (packing_is_hlsl(packing))
		{
			...
			if (begin_word != end_word) packed_alignment = max<uint32_t>(packed_alignment, 16u);
		}

		// Field is not in the specified range anymore and we can ignore any further fields.
		if (actual_offset >= end_offset) break;
		...
		if (actual_offset >= start_offset)
		{
			...
			if (!packing_has_flexible_offset(packing))
			{
				...
				{
					if (failed_validation_index) *failed_validation_index = i;
					return false;
				}
			}
			...
			if (is_array(memb_type) && type_to_packed_array_stride(memb_type, member_flags, packing) != type_struct_member_array_stride(type, i))
			{
				if (failed_validation_index) *failed_validation_index = i;
				return false;
			}
...
bool CompilerGLSL::can_use_io_location(StorageClass storage, bool block)
{
	...
	if ((get_execution_model() != ExecutionModelVertex && storage == StorageClassInput) || (get_execution_model() != ExecutionModelFragment && storage == StorageClassOutput))
	{
...
string CompilerGLSL::layout_for_variable(const SPIRVariable& var)
{
	...
	if (flags.get(DecorationPassthroughNV)) attr.push_back("passthrough");
	...
	if (flags.get(DecorationRowMajor)) attr.push_back("row_major");
	if (flags.get(DecorationColMajor)) attr.push_back("column_major");

	if (options.vulkan_semantics)
	{
		if (flags.get(DecorationInputAttachmentIndex)) attr.push_back(join("input_attachment_index = ", get_decoration(var.self, DecorationInputAttachmentIndex)));
	}
	...
	if (flags.get(DecorationLocation) && can_use_io_location(var.storage, is_block))
	{
		...
		if (!combined_decoration.get(DecorationLocation)) attr.push_back(join("location = ", get_decoration(var.self, DecorationLocation)));
	}

	if (get_execution_model() == ExecutionModelFragment && var.storage == StorageClassOutput && location_is_non_coherent_framebuffer_fetch(get_decoration(var.self, DecorationLocation))) { attr.push_back("noncoherent"); }
	...
	if (is_block && var.storage == StorageClassOutput)
	{
		...
		for (uint32_t i = 0; i < member_count; i++)
		{
			if (has_member_decoration(type.self, i, DecorationStream))
			{
				...
				if (have_geom_stream && member_geom_stream != geom_stream) SPIRV_CROSS_THROW("IO block member Stream mismatch.");
				have_geom_stream = true;
				geom_stream		 = member_geom_stream;
			}

			// Only members with an Offset decoration participate in XFB.
			if (!has_member_decoration(type.self, i, DecorationOffset)) continue;
			...
			if (has_member_decoration(type.self, i, DecorationXfbBuffer))
			{
				...
				if (have_xfb_buffer_stride && buffer_index != xfb_buffer) SPIRV_CROSS_THROW("IO block member XfbBuffer mismatch.");
				have_xfb_buffer_stride = true;
			...
			if (has_member_decoration(type.self, i, DecorationXfbStride))
			{
				...
				if (have_xfb_buffer_stride && stride != xfb_stride) SPIRV_CROSS_THROW("IO block member XfbStride mismatch.");
				have_xfb_buffer_stride = true;
				xfb_stride			   = stride;
			}
		}
		...
		if (have_geom_stream)
		{
			if (get_execution_model() != ExecutionModelGeometry) SPIRV_CROSS_THROW("Geometry streams can only be used in geometry shaders.");
			if (options.es) SPIRV_CROSS_THROW("Multiple geometry streams not supported in ESSL.");
			if (options.version < 400) require_extension_internal("GL_ARB_transform_feedback3");
	...
	if (uses_enhanced_layouts)
	{
		if (!options.es)
		{
			...
			if (!options.es && options.version < 440) require_extension_internal("GL_ARB_enhanced_layouts");
		}
		else if (options.es)
			SPIRV_CROSS_THROW("GL_ARB_enhanced_layouts is not supported in ESSL.");
	}

	if (flags.get(DecorationIndex)) attr.push_back(join("index = ", get_decoration(var.self, DecorationIndex)));
	...
	{
		if (flags.get(DecorationDescriptorSet) && options.vulkan_semantics) attr.push_back(join("set = ", get_decoration(var.self, DecorationDescriptorSet)));
	}

	bool push_constant_block = options.vulkan_semantics && var.storage == StorageClassPushConstant;
	bool ssbo_block			 = var.storage == StorageClassStorageBuffer || var.storage == StorageClassShaderRecordBufferKHR || (var.storage == StorageClassUniform && typeflags.get(DecorationBufferBlock));
	...
	if (ubo_block && options.emit_uniform_buffer_as_plain_uniforms) can_use_buffer_blocks = false;
	...
	if (!can_use_buffer_blocks && var.storage == StorageClassUniform) can_use_binding = false;

	if (var.storage == StorageClassShaderRecordBufferKHR) can_use_binding = false;

	if (can_use_binding && flags.get(DecorationBinding)) attr.push_back(join("binding = ", get_decoration(var.self, DecorationBinding)));

	if (var.storage != StorageClassOutput && flags.get(DecorationOffset)) attr.push_back(join("offset = ", get_decoration(var.self, DecorationOffset)));

	// Instead of adding explicit offsets for every element here, just assume we're using std140 or std430.
	// If SPIR-V does not comply with either layout, we cannot really work around it.
	if (can_use_buffer_blocks && (ubo_block || emulated_ubo)) { attr.push_back(buffer_to_packing_standard(type, false, true)); }
	else if (can_use_buffer_blocks && (push_constant_block || ssbo_block)) { attr.push_back(buffer_to_packing_standard(type, true, true)); }
...
string CompilerGLSL::buffer_to_packing_standard(const SPIRType& type, bool support_std430_without_scalar_layout, bool support_enhanced_layouts)
{
	...
	else if (support_std430_without_scalar_layout && support_enhanced_layouts && buffer_is_packing_standard(type, BufferPackingStd430EnhancedLayout))
	{
		...
		if (!options.es && !options.vulkan_semantics && options.version < 440) require_extension_internal("GL_ARB_enhanced_layouts");
	...
	else if (!support_std430_without_scalar_layout && options.vulkan_semantics && support_enhanced_layouts && buffer_is_packing_standard(type, BufferPackingStd430EnhancedLayout))
	{
		// UBOs can support std430 with GL_EXT_scalar_block_layout.
		set_extended_decoration(type.self, SPIRVCrossDecorationExplicitOffset);
		require_extension_internal("GL_EXT_scalar_block_layout");
...
void CompilerGLSL::emit_push_constant_block_glsl(const SPIRVariable& var)
{
	...
	if (block_flag) set_decoration(type.self, DecorationBlock);
...
void CompilerGLSL::emit_buffer_block_legacy(const SPIRVariable& var)
{
	...
	if (ssbo) SPIRV_CROSS_THROW("SSBOs not supported in legacy targets.");
	...
	emit_struct(type);
	if (block_flag) block_flags.set(DecorationBlock);
	emit_uniform(var);
	statement("");
}
...
void CompilerGLSL::emit_buffer_reference_block(uint32_t type_id, bool forward_declaration)
{
	...
	if (forward_declaration && is_physical_pointer_to_buffer_block(type))
	{
		...
		if (ir.meta[type.self].decoration.alias.empty() || block_ssbo_names.find(buffer_name) != end(block_ssbo_names) || resource_names.find(buffer_name) != end(resource_names)) { buffer_name = join("_", type.self); }
		...
		if (buffer_name.empty()) buffer_name = join("_", type.self);
	...
	else { buffer_name = type_to_glsl(type); }

	if (!forward_declaration)
	{
		auto	 itr	   = physical_storage_type_to_alignment.find(type_id);
		uint32_t alignment = 0;
		if (itr != physical_storage_type_to_alignment.end()) alignment = itr->second.alignment;

		if (is_physical_pointer_to_buffer_block(type))
		{
			SmallVector<std::string> attributes;
			attributes.push_back("buffer_reference");
			if (alignment) attributes.push_back(join("buffer_reference_align = ", alignment));
			attributes.push_back(buffer_to_packing_standard(type, true, true));

			auto   flags = ir.get_buffer_block_type_flags(type);
			string decorations;
			if (flags.get(DecorationRestrict)) decorations += " restrict";
			if (flags.get(DecorationCoherent)) decorations += " coherent";
			if (flags.get(DecorationNonReadable)) decorations += " writeonly";
			if (flags.get(DecorationNonWritable)) decorations += " readonly";
	...
	else { statement("layout(buffer_reference) buffer ", buffer_name, ";"); }
}
...
void CompilerGLSL::emit_buffer_block_native(const SPIRVariable& var)
{
	...
	bool   ssbo			= var.storage == StorageClassStorageBuffer || var.storage == StorageClassShaderRecordBufferKHR || ir.meta[type.self].decoration.decoration_flags.get(DecorationBufferBlock);
	...
	// have to track this other than updating name caches.
	// If we have a collision for any reason, just fallback immediately.
	if (ir.meta[type.self].decoration.alias.empty() || block_namespace.find(buffer_name) != end(block_namespace) || resource_names.find(buffer_name) != end(resource_names)) { buffer_name = get_block_fallback_name(var.self); }
	...
	// This cannot conflict with anything else, so we're safe now.
	// We cannot reuse this fallback name in neither global scope (blocked by block_names) nor block name scope.
	if (buffer_name.empty()) buffer_name = join("_", get<SPIRType>(var.basetype).self, "_", var.self);
	...
	statement(layout_for_variable(var), is_coherent ? "coherent " : "", is_restrict ? "restrict " : "", is_writeonly ? "writeonly " : "", is_readonly ? "readonly " : "", ssbo ? "buffer " : "uniform ", buffer_name);
	...
	if (type_is_empty(type) && !backend.supports_empty_struct) statement("int empty_struct_member;");
...
void CompilerGLSL::emit_buffer_block_flattened(const SPIRVariable& var)
{
	...
	if (get_common_basic_type(type, basic_type))
	{
		...
		tmp.vecsize	 = 4;
		if (basic_type != SPIRType::Float && basic_type != SPIRType::Int && basic_type != SPIRType::UInt) SPIRV_CROSS_THROW("Basic types in a flattened UBO must be float, int or uint.");

		auto flags = ir.get_buffer_block_flags(var);
		statement("uniform ", flags_to_qualifiers_glsl(tmp, flags), type_to_glsl(tmp), " ", buffer_name, "[", buffer_size, "];");
	}
	else
...
const char* CompilerGLSL::to_storage_qualifiers_glsl(const SPIRVariable& var)
{
	...
	else if (var.storage == StorageClassUniformConstant || var.storage == StorageClassUniform || var.storage == StorageClassPushConstant || var.storage == StorageClassAtomicCounter) { return "uniform "; }
	else if (var.storage == StorageClassRayPayloadKHR) { return ray_tracing_is_khr ? "rayPayloadEXT " : "rayPayloadNV "; }
	else if (var.storage == StorageClassIncomingRayPayloadKHR) { return ray_tracing_is_khr ? "rayPayloadInEXT " : "rayPayloadInNV "; }
	else if (var.storage == StorageClassHitAttributeKHR) { return ray_tracing_is_khr ? "hitAttributeEXT " : "hitAttributeNV "; }
	else if (var.storage == StorageClassCallableDataKHR) { return ray_tracing_is_khr ? "callableDataEXT " : "callableDataNV "; }
	else if (var.storage == StorageClassIncomingCallableDataKHR) { return ray_tracing_is_khr ? "callableDataInEXT " : "callableDataInNV "; }
...
void CompilerGLSL::emit_flattened_io_block_member(const std::string& basename, const SPIRType& type, const char* qual, const SmallVector<uint32_t>& indices)
{
	...
	if (parent_type->type_alias) parent_type = &get<SPIRType>(parent_type->type_alias);
...
void CompilerGLSL::emit_flattened_io_block_struct(const std::string& basename, const SPIRType& type, const char* qual, const SmallVector<uint32_t>& indices)
{
	...
	if (!member_type->array.empty()) SPIRV_CROSS_THROW("Cannot flatten array of structs in I/O blocks.");
...
void CompilerGLSL::emit_flattened_io_block(const SPIRVariable& var, const char* qual)
{
	...
	if (!var_type.array.empty()) SPIRV_CROSS_THROW("Array of varying structs cannot be flattened to legacy-compatible varyings.");
...
void CompilerGLSL::emit_interface_block(const SPIRVariable& var)
{
	...
	if (var.storage == StorageClassInput && type.basetype == SPIRType::Double && !options.es && options.version < 410) { require_extension_internal("GL_ARB_vertex_attrib_64bit"); }
	...
	if (block)
	{
		...
		else
		{
			if (options.es && options.version < 320)
			{
				...
				if (!has_extension("GL_EXT_geometry_shader") && !has_extension("GL_EXT_tessellation_shader")) require_extension_internal("GL_EXT_shader_io_blocks");
			}
			...
			// If for some reason buffer_name is an illegal name, make a final fallback to a workaround name.
			// This cannot conflict with anything else, so we're safe now.
			if (block_name.empty()) block_name = join("_", get<SPIRType>(var.basetype).self, "_", var.self);
	...
	else
	{
		...
		// I/O variables which are struct types.
		// To support this, flatten the struct into separate varyings instead.
		if (type.basetype == SPIRType::Struct && (options.force_flattened_io_blocks || (options.es && options.version < 310) || (!options.es && options.version < 150))) { emit_flattened_io_block(var, qual); }
		else
		{
			add_resource_name(var.self);
			...
			if (is_legacy() && var.storage == StorageClassInput && type.basetype == SPIRType::Int) newtype.basetype = SPIRType::Float;
			...
			if (type.storage == StorageClassInput && !type.array.empty() && !has_decoration(var.self, DecorationPatch) && (get_entry_point().model == ExecutionModelTessellationControl || get_entry_point().model == ExecutionModelTessellationEvaluation))
			{
...
void CompilerGLSL::emit_specialization_constant_op(const SPIRConstantOp& constant)
{
	...
	if (type_is_top_level_block(type)) return;
...
void CompilerGLSL::emit_constant(const SPIRConstant& constant)
{
	...
	if (constant.self == workgroup_size_id) return;
	...
	{
		if (options.vulkan_semantics) { statement("layout(constant_id = ", get_decoration(constant.self, DecorationSpecId), ") const ", variable_decl(type, name), " = ", constant_expression(constant), ";"); }
		else
		{
			...
			if (!is_workgroup_size_constant) statement("const ", variable_decl(type, name), " = ", macro_name, ";");
		}
	}
	else { statement("const ", variable_decl(type, name), " = ", constant_expression(constant), ";"); }
}
...
void CompilerGLSL::replace_illegal_names(const unordered_set<string>& keywords)
{
	ir.for_each_typed_id<SPIRVariable>([&](uint32_t, const SPIRVariable& var) {
		...
		if (keywords.find(m.alias) != end(keywords)) m.alias = join("_", m.alias);
	});
	...
	ir.for_each_typed_id<SPIRType>([&](uint32_t, const SPIRType& type) {
		...
		for (auto& memb : meta->members)
			if (keywords.find(memb.alias) != end(keywords)) memb.alias = join("_", memb.alias);
	});
}
...
void CompilerGLSL::replace_fragment_output(SPIRVariable& var)
{
	...
	uint32_t location = 0;
	if (m.decoration_flags.get(DecorationLocation)) location = m.location;
	...
	if (type.array.empty())
	{
		...
		if (is_legacy_es() && location != 0) require_extension_internal("GL_EXT_draw_buffers");
	}
	else if (type.array.size() == 1)
	{
		...
		if (is_legacy_es()) require_extension_internal("GL_EXT_draw_buffers");
	}
	else
...
void CompilerGLSL::replace_fragment_outputs()
{
	ir.for_each_typed_id<SPIRVariable>([&](uint32_t, SPIRVariable& var) {
		...
		if (!is_builtin_variable(var) && !var.remapped_variable && type.pointer && var.storage == StorageClassOutput) replace_fragment_output(var);
	});
}
...
string CompilerGLSL::remap_swizzle(const SPIRType& out_type, uint32_t input_components, const string& expr)
{
	...
	else
	{
		...
		if (backend.swizzle_is_function && out_type.vecsize > 1) e += "()";
...
{
	auto& execution = get_entry_point();
	if (execution.model != ExecutionModelFragment) SPIRV_CROSS_THROW("Pixel local storage only supported in fragment shaders.");

	if (!options.es) SPIRV_CROSS_THROW("Pixel local storage only supported in OpenGL ES.");

	if (options.version < 300) SPIRV_CROSS_THROW("Pixel local storage only supported in ESSL 3.0 and above.");
...
{
	if (!options.enable_storage_image_qualifier_deduction) return;
...
bool CompilerGLSL::should_force_emit_builtin_block(StorageClass storage)
{
	...
	if (storage != StorageClassOutput) return false;
	...
	ir.for_each_typed_id<SPIRVariable>([&](uint32_t, SPIRVariable& var) {
		...
		if (var.storage == storage && block && is_builtin_variable(var))
		{
			...
			for (uint32_t i = 0; i < member_count; i++)
			{
				if (has_member_decoration(type.self, i, DecorationBuiltIn) && is_block_builtin(BuiltIn(get_member_decoration(type.self, i, DecorationBuiltIn))) && has_member_decoration(type.self, i, DecorationOffset)) { should_force = true; }
			}
		}
		else if (var.storage == storage && !block && is_builtin_variable(var))
		{
			if (is_block_builtin(BuiltIn(get_decoration(type.self, DecorationBuiltIn))) && has_decoration(var.self, DecorationOffset)) { should_force = true; }
		}
	});

	// If we're declaring clip/cull planes with control points we need to force block declaration.
	if ((get_execution_model() == ExecutionModelTessellationControl || get_execution_model() == ExecutionModelMeshEXT) && (clip_distance_count || cull_distance_count)) { should_force = true; }

	// Either glslang bug or oversight, but global invariant position does not work in mesh shaders.
	if (get_execution_model() == ExecutionModelMeshEXT && position_invariant) should_force = true;
...
void CompilerGLSL::emit_declared_builtin_block(StorageClass storage, ExecutionModel model)
{
	...
	const auto builtin_is_per_vertex_set = [](BuiltIn builtin) -> bool { return builtin == BuiltInPosition || builtin == BuiltInPointSize || builtin == BuiltInClipDistance || builtin == BuiltInCullDistance; };
	...
	ir.for_each_typed_id<SPIRVariable>([&](uint32_t, SPIRVariable& var) {
		...
		if (var.storage == storage && block && is_builtin_variable(var))
		{
			...
			for (auto& m : ir.meta[type.self].members)
			{
				if (m.builtin && builtin_is_per_vertex_set(m.builtin_type))
				{
					...
					if (is_block_builtin(m.builtin_type) && m.decoration_flags.get(DecorationStream))
					{
						...
						if (have_geom_stream && geom_stream != stream) SPIRV_CROSS_THROW("IO block member Stream mismatch.");
						have_geom_stream = true;
						geom_stream		 = stream;
					}
				}
			...
			if (storage == StorageClassOutput && has_decoration(var.self, DecorationXfbBuffer) && has_decoration(var.self, DecorationXfbStride))
			{
				...
				if (have_xfb_buffer_stride && buffer_index != xfb_buffer) SPIRV_CROSS_THROW("IO block member XfbBuffer mismatch.");
				if (have_xfb_buffer_stride && stride != xfb_stride) SPIRV_CROSS_THROW("IO block member XfbBuffer mismatch.");
				have_xfb_buffer_stride = true;
				xfb_buffer			   = buffer_index;
				xfb_stride			   = stride;
			}
		...
		else if (var.storage == storage && !block && is_builtin_variable(var))
		{
			...
			if (m.builtin && builtin_is_per_vertex_set(m.builtin_type))
			{
				...
				if (is_block_builtin(m.builtin_type) && m.decoration_flags.get(DecorationXfbStride) && m.decoration_flags.get(DecorationXfbBuffer) && m.decoration_flags.get(DecorationOffset))
				{
					have_any_xfb_offset					= true;
					builtin_xfb_offsets[m.builtin_type] = m.offset;
		...
		if (emitted_block) SPIRV_CROSS_THROW("Cannot use more than one builtin I/O block.");
	...
	global_builtins = Bitset(global_builtins.get_lower() & ((1ull << BuiltInPosition) | (1ull << BuiltInPointSize) | (1ull << BuiltInClipDistance) | (1ull << BuiltInCullDistance)));

	// Try to collect all other declared builtins.
	if (!emitted_block) emitted_builtins = global_builtins;

	// Can't declare an empty interface block.
	if (emitted_builtins.empty()) return;
	...
	bool builtin_array = model == ExecutionModelTessellationControl || (model == ExecutionModelMeshEXT && storage == StorageClassOutput) || (model == ExecutionModelGeometry && storage == StorageClassInput) ||
...
bool CompilerGLSL::variable_is_lut(const SPIRVariable& var) const
{
	...
	if (statically_assigned)
	{
		...
		if (constant && constant->is_used_as_lut) return true;
	}
...
void CompilerGLSL::emit_resources()
{
	...
	// with builtins.
	if (execution.model == ExecutionModelFragment && is_legacy()) replace_fragment_outputs();

	// Emit PLS blocks if we have such variables.
	if (!pls_inputs.empty() || !pls_outputs.empty()) emit_pls();
	...
	else
	{
		...
		if (clip_distance_count != 0) statement(storage, " float gl_ClipDistance[", clip_distance_count, "];");
		if (cull_distance_count != 0) statement(storage, " float gl_CullDistance[", cull_distance_count, "];");
		if (clip_distance_count != 0 || cull_distance_count != 0) statement("");
	}
	...
	//
	{
		...
		for (auto& id_ : ir.ids_for_constant_undef_or_type)
		{
			...
			if (id.get_type() == TypeConstant)
			{
				...
				{
					if (!options.vulkan_semantics && c.specialization) { c.specialization_constant_macro_name = constant_value_macro_name(get_decoration(c.self, DecorationSpecId)); }
					emit_constant(c);
					emitted = true;
				}
			}
			...
			else if (id.get_type() == TypeType)
			{
				...
				bool is_natural_struct = type->basetype == SPIRType::Struct && type->array.empty() && !type->pointer && (!has_decoration(type->self, DecorationBlock) && !has_decoration(type->self, DecorationBufferBlock));
				...
				if (type->basetype == SPIRType::Struct && type->pointer && has_decoration(type->self, DecorationBlock) &&
					(type->storage == StorageClassRayPayloadKHR || type->storage == StorageClassIncomingRayPayloadKHR || type->storage == StorageClassHitAttributeKHR))
				{
			...
			else if (id.get_type() == TypeUndef)
			{
				...
				string initializer;
				if (options.force_zero_initialized_variables && type_can_zero_initialize(type)) initializer = join(" = ", to_zero_initialized_expression(undef.basetype));
	...
	if (execution.model == ExecutionModelGLCompute && !options.vulkan_semantics && (execution.workgroup_size.constant != 0 || execution.flags.get(ExecutionModeLocalSizeId)))
	{
		SpecializationConstant wg_x, wg_y, wg_z;
		get_work_group_size_specialization_constants(wg_x, wg_y, wg_z);
	...
	if (ir.addressing_model == AddressingModelPhysicalStorageBuffer64EXT)
	{
		...
		ir.for_each_typed_id<SPIRType>([&](uint32_t id, SPIRType& type) {
			if (is_physical_pointer(type))
			{
				...
				if (!is_physical_pointer_to_buffer_block(type))
				{
					...
					emit_type = std::find(physical_storage_non_block_pointer_types.begin(), physical_storage_non_block_pointer_types.end(), id) != physical_storage_non_block_pointer_types.end();
				}

				if (emit_type) emit_buffer_reference_block(id, true);
			}
		});
		...
		ir.for_each_typed_id<SPIRType>([&](uint32_t id, SPIRType& type) {
			if (is_physical_pointer_to_buffer_block(type)) emit_buffer_reference_block(id, false);
		});
	}
	...
	ir.for_each_typed_id<SPIRVariable>([&](uint32_t, SPIRVariable& var) {
		...
		bool is_block_storage = type.storage == StorageClassStorageBuffer || type.storage == StorageClassUniform || type.storage == StorageClassShaderRecordBufferKHR;
		bool has_block_flags  = ir.meta[type.self].decoration.decoration_flags.get(DecorationBlock) || ir.meta[type.self].decoration.decoration_flags.get(DecorationBufferBlock);

		if (var.storage != StorageClassFunction && type.pointer && is_block_storage && !is_hidden_variable(var) && has_block_flags) { emit_buffer_block(var); }
	});

	// Output push constant blocks
	ir.for_each_typed_id<SPIRVariable>([&](uint32_t, SPIRVariable& var) {
		auto& type = this->get<SPIRType>(var.basetype);
		if (var.storage != StorageClassFunction && type.pointer && type.storage == StorageClassPushConstant && !is_hidden_variable(var)) { emit_push_constant_block(var); }
	});
	...
	ir.for_each_typed_id<SPIRVariable>([&](uint32_t, SPIRVariable& var) {
		...
		if (skip_separate_image_sampler)
		{
			...
			if (!sampler_buffer && (separate_image || separate_sampler)) return;
		}

		if (var.storage != StorageClassFunction && type.pointer &&
			(type.storage == StorageClassUniformConstant || type.storage == StorageClassAtomicCounter || type.storage == StorageClassRayPayloadKHR || type.storage == StorageClassIncomingRayPayloadKHR || type.storage == StorageClassCallableDataKHR ||
	...
	ir.for_each_typed_id<SPIRVariable>([&](uint32_t, SPIRVariable& var) {
		...
		if (var.storage == StorageClassOutput && !is_legacy() && location_is_framebuffer_fetch(get_decoration(var.self, DecorationLocation)) != 0) { is_hidden = false; }

		if (var.storage != StorageClassFunction && type.pointer && (var.storage == StorageClassInput || var.storage == StorageClassOutput) && interface_variable_exists_in_entry_point(var.self) && !is_hidden)
		{
			if (options.es && get_execution_model() == ExecutionModelVertex && var.storage == StorageClassInput && type.array.size() == 1) { SPIRV_CROSS_THROW("OpenGL ES doesn't support array input variables in vertex shader."); }
		...
		else if (is_builtin_variable(var))
		{
			...
			if (!options.vulkan_semantics)
			{
				if (!emitted_base_instance && ((options.vertex.support_nonzero_base_instance && builtin == BuiltInInstanceIndex) || (builtin == BuiltInBaseInstance)))
				{
	...
	for (auto global : global_variables)
	{
		...
		if (is_hidden_variable(var, true)) continue;
		...
		if (var.storage != StorageClassOutput)
		{
			if (!variable_is_lut(var))
			{
				...
				string initializer;
				if (options.force_zero_initialized_variables && var.storage == StorageClassPrivate && !var.initializer && !var.static_expression && type_can_zero_initialize(get_variable_data_type(var)))
				{
		...
		else if (var.initializer && maybe_get<SPIRConstant>(var.initializer) != nullptr) { emit_output_variable_initializer(var); }
	}
...
void CompilerGLSL::emit_output_variable_initializer(const SPIRVariable& var)
{
	...
	if (is_block)
	{
		...
		if (type_is_array) array_size = to_array_size_literal(type);
		...
		for (uint32_t i = 0; i < member_count; i++)
		{
			...
			{
				if (get_member_decoration(type.self, i, DecorationBuiltIn) == BuiltInCullDistance && !cull_distance_count) continue;

				if (get_member_decoration(type.self, i, DecorationBuiltIn) == BuiltInClipDistance && !clip_distance_count) continue;
			}
			...
			if (type_is_array)
			{
				...
				statement("const ", type_to_glsl(array_type), " ", lut_name, type_to_array_glsl(array_type), " = ", type_to_glsl_constructor(array_type), "(", merge(exprs, ", "), ");");
			}
	...
	else if (is_control_point)
	{
		...
		statement("const ", type_to_glsl(type), " ", lut_name, type_to_array_glsl(type), " = ", to_expression(var.initializer), ";");
...
void CompilerGLSL::emit_subgroup_arithmetic_workaround(const std::string& func, Op op, GroupOperation group_op)
{
	...
	if (op_is_addition) { op_symbol = "+="; }
	else if (op_is_multiplication) { op_symbol = "*="; }
	...
	for (const TypeInfo& t : type_infos)
	{
		...
		if (group_op == GroupOperationReduce) { statement(t.type, " s = shuffleXorNV(", result, ", i, gl_SubgroupSize, valid);"); }
		else if (group_op == GroupOperationExclusiveScan || group_op == GroupOperationInclusiveScan) { statement(t.type, " s = shuffleUpNV(", result, ", i, gl_SubgroupSize, valid);"); }
		if (op_is_addition || op_is_multiplication) { statement(result, " ", op_symbol, " valid ? s : ", t.identity, ";"); }
		end_scope();
		...
		statement("else");
		begin_scope();
		if (group_op == GroupOperationExclusiveScan) { statement("uint total = subgroupBallotBitCount(gl_SubgroupLtMask);"); }
		else if (group_op == GroupOperationInclusiveScan) { statement("uint total = subgroupBallotBitCount(gl_SubgroupLeMask);"); }
		...
		if (group_op == GroupOperationExclusiveScan || group_op == GroupOperationInclusiveScan) { statement("valid = valid && (i < total);"); }
		if (op_is_addition || op_is_multiplication) { statement(result, " ", op_symbol, " valid ? s : ", t.identity, ";"); }
		end_scope();
		end_scope();
...
{
	static const char* workaround_types[] = {"int", "ivec2", "ivec3", "ivec4", "uint", "uvec2", "uvec3", "uvec4", "float", "vec2", "vec3", "vec4", "double", "dvec2", "dvec3", "dvec4"};
	...
	if (!options.vulkan_semantics)
	{
		...
		auto arithmetic_feature_helper = [&](Supp::Feature feat, std::string func_name, spv::Op op, spv::GroupOperation group_op) {
	...
	if (!workaround_ubo_load_overload_types.empty())
	{
		for (auto& type_id : workaround_ubo_load_overload_types)
		{
			...
			else { statement(type_to_glsl(type), " spvWorkaroundRowMajor(", type_to_glsl(type), " wrap) { return wrap; }"); }
		}
		statement("");
	}
}
...
void CompilerGLSL::emit_polyfills(uint32_t polyfills, bool relaxed)
{
	...
	if (options.es) qual = relaxed ? "mediump " : "highp ";
...
string CompilerGLSL::to_func_call_arg(const SPIRFunction::Parameter&, uint32_t id)
{
	...
	if (var && var->basevariable) name_id = var->basevariable;
...
uint32_t CompilerGLSL::consume_temporary_in_precision_context(uint32_t type_id, uint32_t id, Options::Precision precision)
{
	...
	if (handle_type == TypeConstant || handle_type == TypeConstantOp || handle_type == TypeUndef) return id;

	// Ignore anything that isn't 32-bit values.
	auto& type = get<SPIRType>(type_id);
	if (type.pointer) return id;
	if (type.basetype != SPIRType::Float && type.basetype != SPIRType::UInt && type.basetype != SPIRType::Int) return id;
	...
	if (itr == temporary_to_mirror_precision_alias.end())
	{
		...
		if (auto* input_m = ir.find_meta(id)) m = *input_m;
...
void CompilerGLSL::handle_invalid_expression(uint32_t id)
{
	...
	if (composite_insert_overwritten.count(id)) block_composite_insert_overwrite.insert(id);
}
...
{
	if (expr.size() < 2 || expr.front() != '(' || expr.back() != ')') return;
	...
	for (auto& c : expr)
	{
		...
		else if (c == ')')
		{
			...
			if (paren_count == 0 && &c != &expr.back()) return;
		}
	}
...
bool CompilerGLSL::needs_enclose_expression(const std::string& expr)
{
	...
	{
		auto c = expr.front();
		if (c == '-' || c == '+' || c == '!' || c == '~' || c == '&' || c == '*') need_parens = true;
	}
...
{
	return unpack_expression_type(to_expression(id), expression_type(id), get_extended_decoration(id, SPIRVCrossDecorationPhysicalTypeID), has_extended_decoration(id, SPIRVCrossDecorationPhysicalTypePacked), true);
}
...
string CompilerGLSL::to_unpacked_expression(uint32_t id, bool register_expression_read)
{
	...
	if (!need_transpose && (is_remapped || is_packed))
	{
		return unpack_expression_type(
			to_expression(id, register_expression_read), get_pointee_type(expression_type_id(id)), get_extended_decoration(id, SPIRVCrossDecorationPhysicalTypeID), has_extended_decoration(id, SPIRVCrossDecorationPhysicalTypePacked), false);
	}
	else
...
{
	bool remapped_boolean = parent_type.basetype == SPIRType::Struct && type.basetype == SPIRType::Boolean && backend.boolean_in_struct_remapped_type != SPIRType::Boolean;
	...
	for (uint32_t i = 0; i < size; i++)
	{
		...
		if (!is_array(parent))
		{
			if (remapped_boolean) subexpr = join(type_to_glsl(tmp_type), "(", subexpr, ")");
			expr += subexpr;
		}
		else
...
string CompilerGLSL::to_non_uniform_aware_expression(uint32_t id)
{
	...
	if (has_decoration(id, DecorationNonUniform)) convert_non_uniform_expression(expr, id);
...
string CompilerGLSL::to_expression(uint32_t id, bool register_expression_read)
{
	...
	if (itr != end(invalid_expressions)) handle_invalid_expression(id);
	...
	if (ir.ids[id].get_type() == TypeExpression)
	{
		...
		for (uint32_t dep : expr.expression_dependencies)
			if (invalid_expressions.find(dep) != end(invalid_expressions)) handle_invalid_expression(dep);
	}

	if (register_expression_read) track_expression_read(id);
	...
	case TypeExpression: {
		...
		else if (flattened_structs.count(id)) { return load_flattened_struct(e.expression, get<SPIRType>(e.expression_type)); }
		else
		{
		...
		else if (c.specialization)
		{
			if (backend.workgroup_size_is_hidden)
			{
				...
				if (wg_index >= 0)
				{
					...
					if (type.basetype != SPIRType::UInt) wg_size = bitcast_expression(type, SPIRType::UInt, wg_size);
					return wg_size;
				}
			}

			if (expression_is_forwarded(id)) return constant_expression(c);
	...
	case TypeVariable: {
		...
		else if (flattened_structs.count(id)) { return load_flattened_struct(to_name(id), get<SPIRType>(var.basetype)); }
		else
		{
...
SmallVector<ConstantID> CompilerGLSL::get_composite_constant_ids(ConstantID const_id)
{
	if (auto* constant = maybe_get<SPIRConstant>(const_id))
	{
		...
		if (is_array(type) || type.basetype == SPIRType::Struct) return constant->subconstants;
		if (is_matrix(type)) return SmallVector<ConstantID>(constant->m.id);
		if (is_vector(type)) return SmallVector<ConstantID>(constant->m.c[0].id);
		SPIRV_CROSS_THROW("Unexpected scalar constant!");
	}
	if (!const_composite_insert_ids.count(const_id)) SPIRV_CROSS_THROW("Unimplemented for this OpSpecConstantOp!");
...
void CompilerGLSL::fill_composite_constant(SPIRConstant& constant, TypeID type_id, const SmallVector<ConstantID>& initializers)
{
	...
	if (is_array(type) || type.basetype == SPIRType::Struct) { constant.subconstants = initializers; }
...
{
	auto& type = get<SPIRType>(type_id);
	if (is_array(type)) return type.parent_type;
	if (type.basetype == SPIRType::Struct) return type.member_types[member_idx];
	if (is_matrix(type)) return type.parent_type;
	if (is_vector(type)) return type.parent_type;
...
string CompilerGLSL::constant_op_expression(const SPIRConstantOp& cop)
{
	...
	if (is_legacy() && is_unsigned_opcode(cop.opcode)) SPIRV_CROSS_THROW("Unsigned integers are not supported on legacy targets.");
...
#define GLSL_UOP(opname, x)                                                                                                                                                                                                                                        \
	...
	case OpSRem: {
		...
		return join(to_enclosed_expression(op0), " - ", to_enclosed_expression(op1), " * ", "(", to_enclosed_expression(op0), " / ", to_enclosed_expression(op1), ")");
	}

	case OpSelect: {
		if (cop.arguments.size() < 3) SPIRV_CROSS_THROW("Not enough arguments to OpSpecConstantOp.");
	...
	case OpVectorShuffle: {
		...
		for (uint32_t i = 2; i < uint32_t(cop.arguments.size()); i++)
		{
			...
			else if (index >= left_components) { expr += right_arg + "." + "xyzw"[index - left_components]; }
			else { expr += left_arg + "." + "xyzw"[index]; }

			if (i + 1 < uint32_t(cop.arguments.size())) expr += ", ";
		}
	...
	case OpCompositeExtract: {
		auto expr = access_chain_internal(cop.arguments[0], &cop.arguments[1], uint32_t(cop.arguments.size() - 1), ACCESS_CHAIN_INDEX_IS_LITERAL_BIT, nullptr);
		return expr;
	}
	...
	if (unary || binary || cop.opcode == OpSConvert || cop.opcode == OpUConvert) bit_width = expression_type(cop.arguments[0]).width;
...
#undef GLSL_UOP
	if (binary)
	{
		if (cop.arguments.size() < 2) SPIRV_CROSS_THROW("Not enough arguments to OpSpecConstantOp.");
	...
	{
		if (cop.arguments.size() < 1) SPIRV_CROSS_THROW("Not enough arguments to OpSpecConstantOp.");
...
string CompilerGLSL::constant_expression(const SPIRConstant& c, bool inside_block_like_struct_scope, bool inside_struct_scope)
{
	...
	if (is_pointer(type)) { return backend.null_pointer_literal; }
	else if (!c.subconstants.empty())
	{
		...
		if (backend.use_initializer_list && backend.use_typed_initializer_list && type.basetype == SPIRType::Struct && !is_array(type)) { res = type_to_glsl_constructor(type) + "{ "; }
		...
		else if (backend.use_initializer_list) { res = "{ "; }
		else { res = type_to_glsl_constructor(type) + "("; }

		uint32_t subconstant_index = 0;
		for (auto& elem : c.subconstants)
		{
			if (auto* op = maybe_get<SPIRConstantOp>(elem)) { res += constant_op_expression(*op); }
			else if (maybe_get<SPIRUndef>(elem) != nullptr) { res += to_name(elem); }
			else
			{
				...
				else
				{
					...
					if (type.basetype == SPIRType::Struct) inside_struct_scope = true;

					res += constant_expression(subc, inside_block_like_struct_scope, inside_struct_scope);
				}
			}

			if (&elem != &c.subconstants.back()) res += ", ";

			subconstant_index++;
		}

		res += backend.use_initializer_list ? " }" : ")";
		if (needs_trailing_tracket) res += ")";
...
string CompilerGLSL::convert_float_to_string(const SPIRConstant& c, uint32_t col, uint32_t row)
{
	...
	else
	{
		...
		if (backend.float_literal_suffix) res += "f";
	}
...
std::string CompilerGLSL::convert_double_to_string(const SPIRConstant& c, uint32_t col, uint32_t row)
{
	...
	if (std::isnan(double_value) || std::isinf(double_value))
	{
...
#else
			snprintf(print_buffer, sizeof(print_buffer), "0x%llx%s", static_cast<unsigned long long>(u64_value), backend.long_long_literal_suffix ? "ull" : "ul");
#endif
		...
		{
			if (options.es) SPIRV_CROSS_THROW("FP64 not supported in ES profile.");
			if (options.version < 400) require_extension_internal("GL_ARB_gpu_shader_fp64");
	...
	else
	{
		...
		if (backend.double_literal_suffix) res += "lf";
	}
...
string CompilerGLSL::constant_expression_vector(const SPIRConstant& c, uint32_t vector)
{
	...
	if (c.vector_size() > 1 && !swizzle_splat) res += type_to_glsl(type) + "(";
	...
	case SPIRType::Half:
		if (splat || swizzle_splat)
		{
			...
			if (swizzle_splat) res = remap_swizzle(get<SPIRType>(c.constant_type), 1, res);
		}
		else
		{
			for (uint32_t i = 0; i < c.vector_size(); i++)
	...
	case SPIRType::Int64: {
		...
		if (splat) { res += convert_to_string(c.scalar_i64(vector, 0), int64_type, backend.long_long_literal_suffix); }
		else
		{
			for (uint32_t i = 0; i < c.vector_size(); i++)
			{
	...
	case SPIRType::UInt:
		if (splat)
		{
			...
			if (is_legacy())
			{
				...
				if (c.scalar_i32(vector, 0) < 0) SPIRV_CROSS_THROW("Tried to convert uint literal into int, but this made the literal negative.");
			}
	...
	case SPIRType::UShort:
		if (splat) { res += convert_to_string(c.scalar(vector, 0)); }
		else
		{
			for (uint32_t i = 0; i < c.vector_size(); i++)
			{
	...
	case SPIRType::Short:
		if (splat) { res += convert_to_string(c.scalar_i16(vector, 0)); }
		else
		{
			for (uint32_t i = 0; i < c.vector_size(); i++)
			{
	...
	case SPIRType::UByte:
		if (splat) { res += convert_to_string(c.scalar_u8(vector, 0)); }
		else
		{
			for (uint32_t i = 0; i < c.vector_size(); i++)
			{
	...
	case SPIRType::SByte:
		if (splat) { res += convert_to_string(c.scalar_i8(vector, 0)); }
		else
		{
			for (uint32_t i = 0; i < c.vector_size(); i++)
			{
	...
	if (c.vector_size() > 1 && !swizzle_splat) res += ")";
...
void CompilerGLSL::emit_uninitialized_temporary(uint32_t result_type, uint32_t result_id)
{
	...
	if (!block_temporary_hoisting && current_continue_block && !hoisted_temporaries.count(result_id))
	{
		...
		if (find_if(begin(header.declare_temporary), end(header.declare_temporary), [result_type, result_id](const pair<uint32_t, uint32_t>& tmp) { return tmp.first == result_type && tmp.second == result_id; }) == end(header.declare_temporary))
		{
	...
	else if (hoisted_temporaries.count(result_id) == 0)
	{
		...
		string initializer;
		if (options.force_zero_initialized_variables && type_can_zero_initialize(type)) initializer = join(" = ", to_zero_initialized_expression(result_type));
...
SPIRExpression& CompilerGLSL::emit_op(uint32_t result_type, uint32_t result_id, const string& rhs, bool forwarding, bool suppress_usage_tracking)
{
	if (forwarding && (forced_temporaries.find(result_id) == end(forced_temporaries)))
	{
		...
		if (suppress_usage_tracking) suppressed_usage_tracking.insert(result_id);
...
{
	statement("EmitMeshTasksEXT(", to_unpacked_expression(block.mesh.groups[0]), ", ", to_unpacked_expression(block.mesh.groups[1]), ", ", to_unpacked_expression(block.mesh.groups[2]), ");");
}
...
void CompilerGLSL::emit_binary_op(uint32_t result_type, uint32_t result_id, uint32_t op0, uint32_t op1, const char* op)
{
	...
	bool force_temporary_precise = backend.support_precise_qualifier && has_decoration(result_id, DecorationNoContraction) && type_is_floating_point(get<SPIRType>(result_type));
...
void CompilerGLSL::emit_unrolled_binary_op(uint32_t result_type, uint32_t result_id, uint32_t op0, uint32_t op1, const char* op, bool negate, SPIRType::BaseType expected_type)
{
...
SPIRType CompilerGLSL::binary_op_bitcast_helper(string& cast_op0, string& cast_op1, SPIRType::BaseType& input_type, uint32_t op0, uint32_t op1, bool skip_cast_if_equal_type)
{
	auto& type0 = expression_type(op0);
	auto& type1 = expression_type(op1);
...
void CompilerGLSL::emit_binary_op_cast(uint32_t result_type, uint32_t result_id, uint32_t op0, uint32_t op1, const char* op, SPIRType::BaseType input_type, bool skip_cast_if_equal_type, bool implicit_integer_promotion)
{
...
void CompilerGLSL::emit_atomic_func_op(uint32_t result_type, uint32_t result_id, uint32_t op0, uint32_t op1, const char* op)
{
	...
	if (type_is_floating_point(type))
	{
		if (!options.vulkan_semantics) SPIRV_CROSS_THROW("Floating point atomics requires Vulkan semantics.");
		if (options.es) SPIRV_CROSS_THROW("Floating point atomics requires desktop GLSL.");
		require_extension_internal("GL_EXT_shader_atomic_float");
	}

	forced_temporaries.insert(result_id);
	emit_op(result_type, result_id, join(op, "(", to_non_uniform_aware_expression(op0), ", ", to_unpacked_expression(op1), ")"), false);
...
{
	forced_temporaries.insert(result_id);
	emit_op(result_type, result_id, join(op, "(", to_non_uniform_aware_expression(op0), ", ", to_unpacked_expression(op1), ", ", to_unpacked_expression(op2), ")"), false);
	flush_all_atomic_capable_variables();
}

void CompilerGLSL::emit_unary_func_op_cast(uint32_t result_type, uint32_t result_id, uint32_t op0, const char* op, SPIRType::BaseType input_type, SPIRType::BaseType expected_result_type)
{
	auto& out_type		= get<SPIRType>(result_type);
...
void CompilerGLSL::emit_trinary_func_op_bitextract(
	uint32_t result_type, uint32_t result_id, uint32_t op0, uint32_t op1, uint32_t op2, const char* op, SPIRType::BaseType expected_result_type, SPIRType::BaseType input_type0, SPIRType::BaseType input_type1, SPIRType::BaseType input_type2)
{
	auto& out_type		   = get<SPIRType>(result_type);
	auto  expected_type	   = out_type;
	...
	else { expr += join(op, "(", cast_op0, ", ", cast_op1, ", ", cast_op2, ")"); }
...
void CompilerGLSL::emit_trinary_func_op_cast(uint32_t result_type, uint32_t result_id, uint32_t op0, uint32_t op1, uint32_t op2, const char* op, SPIRType::BaseType input_type)
{
	auto& out_type		   = get<SPIRType>(result_type);
	auto  expected_type	   = out_type;
	expected_type.basetype = input_type;
...
void CompilerGLSL::emit_binary_func_op_cast_clustered(uint32_t result_type, uint32_t result_id, uint32_t op0, uint32_t op1, const char* op, SPIRType::BaseType input_type)
{
	...
	else { expr += join(op, "(", cast_op0, ", ", to_expression(op1), ")"); }

	emit_op(result_type, result_id, expr, should_forward(op0));
	inherit_expression_dependencies(result_id, op0);
}

void CompilerGLSL::emit_binary_func_op_cast(uint32_t result_type, uint32_t result_id, uint32_t op0, uint32_t op1, const char* op, SPIRType::BaseType input_type, bool skip_cast_if_equal_type)
{
	string cast_op0, cast_op1;
	auto   expected_type = binary_op_bitcast_helper(cast_op0, cast_op1, input_type, op0, op1, skip_cast_if_equal_type);
	auto&  out_type		 = get<SPIRType>(result_type);
	...
	else { expr += join(op, "(", cast_op0, ", ", cast_op1, ")"); }
...
void CompilerGLSL::emit_trinary_func_op(uint32_t result_type, uint32_t result_id, uint32_t op0, uint32_t op1, uint32_t op2, const char* op)
{
	...
	emit_op(result_type, result_id, join(op, "(", to_unpacked_expression(op0), ", ", to_unpacked_expression(op1), ", ", to_unpacked_expression(op2), ")"), forward);
...
void CompilerGLSL::emit_quaternary_func_op(uint32_t result_type, uint32_t result_id, uint32_t op0, uint32_t op1, uint32_t op2, uint32_t op3, const char* op)
{
	...
	emit_op(result_type, result_id, join(op, "(", to_unpacked_expression(op0), ", ", to_unpacked_expression(op1), ", ", to_unpacked_expression(op2), ", ", to_unpacked_expression(op3), ")"), forward);
...
void CompilerGLSL::emit_bitfield_insert_op(uint32_t result_type, uint32_t result_id, uint32_t op0, uint32_t op1, uint32_t op2, uint32_t op3, const char* op, SPIRType::BaseType offset_count_type)
{
...
string CompilerGLSL::legacy_tex_op(const std::string& op, const SPIRType& imgtype, uint32_t tex)
{
	...
	{
		if (is_legacy_es()) SPIRV_CROSS_THROW(join(op, " not allowed in legacy ES"));
	...
	if (is_comparison && is_legacy_es())
	{
		...
		if (imgtype.image.dim == spv::DimCube) return "shadowCubeNV";
	}

	if (op == "textureSize")
	{
		if (is_legacy_es()) SPIRV_CROSS_THROW("textureSize not supported in legacy ES");
		if (is_comparison) SPIRV_CROSS_THROW("textureSize not supported on shadow sampler in legacy GLSL");
		require_extension_internal("GL_EXT_gpu_shader4");
	}

	if (op == "texelFetch" && is_legacy_es()) SPIRV_CROSS_THROW("texelFetch not supported in legacy ES");
	...
	else { SPIRV_CROSS_THROW(join("Unsupported legacy texture op: ", op)); }
}
...
bool CompilerGLSL::to_trivial_mix_op(const SPIRType& type, string& op, uint32_t left, uint32_t right, uint32_t lerp)
{
	...
	if (cleft->specialization || cright->specialization) return false;

	auto& value_type = get<SPIRType>(cleft->constant_type);

	if (lerptype.basetype != SPIRType::Boolean) return false;
	if (value_type.basetype == SPIRType::Struct || is_array(value_type)) return false;
	if (!backend.use_constructor_splatting && value_type.vecsize != lerptype.vecsize) return false;
	...
	if (value_type.columns > 1) return false;
	...
	if (ret) op = type_to_glsl_constructor(type);
	return ret;
}
...
string CompilerGLSL::to_combined_image_sampler(VariableID image_id, VariableID samp_id)
{
	...
	if (array_index != string::npos) array_expr = image_expr.substr(array_index, string::npos);
	...
	if (image) image_id = image->self;
	if (samp) samp_id = samp->self;
	...
	if (image_itr != end(args) || sampler_itr != end(args))
	{
		...
		auto  itr	   = find_if(begin(combined), end(combined), [=](const SPIRFunction::CombinedImageSamplerParameter& p) { return p.global_image == global_image && p.global_sampler == global_sampler && p.image_id == iid && p.sampler_id == sid; });
	...
	else
	{
		...
		auto  itr	  = find_if(begin(mapping), end(mapping), [image_id, samp_id](const CombinedImageSampler& combined) { return combined.image_id == image_id && combined.sampler_id == samp_id; });
...
bool CompilerGLSL::is_supported_subgroup_op_in_opengl(spv::Op op, const uint32_t* ops)
{
	...
	case OpGroupNonUniformFMul: {
		...
		if (operation == GroupOperationReduce || operation == GroupOperationInclusiveScan || operation == GroupOperationExclusiveScan) { return true; }
		else { return false; }
	}
	default:
		return false;
...
{
	if (options.vulkan_semantics && combined_image_samplers.empty()) { emit_binary_func_op(result_type, result_id, image_id, samp_id, type_to_glsl(get<SPIRType>(result_type), result_id).c_str()); }
	else
	{
...
void CompilerGLSL::emit_sparse_feedback_temporaries(uint32_t result_type_id, uint32_t id, uint32_t& feedback_id, uint32_t& texel_id)
{
	...
	if (options.es) SPIRV_CROSS_THROW("Sparse texture feedback is not supported on ESSL.");
	require_extension_internal("GL_ARB_sparse_texture2");

	auto& temps = extra_sub_expressions[id];
	if (temps == 0) temps = ir.increase_bound_by(2);

	feedback_id = temps + 0;
	texel_id	= temps + 1;

	auto& return_type = get<SPIRType>(result_type_id);
	if (return_type.basetype != SPIRType::Struct || return_type.member_types.size() != 2) SPIRV_CROSS_THROW("Invalid return type for sparse feedback.");
...
void CompilerGLSL::emit_texture_op(const Instruction& i, bool sparse)
{
	...
	if (sparse) emit_sparse_feedback_temporaries(result_type_id, id, sparse_code_id, sparse_texel_id);
...
std::string CompilerGLSL::to_texture_op(const Instruction& i, bool sparse, bool* forward, SmallVector<uint32_t>& inherited_expressions)
{
	...
	if (has_decoration(img, DecorationNonUniform) && !maybe_get_backing_variable(img)) nonuniform_expression = true;
	...
	case OpImageSparseGather:
		...
		{
			if (!expression_is_constant_null(comp)) SPIRV_CROSS_THROW("textureGather with component requires GLSL 400.");
	...
	if (dref) inherited_expressions.push_back(dref);

	if (proj) coord_components++;
	if (imgtype.image.arrayed) coord_components++;
	...
	uint32_t sparse_texel_id = 0;
	if (sparse) sparse_texel_id = get_sparse_feedback_texel_id(ops[1]);
	...
	if (is_legacy() && !options.es && is_depth_image(imgtype, img)) expr += ".r";
	...
	if (backend.comparison_image_samples_scalar && image_opcode_is_sample_no_dref(op))
	{
		...
		if (image_variable && is_depth_image(get<SPIRType>(image_variable->basetype), image_variable->self)) image_is_depth = true;

		if (image_is_depth) expr = remap_swizzle(result_type, 1, expr);
	}
	...
	if (op == OpImageRead) expr = remap_swizzle(result_type, 4, expr);
...
bool CompilerGLSL::expression_is_non_value_type_array(uint32_t ptr)
{
	...
	if (!backend.array_is_value_type) return true;

	auto* var = maybe_get_backing_variable(ptr);
	if (!var) return false;

	auto& backed_type = get<SPIRType>(var->basetype);
	return !backend.array_is_value_type_in_buffer_blocks && backed_type.basetype == SPIRType::Struct && has_member_decoration(backed_type.self, 0, DecorationOffset);
}
...
string CompilerGLSL::to_function_name(const TextureFunctionNameArguments& args)
{
	if (args.has_min_lod)
	{
		if (options.es) SPIRV_CROSS_THROW("Sparse residency is not supported in ESSL.");
	...
	if (((imgtype.image.arrayed && imgtype.image.dim == Dim2D) || imgtype.image.dim == DimCube) && is_depth_image(imgtype, tex) && args.lod && !args.base.is_fetch)
	{
	...
	if (args.is_sparse_feedback) fname += "sparse";
	...
	else
	{
		...
		if (args.base.is_gather) fname += "Gather";
		if (args.has_array_offsets) fname += "Offsets";
		if (args.base.is_proj) fname += "Proj";
		if (args.has_grad || workaround_lod_array_shadow_as_grad) fname += "Grad";
		if (args.lod != 0 && !workaround_lod_array_shadow_as_grad) fname += "Lod";
	}

	if (args.has_offset) fname += "Offset";

	if (args.has_min_lod) fname += "Clamp";

	if (args.is_sparse_feedback || args.has_min_lod) fname += "ARB";
...
string CompilerGLSL::to_function_args(const TextureFunctionArguments& args, bool* p_forward)
{
	...
	// The workaround will assert that the LOD is in fact constant 0, or we cannot emit correct code.
	// This happens for HLSL SampleCmpLevelZero on Texture2DArray and TextureCube.
	bool workaround_lod_array_shadow_as_grad = ((imgtype.image.arrayed && imgtype.image.dim == Dim2D) || imgtype.image.dim == DimCube) && is_depth_image(imgtype, img) && args.lod != 0 && !args.base.is_fetch;
	...
	if (args.dref)
	{
		...
		else
		{
			...
			if (imgtype.image.dim == Dim1D && options.es) type.vecsize++;
			farg_str += ", ";
	...
	else
	{
		if (imgtype.image.dim == Dim1D && options.es)
		{
			...
			if (type_is_floating_point(coord_type))
			{
				...
				if (imgtype.image.arrayed || args.base.is_proj) { coord_expr = join("vec3(", enclose_expression(coord_expr), ".x, 0.0, ", enclose_expression(coord_expr), ".y)"); }
				else
					coord_expr = join("vec2(", coord_expr, ", 0.0)");
			}
			else
			{
				if (imgtype.image.arrayed) { coord_expr = join("ivec3(", enclose_expression(coord_expr), ".x, 0, ", enclose_expression(coord_expr), ".y)"); }
				else
...
void CompilerGLSL::emit_glsl_op(uint32_t result_type, uint32_t id, uint32_t eop, const uint32_t* args, uint32_t length)
{
	...
	if (is_legacy() && is_unsigned_glsl_opcode(op)) SPIRV_CROSS_THROW("Unsigned integers are not supported on legacy GLSL targets.");
	...
	case GLSLstd450ModfStruct: {
		...
		if (!is_legacy()) { statement(to_expression(id), ".", to_member_name(type, 0), " = ", "modf(", to_expression(args[0]), ", ", to_expression(id), ".", to_member_name(type, 1), ");"); }
		else
		{
			// NB. legacy GLSL doesn't have trunc() either, so we do a value cast
			auto& op0_type	  = expression_type(args[0]);
			...
			statement(to_expression(id), ".", to_member_name(type, 0), " = ", to_enclosed_expression(args[0]), " - ", to_expression(id), ".", to_member_name(type, 1), ";");
		}
		break;
	}
	...
	case GLSLstd450Tanh:
		...
		else
		{
			...
			auto expr = join("(",
							 to_enclosed_expression(epos_id),
							 " - ",
							 to_enclosed_expression(eneg_id),
							 ") / "
							 "(",
							 to_enclosed_expression(epos_id),
							 " + ",
	...
	case GLSLstd450Determinant: {
		...
		if (old_transpose) e->need_transpose = false;
		...
		if (options.version < 150) // also matches ES 100
		{
			...
			if (type.basetype != SPIRType::Float) SPIRV_CROSS_THROW("Unsupported type for matrix determinant");
		...
		if (old_transpose) e->need_transpose = true;
		break;
	}
	...
	case GLSLstd450MatrixInverse: {
		...
		if (old_transpose) a->need_transpose = false;
		...
		if (options.version < 140) // also matches ES 100
		{
			...
			if (type.basetype != SPIRType::Float) SPIRV_CROSS_THROW("Unsupported type for matrix inverse");
	...
	case GLSLstd450FrexpStruct: {
		auto& type = get<SPIRType>(result_type);
		emit_uninitialized_temporary_expression(result_type, id);
		statement(to_expression(id), ".", to_member_name(type, 0), " = ", "frexp(", to_expression(args[0]), ", ", to_expression(id), ".", to_member_name(type, 1), ");");
		break;
	}
	...
	case GLSLstd450NClamp: {
		...
		if (!max_id) max_id = ir.increase_bound_by(1);
...
void CompilerGLSL::emit_subgroup_op(const Instruction& i)
{
	...
	if (!options.vulkan_semantics && !is_supported_subgroup_op_in_opengl(op, ops)) SPIRV_CROSS_THROW("This subgroup operation is only supported in Vulkan semantics.");
...
#undef GLSL_GROUP_OP
	...
	case OpGroupNonUniformLogicalXor: {
		auto operation = static_cast<GroupOperation>(ops[3]);
		if (operation == GroupOperationClusteredReduce) { require_extension_internal("GL_KHR_shader_subgroup_clustered"); }
		else if (operation == GroupOperationExclusiveScan || operation == GroupOperationInclusiveScan || operation == GroupOperationReduce) { require_extension_internal("GL_KHR_shader_subgroup_arithmetic"); }
		else
			SPIRV_CROSS_THROW("Invalid group operation.");
		break;
	}
...
string CompilerGLSL::bitcast_glsl_op(const SPIRType& out_type, const SPIRType& in_type)
{
	...
	{
		if (out_type.vecsize == 2 || in_type.vecsize == 2) require_extension_internal("GL_EXT_buffer_reference_uvec2");
	...
	if (integral_cast && same_size_cast) return type_to_glsl(out_type);
...
{
	if (target_type.basetype == expr_type) return expr;
...
string CompilerGLSL::builtin_to_glsl(BuiltIn builtin, StorageClass storage)
{
	...
	case BuiltInClipDistance: {
		if (options.es) require_extension_internal("GL_EXT_clip_cull_distance");
	...
	case BuiltInInstanceId:
		...
		if (!options.es && options.version < 140) { require_extension_internal("GL_ARB_draw_instanced"); }
	...
	case BuiltInInstanceIndex:
		if (options.vulkan_semantics) return "gl_InstanceIndex";
	...
	case BuiltInBaseVertex:
		if (options.es) SPIRV_CROSS_THROW("BaseVertex not supported in ES profile.");
	...
	case BuiltInBaseInstance:
		if (options.es) SPIRV_CROSS_THROW("BaseInstance not supported in ES profile.");
	...
	case BuiltInDrawIndex:
		if (options.es) SPIRV_CROSS_THROW("DrawIndex not supported in ES profile.");
	...
	case BuiltInPrimitiveShadingRateKHR: {
		if (!options.vulkan_semantics) SPIRV_CROSS_THROW("Can only use PrimitiveShadingRateKHR in Vulkan GLSL.");
		require_extension_internal("GL_EXT_fragment_shading_rate");
		return "gl_PrimitiveShadingRateEXT";
	}

	case BuiltInShadingRateKHR: {
		if (!options.vulkan_semantics) SPIRV_CROSS_THROW("Can only use ShadingRateKHR in Vulkan GLSL.");
		require_extension_internal("GL_EXT_fragment_shading_rate");
		return "gl_ShadingRateEXT";
	}

	case BuiltInDeviceIndex:
		if (!options.vulkan_semantics) SPIRV_CROSS_THROW("Need Vulkan semantics for device group support.");
...
void CompilerGLSL::access_chain_internal_append_index(std::string& expr, uint32_t /*base*/, const SPIRType* /*type*/, AccessChainFlags flags, bool& access_chain_is_arrayed, uint32_t index)
{
...
string CompilerGLSL::access_chain_internal(uint32_t base, const uint32_t* indices, uint32_t count, AccessChainFlags flags, AccessChainMeta* meta)
{
	...
	if (!chain_only)
	{
		...
		bool  old_transpose = e && e->need_transpose;
		if (e) e->need_transpose = false;
		expr = to_enclosed_expression(base, register_expression_read);
		if (e) e->need_transpose = old_transpose;
	}
	...
	{
		if (ptr_chain) SPIRV_CROSS_THROW("Backend does not support native pointers and does not support OpPtrAccessChain.");
		...
		if (should_dereference(base)) expr = dereference_expression(get<SPIRType>(type_id), expr);
	}
	...
	if (auto* base_expr = maybe_get<SPIRExpression>(base)) { access_meshlet_position_y = base_expr->access_meshlet_position_y; }
	...
	const auto append_index = [&](uint32_t index, bool is_literal, bool is_ptr_chain = false) {
		...
		if (!is_literal) mod_flags &= ~ACCESS_CHAIN_INDEX_IS_LITERAL_BIT;
		if (!is_ptr_chain) mod_flags &= ~ACCESS_CHAIN_PTR_CHAIN_BIT;
	...
	for (uint32_t i = 0; i < count; i++)
	{
		...
		if (ptr_chain_array_entry)
		{
			...
			if (!should_dereference(base)) expr = enclose_expression(address_of_expression(expr));
		}
		...
		if (ptr_chain && i == 0)
		{
			...
			else { append_index(index, is_literal, true); }
			...
			if (ptr_chain_array_entry) expr = join("(", expr, ")");
		}
		// Arrays
		else if (!type->array.empty())
		{
			...
			if (var && has_decoration(var->self, DecorationBuiltIn) && get_decoration(var->self, DecorationBuiltIn) == BuiltInPosition && get_execution_model() == ExecutionModelMeshEXT) { access_meshlet_position_y = true; }
		...
		{
			if (!is_literal) index = evaluate_constant_u32(index);

			if (index < uint32_t(type->member_type_index_redirection.size())) index = type->member_type_index_redirection[index];

			if (index >= type->member_types.size()) SPIRV_CROSS_THROW("Member index is out of bounds!");
			...
			else
			{
				...
				if (is_member_builtin(*type, index, &builtin) && access_chain_needs_stage_io_builtin_translation(base))
				{
					...
					if (builtin == BuiltInPosition && get_execution_model() == ExecutionModelMeshEXT) { access_meshlet_position_y = true; }
				}
				else
				{
			...
			if (has_member_decoration(type->self, index, DecorationInvariant)) is_invariant = true;
			if (has_member_decoration(type->self, index, DecorationRelaxedPrecision)) relaxed_precision = true;
		...
		else if (type->columns > 1)
		{
			...
			{
				auto& phys = get<SPIRType>(physical_type);
				if (phys.vecsize > 4 || phys.columns > 4) expr += ".data";
			}
		...
		else if (type->vecsize > 1)
		{
			...
			{
				if (expression_type(base).pointer) effective_storage = get_expression_effective_storage_class(base);
				...
				if (var && var->storage == StorageClassOutput && get_execution_model() == ExecutionModelTessellationControl && !has_decoration(var->self, DecorationPatch)) { ignore_potential_sliced_writes = true; }
			}
			else
			...
			if (row_major_matrix_needs_conversion && !ignore_potential_sliced_writes)
			{
				if (prepare_access_chain_for_scalar_access(expr, get<SPIRType>(type->parent_type), effective_storage, is_packed))
				{
					...
					if (first_index != string::npos && first_index != 0) deferred_index = deferred_index.substr(first_index);
				}
			}

			if (access_meshlet_position_y)
			{
				if (is_literal) { access_meshlet_position_y = index == 1; }
				else
				{
...
string CompilerGLSL::access_chain(uint32_t base, const uint32_t* indices, uint32_t count, const SPIRType& target_type, AccessChainMeta* meta, bool ptr_chain)
{
	...
	else if (flattened_structs.count(base) && count > 0)
	{
		...
		if (ptr_chain) flags |= ACCESS_CHAIN_PTR_CHAIN_BIT;

		if (flattened_structs[base])
		{
			flags |= ACCESS_CHAIN_FLATTEN_ALL_MEMBERS_BIT;
			if (meta) meta->flattened_struct = target_type.basetype == SPIRType::Struct;
		}
...
void CompilerGLSL::store_flattened_struct(const string& basename, uint32_t rhs_id, const SPIRType& type, const SmallVector<uint32_t>& indices)
{
	...
	for (uint32_t i = 0; i < uint32_t(member_type->member_types.size()); i++)
	{
		...
		if (get<SPIRType>(member_type->member_types[i]).basetype == SPIRType::Struct) { store_flattened_struct(lhs, rhs_id, type, sub_indices); }
		else
		{
...
std::string CompilerGLSL::flattened_access_chain(uint32_t base, const uint32_t* indices, uint32_t count, const SPIRType& target_type, uint32_t offset, uint32_t matrix_stride, uint32_t /* array_stride */, bool need_transpose)
{
...
std::string CompilerGLSL::flattened_access_chain_matrix(uint32_t base, const uint32_t* indices, uint32_t count, const SPIRType& target_type, uint32_t offset, uint32_t matrix_stride, bool need_transpose)
{
	assert(matrix_stride);
	SPIRType tmp_type = target_type;
	if (need_transpose) swap(tmp_type.vecsize, tmp_type.columns);
	...
	for (uint32_t i = 0; i < tmp_type.columns; i++)
	{
		...
		expr += flattened_access_chain_vector(base,
											  indices,
											  count,
											  tmp_type,
											  offset + i * matrix_stride,
											  matrix_stride,
...
std::string CompilerGLSL::flattened_access_chain_vector(uint32_t base, const uint32_t* indices, uint32_t count, const SPIRType& target_type, uint32_t offset, uint32_t matrix_stride, bool need_transpose)
{
...
std::pair<std::string, uint32_t> CompilerGLSL::flattened_access_chain_offset(
	const SPIRType& basetype, const uint32_t* indices, uint32_t count, uint32_t offset, uint32_t word_stride, bool* need_transpose, uint32_t* out_matrix_stride, uint32_t* out_array_stride, bool ptr_chain)
{
	// Start traversing type hierarchy at the proper non-pointer types.
	...
	for (uint32_t i = 0; i < count; i++)
	{
		...
		if (ptr_chain && i == 0)
		{
			...
			if (!array_stride) SPIRV_CROSS_THROW("SPIR-V does not define ArrayStride for buffer block.");
		...
		else if (!type->array.empty())
		{
			...
			if (!type->array.empty()) array_stride = get_decoration(parent_type, DecorationArrayStride);
		}
		...
		else if (type->basetype == SPIRType::Struct)
		{
			...
			if (!type->array.empty()) array_stride = type_struct_member_array_stride(struct_type, index);
		}
		// Matrix -> Vector
		else if (type->columns > 1)
		{
	...
	if (need_transpose) *need_transpose = row_major_matrix_needs_conversion;
	if (out_matrix_stride) *out_matrix_stride = matrix_stride;
	if (out_array_stride) *out_array_stride = array_stride;
...
bool CompilerGLSL::should_dereference(uint32_t id)
{
	...
	if (auto* expr = maybe_get<SPIRExpression>(id))
	{
		...
		while (expr->loaded_from && expression_is_forwarded(expr->self))
		{
			...
			if (src_type.pointer != type.pointer || src_type.pointer_depth != type.pointer_depth || src_type.parent_type != type.parent_type) break;
			if ((var = maybe_get<SPIRVariable>(expr->loaded_from))) break;
			if (!(expr = maybe_get<SPIRExpression>(expr->loaded_from))) break;
		}
...
bool CompilerGLSL::should_forward(uint32_t id) const
{
	...
	if (options.force_temporary) return false;
	...
	if (expr && expr->expression_dependencies.size() >= max_expression_dependencies) return false;
	...
	if (is_immutable(id)) return true;
...
void CompilerGLSL::track_expression_read(uint32_t id)
{
	...
	if (expression_is_forwarded(id) && !expression_suppresses_usage_tracking(id))
	{
		...
		if (expression_read_implies_multiple_reads(id)) v++;
...
{
	if (forced_temporaries.find(id) != end(forced_temporaries)) return false;

	for (uint32_t i = 0; i < num_args; i++)
		if (!should_forward(args[i])) return false;

	// We need to forward globals as well.
	if (!pure)
	{
		for (auto global : global_variables)
			if (!should_forward(global)) return false;
		for (auto aliased : aliased_variables)
			if (!should_forward(aliased)) return false;
	}
...
void CompilerGLSL::register_call_out_argument(uint32_t id)
{
	...
	auto* var = maybe_get<SPIRVariable>(id);
	if (var) flush_variable_declaration(var->self);
}
...
void CompilerGLSL::flush_variable_declaration(uint32_t id)
{
	...
	{
		string initializer;
		if (options.force_zero_initialized_variables && (var->storage == StorageClassFunction || var->storage == StorageClassGeneric || var->storage == StorageClassPrivate) && !var->initializer && type_can_zero_initialize(get_variable_data_type(*var)))
		{
			initializer = join(" = ", to_zero_initialized_expression(get_variable_data_type_id(*var)));
		}
	...
	if (var) { emit_variable_temporary_copies(*var); }
}
...
bool CompilerGLSL::remove_duplicate_swizzle(string& op)
{
	...
	{
		if (final_swiz.size() < 2) return false;
	...
	for (uint32_t i = 0; i < final_swiz.size(); i++)
	{
		...
		if (i >= 4 || final_swiz[i] != expected[i]) return false;
	}

	auto prevpos = op.find_last_of('.', pos - 1);
	if (prevpos == string::npos) return false;
	...
	for (auto i = prevpos; i < pos; i++)
	{
		if (op[i] < 'w' || op[i] > 'z')
		{
			...
			if (backend.swizzle_is_function && i + 2 == pos && op[i] == '(' && op[i + 1] == ')') break;
			return false;
		}
	}
	...
	if (pos - prevpos >= final_swiz.size())
	{
		...
		if (backend.swizzle_is_function) op += "()";
	}
	return true;
}
...
bool CompilerGLSL::remove_unity_swizzle(uint32_t base, string& op)
{
	...
	if (type.vecsize == final_swiz.size()) op.erase(pos, string::npos);
	return true;
}
...
string CompilerGLSL::build_composite_combiner(uint32_t return_type, const uint32_t* elems, uint32_t length)
{
	...
	for (uint32_t i = 0; i < length; i++)
	{
		...
		else
		{
			...
			{
				if (backend.swizzle_is_function) subop += "()";
				...
				if (!remove_duplicate_swizzle(subop)) remove_unity_swizzle(base, subop);
...
bool CompilerGLSL::skip_argument(uint32_t id) const
{
	...
	{
		auto& type = expression_type(id);
		if (type.basetype == SPIRType::Sampler || (type.basetype == SPIRType::Image && type.image.sampled == 1)) return true;
	}
	return false;
}
...
bool CompilerGLSL::optimize_read_modify_write(const SPIRType& type, const string& lhs, const string& rhs)
{
	...
	if (rhs.size() < lhs.size() + 3) return false;
	...
	if (type.vecsize > 1 && type.columns > 1) return false;
	...
	if (needs_enclose_expression(expr)) return false;
...
{
	if (forwarded_temporaries.find(expr) == end(forwarded_temporaries)) return;
...
void CompilerGLSL::emit_block_instructions(SPIRBlock& block)
{
	...
	if (backend.requires_relaxed_precision_analysis)
	{
		...
		for (size_t i = 0, n = block.phi_variables.size(); i < n; i++)
		{
			...
			if (i && block.phi_variables[i - 1].function_variable == phi.function_variable) continue;
...
void CompilerGLSL::disallow_forwarding_in_expression_chain(const SPIRExpression& expr)
{
	...
	if (expression_is_forwarded(expr.self) && !expression_suppresses_usage_tracking(expr.self) && forced_invariant_temporaries.count(expr.self) == 0)
	{
...
void CompilerGLSL::handle_store_to_invariant_variable(uint32_t store_id, uint32_t value_id)
{
	...
	if (!has_decoration(store_id, DecorationInvariant)) return;
...
void CompilerGLSL::emit_store_statement(uint32_t lhs_expression, uint32_t rhs_expression)
{
	...
	if (!rhs.empty())
	{
		...
		if (!unroll_array_to_complex_store(lhs_expression, rhs_expression))
		{
			...
			if (has_decoration(lhs_expression, DecorationNonUniform)) convert_non_uniform_expression(lhs, lhs_expression);
...
void CompilerGLSL::forward_relaxed_precision(uint32_t dst_id, const uint32_t* args, uint32_t length)
{
	...
	if (!backend.requires_relaxed_precision_analysis) return;
	...
	if (input_precision == Options::Mediump) set_decoration(dst_id, DecorationRelaxedPrecision);
}
...
CompilerGLSL::Options::Precision CompilerGLSL::analyze_expression_precision(const uint32_t* args, uint32_t length) const
{
	...
	for (uint32_t i = 0; i < length; i++)
	{
		...
		if (handle_type == TypeConstant || handle_type == TypeConstantOp || handle_type == TypeUndef) continue;
...
void CompilerGLSL::analyze_precision_requirements(uint32_t type_id, uint32_t dst_id, uint32_t* args, uint32_t length)
{
	...
	if (type.basetype != SPIRType::Float && type.basetype != SPIRType::Int && type.basetype != SPIRType::UInt) return;
...
CompilerGLSL::TemporaryCopy CompilerGLSL::handle_instruction_precision(const Instruction& instruction)
{
	...
	if (backend.requires_relaxed_precision_analysis)
	{
		...
		if (instruction_to_result_type(result_type, result_id, opcode, ops, length))
		{
			...
			if (itr != temporary_to_mirror_precision_alias.end()) return {itr->second, itr->first};
		}
	}
...
#define GLSL_UFOP(op)			 emit_unary_func_op(ops[0], ops[1], ops[2], #op)
	...
	bool implicit_integer_promotion = integer_width < 32 && backend.implicit_c_integer_promotion_rules && opcode_can_promote_integer_implicitly(opcode) && get<SPIRType>(ops[0]).vecsize == 1;
	...
	case OpLoad: {
		...
		if (expr_type.vecsize > type.vecsize) expr = enclose_expression(expr + vector_swizzle(type.vecsize, 0));

		if (forward && ptr_expression) ptr_expression->need_transpose = old_need_transpose;

		// We might need to cast in order to load from a builtin.
		cast_from_variable_load(ptr, expr, type);

		if (forward && ptr_expression) ptr_expression->need_transpose = false;
		...
		if (backend.needs_row_major_load_workaround && !is_non_native_row_major_matrix(ptr) && !flattened) rewrite_load_for_wrapped_row_major(expr, result_type, ptr);
		...
		if (forward)
		{
			...
			if (has_extended_decoration(ptr, SPIRVCrossDecorationPhysicalTypePacked)) set_extended_decoration(id, SPIRVCrossDecorationPhysicalTypePacked);
			if (has_extended_decoration(ptr, SPIRVCrossDecorationPhysicalTypeID)) { set_extended_decoration(id, SPIRVCrossDecorationPhysicalTypeID, get_extended_decoration(ptr, SPIRVCrossDecorationPhysicalTypeID)); }
		}
		else
		{
		...
		if (forward) add_implied_read_expression(*e, ptr);
		break;
	}
	...
	case OpPtrAccessChain: {
		...
		if (flattened_buffer_blocks.count(ops[2]) && target_type.basetype == SPIRType::Struct) requires_temporary = !backend.can_declare_struct_inline;

		auto& expr = requires_temporary ? emit_op(ops[0], ops[1], std::move(e), false) : set<SPIRExpression>(ops[1], std::move(e), ops[0], should_forward(ops[2]));
		...
		if (meta.flattened_struct) flattened_structs[ops[1]] = true;
		if (meta.relaxed_precision && backend.requires_relaxed_precision_analysis) set_decoration(ops[1], DecorationRelaxedPrecision);
	...
	case OpArrayLength: {
		...
		if (has_decoration(ops[2], DecorationNonUniform)) convert_non_uniform_expression(e, ops[2]);
	...
	case OpFunctionCall: {
		...
		if (!pure) register_impure_function_call();
		...
		for (uint32_t i = 0; i < length; i++)
		{
			...
			if (skip_argument(arg[i])) continue;
		...
		if (return_type.basetype != SPIRType::Void)
		{
			...
			bool forward = args_will_forward(id, arg, length, pure) && !callee_has_out_variables && pure && (forced_temporaries.find(id) == end(forced_temporaries));
			...
			if (forward) register_global_read_dependencies(callee, id);
		}
		else
			statement(funexpr, ";");
	...
	case OpCompositeConstruct: {
		...
		if (out_type.basetype == SPIRType::Struct && !backend.can_declare_struct_inline) forward = false;
		if (!out_type.array.empty() && !backend.can_declare_arrays_inline) forward = false;
		if (type_is_empty(out_type) && !backend.supports_empty_struct) forward = false;
		...
		if (backend.use_initializer_list && composite)
		{
			...
			constructor_op += " }";
			if (needs_trailing_tracket) constructor_op += ")";
		}
		else if (swizzle_splat && !composite) { constructor_op = remap_swizzle(get<SPIRType>(result_type), 1, to_unpacked_expression(elems[0])); }
		else
		{
			constructor_op = type_to_glsl_constructor(get<SPIRType>(result_type)) + "(";
			if (type_is_empty(out_type) && !backend.supports_empty_struct)
	...
	case OpCompositeExtract: {
		...
		if (composite_type_is_complex) allow_base_expression = false;

		// Packed expressions or physical ID mapped expressions cannot be split up.
		if (has_extended_decoration(ops[2], SPIRVCrossDecorationPhysicalTypePacked) || has_extended_decoration(ops[2], SPIRVCrossDecorationPhysicalTypeID)) allow_base_expression = false;
		...
		if (is_non_native_row_major_matrix(ops[2])) allow_base_expression = false;
		...
		else if (allow_base_expression && should_forward(ops[2]) && type.vecsize == 1 && type.columns == 1 && length == 1)
		{
			...
			auto expr = access_chain_internal(ops[2], &ops[3], length, ACCESS_CHAIN_INDEX_IS_LITERAL_BIT | ACCESS_CHAIN_CHAIN_ONLY_BIT | ACCESS_CHAIN_FORCE_COMPOSITE_BIT, &meta);
		...
		if (meta.storage_is_packed) set_extended_decoration(id, SPIRVCrossDecorationPhysicalTypePacked);
		if (meta.storage_physical_type != 0) set_extended_decoration(id, SPIRVCrossDecorationPhysicalTypeID, meta.storage_physical_type);
		if (meta.storage_is_invariant) set_decoration(id, DecorationInvariant);
	...
	case OpCompositeInsert: {
		...
		if (invalid_expressions.count(composite) || block_composite_insert_overwrite.count(composite) || hoisted_temporaries.count(id) || hoisted_temporaries.count(composite) || maybe_get<SPIRExpression>(composite) == nullptr) { can_modify_in_place = false; }
		else if (backend.requires_relaxed_precision_analysis && has_decoration(composite, DecorationRelaxedPrecision) != has_decoration(id, DecorationRelaxedPrecision) && get<SPIRType>(result_type).basetype != SPIRType::Struct)
		{
		...
		if (can_modify_in_place)
		{
			...
			if (!forced_temporaries.count(composite)) force_temporary_and_recompile(composite);
		...
		{
			if (maybe_get<SPIRUndef>(composite) != nullptr) { emit_uninitialized_temporary_expression(result_type, id); }
			else
			{
				// Make a copy, then use access chain to store the variable.
	...
	case OpVectorShuffle: {
		...
		bool shuffle = false;
		for (uint32_t i = 0; i < length; i++)
			if (elems[i] >= type0.vecsize || elems[i] == 0xffffffffu) shuffle = true;

		// Cannot use swizzles with packed expressions, force shuffle path.
		if (!shuffle && has_extended_decoration(vec0, SPIRVCrossDecorationPhysicalTypePacked)) shuffle = true;
		...
		else
		{
			...
			if (backend.swizzle_is_function && length > 1) expr += "()";
		}
		...
		if (vec0 != vec1) inherit_expression_dependencies(id, vec1);
		break;
	}
	...
	case OpMatrixTimesMatrix: {
		...
		if (a && b && a->need_transpose && b->need_transpose)
		{
			...
			auto  expr		  = join(enclose_expression(to_unpacked_row_major_matrix_expression(ops[3])), " * ", enclose_expression(to_unpacked_row_major_matrix_expression(ops[2])));
			bool  forward	  = should_forward(ops[2]) && should_forward(ops[3]);
	...
	case OpTranspose:
		if (options.version < 120) // Matches GLSL 1.10 / ESSL 1.00
		{
			...
			if (input_e) swap(need_transpose, input_e->need_transpose);
			...
			if (input_e) input_e->need_transpose = need_transpose;
		}
		else
			GLSL_UFOP(transpose);
		break;
	...
	case OpSRem: {
		...
		auto expr	 = join(to_enclosed_expression(op0), " - ", to_enclosed_expression(op1), " * ", "(", to_enclosed_expression(op0), " / ", to_enclosed_expression(op1), ")");

		if (implicit_integer_promotion) expr = join(type_to_glsl(get<SPIRType>(result_type)), '(', expr, ')');
	...
	case OpISubBorrow: {
		...
		statement(to_expression(result_id), ".", to_member_name(type, 0), " = ", op, "(", to_expression(op0), ", ", to_expression(op1), ", ", to_expression(result_id), ".", to_member_name(type, 1), ");");
		break;
	}
	...
	case OpSMulExtended: {
		...
		statement(op, "(", to_expression(op0), ", ", to_expression(op1), ", ", to_expression(result_id), ".", to_member_name(type, 1), ", ", to_expression(result_id), ".", to_member_name(type, 0), ");");
		break;
	}
	...
	case OpFRem: {
		...
		bool		forward = should_forward(op0) && should_forward(op1);
		std::string expr;
		if (!is_legacy()) { expr = join(to_enclosed_expression(op0), " - ", to_enclosed_expression(op1), " * ", "trunc(", to_enclosed_expression(op0), " / ", to_enclosed_expression(op1), ")"); }
		else
		{
			...
			auto  via_type	  = op0_type;
			via_type.basetype = SPIRType::Int;
			expr			  = join(to_enclosed_expression(op0), " - ", to_enclosed_expression(op1), " * ", type_to_glsl(op0_type), "(", type_to_glsl(via_type), "(", to_enclosed_expression(op0), " / ", to_enclosed_expression(op1), "))");
		}
	...
	case OpDPdx:
		GLSL_UFOP(dFdx);
		if (is_legacy_es()) require_extension_internal("GL_OES_standard_derivatives");
	...
	case OpDPdxFine:
		GLSL_UFOP(dFdxFine);
		if (options.es) { SPIRV_CROSS_THROW("GL_ARB_derivative_control is unavailable in OpenGL ES."); }
		if (options.version < 450) require_extension_internal("GL_ARB_derivative_control");
		register_control_dependent_expression(ops[1]);
		break;
	...
	case OpEmitStreamVertex: {
		...
		if (expression_type(ops[0]).basetype != SPIRType::Int) stream_expr = join("int(", stream_expr, ")");
	...
	case OpImageSparseTexelsResident:
		if (options.es) SPIRV_CROSS_THROW("Sparse feedback is not supported in GLSL.");
		require_extension_internal("GL_ARB_sparse_texture2");
	...
	case OpImageQueryLod: {
		...
		{
			if (options.version < 300) SPIRV_CROSS_THROW("textureQueryLod not supported in legacy ES");
	...
	case OpImageQueryLevels: {
		...
		if (!options.es && options.version < 430) require_extension_internal("GL_ARB_texture_query_levels");
		if (options.es) SPIRV_CROSS_THROW("textureQueryLevels not supported in ES profile.");
	...
	case OpImageQuerySizeLod: {
		...
		if (is_legacy_desktop()) { fname = legacy_tex_op(fname, imgtype, img); }
		...
		if (type.image.dim == Dim1D && options.es) expr = join(expr, ".x");
	...
	case OpImageSparseRead: {
		...
		{
			if (type.image.ms) SPIRV_CROSS_THROW("Trying to remap multisampled image to variable, this is not possible.");
			...
			if (itr == end(pls_inputs))
			{
				...
				if (!var->remapped_components) SPIRV_CROSS_THROW("subpassInput was remapped, but remap_components is not set correctly.");
		...
		{
			if (var && subpass_input_is_framebuffer_fetch(var->self)) { imgexpr = to_expression(var->self); }
			else if (options.vulkan_semantics)
			{
		...
		else
		{
			...
			uint32_t sparse_code_id	 = 0;
			uint32_t sparse_texel_id = 0;
			if (sparse) emit_sparse_feedback_temporaries(ops[0], ops[1], sparse_code_id, sparse_texel_id);
			...
			// ES needs to emulate 1D images as 2D.
			if (type.image.dim == Dim1D && options.es) coord_expr = join("ivec2(", coord_expr, ", 0)");
			...
			if (sparse)
			{
				if (type.image.ms)
				{
					...
					uint32_t samples = ops[5];
					statement(to_expression(sparse_code_id), " = sparseImageLoadARB(", to_non_uniform_aware_expression(ops[2]), ", ", coord_expr, ", ", to_expression(samples), ", ", to_expression(sparse_texel_id), ");");
				}
				else { statement(to_expression(sparse_code_id), " = sparseImageLoadARB(", to_non_uniform_aware_expression(ops[2]), ", ", coord_expr, ", ", to_expression(sparse_texel_id), ");"); }
			...
			if (!sparse) imgexpr = remap_swizzle(get<SPIRType>(result_type), 4, imgexpr);
			pure = false;
		}
		...
		if (var)
		{
			...
			if (!pure)
			{
				...
				if (forward) var->dependees.push_back(id);
			}
		}
		else
		...
		inherit_expression_dependencies(id, ops[2]);
		if (type.image.ms) inherit_expression_dependencies(id, ops[5]);
		break;
	}
	...
	case OpImageWrite: {
		...
		if (type.image.ms)
		{
			...
			if (operands != ImageOperandsSampleMask || length != 5) SPIRV_CROSS_THROW("Multisampled image used in OpImageWrite, but unexpected operand mask was used.");
			uint32_t samples = ops[4];
			statement("imageStore(", to_non_uniform_aware_expression(ops[0]), ", ", coord_expr, ", ", to_expression(samples), ", ", remap_swizzle(store_type, value_type.vecsize, to_expression(ops[2])), ");");
		}
		else
			statement("imageStore(", to_non_uniform_aware_expression(ops[0]), ", ", coord_expr, ", ", remap_swizzle(store_type, value_type.vecsize, to_expression(ops[2])), ");");

		if (var && variable_storage_is_aliased(*var)) flush_all_aliased_variables();
		break;
	}
	...
	case OpImageBlockMatchSADQCOM: {
		...
		inherit_expression_dependencies(id, ops[3]);
		if (opcode == OpImageBlockMatchSSDQCOM || opcode == OpImageBlockMatchSADQCOM) inherit_expression_dependencies(id, ops[5]);
	...
	case OpMemoryBarrier: {
		...
		if (execution_scope == ScopeSubgroup || memory == ScopeSubgroup)
		{
			...
			if (opcode != OpControlBarrier) { request_subgroup_feature(ShaderSubgroupSupportHelper::SubgroupMemBarrier); }
			else { request_subgroup_feature(ShaderSubgroupSupportHelper::SubgroupBarrier); }
		}
		...
		if (execution_scope != ScopeSubgroup && get_entry_point().model == ExecutionModelTessellationControl)
		{
			...
			if (opcode == OpControlBarrier) statement("barrier();");
			break;
		}
		...
		if (opcode == OpMemoryBarrier)
		{
			...
			if (next && next->op == OpControlBarrier)
			{
				...
				else if (next_semantics == MemorySemanticsWorkgroupMemoryMask)
				{
					...
					if ((next_memory == ScopeDevice || next_memory == ScopeWorkgroup) && (memory == ScopeDevice || memory == ScopeWorkgroup)) { memory_scope_covered = true; }
				}
				...
				if (memory_scope_covered && (semantics & next_semantics) == semantics) break;
			}
		}
		...
		if (memory == ScopeWorkgroup) // Only need to consider memory within a group
		{
			if (semantics == MemorySemanticsWorkgroupMemoryMask)
			{
				...
				if (!implies_shared_barrier) statement("memoryBarrierShared();");
			}
			else if (semantics != 0)
		...
		else if (memory == ScopeSubgroup)
		{
			...
			else
			{
				...
				if (semantics & MemorySemanticsWorkgroupMemoryMask) statement("subgroupMemoryBarrierShared();");
				if (semantics & MemorySemanticsUniformMemoryMask) statement("subgroupMemoryBarrierBuffer();");
				if (semantics & MemorySemanticsImageMemoryMask) statement("subgroupMemoryBarrierImage();");
			}
		}
		else
		{
			...
			{
				// Pick out individual barriers.
				if (semantics & MemorySemanticsWorkgroupMemoryMask) statement("memoryBarrierShared();");
				if (semantics & MemorySemanticsUniformMemoryMask) statement("memoryBarrierBuffer();");
				if (semantics & MemorySemanticsImageMemoryMask) statement("memoryBarrierImage();");
			}
		}
	...
	case OpExtInst: {
		...
		if (ext == SPIRExtension::GLSL) { emit_glsl_op(ops[0], ops[1], ops[3], &ops[4], length - 4); }
		else if (ext == SPIRExtension::SPV_AMD_shader_ballot) { emit_spv_amd_shader_ballot_op(ops[0], ops[1], ops[3], &ops[4], length - 4); }
		else if (ext == SPIRExtension::SPV_AMD_shader_explicit_vertex_parameter) { emit_spv_amd_shader_explicit_vertex_parameter_op(ops[0], ops[1], ops[3], &ops[4], length - 4); }
		else if (ext == SPIRExtension::SPV_AMD_shader_trinary_minmax) { emit_spv_amd_shader_trinary_minmax_op(ops[0], ops[1], ops[3], &ops[4], length - 4); }
		else if (ext == SPIRExtension::SPV_AMD_gcn_shader) { emit_spv_amd_gcn_shader_op(ops[0], ops[1], ops[3], &ops[4], length - 4); }
		else if (ext == SPIRExtension::SPV_debug_info || ext == SPIRExtension::NonSemanticShaderDebugInfo || ext == SPIRExtension::NonSemanticGeneric)
		{
		...
		else if (ext == SPIRExtension::NonSemanticDebugPrintf)
		{
			...
			if (ops[3] == 1)
			{
				if (!options.vulkan_semantics) SPIRV_CROSS_THROW("Debug printf is only supported in Vulkan GLSL.\n");
	...
	case OpFragmentMaskFetchAMD: {
		...
		if (type.image.dim == spv::DimSubpassData) { emit_unary_func_op(result_type, id, ops[2], "fragmentMaskFetchAMD"); }
		else { emit_binary_func_op(result_type, id, ops[2], ops[3], "fragmentMaskFetchAMD"); }
	...
	case OpFragmentFetchAMD: {
		...
		if (type.image.dim == spv::DimSubpassData) { emit_binary_func_op(result_type, id, ops[2], ops[4], "fragmentFetchAMD"); }
		else { emit_trinary_func_op(result_type, id, ops[2], ops[3], ops[4], "fragmentFetchAMD"); }
	...
	case OpTraceNV:
		statement("traceNV(",
				  to_non_uniform_aware_expression(ops[0]),
				  ", ",
				  to_expression(ops[1]),
				  ", ",
				  to_expression(ops[2]),
				  ...
				  to_expression(ops[9]),
				  ", ",
				  to_expression(ops[10]),
				  ");");
		flush_control_dependent_expressions(current_emitting_block->self);
		break;
	case OpTraceRayKHR:
		if (!has_decoration(ops[10], DecorationLocation)) SPIRV_CROSS_THROW("A memory declaration object must be used in TraceRayKHR.");
		statement("traceRayEXT(",
				  to_non_uniform_aware_expression(ops[0]),
				  ", ",
				  to_expression(ops[1]),
				  ", ",
	...
	case OpExecuteCallableKHR:
		if (!has_decoration(ops[1], DecorationLocation)) SPIRV_CROSS_THROW("A memory declaration object must be used in ExecuteCallableKHR.");
...
#undef GLSL_RAY_QUERY_GET_OP2
	...
	case OpConvertUToAccelerationStructureKHR: {
		...
		if (elide_temporary) { GLSL_UFOP(accelerationStructureEXT); }
		else
		{
	...
	case OpConvertUToPtr: {
		auto& type = get<SPIRType>(ops[0]);
		if (type.storage != StorageClassPhysicalStorageBufferEXT) SPIRV_CROSS_THROW("Only StorageClassPhysicalStorageBufferEXT is supported by OpConvertUToPtr.");

		auto& in_type = expression_type(ops[2]);
		if (in_type.vecsize == 2) require_extension_internal("GL_EXT_buffer_reference_uvec2");
	...
	case OpConvertPtrToU: {
		...
		if (ptr_type.storage != StorageClassPhysicalStorageBufferEXT) SPIRV_CROSS_THROW("Only StorageClassPhysicalStorageBufferEXT is supported by OpConvertPtrToU.");

		if (type.vecsize == 2) require_extension_internal("GL_EXT_buffer_reference_uvec2");
	...
	case OpDemoteToHelperInvocationEXT:
		if (!options.vulkan_semantics) SPIRV_CROSS_THROW("GL_EXT_demote_to_helper_invocation is only supported in Vulkan GLSL.");
...
void CompilerGLSL::append_global_func_args(const SPIRFunction& func, uint32_t index, SmallVector<string>& arglist)
{
	...
	for (uint32_t arg_idx = index; arg_idx < arg_cnt; arg_idx++)
	{
		...
		if (var_id) flush_variable_declaration(var_id);
...
{
	if (type.type_alias != TypeID(0) && !has_extended_decoration(type.type_alias, SPIRVCrossDecorationBufferBlockRepacked)) { return to_member_name(get<SPIRType>(type.type_alias), index); }
...
bool CompilerGLSL::is_non_native_row_major_matrix(uint32_t id)
{
	...
	if (backend.native_row_major_matrix && !is_legacy()) return false;
...
bool CompilerGLSL::member_is_non_native_row_major_matrix(const SPIRType& type, uint32_t index)
{
	...
	if (!has_member_decoration(type.self, index, DecorationRowMajor)) return false;
	...
	if (mbr_type.columns != mbr_type.vecsize) SPIRV_CROSS_THROW("Row-major matrices must be square on this platform.");
...
string CompilerGLSL::convert_row_major_matrix(string exp_str, const SPIRType& exp_type, uint32_t /* physical_type_id */, bool /*is_packed*/, bool relaxed)
{
	...
	if (!is_matrix(exp_type))
	{
		...
		if (column_index == string::npos) return exp_str;
		...
		for (uint32_t c = 0; c < exp_type.vecsize; c++)
		{
			...
			if (c + 1 < exp_type.vecsize) transposed_expr += ", ";
		}
...
void CompilerGLSL::emit_struct_member(const SPIRType& type, uint32_t member_type_id, uint32_t index, const string& qualifier, uint32_t)
{
	...
	string qualifiers;
	bool   is_block = ir.meta[type.self].decoration.decoration_flags.get(DecorationBlock) || ir.meta[type.self].decoration.decoration_flags.get(DecorationBufferBlock);

	if (is_block) qualifiers = to_interpolation_qualifiers(memberflags);

	statement(layout_for_member(type, index), qualifiers, qualifier, flags_to_qualifiers_glsl(membertype, memberflags), variable_decl(membertype, to_member_name(type, index)), ";");
}
...
string CompilerGLSL::flags_to_qualifiers_glsl(const SPIRType& type, const Bitset& flags)
{
	...
	if (flags.get(DecorationRestrictPointerEXT)) return "restrict ";

	string qual;

	if (type_is_floating_point(type) && flags.get(DecorationNoContraction) && backend.support_precise_qualifier) qual = "precise ";

	// Structs do not have precision qualifiers, neither do doubles (desktop only anyways, so no mediump/highp).
	bool type_supports_precision = type.basetype == SPIRType::Float || type.basetype == SPIRType::Int || type.basetype == SPIRType::UInt || type.basetype == SPIRType::Image || type.basetype == SPIRType::SampledImage || type.basetype == SPIRType::Sampler;

	if (!type_supports_precision) return qual;
	...
	if (options.es)
	{
		...
		{
			bool implied_fmediump = type.basetype == SPIRType::Float && options.fragment.default_float_precision == Options::Mediump && execution.model == ExecutionModelFragment;

			bool implied_imediump = (type.basetype == SPIRType::Int || type.basetype == SPIRType::UInt) && options.fragment.default_int_precision == Options::Mediump && execution.model == ExecutionModelFragment;

			qual += (implied_fmediump || implied_imediump) ? "" : "mediump ";
		}
		else
		{
			bool implied_fhighp = type.basetype == SPIRType::Float && ((options.fragment.default_float_precision == Options::Highp && execution.model == ExecutionModelFragment) || (execution.model != ExecutionModelFragment));

			bool implied_ihighp = (type.basetype == SPIRType::Int || type.basetype == SPIRType::UInt) && ((options.fragment.default_int_precision == Options::Highp && execution.model == ExecutionModelFragment) || (execution.model != ExecutionModelFragment));
	...
	else if (backend.allow_precision_qualifiers)
	{
		...
		if (flags.get(DecorationRelaxedPrecision)) qual += "mediump ";
	}
...
string CompilerGLSL::to_precision_qualifiers_glsl(uint32_t id)
{
	...
	if (use_precision_qualifiers && (type.basetype == SPIRType::Image || type.basetype == SPIRType::SampledImage))
	{
		...
		if (result_type.width < 32) return "mediump ";
	}
...
string CompilerGLSL::to_qualifiers_glsl(uint32_t id)
{
	...
	if (var) res += to_storage_qualifiers_glsl(*var);

	auto& type = expression_type(id);
	if (type.image.dim != DimSubpassData && type.image.sampled == 2)
	{
		if (flags.get(DecorationCoherent)) res += "coherent ";
		if (flags.get(DecorationRestrict)) res += "restrict ";

		if (flags.get(DecorationNonWritable)) res += "readonly ";
...
bool CompilerGLSL::type_can_zero_initialize(const SPIRType& type) const
{
	...
	if (!type.array.empty() && options.flatten_multidimensional_arrays) return false;

	for (auto& literal : type.array_size_literal)
		if (!literal) return false;

	for (auto& memb : type.member_types)
		if (!type_can_zero_initialize(get<SPIRType>(memb))) return false;
...
string CompilerGLSL::variable_decl(const SPIRVariable& variable)
{
	...
	if (type.pointer_depth > 1 && !backend.support_pointer_to_pointer) SPIRV_CROSS_THROW("Cannot declare pointer-to-pointer types.");
...
uint32_t CompilerGLSL::to_array_size_literal(const SPIRType& type, uint32_t index) const
{
	...
	if (type.array_size_literal[index]) { return type.array[index]; }
	else
	{
...
string CompilerGLSL::image_type_glsl(const SPIRType& type, uint32_t id, bool /*member*/)
{
	...
	case DimCube:
		...
		break;
	case DimRect:
		if (options.es) SPIRV_CROSS_THROW("Rectangle textures are not supported on OpenGL ES.");

		if (is_legacy_desktop()) require_extension_internal("GL_ARB_texture_rectangle");
	...
	if (type.image.ms) res += "MS";
	if (type.image.arrayed)
	{
		if (is_legacy_desktop()) require_extension_internal("GL_EXT_texture_array");
		res += "Array";
	}
...
{
	if (!has_extension(ext)) forced_extensions.push_back(ext);
}
...
void CompilerGLSL::flatten_buffer_block(VariableID id)
{
	...
	if (!type.array.empty()) SPIRV_CROSS_THROW(name + " is an array of UBOs.");
	if (type.basetype != SPIRType::Struct) SPIRV_CROSS_THROW(name + " is not a struct.");
	if (!flags.get(DecorationBlock)) SPIRV_CROSS_THROW(name + " is not a block.");
	if (type.member_types.empty()) SPIRV_CROSS_THROW(name + " is an empty struct.");
...
bool CompilerGLSL::check_atomic_image(uint32_t id)
{
	...
	{
		if (options.es && options.version < 320) require_extension_internal("GL_OES_shader_image_atomic");
...
void CompilerGLSL::add_function_overload(const SPIRFunction& func)
{
	...
	for (auto& arg : func.arguments)
	{
		...
		if (!combined_image_samplers.empty())
		{
			...
			if (type.basetype == SPIRType::SampledImage || (type.basetype == SPIRType::Image && type.image.sampled == 1) || type.basetype == SPIRType::Sampler) { continue; }
		}
...
void CompilerGLSL::emit_function_prototype(SPIRFunction& func, const Bitset& return_flags)
{
	...
	for (auto& arg : func.arguments)
	{
		// Do not pass in separate images or samplers if we're remapping
		// to combined image samplers.
		if (skip_argument(arg.id)) continue;
...
void CompilerGLSL::emit_function(SPIRFunction& func, const Bitset& return_flags)
{
	...
	if (func.entry_line.file_id != 0) emit_line_directive(func.entry_line.file_id, func.entry_line.line_literal);
	emit_function_prototype(func, return_flags);
	begin_scope();

	if (func.self == ir.default_entry_point) emit_entry_point_declarations();
	...
	for (auto& v : func.local_variables)
	{
		...
		else if (var.storage == StorageClassPrivate)
		{
			...
			else
			{
				...
				if (find(begin(dominated), end(dominated), var.self) == end(dominated)) entry_block.dominated_variables.push_back(var.self);
...
void CompilerGLSL::emit_fixup()
{
	if (is_vertex_like_shader())
	{
		...
		if (options.vertex.flip_vert_y) statement("gl_Position.y = -gl_Position.y;");
	}
}
...
void CompilerGLSL::flush_phi(BlockID from, BlockID to)
{
	...
	if (child.ignore_phi_from_block == from) return;
	...
	for (auto itr = begin(child.phi_variables); itr != end(child.phi_variables); ++itr)
	{
		...
		if (phi.parent == from)
		{
			...
			else
			{
				...
				bool need_saved_temporary = find_if(itr + 1, end(child.phi_variables), [&](const SPIRBlock::Phi& future_phi) -> bool { return future_phi.local_variable == ID(phi.function_variable) && future_phi.parent == from; }) != end(child.phi_variables);
				...
				if (!optimize_read_modify_write(get<SPIRType>(var.basetype), lhs, rhs)) statement(lhs, " = ", rhs, ";");
			}
...
void CompilerGLSL::branch_to_continue(BlockID from, BlockID to)
{
	...
	else
	{
		...
		else if (from_block.loop_dominator != BlockID(SPIRBlock::NoDominator)) { loop_dominator = from_block.loop_dominator; }
		...
		if (!outside_control_flow) statement("continue;");
	}
}
...
void CompilerGLSL::branch(BlockID from, BlockID to)
{
	...
	else if (from != to && is_break(to))
	{
		...
		if (is_loop_break(to))
		{
			for (size_t n = current_emitting_switch_stack.size(); n; n--)
			{
				...
				if (current_emitting_switch && current_emitting_switch->loop_dominator != BlockID(SPIRBlock::NoDominator) && get<SPIRBlock>(current_emitting_switch->loop_dominator).merge_block == to)
				{
	...
	else if (to_is_continue || from == to)
	{
		...
		bool  branching_to_merge = (block_meta & (ParsedIR::BLOCK_META_SELECTION_MERGE_BIT | ParsedIR::BLOCK_META_MULTISELECT_MERGE_BIT | ParsedIR::BLOCK_META_LOOP_MERGE_BIT)) != 0;
		if (!to_is_continue || !branching_to_merge) branch_to_continue(from, to);
	}
...
void CompilerGLSL::branch(BlockID from, uint32_t cond, BlockID true_block, BlockID false_block)
{
	...
	if (!true_block_needs_code && !false_block_needs_code) return;

	// We might have a loop merge here. Only consider selection flattening constructs.
	// Loop hints are handled explicitly elsewhere.
	if (from_block.hint == SPIRBlock::HintFlatten || from_block.hint == SPIRBlock::HintDontFlatten) emit_block_hints(from_block);
...
string CompilerGLSL::emit_continue_block(uint32_t continue_block, bool follow_true_block, bool follow_false_block)
{
	...
	while ((ir.block_meta[block->self] & ParsedIR::BLOCK_META_LOOP_HEADER_BIT) == 0)
	{
		...
		else { SPIRV_CROSS_THROW("Invalid continue block detected!"); }
	}
...
{
	if (block.loop_variables.empty()) return "";
	...
	for (auto& variable : block.loop_variables)
	{
		...
		if (expr == 0 || ir.ids[expr].get_type() == TypeUndef) missing_initializers++;
	}

	if (block.loop_variables.size() == 1 && missing_initializers == 0) { return variable_decl(get<SPIRVariable>(block.loop_variables.front())); }
	...
	else
	{
		...
		for (auto& loop_var : block.loop_variables)
		{
			...
			if (static_expr == 0 || ir.ids[static_expr].get_type() == TypeUndef) { statement(variable_decl(get<SPIRVariable>(loop_var)), ";"); }
			else
			{
				auto& var  = get<SPIRVariable>(loop_var);
...
{
	if (block.loop_variables.size() <= 1) return true;
	...
	for (auto& var : block.loop_variables)
	{
		...
		if (expr == 0 || ir.ids[expr].get_type() == TypeUndef) continue;
		...
		if (expected_flags != get_decoration_bitset(var)) return false;
	}
	...
	if (method == SPIRBlock::MergeToSelectForLoop || method == SPIRBlock::MergeToSelectContinueForLoop)
	{
		...
		if (!flushes_phi && current_count == statement_count && condition_is_temporary)
		{
			...
			case SPIRBlock::ForLoop: {
				...
				if (execution_is_noop(get<SPIRBlock>(block.true_block), get<SPIRBlock>(block.merge_block))) condition = join("!", enclose_expression(condition));
...
void CompilerGLSL::emit_hoisted_temporaries(SmallVector<pair<TypeID, ID>>& temporaries)
{
	...
	for (auto& tmp : temporaries)
	{
		...
		if (type.pointer && !backend.native_pointers) continue;
		...
		string initializer;
		if (options.force_zero_initialized_variables && type_can_zero_initialize(type)) initializer = join(" = ", to_zero_initialized_expression(tmp.first));
		...
		if (mirrored_precision_itr != temporary_to_mirror_precision_alias.end())
		{
			...
			statement(flags_to_qualifiers_glsl(type, mirror_flags), variable_decl(type, to_name(mirror_id)), initializer, ";");
			// The temporary might be read from before it's assigned, set up the expression now.
...
void CompilerGLSL::emit_block_chain(SPIRBlock& block)
{
	...
	if (block.merge == SPIRBlock::MergeLoop) add_loop_level();
	...
	for (auto var_id : block.dominated_variables)
	{
		...
		if (var.phi_variable)
		{
			...
			if (mirrored_precision_itr != temporary_to_mirror_precision_alias.end() &&
				find_if(block.declare_temporary.begin(), block.declare_temporary.end(), [mirrored_precision_itr](const std::pair<TypeID, VariableID>& p) { return p.second == mirrored_precision_itr->second; }) == block.declare_temporary.end())
			{
	...
	if (block.continue_block)
	{
		...
		if (continue_type == SPIRBlock::ComplexLoop) block.complex_continue = true;
	}
	...
	else { emit_block_instructions(block); }
	...
	case SPIRBlock::MultiSelect: {
		...
		bool  unsigned_case = type.basetype == SPIRType::UInt || type.basetype == SPIRType::UShort || type.basetype == SPIRType::UByte || type.basetype == SPIRType::UInt64;

		if (block.merge == SPIRBlock::MergeNone) SPIRV_CROSS_THROW("Switch statement is not structured");
		...
		if (block.need_ladder_break) statement("bool _", block.self, "_ladder_break = false;");
		...
		for (auto& c : cases)
		{
			...
			{
				if (!case_constructs.count(c.block)) block_declaration_order.push_back(c.block);
		...
		for (size_t i = 0; i < num_blocks; i++)
		{
			...
			{
				if (is_legacy_es()) { statement((i ? "else " : ""), "if (", to_legacy_case_label(block.condition, literals, label_suffix), ")"); }
				else
				{
			...
			if (backend.support_case_fallthrough && i + 1 < num_blocks && execution_is_direct_branch(case_block, get<SPIRBlock>(block_declaration_order[i + 1])))
			{
			...
			if (!block_like_switch) begin_scope();
			branch(block.self, target_block);
			if (!block_like_switch) end_scope();
	...
	case SPIRBlock::Return: {
		...
		if (processing_entry_point) emit_fixup();
		...
		if (block.return_value)
		{
			...
			if (!type.array.empty() && !backend.can_return_array)
			{
				...
				if (ir.ids[block.return_value].get_type() != TypeUndef) { emit_array_copy("spvReturnValue", 0, block.return_value, StorageClassFunction, get_expression_effective_storage_class(block.return_value)); }

				if (!cfg.node_terminates_control_flow_in_sub_graph(current_function->entry_block, block.self) || block.loop_dominator != BlockID(SPIRBlock::NoDominator)) { statement("return;"); }
			}
			else
			{
				// OpReturnValue can return Undef, so don't emit anything for this case.
				if (ir.ids[block.return_value].get_type() != TypeUndef) statement("return ", to_unpacked_expression(block.return_value), ";");
			}
		}
		else if (!cfg.node_terminates_control_flow_in_sub_graph(current_function->entry_block, block.self) || block.loop_dominator != BlockID(SPIRBlock::NoDominator))
		{
	...
	case SPIRBlock::Kill:
		...
		if (block.return_value) statement("return ", to_unpacked_expression(block.return_value), ";");
		break;
	...
	case SPIRBlock::Unreachable: {
		...
		while (id)
		{
			...
			if (cfg.get_preceding_edges(id).empty()) break;

			id = cfg.get_immediate_dominator(id);
		}

		if (inner_dominator_is_switch) statement("break; // unreachable workaround");
	...
	if (block.merge == SPIRBlock::MergeLoop)
	{
		if (continue_type == SPIRBlock::DoWhileLoop)
		{
			...
			if (!positive_test) condition = join("!", enclose_expression(condition));
...
{
	if (!indent) SPIRV_CROSS_THROW("Popping empty indent stack.");
	indent--;
	statement("}");
}
...
void CompilerGLSL::check_function_call_constraints(const uint32_t* args, uint32_t length)
{
	...
	for (uint32_t i = 0; i < length; i++)
	{
		...
		if (!var || !var->remapped_variable) continue;
...
{
	return semantics & (MemorySemanticsAtomicCounterMemoryMask | MemorySemanticsImageMemoryMask | MemorySemanticsWorkgroupMemoryMask | MemorySemanticsUniformMemoryMask | MemorySemanticsCrossWorkgroupMemoryMask | MemorySemanticsSubgroupMemoryMask);
}
...
{
	if (!backend.force_gl_in_out_block) return false;
	// This path is only relevant for GL backends.

	auto* var = maybe_get<SPIRVariable>(target_id);
	if (!var || var->storage != StorageClassOutput) return false;

	if (!is_builtin_variable(*var) || BuiltIn(get_decoration(var->self, DecorationBuiltIn)) != BuiltInSampleMask) return false;
	...
	if (type.array_size_literal.back())
	{
		...
		if (type.array.back() == 0) SPIRV_CROSS_THROW("Cannot unroll an array copy from unsized array.");
	}
	else
...
{
	if (!backend.force_gl_in_out_block) return;
	// This path is only relevant for GL backends.
...
void CompilerGLSL::cast_from_variable_load(uint32_t source_id, std::string& expr, const SPIRType& expr_type)
{
	...
	if (!expr_type.array.empty()) return;

	auto* var = maybe_get_backing_variable(source_id);
	if (var) source_id = var->self;

	// Only interested in standalone builtin variables.
	if (!has_decoration(source_id, DecorationBuiltIn))
	{
		// Except for int attributes in legacy GLSL, which are cast from float.
		if (is_legacy() && expr_type.basetype == SPIRType::Int && var && var->storage == StorageClassInput) expr = join(type_to_glsl(expr_type), "(", expr, ")");
		return;
	}
	...
	if (expected_type != expr_type.basetype) expr = bitcast_expression(expr_type, expected_type, expr);
}
...
{
	if (*backend.nonuniform_qualifier == '\0') return;

	auto* var = maybe_get_backing_variable(ptr_id);
	if (!var) return;

	if (var->storage != StorageClassUniformConstant && var->storage != StorageClassStorageBuffer && var->storage != StorageClassUniform) return;

	auto& backing_type = get<SPIRType>(var->basetype);
	if (backing_type.array.empty()) return;
	...
	if (start_array_index == string::npos) return;
	...
	if (start_array_index == string::npos || end_array_index == string::npos || end_array_index < start_array_index) return;

	start_array_index++;

	expr = join(expr.substr(0, start_array_index), backend.nonuniform_qualifier, "(", expr.substr(start_array_index, end_array_index - start_array_index), ")", expr.substr(end_array_index, string::npos));
}

void CompilerGLSL::emit_block_hints(const SPIRBlock& block)
{
	if ((options.es && options.version < 310) || (!options.es && options.version < 140)) return;
...
{
	if (visited.count(type.self)) return;
...
void CompilerGLSL::fixup_anonymous_struct_names()
{
	...
	ir.for_each_typed_id<SPIRType>([&](uint32_t, SPIRType& type) {
		if (type.basetype == SPIRType::Struct && (has_decoration(type.self, DecorationBlock) || has_decoration(type.self, DecorationBufferBlock))) { fixup_anonymous_struct_names(visited, type); }
	});
}
	...
	ir.for_each_typed_id<SPIRType>([&](uint32_t self, SPIRType& type) {
		...
		else if (type_is_block_like(type) && type.self == ID(self))
		{
			...
			ir.for_each_typed_id<SPIRType>([&](uint32_t other_id, SPIRType& other_type) {
				...
				if (other_type.type_alias == type.type_alias) other_type.type_alias = self;
			});
...
void CompilerGLSL::emit_line_directive(uint32_t file_id, uint32_t line_literal)
{
	...
	if (block_debug_directives) return;
...
void CompilerGLSL::emit_copy_logical_type(uint32_t lhs_id, uint32_t lhs_type_id, uint32_t rhs_id, uint32_t rhs_type_id, SmallVector<uint32_t> chain)
{
	...
	else
	{
...

		{
			...
			if (lhs_meta.storage_is_packed) set_extended_decoration(lhs_id, SPIRVCrossDecorationPhysicalTypePacked);
			if (lhs_meta.storage_physical_type != 0) set_extended_decoration(lhs_id, SPIRVCrossDecorationPhysicalTypeID, lhs_meta.storage_physical_type);
...

		{
			...
			if (rhs_meta.storage_is_packed) set_extended_decoration(rhs_id, SPIRVCrossDecorationPhysicalTypePacked);
			if (rhs_meta.storage_physical_type != 0) set_extended_decoration(rhs_id, SPIRVCrossDecorationPhysicalTypeID, rhs_meta.storage_physical_type);
...
{
	if (!has_decoration(id, DecorationInputAttachmentIndex)) return false;
	...
	for (auto& remap : subpass_to_framebuffer_fetch_attachment)
		if (remap.first == input_attachment_index) return true;
...
const SPIRVariable* CompilerGLSL::find_subpass_input_by_attachment_index(uint32_t index) const
{
	...
	ir.for_each_typed_id<SPIRVariable>([&](uint32_t, const SPIRVariable& var) {
		if (has_decoration(var.self, DecorationInputAttachmentIndex) && get_decoration(var.self, DecorationInputAttachmentIndex) == index) { ret = &var; }
	});
	return ret;
}
...
const SPIRVariable* CompilerGLSL::find_color_output_by_location(uint32_t location) const
{
	const SPIRVariable* ret = nullptr;
	ir.for_each_typed_id<SPIRVariable>([&](uint32_t, const SPIRVariable& var) {
		if (var.storage == StorageClassOutput && get_decoration(var.self, DecorationLocation) == location) ret = &var;
	});
	return ret;
}
...
void CompilerGLSL::emit_inout_fragment_outputs_copy_to_subpass_inputs()
{
	for (auto& remap : subpass_to_framebuffer_fetch_attachment)
	{
		...
		if (!subpass_var) continue;
		if (!output_var)
			SPIRV_CROSS_THROW("Need to declare the corresponding fragment output variable to be able "
							  "to read from it.");
		if (is_array(get<SPIRType>(output_var->basetype))) SPIRV_CROSS_THROW("Cannot use GL_EXT_shader_framebuffer_fetch with arrays of color outputs.");

		auto& func = get<SPIRFunction>(get_entry_point().self);
		func.fixup_hooks_in.push_back([=]() {
			if (is_legacy()) { statement(to_expression(subpass_var->self), " = ", "gl_LastFragData[", get_decoration(output_var->self, DecorationLocation), "];"); }
			else
			{
...
CompilerGLSL::ShaderSubgroupSupportHelper::Candidate CompilerGLSL::ShaderSubgroupSupportHelper::get_KHR_extension_for_feature(Feature feature)
{
	static const Candidate extensions[FeatureCount] = {
		KHR_shader_subgroup_ballot,		KHR_shader_subgroup_basic,		KHR_shader_subgroup_basic,		KHR_shader_subgroup_basic,		KHR_shader_subgroup_basic,		KHR_shader_subgroup_ballot,		KHR_shader_subgroup_ballot,
		KHR_shader_subgroup_vote,		KHR_shader_subgroup_vote,		KHR_shader_subgroup_basic,		KHR_shader_subgroup_basic,		KHR_shader_subgroup_basic,		KHR_shader_subgroup_ballot,		KHR_shader_subgroup_ballot,
		KHR_shader_subgroup_ballot,		KHR_shader_subgroup_ballot,		KHR_shader_subgroup_arithmetic, KHR_shader_subgroup_arithmetic, KHR_shader_subgroup_arithmetic, KHR_shader_subgroup_arithmetic, KHR_shader_subgroup_arithmetic,
		KHR_shader_subgroup_arithmetic, KHR_shader_subgroup_arithmetic, KHR_shader_subgroup_arithmetic, KHR_shader_subgroup_arithmetic, KHR_shader_subgroup_arithmetic, KHR_shader_subgroup_arithmetic, KHR_shader_subgroup_arithmetic,
	};
	...
	for (uint32_t i = 0u; i < FeatureCount; ++i)
	{
		if (feature_mask & (1u << i))
		{
			...
			for (Feature d : deps)
			{
				...
				if (!candidates.empty()) unique_candidates.insert(candidates.begin(), candidates.end());
			}
...
CompilerGLSL::ShaderSubgroupSupportHelper::CandidateVector CompilerGLSL::ShaderSubgroupSupportHelper::get_candidates_for_feature(Feature ft, const Result& r)
{
	...
	auto cmp = [&r](Candidate a, Candidate b) {
		if (r.weights[a] == r.weights[b]) return a < b; // Prefer candidates with lower enum value
...
void CompilerGLSL::rewrite_load_for_wrapped_row_major(std::string& expr, TypeID loaded_type, ID ptr)
{
	...
	auto& backing_type = get<SPIRType>(var->basetype);
	bool  is_ubo	   = backing_type.basetype == SPIRType::Struct && backing_type.storage == StorageClassUniform && has_decoration(backing_type.self, DecorationBlock);
	if (!is_ubo) return;
	...
	if (type->basetype == SPIRType::Struct)
	{
		...
		for (uint32_t i = 0; i < uint32_t(type->member_types.size()); i++)
		{
			...
			if (decorations.get(DecorationRowMajor)) rewrite = true;

			// Since we decide on a per-struct basis, only use mediump wrapper if all candidates are mediump.
			if (!decorations.get(DecorationRelaxedPrecision)) relaxed = false;
		}
	}
...
bool CompilerGLSL::is_stage_output_variable_masked(const SPIRVariable& var) const
{
	...
	if (is_builtin) { return is_stage_output_builtin_masked(BuiltIn(get_decoration(var.self, DecorationBuiltIn))); }
	else
	{
		if (!has_decoration(var.self, DecorationLocation)) return false;
...
bool CompilerGLSL::is_stage_output_block_member_masked(const SPIRVariable& var, uint32_t index, bool strip_array) const
{
	...
	BuiltIn builtin = BuiltInMax;
	if (is_member_builtin(type, index, &builtin)) { return is_stage_output_builtin_masked(builtin); }
	else
	{
...
{
	if (has_decoration(var.self, DecorationPerPrimitiveEXT)) return true;

	auto& type = get<SPIRType>(var.basetype);
	if (!has_decoration(type.self, DecorationBlock)) return false;

	for (uint32_t i = 0, n = uint32_t(type.member_types.size()); i < n; i++)
		if (!has_member_decoration(type.self, i, DecorationPerPrimitiveEXT)) return false;
...
uint32_t CompilerGLSL::get_accumulated_member_location(const SPIRVariable& var, uint32_t mbr_idx, bool strip_array) const
{
	...
	for (uint32_t i = 0; i < mbr_idx; i++)
	{
		...
		if (has_member_decoration(type.self, mbr_idx, DecorationLocation)) location = get_member_decoration(type.self, mbr_idx, DecorationLocation);
...
StorageClass CompilerGLSL::get_expression_effective_storage_class(uint32_t ptr)
{
	...
	bool forced_temporary = ir.ids[ptr].get_type() == TypeExpression && !get<SPIRExpression>(ptr).access_chain && (forced_temporaries.count(ptr) != 0 || forwarded_temporaries.count(ptr) == 0);

	if (var && !forced_temporary)
	{
		if (variable_decl_is_remapped_storage(*var, StorageClassWorkgroup)) return StorageClassWorkgroup;
		if (variable_decl_is_remapped_storage(*var, StorageClassStorageBuffer)) return StorageClassStorageBuffer;
...
uint32_t CompilerGLSL::type_to_location_count(const SPIRType& type) const
{
	...
	else { count = type.columns > 1 ? type.columns : 1; }
...
{
	if (float_formatter) return float_formatter->format_float(value);
...
{
	if (float_formatter) return float_formatter->format_double(value);
