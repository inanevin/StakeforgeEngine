...
namespace SPIRV_CROSS_NAMESPACE
{
	...
	static bool is_valid_identifier(const string& name)
	{
		...
		if (is_numeric(name[0])) return false;

		for (auto c : name)
			if (!is_alphanumeric(c) && c != '_') return false;
		...
		for (auto c : name)
		{
			...
			if (is_underscore && saw_underscore) return false;
	...
	{
		if (!allow_reserved_prefixes && is_reserved_prefix(name)) return true;
		...
		if (member)
		{
			...
			if (name.compare(0, 2, "_m", 2) != 0) return false;
		...
		else
		{
			...
			if (name[0] != '_' || !is_numeric(name[1])) return false;
	...
	void ParsedIR::sanitize_underscores(std::string& str)
	{
		...
		while (src != str.end())
		{
			...
			if (saw_underscore && is_underscore) { src++; }
			else
			{
				if (dst != src) *dst = *src;
				dst++;
	...
	static string ensure_valid_identifier(const string& name)
	{
		...
		if (is_numeric(str[0])) str[0] = '_';

		for (auto& c : str)
			if (!is_alphanumeric(c) && c != '_') c = '_';
	...
	const string& ParsedIR::get_member_name(TypeID id, uint32_t index) const
	{
		...
		if (m)
		{
			if (index >= m->members.size()) return empty_string;
	...
	{
		if (!is_valid_identifier(name)) name = ensure_valid_identifier(name);
		if (is_reserved_identifier(name, member, allow_reserved_prefixes)) name = make_unreserved_identifier(name);
	}
	...
	void ParsedIR::fixup_reserved_names()
	{
		for (uint32_t id : meta_needing_name_fixup)
		{
			...
			if (ids[id].get_type() == TypeVariable && get<SPIRVariable>(id).remapped_variable) continue;
	...
	void ParsedIR::set_name(ID id, const string& name)
	{
		...
		if (!is_valid_identifier(name) || is_reserved_identifier(name, false, false)) meta_needing_name_fixup.insert(id);
	}
	...
	void ParsedIR::set_member_name(TypeID id, uint32_t index, const string& name)
	{
		...
		if (!is_valid_identifier(name) || is_reserved_identifier(name, true, false)) meta_needing_name_fixup.insert(id);
	}
	...
	{
		if (type.member_types.empty()) return {};
	...
	Bitset ParsedIR::get_buffer_block_flags(const SPIRVariable& var) const
	{
		...
		if (m) base_flags = m->decoration.decoration_flags;

		if (type.member_types.empty()) return base_flags;
	...
	const Bitset& ParsedIR::get_member_decoration_bitset(TypeID id, uint32_t index) const
	{
		...
		if (m)
		{
			if (index >= m->members.size()) return cleared_bitset;
	...
	uint32_t ParsedIR::get_decoration(ID id, Decoration decoration) const
	{
		...
		if (!dec.decoration_flags.get(decoration)) return 0;
	...
	const string& ParsedIR::get_decoration_string(ID id, Decoration decoration) const
	{
		...
		if (!dec.decoration_flags.get(decoration)) return empty_string;
	...
	const string& ParsedIR::get_member_decoration_string(TypeID id, uint32_t index, Decoration decoration) const
	{
		...
		if (m)
		{
			if (!has_member_decoration(id, index, decoration)) return empty_string;
	...
	{
		auto& m = meta[id];
		if (index >= m.members.size()) return;
	...
	void ParsedIR::reset_all_of_type(Types type)
	{
		for (auto& id : ids_for_type[type])
			if (ids[id].get_type() == type) ids[id].reset();
	...
	{
		if (loop_iteration_depth_hard != 0) SPIRV_CROSS_THROW("Cannot add typed ID while looping over it.");

		if (loop_iteration_depth_soft != 0)
		{
			if (!ids[id].empty()) SPIRV_CROSS_THROW("Cannot override IDs when loop is soft locked.");
			return;
		}
		...
		if (ids[id].empty()) { ids_for_type[type].push_back(id); }
	...
	void ParsedIR::make_constant_null(uint32_t id, uint32_t type, bool add_to_typed_id_set)
	{
		...
		{
			if (add_to_typed_id_set) add_typed_id(TypeConstant, id);
