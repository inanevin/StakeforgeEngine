...
namespace SPIRV_CROSS_NAMESPACE
{
	...
	public:
		...
		SmallVector& operator=(SmallVector&& other) SPIRV_CROSS_NOEXCEPT
		{
			...
			if (other.ptr != other.stack_storage.data())
			{
				...
				if (this->ptr != stack_storage.data()) free(this->ptr);
				this->ptr			  = other.ptr;
		...
		void pop_back() SPIRV_CROSS_NOEXCEPT
		{
			...
			if (!this->empty()) resize(this->buffer_size - 1);
		}
		...
		void reserve(size_t count) SPIRV_CROSS_NOEXCEPT
		{
			...
			if (count > buffer_capacity)
			{
				...
				if (target_capacity == 0) target_capacity = 1;
		...
		void insert(T* itr, const T* insert_begin, const T* insert_end) SPIRV_CROSS_NOEXCEPT
		{
			...
			else
			{
				if (this->buffer_size + count > buffer_capacity)
				{
					...
					if (target_capacity == 0) target_capacity = 1;
					if (target_capacity < N) target_capacity = N;
		...
		{
			if (end_erase == this->end()) { resize(size_t(start_erase - this->begin())); }
			else
			{
	...
	public:
		...
		{
			for (auto& saved : saved_buffers)
				if (saved.buffer != stack_buffer) free(saved.buffer);
			if (current_buffer.buffer != stack_buffer) free(current_buffer.buffer);
	...
	private:
		...
		void append(const char* s, size_t len)
		{
			...
			if (avail < len)
			{
				...
				if (!current_buffer.buffer) SPIRV_CROSS_THROW("Out of memory.");
