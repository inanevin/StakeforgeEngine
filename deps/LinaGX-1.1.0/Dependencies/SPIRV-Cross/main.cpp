...
struct CLIParser
{
	CLIParser(CLICallbacks cbs_, int argc_, char* argv_[]) : cbs(std::move(cbs_)), argc(argc_), argv(argv_)
	{
	}
	...
	bool parse()
	{
...
#endif
		{
			while (argc && !ended_state)
			{
				...
				if (*next != '-' && cbs.default_handler) { cbs.default_handler(next); }
				else
				{
					auto itr = cbs.callbacks.find(next);
					if (itr == ::end(cbs.callbacks)) { THROW("Invalid argument"); }
	...
	uint32_t next_uint()
	{
		if (!argc) { THROW("Tried to parse uint, but nothing left in arguments"); }

		uint64_t val = stoul(*argv);
		if (val > numeric_limits<uint32_t>::max()) { THROW("next_uint() out of range"); }
	...
	double next_double()
	{
		if (!argc) { THROW("Tried to parse double, but nothing left in arguments"); }
	...
	{
		if (!argc) { return default_value; }

		if (0 == strncmp("--", *argv, 2)) { return default_value; }
	...
	{
		if (!argc) { THROW("Tried to parse string, but nothing left in arguments"); }
...
{
	if (path[0] == '-' && path[1] == '\0') return read_spirv_file_stdin();
	...
	if (fread(spirv.data(), sizeof(uint32_t), len, file) != size_t(len)) spirv.clear();
...
static void print_resources(const Compiler& compiler, spv::StorageClass storage, const SmallVector<BuiltInResource>& resources)
{
	...
	for (auto& res : resources)
	{
		...
		if (type.vecsize > 1) type_str += std::to_string(type.vecsize);
...
static void print_resources(const Compiler& compiler, const char* tag, const SmallVector<Resource>& resources)
{
	...
	for (auto& res : resources)
	{
		...
		if (print_ssbo && compiler.buffer_is_hlsl_counter_buffer(res.id)) continue;

		// If we don't have a name, use the fallback for the type instead of the variable
		// for SSBOs and UBOs since those are the only meaningful names to use externally.
		// Push constant blocks are still accessed by name and not block name, even though they are technically Blocks.
		bool is_push_constant = compiler.get_storage_class(res.id) == StorageClassPushConstant;
		bool is_block		  = compiler.get_decoration_bitset(type.self).get(DecorationBlock) || compiler.get_decoration_bitset(type.self).get(DecorationBufferBlock);
		bool is_sized_block	  = is_block && (compiler.get_storage_class(res.id) == StorageClassUniform || compiler.get_storage_class(res.id) == StorageClassUniformConstant);
		...
		fprintf(stderr, " ID %03u : %s%s", uint32_t(res.id), !res.name.empty() ? res.name.c_str() : compiler.get_fallback_name(fallback_id).c_str(), array.c_str());

		if (mask.get(DecorationLocation)) fprintf(stderr, " (Location : %u)", compiler.get_decoration(res.id, DecorationLocation));
		if (mask.get(DecorationDescriptorSet)) fprintf(stderr, " (Set : %u)", compiler.get_decoration(res.id, DecorationDescriptorSet));
		if (mask.get(DecorationBinding)) fprintf(stderr, " (Binding : %u)", compiler.get_decoration(res.id, DecorationBinding));
		if (static_cast<const CompilerGLSL&>(compiler).variable_is_depth_or_compare(res.id)) fprintf(stderr, " (comparison)");
		if (mask.get(DecorationInputAttachmentIndex)) fprintf(stderr, " (Attachment : %u)", compiler.get_decoration(res.id, DecorationInputAttachmentIndex));
		if (mask.get(DecorationNonReadable)) fprintf(stderr, " writeonly");
		if (mask.get(DecorationNonWritable)) fprintf(stderr, " readonly");
		if (mask.get(DecorationRestrict)) fprintf(stderr, " restrict");
		if (mask.get(DecorationCoherent)) fprintf(stderr, " coherent");
		if (mask.get(DecorationVolatile)) fprintf(stderr, " volatile");
		if (is_sized_block)
		{
			fprintf(stderr, " (BlockSize : %u bytes)", block_size);
			if (runtime_array_stride) fprintf(stderr, " (Unsized array stride: %u bytes)", runtime_array_stride);
		}

		uint32_t counter_id = 0;
		if (print_ssbo && compiler.buffer_get_hlsl_counter_buffer(res.id, counter_id)) fprintf(stderr, " (HLSL counter buffer ID: %u)", counter_id);
		fprintf(stderr, "\n");
	}
	fprintf(stderr, "=============\n\n");
}
...
static void print_push_constant_resources(const Compiler& compiler, const SmallVector<Resource>& res)
{
	for (auto& block : res)
	{
		...
		for (auto& range : ranges)
		{
			...
			fprintf(stderr, "Member #%3u (%s): Offset: %4u, Range: %4u\n", range.index, !name.empty() ? name.c_str() : compiler.get_fallback_member_name(range.index).c_str(), unsigned(range.offset), unsigned(range.range));
		}
		fprintf(stderr, "==================\n\n");
	}
}
...
{
	auto itr = find_if(begin(resources), end(resources), [&remap](const Resource& res) { return res.name == remap.src_name; });
...
static vector<PlsRemap> remap_pls(const SmallVector<PLSArg>& pls_variables, const SmallVector<Resource>& resources, const SmallVector<Resource>* secondary_resources)
{
	vector<PlsRemap> ret;
	...
	for (auto& pls : pls_variables)
	{
		...
		if (!found) fprintf(stderr, "Did not find stage input/output/target with name \"%s\".\n", pls.name.c_str());
	}
...
static string compile_iteration(const CLIArguments& args, std::vector<uint32_t> spirv_file)
{
	...
	if (args.cpp)
	{
		...
		if (args.cpp_interface_name) static_cast<CompilerCPP*>(compiler.get())->set_interface_name(args.cpp_interface_name);
	}
	else if (args.msl)
	{
		...
		if (args.set_msl_version) msl_opts.msl_version = args.msl_version;
		...
		if (args.msl_combined_sampler_suffix) msl_comp->set_combined_sampler_suffix(args.msl_combined_sampler_suffix);
	}
	else if (args.hlsl)
	...
	else
	{
		...
		if (!args.vulkan_semantics || args.vulkan_glsl_disable_ext_samplerless_texture_functions) build_dummy_sampler = true;
	...
	if (!args.variable_type_remaps.empty())
	{
		auto remap_cb = [&](const SPIRType&, const string& name, string& out) -> void {
			for (const VariableTypeRemap& remap : args.variable_type_remaps)
				if (name == remap.variable_name) out = remap.new_variable_type;
		};
	...
	if (!entry_point.empty()) compiler->set_entry_point(entry_point, model);
	...
	if (args.set_version) opts.version = args.version;
	if (args.set_es) opts.es = args.es;
	...
	if (args.hlsl)
	{
		...
		if (!args.entry_point_rename.empty()) hlsl_opts.use_entry_point_name = true;
		...
		if (args.hlsl_base_vertex_index_explicit_binding) { hlsl->set_hlsl_aux_buffer_binding(HLSL_AUX_BINDING_BASE_VERTEX_INSTANCE, args.hlsl_base_vertex_index_register_index, args.hlsl_base_vertex_index_register_space); }
	}
	...
	{
		if (remap_generic(*compiler, res.stage_inputs, remap)) continue;
		if (remap_generic(*compiler, res.stage_outputs, remap)) continue;
		if (remap_generic(*compiler, res.subpass_inputs, remap)) continue;
	}

	for (auto& rename : args.interface_variable_renames)
	{
		if (rename.storageClass == StorageClassInput)
			spirv_cross_util::rename_interface_variable(*compiler, res.stage_inputs, rename.location, rename.variable_name);
		else if (rename.storageClass == StorageClassOutput)
			spirv_cross_util::rename_interface_variable(*compiler, res.stage_outputs, rename.location, rename.variable_name);
		else
		{
	...
	if (combined_image_samplers)
	{
		...
		if (args.combined_samplers_inherit_bindings) spirv_cross_util::inherit_combined_sampler_bindings(*compiler);
	...
	if (args.hlsl)
	{
		...
		{
			auto itr = std::find_if(res.stage_inputs.begin(), res.stage_inputs.end(), [&](const Resource& input_res) { return input_res.name == named_remap.name; });
...
static int main_inner(int argc, char* argv[])
{
	...
	cbs.add("--glsl-ext-framebuffer-fetch-noncoherent", [&args](CLIParser&) { args.glsl_ext_framebuffer_fetch_noncoherent = true; });
	...
	cbs.add("--force-zero-initialized-variables", [&args](CLIParser&) { args.force_zero_initialized_variables = true; });
	...
	cbs.add("--hlsl-auto-binding", [&args](CLIParser& parser) { args.hlsl_binding_flags |= hlsl_resource_type_to_flag(parser.next_string()); });
	cbs.add("--hlsl-force-storage-buffer-as-uav", [&args](CLIParser&) { args.hlsl_force_storage_buffer_as_uav = true; });
	cbs.add("--hlsl-nonwritable-uav-texture-as-srv", [&args](CLIParser&) { args.hlsl_nonwritable_uav_texture_as_srv = true; });
	...
	cbs.add("--msl-argument-buffer-tier", [&args](CLIParser& parser) { args.msl_argument_buffers_tier = parser.next_uint(); });
	cbs.add("--msl-discrete-descriptor-set", [&args](CLIParser& parser) { args.msl_discrete_descriptor_sets.push_back(parser.next_uint()); });
	cbs.add("--msl-device-argument-buffer", [&args](CLIParser& parser) { args.msl_device_argument_buffers.push_back(parser.next_uint()); });
	...
	cbs.add("--msl-multiview-no-layered-rendering", [&args](CLIParser&) { args.msl_multiview_layered_rendering = false; });
	cbs.add("--msl-view-index-from-device-index", [&args](CLIParser&) { args.msl_view_index_from_device_index = true; });
	...
	cbs.add("--msl-force-active-argument-buffer-resources", [&args](CLIParser&) { args.msl_force_active_argument_buffer_resources = true; });
	...
	cbs.add("--msl-disable-frag-stencil-ref-builtin", [&args](CLIParser&) { args.msl_enable_frag_stencil_ref_builtin = false; });
	cbs.add("--msl-enable-frag-output-mask", [&args](CLIParser& parser) { args.msl_enable_frag_output_mask = parser.next_hex_uint(); });
	cbs.add("--msl-no-clip-distance-user-varying", [&args](CLIParser&) { args.msl_enable_clip_distance_user_varying = false; });
	...
	cbs.add("--msl-additional-fixed-sample-mask", [&args](CLIParser& parser) { args.msl_additional_fixed_sample_mask = parser.next_hex_uint(); });
	cbs.add("--msl-arrayed-subpass-input", [&args](CLIParser&) { args.msl_arrayed_subpass_input = true; });
	cbs.add("--msl-r32ui-linear-texture-align", [&args](CLIParser& parser) { args.msl_r32ui_linear_texture_alignment = parser.next_uint(); });
	cbs.add("--msl-r32ui-linear-texture-align-constant-id", [&args](CLIParser& parser) { args.msl_r32ui_alignment_constant_id = parser.next_uint(); });
	...
	cbs.add("--msl-fixed-subgroup-size", [&args](CLIParser& parser) { args.msl_fixed_subgroup_size = parser.next_uint(); });
	cbs.add("--msl-force-sample-rate-shading", [&args](CLIParser&) { args.msl_force_sample_rate_shading = true; });
	cbs.add("--msl-no-manual-helper-invocation-updates", [&args](CLIParser&) { args.msl_manual_helper_invocation_updates = false; });
	cbs.add("--msl-check-discarded-frag-stores", [&args](CLIParser&) { args.msl_check_discarded_frag_stores = true; });
	cbs.add("--msl-sample-dref-lod-array-as-grad", [&args](CLIParser&) { args.msl_sample_dref_lod_array_as_grad = true; });
	...
	cbs.add("--msl-combined-sampler-suffix", [&args](CLIParser& parser) { args.msl_combined_sampler_suffix = parser.next_string(); });
	cbs.add("--msl-runtime-array-rich-descriptor", [&args](CLIParser&) { args.msl_runtime_array_rich_descriptor = true; });
	...
	cbs.add("--force-recompile-max-debug-iterations", [&](CLIParser& parser) { args.force_recompile_max_debug_iterations = parser.next_uint(); });
	...
	if (spirv_file.empty()) return EXIT_FAILURE;
