...
namespace SPIRV_CROSS_NAMESPACE
{
	...
	void Parser::parse()
	{
		...
		auto len = spirv.size();
		if (len < 5) SPIRV_CROSS_THROW("SPIRV file too small.");

		auto s = spirv.data();

		// Endian-swap if we need to.
		if (s[0] == swap_endian(MagicNumber)) transform(begin(spirv), end(spirv), begin(spirv), [](uint32_t c) { return swap_endian(c); });

		if (s[0] != MagicNumber || !is_valid_spirv_version(s[1])) SPIRV_CROSS_THROW("Invalid SPIRV format.");

		uint32_t bound = s[3];

		const uint32_t MaximumNumberOfIDs = 0x3fffff;
		if (bound > MaximumNumberOfIDs) SPIRV_CROSS_THROW("ID bound exceeds limit of 0x3fffff.\n");
		...
		while (offset < len)
		{
			...
			if (instr.count == 0) SPIRV_CROSS_THROW("SPIR-V instructions cannot consume 0 words. Invalid SPIR-V file.");
			...
			if (offset > spirv.size()) SPIRV_CROSS_THROW("SPIR-V instruction goes out of bounds.");
		...
		if (current_function) SPIRV_CROSS_THROW("Function was not terminated.");
		if (current_block) SPIRV_CROSS_THROW("Block was not terminated.");
		if (ir.default_entry_point == 0) SPIRV_CROSS_THROW("There is no entry point in the SPIR-V module.");
	}
	...
	const uint32_t* Parser::stream(const Instruction& instr) const
	{
		...
		if (!instr.length) return nullptr;

		if (instr.offset + instr.length > ir.spirv.size()) SPIRV_CROSS_THROW("Compiler::stream() out of range.");
	...
	void Parser::parse(const Instruction& instruction)
	{
		...
		if (ignore_trailing_block_opcodes)
		{
			...
			if (op == OpReturn || op == OpBranch || op == OpUnreachable) return;
		}
		...
		case OpUndef: {
			...
			if (current_block) current_block->ops.push_back(instruction);
			break;
		}

		case OpCapability: {
			uint32_t cap = ops[0];
			if (cap == CapabilityKernel) SPIRV_CROSS_THROW("Kernel capability not supported.");
		...
		case OpExtInst: {
			...
			if (current_block)
			{
				...
				if (length >= 2)
				{
					...
					if (type) ir.load_type_width.insert({ops[1], type->width});
				}
			}
			break;
		}
		...
		case OpEntryPoint: {
			auto  itr = ir.entry_points.insert(make_pair(ops[1], SPIREntryPoint(ops[1], static_cast<ExecutionModel>(ops[0]), extract_string(ir.spirv, instruction.offset + 2))));
			...
			if (!ir.default_entry_point) ir.default_entry_point = ops[1];
			break;
		}
		...
		case OpGroupDecorate: {
			...
			for (uint32_t i = 1; i < length; i++)
			{
				...
				flags.for_each_bit([&](uint32_t bit) {
					...
					if (decoration_is_string(decoration)) { ir.set_decoration_string(target, decoration, ir.get_decoration_string(group_id, decoration)); }
					else
					{
		...
		case OpTypeArray: {
			...
			// references.
			if (base.forward_pointer) forward_pointer_fixups.push_back({id, tid});
		...
		case OpTypeRuntimeArray: {
			...
			// We're copying type information into Array types, so we'll need a fixup for any physical pointer
			// references.
			if (base.forward_pointer) forward_pointer_fixups.push_back({id, ops[1]});
		...
		case OpTypePointer: {
			...
			if (ptrbase.storage == StorageClassAtomicCounter) ptrbase.basetype = SPIRType::AtomicCounter;

			if (base && base->forward_pointer) forward_pointer_fixups.push_back({id, ops[2]});
		...
		case OpTypeStruct: {
			...
			if (consider_aliasing)
			{
				...
				if (type.type_alias == TypeID(0)) global_struct_cache.push_back(id);
			}
			break;
		}
		...
		case OpVariable: {
			...
			{
				if (!current_function) SPIRV_CROSS_THROW("No function currently in scope");
		...
		case OpPhi: {
			if (!current_function) SPIRV_CROSS_THROW("No function currently in scope");
			if (!current_block) SPIRV_CROSS_THROW("No block currently in scope");
		...
		case OpConstantComposite: {
			...
			// can refer to.
			if (ctype.basetype == SPIRType::Struct || !ctype.array.empty()) { set<SPIRConstant>(id, type, ops + 2, length - 2, op == OpSpecConstantComposite); }
			else
			{
				uint32_t elements = length - 2;
				if (elements > 4) SPIRV_CROSS_THROW("OpConstantComposite only supports 1, 2, 3 and 4 elements.");
				...
				for (uint32_t i = 0; i < elements; i++)
				{
					...
					if (constant_op)
					{
						if (op == OpConstantComposite) SPIRV_CROSS_THROW("Specialization constant operation used in OpConstantComposite.");
		...
		case OpFunction: {
			...
			if (current_function) SPIRV_CROSS_THROW("Must end a function before starting a new one!");
		...
		case OpFunctionParameter: {
			...
			if (!current_function) SPIRV_CROSS_THROW("Must be in a function!");
		...
		case OpLabel: {
			...
			if (!current_function) SPIRV_CROSS_THROW("Blocks cannot exist outside functions!");

			uint32_t id = ops[0];

			current_function->blocks.push_back(id);
			if (!current_function->entry_block) current_function->entry_block = id;

			if (current_block) SPIRV_CROSS_THROW("Cannot start a block before ending the current block.");
		...
		case OpBranch: {
			if (!current_block) SPIRV_CROSS_THROW("Trying to end a non-existing block.");
		...
		case OpSelectionMerge: {
			if (!current_block) SPIRV_CROSS_THROW("Trying to modify a non-existing block.");
		...
		case OpLoopMerge: {
			...
			if (current_block->continue_block != BlockID(current_block->self)) ir.block_meta[current_block->continue_block] |= ParsedIR::BLOCK_META_CONTINUE_BIT;
		...
		case OpSpecConstantOp: {
			if (length < 3) SPIRV_CROSS_THROW("OpSpecConstantOp not enough arguments.");
		...
		default: {
			...
			if (!current_block) SPIRV_CROSS_THROW("Currently no block to insert opcode.");
