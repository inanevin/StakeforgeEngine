...
int main()
{
	...
	auto [request_str, inserted] = client.request<"foo">(uuid, foo_params{.foo_a = 1337, .foo_b = "hello world"}, client_cb);
   ...
   timeout();
	...
	std::tie(request_str, inserted) = client.request<"foo">(uuid, foo_params{.foo_a = -1, .foo_b = "invalid data"}, client_cb);
