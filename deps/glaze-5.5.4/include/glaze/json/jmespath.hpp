...
namespace glz
{
	namespace jmespath
	{
			...
			while (pos < len)
			{
				...
				if (in_string)
				{
					...
					else if (current == '\\')
					{
						...
						if (next_char != '"' && next_char != '\'' && next_char != '\\' && next_char != '/' && next_char != 'b' && next_char != 'f' && next_char != 'n' && next_char != 'r' && next_char != 't' && next_char != 'u')
						{
							return {"", "", tokenization_error::invalid_escape_sequence};
						}
					}
		 ...
		 */
		inline constexpr jmespath::tokenization_error tokenize_full_jmespath(std::string_view expression, std::vector<std::string_view>& tokens)
		{
			tokens.clear();
		...
		template <const std::string_view& expression> consteval auto tokenize_as_array()
		{
			constexpr auto N = [] {
	...
	struct jmespath_expression
	{
		...
		template <size_t N> jmespath_expression(const char (&input_path)[N]) noexcept : path(input_path)
		{
			error = jmespath::tokenize_full_jmespath(path, tokens);
		}
