...
namespace glz
{
	template <class T> struct from<JSON, quoted_t<T>>
	{
	...
	template <class T> struct to<JSON, quoted_t<T>>
	{
	...
	template <is_opts_wrapper T> struct from<JSON, T>
	{
	...
	template <is_opts_wrapper T> struct to<JSON, T>
	{
	...
	template <auto MemPtr> inline constexpr decltype(auto) quoted_impl() noexcept
	{
	...
	template <auto MemPtr> constexpr auto quoted = quoted_impl<MemPtr>();
}
