...
namespace glz
{
	using basic = std::variant<bool, char, char8_t, unsigned char, signed char, short, unsigned short, float, int, unsigned int, long, unsigned long, double, long long, unsigned long long, std::string>;

	// Explicitly defining basic_ptr to avoid additional template instantiations
	using basic_ptr = std::variant<bool*, char*, char8_t*, unsigned char*, signed char*, short*, unsigned short*, float*, int*, unsigned int*, long*, unsigned long*, double*, long long*, unsigned long long*, std::string*>;
	...
	template <> struct meta<study::param>
	{
		using T						= study::param;
		static constexpr auto value = object("ptr", &T::ptr, "dist", &T::distribution, "values", &T::range);
	};

	template <> struct meta<study::design>
	{
		using T						= study::design;
		static constexpr auto value = object("params", &T::params, "states", &T::states, "overwrite", &T::overwrite, "seed", &T::seed, "random_samples", &T::random_samples);
	};
	...
	namespace study
	{
		...
		template <class State> struct full_factorial
		{
			State				   state{};
			...
			expected<param_set, error_ctx> param_set_from_dist(const param& dist)
			{
				...
				else if (dist.distribution == "linspace")
				{
					...
					{
						return unexpected(error_ctx{error_code::invalid_distribution_elements}); // distribution's range must have 3 elements
					}
		...
		struct random_param
		{
			...
			void				   apply()
			{
				std::visit([&](auto&& p) { *p = *std::get_if<std::remove_pointer_t<std::decay_t<decltype(p)>>>(&value); }, param_ptr);
			}
		};

		template <class State> struct random_doe
		{
			State state{};
			...
			bool done() const
			{
				return index >= params_per_state.size();
			}
			...
			expected<random_param, error_ctx> param_from_dist(const param& dist)
			{
				...
				if (dist.distribution == "elements")
				{
					...
					std::visit(
						...
						result.param_ptr);
					result.gen = [this, dist = std::uniform_int_distribution<std::size_t>(0, dist.range.size() - 1), elements = std::move(elements)]() mutable {
				...
				else if (dist.distribution == "linspace")
				{
					...
					result.gen = [this, dist = std::uniform_real_distribution<double>(start, stop)]() mutable { return dist(this->engine); };
				}
				...
				else if (dist.distribution == "normal")
				{
					...
					result.gen = [this, dist = std::normal_distribution<double>(mean, std_dev)]() mutable { return dist(this->engine); };
				}
				else
				{
					return unexpected(error_ctx{error_code::unknown_distribution});
