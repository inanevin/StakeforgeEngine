...
namespace glz
{
	...
	template <> struct parse<JSON>
	{
		template <auto Opts, class T, is_context Ctx, class It0, class It1> GLZ_ALWAYS_INLINE static void op(T&& value, Ctx&& ctx, It0&& it, It1&& end)
		{
			...
			else
			{
				...
				from<JSON, V>::template op<Opts>(std::forward<T>(value), std::forward<Ctx>(ctx), std::forward<It0>(it), std::forward<It1>(end));
			}
		}

		// This unknown key handler should not be given unescaped keys, that is for the user to handle.
		template <auto Opts, class T, is_context Ctx, class It0, class It1> static void handle_unknown(const sv& key, T&& value, Ctx&& ctx, It0&& it, It1&& end)
		{
	...
	struct from<JSON, T>
	{
		template <auto Opts, class Value, is_context Ctx, class It0, class It1> GLZ_ALWAYS_INLINE static void op(Value&& value, Ctx&& ctx, It0&& it, It1&& end)
		{
			using V = std::decay_t<decltype(get_member(std::declval<Value>(), meta_wrapper_v<T>))>;
			from<JSON, V>::template op<Opts>(get_member(std::forward<Value>(value), meta_wrapper_v<T>), std::forward<Ctx>(ctx), std::forward<It0>(it), std::forward<It1>(end));
		}
	};

	template <auto Opts> GLZ_ALWAYS_INLINE bool parse_ws_colon(is_context auto& ctx, auto&& it, auto&& end) noexcept
	{
	...
	void decode_index(Value&& value, is_context auto&& ctx, auto&& it, auto&& end, SelectedIndex&&... selected_index)
	{
		...
		if (((it + Length) < end) && comparitor<KeyWithEndQuote>(it)) [[likely]]
		{
			...
			else
			{
				...
				{
					from<JSON, std::remove_cvref_t<V>>::template op<ws_handled<Opts>()>(get_member(value, get<I>(reflect<T>::values)), ctx, it, end);
				}
				else
				{
					from<JSON, std::remove_cvref_t<V>>::template op<ws_handled<Opts>()>(get_member(value, get<I>(to_tie(value))), ctx, it, end);
				}
			}
	...
	template <auto Opts, class T, auto& HashInfo, class Value, class... SelectedIndex>
		requires(glaze_object_t<T> || reflectable<T>)
	GLZ_ALWAYS_INLINE constexpr void parse_and_invoke(Value&& value, is_context auto&& ctx, auto&& it, auto&& end, SelectedIndex&&... selected_index)
	{
	...
	template <is_member_function_pointer T> struct from<JSON, T>
	{
		template <auto Opts> GLZ_ALWAYS_INLINE static void op(auto&&, is_context auto&& ctx, auto&&...) noexcept
		{
			ctx.error = error_code::attempt_member_func_read;
		}
	};

	template <is_includer T> struct from<JSON, T>
	{
		template <auto Opts, class... Args> static void op(auto&&, is_context auto&& ctx, auto&& it, auto&& end) noexcept
		{
	...
	template <is_bitset T> struct from<JSON, T>
	{
		template <auto Opts> static void op(auto&& value, is_context auto&& ctx, auto&& it, auto&& end) noexcept
		{
			if (match_invalid_end<'"', Opts>(ctx, it, end))
			{
				return;
	...
	template <> struct from<JSON, skip>
	{
		template <auto Opts> GLZ_ALWAYS_INLINE static void op(auto&&, is_context auto&& ctx, auto&&... args) noexcept
		{
			skip_value<JSON>::op<Opts>(ctx, args...);
		}
	};

	template <is_reference_wrapper T> struct from<JSON, T>
	{
		template <auto Opts, class... Args> GLZ_ALWAYS_INLINE static void op(auto&& value, Args&&... args)
		{
	...
	template <> struct from<JSON, hidden>
	{
		template <auto Opts> GLZ_ALWAYS_INLINE static void op(auto&&, is_context auto&& ctx, auto&&...) noexcept
		{
	...
	template <complex_t T> struct from<JSON, T>
	{
		template <auto Options> static void op(auto&& v, is_context auto&& ctx, auto&& it, auto&& end) noexcept
		{
	...
	template <always_null_t T> struct from<JSON, T>
	{
		template <auto Opts> GLZ_ALWAYS_INLINE static void op(auto&&, is_context auto&& ctx, auto&& it, auto&& end) noexcept
		{
			if constexpr (!check_ws_handled(Opts))
			{
				if (skip_ws<Opts>(ctx, it, end))
	...
	template <bool_t T> struct from<JSON, T>
	{
		template <auto Opts> static void op(bool_t auto&& value, is_context auto&& ctx, auto&& it, auto&& end) noexcept
		{
	...
	template <num_t T> struct from<JSON, T>
	{
		template <auto Opts, class It> GLZ_ALWAYS_INLINE static void op(auto&& value, is_context auto&& ctx, It&& it, auto&& end) noexcept
		{
			if constexpr (Opts.quoted_num)
			{
				if (skip_ws<Opts>(ctx, it, end))
	...
	template <string_t T> struct from<JSON, T>
	{
	...
	struct from<JSON, T>
	{
		template <auto Opts, class It, class End> GLZ_ALWAYS_INLINE static void op(auto& value, is_context auto&& ctx, It&& it, End&& end) noexcept
		{
			if constexpr (!check_opening_handled(Opts))
			{
				if constexpr (!check_ws_handled(Opts))
	...
	template <char_t T> struct from<JSON, T>
	{
		template <auto Opts> static void op(auto& value, is_context auto&& ctx, auto&& it, auto&& end) noexcept
		{
			if constexpr (!check_opening_handled(Opts))
	...
	template <func_t T> struct from<JSON, T>
	{
	...
	template <class T> struct from<JSON, basic_raw_json<T>>
	{
		template <auto Opts> GLZ_ALWAYS_INLINE static void op(auto&& value, is_context auto&& ctx, auto&& it, auto&& end)
		{
			auto it_start = it;
			if (*it == 'n')
			{
	...
	template <class T> struct from<JSON, basic_text<T>>
	{
		template <auto Opts> GLZ_ALWAYS_INLINE static void op(auto&& value, is_context auto&&, auto&& it, auto&& end)
		{
	...
	struct from<JSON, T>
	{
		template <auto Options> static void op(auto& value, is_context auto&& ctx, auto&& it, auto&& end)
		{
			constexpr auto Opts = ws_handled_off<Options>();
			if constexpr (!check_ws_handled(Options))
			{
	...
	struct from<JSON, T>
	{
		...
		static void op(auto&& value, is_context auto&& ctx, auto&& it, auto&& end)
		{
			...
			while (it < end)
			{
				...
				if constexpr (str_t<typename V::first_type> || (std::is_enum_v<typename V::first_type> && glaze_t<typename V::first_type>))
				{
	...
	template <auto Opts> [[nodiscard]] size_t number_of_array_elements(is_context auto&& ctx, auto it, auto&& end) noexcept
	{
	...
	template <glaze_flags_t T> struct from<JSON, T>
	{
		template <auto Opts> static void op(auto&& value, is_context auto&& ctx, auto&& it, auto&& end)
		{
			if constexpr (!check_ws_handled(Opts))
	...
	template <class T> struct from<JSON, includer<T>>
	{
		template <auto Options> static void op(auto&& value, is_context auto&& ctx, auto&& it, auto&& end)
		{
			constexpr auto Opts = ws_handled_off<Options>();
			...
			const auto file_path = relativize_if_not_absolute(std::filesystem::path(ctx.current_file).parent_path(), std::filesystem::path{buffer});
	...
	template <pair_t T> struct from<JSON, T>
	{
		template <auto Options, string_literal tag = ""> static void op(T& value, is_context auto&& ctx, auto&& it, auto&& end)
		{
	...
	template <class T, string_literal Tag> inline consteval bool contains_tag()
	{
	...
	struct from<JSON, T>
	{
		template <auto Options, string_literal tag = ""> static void op(auto&& value, is_context auto&& ctx, auto&& it, auto&& end)
		{
			...
			else
			{
				...
				while (true)
				{
					...
					if (*it == '}')
					{
						...
						if constexpr ((glaze_object_t<T> || reflectable<T>)&&(Opts.partial_read && Opts.error_on_missing_keys))
						{
							ctx.error = error_code::missing_key;
							return;
						}
	...
	template <is_variant T> consteval auto variant_is_auto_deducible()
	{
	...
	template <class> struct variant_types;

	template <class... Ts> struct variant_types<std::variant<Ts...>>
	{
		// TODO: this way of filtering types is compile time intensive.
		using bool_types	   = decltype(tuplet::tuple_cat(std::conditional_t<bool_t<remove_meta_wrapper_t<Ts>>, tuple<Ts>, tuple<>>{}...));
		using number_types	   = decltype(tuplet::tuple_cat(std::conditional_t<num_t<remove_meta_wrapper_t<Ts>>, tuple<Ts>, tuple<>>{}...));
		...
		using array_types	   = decltype(tuplet::tuple_cat(std::conditional_t < array_t<remove_meta_wrapper_t<Ts>> || glaze_array_t<Ts>, tuple<Ts>, tuple < >> {}...));
		using nullable_types   = decltype(tuplet::tuple_cat(std::conditional_t<null_t<Ts>, tuple<Ts>, tuple<>>{}...));
		using nullable_objects = decltype(tuplet::tuple_cat(std::conditional_t<is_memory_object<Ts>, tuple<Ts>, tuple<>>{}...));
	};

	// post process output of variant_types
	template <class> struct tuple_types;

	template <class... Ts> struct tuple_types<tuple<Ts...>>
	{
		using glaze_const_types		= decltype(tuplet::tuple_cat(std::conditional_t<glaze_const_value_t<Ts>, tuple<Ts>, tuple<>>{}...));
		using glaze_non_const_types = decltype(tuplet::tuple_cat(std::conditional_t<!glaze_const_value_t<Ts>, tuple<Ts>, tuple<>>{}...));
	};

	template <class> struct variant_type_count;

	template <class... Ts> struct variant_type_count<std::variant<Ts...>>
	{
	...
	template <class Tuple> struct process_arithmetic_boolean_string_or_array
	{
		template <auto Options> static void op(auto&& value, is_context auto&& ctx, auto&& it, auto&& end)
		{
	...
	template <is_variant T> struct from<JSON, T>
	{
	...
	template <class T> struct from<JSON, array_variant_wrapper<T>>
	{
		template <auto Options> static void op(auto&& wrapper, is_context auto&& ctx, auto&& it, auto&& end)
		{
	...
	template <is_expected T> struct from<JSON, T>
	{
		template <auto Opts, class... Args> static void op(auto&& value, is_context auto&& ctx, auto&& it, auto&& end)
		{
			if constexpr (!check_ws_handled(Opts))
			{
				if (skip_ws<Opts>(ctx, it, end))
	...
	struct from<JSON, T>
	{
		template <auto Opts, class V, size_t N> GLZ_ALWAYS_INLINE static void op(V (&value)[N], is_context auto&& ctx, auto&& it, auto&& end) noexcept
		{
	...
	template <filesystem_path T> struct from<JSON, T>
	{
		template <auto Opts> static void op(auto&& value, is_context auto&& ctx, auto&& it, auto&& end)
		{
	...
	template <is_buffer Buffer> [[nodiscard]] error_ctx validate_json(Buffer&& buffer) noexcept
	{
		context	  ctx{};
		glz::skip skip_value{};
	...
	template <is_buffer Buffer> [[nodiscard]] error_ctx validate_jsonc(Buffer&& buffer) noexcept
	{
		context	  ctx{};
		glz::skip skip_value{};
	...
	template <read_supported<JSON> T, is_buffer Buffer> [[nodiscard]] error_ctx read_json(T& value, Buffer&& buffer)
	{
		context ctx{};
		return read<opts{}>(value, std::forward<Buffer>(buffer), ctx);
	}

	template <read_supported<JSON> T, is_buffer Buffer> [[nodiscard]] expected<T, error_ctx> read_json(Buffer&& buffer)
	{
		T				value{};
		context			ctx{};
	...
	template <read_supported<JSON> T, is_buffer Buffer> [[nodiscard]] error_ctx read_jsonc(T& value, Buffer&& buffer)
	{
		context ctx{};
		return read<opts{.comments = true}>(value, std::forward<Buffer>(buffer), ctx);
	}

	template <read_supported<JSON> T, is_buffer Buffer> [[nodiscard]] expected<T, error_ctx> read_jsonc(Buffer&& buffer)
	{
		T				value{};
		context			ctx{};
	...
	template <auto Opts = opts{}, read_supported<JSON> T, is_buffer Buffer> [[nodiscard]] error_ctx read_file_json(T& value, const sv file_name, Buffer&& buffer)
	{
		context ctx{};
	...
	template <auto Opts = opts{}, read_supported<JSON> T, is_buffer Buffer> [[nodiscard]] error_ctx read_file_jsonc(T& value, const sv file_name, Buffer&& buffer)
	{
		context ctx{};
		ctx.current_file = file_name;
