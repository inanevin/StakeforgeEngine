...
namespace glz
{
	...
	struct json_t
	{
		...
		template <class T> [[nodiscard]] T* get_if() noexcept
		{
			return std::get_if<T>(&data);
		}

		template <class T> [[nodiscard]] const T* get_if() const noexcept
		{
			return std::get_if<T>(&data);
		}
		...
		const json_t& operator[](std::integral auto&& index) const
		{
			return std::get<array_t>(data)[index];
		}
		...
		// for integers
		template <int_t T> json_t& operator=(const T value)
		{
		...
		[[nodiscard]] json_t& at(std::convertible_to<std::string_view> auto&& key)
		{
			return operator[](key);
		}

		[[nodiscard]] const json_t& at(std::convertible_to<std::string_view> auto&& key) const
		{
			return operator[](key);
		}
		...
		{
			return !std::holds_alternative<null_t>(data);
		}
		...
		template <class T>
			requires std::convertible_to<T, double> && (!std::derived_from<std::decay_t<T>, json_t>) && (!std::convertible_to<T, val_t>)
		json_t(T&& val)
		{
		...
		template <bool deprioritize = true> json_t(std::initializer_list<json_t>&& arr)
		{
	...
	[[nodiscard]] inline bool is_array(const json_t& value) noexcept
	{
		return value.is_array();
	}

	[[nodiscard]] inline bool is_object(const json_t& value) noexcept
	{
		return value.is_object();
	}

	[[nodiscard]] inline bool is_number(const json_t& value) noexcept
	{
		return value.is_number();
	}

	[[nodiscard]] inline bool is_string(const json_t& value) noexcept
	{
		return value.is_string();
	}

	[[nodiscard]] inline bool is_boolean(const json_t& value) noexcept
	{
		return value.is_boolean();
	}

	[[nodiscard]] inline bool is_null(const json_t& value) noexcept
	{
		return value.is_null();
	}
}

template <> struct glz::meta<glz::json_t>
{
...
namespace glz
{
	...
	template <read_supported<JSON> T> [[nodiscard]] error_ctx read_json(T& value, const json_t& source)
	{
		auto buffer = source.dump();
		if (buffer)
		{
	...
	template <read_supported<JSON> T> [[nodiscard]] expected<T, error_ctx> read_json(const json_t& source)
	{
		auto buffer = source.dump();
		if (buffer)
		{
...
namespace glz
{
	...
	template <> struct seek_op<glz::json_t>
	{
