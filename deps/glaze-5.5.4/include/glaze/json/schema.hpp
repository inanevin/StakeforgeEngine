...
namespace glz
{
	...
	struct schema final
	{
		...
		std::optional<bool>				ExtAdvanced{}; // flag to indicate that the parameter is advanced and can be hidden in default views
...
template <> struct glz::meta<glz::detail::defined_formats>
{
...
template <> struct glz::meta<glz::detail::schematic>
{
...
namespace glz
{
	namespace detail
	{
		template <class T = void> struct to_json_schema
		{
			template <auto Opts> static void op(auto& s, auto& defs)
			{
				// &T::member
				...
				else if constexpr (glaze_const_value_t<T>)
				{ // &T::constexpr_member
		...
		{
			template <auto Opts> static void op(auto& s, auto&)
			{
				s.type = {"boolean"};
			}
		};

		template <num_t T> struct to_json_schema<T>
		{
			template <auto Opts> static void op(auto& s, auto&)
			{
		...
		template <always_null_t T> struct to_json_schema<T>
		{
			template <auto Opts> static void op(auto& s, auto&)
			{
				s.type				  = {"null"};
		...
		template <glaze_enum_t T> struct to_json_schema<T>
		{
			template <auto Opts> static void op(auto& s, auto&)
			{
				s.type = {"string"};
		...
		template <class T> struct to_json_schema<basic_raw_json<T>>
		{
			template <auto Opts> static void op(auto& s, auto&)
			{
				s.type = {"number", "string", "boolean", "object", "array", "null"};
		...
		template <array_t T> struct to_json_schema<T>
		{
			template <auto Opts> static void op(auto& s, auto& defs)
			{
		...
		template <writable_map_t T> struct to_json_schema<T>
		{
			template <auto Opts> static void op(auto& s, auto& defs)
			{
		...
		template <nullable_t T> struct to_json_schema<T>
		{
			template <auto Opts> static void op(auto& s, auto& defs)
			{
		...
		template <is_variant T> struct to_json_schema<T>
		{
			template <auto Opts> static void op(auto& s, auto& defs)
			{
		...
		template <class T> consteval bool json_schema_matches_object_keys()
		{
		...
		template <const std::string_view& ref> auto consteval validate_ref() noexcept -> void
		{
#if (__cpp_static_assert >= 202306L) && (__cplusplus > 202302L)
			static_assert(!has_slash(ref), join_v<chars<"Slash in name: \"">, ref, chars<"\" in json schema references is not allowed">>);
#else
	...
	template <class T, auto Opts = opts{}, class Buffer> [[nodiscard]] error_ctx write_json_schema(Buffer&& buffer)
	{
		detail::schematic s{};
		s.defs.emplace();
	...
	template <class T, auto Opts = opts{}> [[nodiscard]] glz::expected<std::string, error_ctx> write_json_schema()
	{
		std::string		buffer{};
