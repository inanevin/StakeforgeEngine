...
namespace glz
{
	...
	template <class T> struct invoke_t;
	...
	template <class T> struct from<JSON, invoke_t<T>>
	{
	...
	template <class T> struct to<JSON, invoke_t<T>>
	{
	...
	template <auto MemPtr> inline constexpr decltype(auto) invoke_impl()
	{
	...
	template <auto MemPtr> constexpr auto invoke = invoke_impl<MemPtr>();
}
...
namespace glz
{
	...
	struct invoke_update
	{
		...
		template <class F> invoke_update(F&& f) : func(std::forward<F>(f))
		{
		}
	...
	template <is_invoke_update T> struct from<JSON, T>
	{
		template <auto Opts> static void op(auto&& value, is_context auto&& ctx, auto&& it, auto&& end)
		{
	...
	template <is_invoke_update T> struct to<JSON, T>
	{
		template <auto Opts> static void op(auto&& value, is_context auto&& ctx, auto&&... args)
		{
			using V = std::decay_t<decltype(value.val)>;
