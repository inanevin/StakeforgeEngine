...
namespace glz
{
	...
	template <class Key, class T, class Compare = std::less<>, class Container = std::vector<std::pair<Key, T>>> struct flat_map
	{
		// Member types
		using key_type				 = Key;
		using mapped_type			 = T;
	...
	private:
		...
		template <typename Comp> static constexpr bool IsDefaultComparator = std::same_as<Comp, std::less<>> || std::same_as<Comp, std::less<Key>>;
	...
	public:
		...
		template <class InputIt> flat_map(InputIt first, InputIt last, const Compare& comp = Compare()) : data_(), comp_(comp)
		{
			insert(first, last);
		}
		...
		void swap(flat_map& other) noexcept(std::allocator_traits<typename Container::allocator_type>::is_always_equal::value && std::is_nothrow_swappable_v<Compare>)
		{
		...
		{
			return std::lower_bound(data_.begin(), data_.end(), key, [this](const value_type& lhs, const K& rhs) { return comp_(lhs.first, rhs); });
		}
		...
		{
			return std::upper_bound(data_.begin(), data_.end(), key, [this](const K& lhs, const value_type& rhs) { return comp_(lhs, rhs.first); });
		}
	...
	template <class Key, class T, class Compare, class Container> bool operator==(const flat_map<Key, T, Compare, Container>& lhs, const flat_map<Key, T, Compare, Container>& rhs)
	{
	...
	template <class Key, class T, class Compare, class Container> void swap(flat_map<Key, T, Compare, Container>& lhs, flat_map<Key, T, Compare, Container>& rhs) noexcept(noexcept(lhs.swap(rhs)))
	{
		lhs.swap(rhs);
	}
}
