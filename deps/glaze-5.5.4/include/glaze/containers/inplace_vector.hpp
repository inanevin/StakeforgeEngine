...
namespace glz
{
	...
	{
		template <class T, size_t N> struct byte_storage
		{
			...
			constexpr void set_storage_size(size_t new_size) noexcept
			{
				size_ = new_size;
			}
		};
		...
		template <class T, size_t N> using storage_type = std::conditional_t<N == 0, zero_storage<T>, byte_storage<T, N>>;

		template <class T, size_t N> class inplace_vector_base : protected storage_type<T, N>
		{
		protected:
		...
		public:
			...
			constexpr inplace_vector_base(inplace_vector_base&& other) noexcept(N == 0 || std::is_nothrow_move_constructible_v<T>)
				requires std::is_trivially_copyable_v<T>
			= default;
			...
			constexpr inplace_vector_base& operator=(inplace_vector_base&& other) noexcept(N == 0 || (std::is_nothrow_move_assignable_v<T> && std::is_nothrow_move_constructible_v<T>))
				requires std::is_trivially_copyable_v<T>
			= default;
			...
			{
				return this->data_ptr() + storage_size();
			}
			...
			{
				return this->data_ptr()[storage_size() - 1];
			}
			...
			{
				return *try_push_back(x);
			}

			constexpr reference unchecked_push_back(T&& x)
			{
				return *try_push_back(std::move(x));
			}
			...
			constexpr iterator erase(const_iterator position)
			{
				...
				if constexpr (std::is_trivially_copyable_v<T> && std::is_trivially_destructible_v<T>)
				{
					...
					{
						std::memmove(this->data_ptr() + pos_idx, this->data_ptr() + pos_idx + 1, (storage_size() - pos_idx - 1) * sizeof(T));
					}
				}
				else
				{
			...
			constexpr iterator erase(const_iterator first, const_iterator last)
			{
				...
				if constexpr (std::is_trivially_copyable_v<T> && std::is_trivially_destructible_v<T>)
				{
					...
					{
						std::memmove(this->data_ptr() + first_idx, this->data_ptr() + last_idx, (storage_size() - last_idx) * sizeof(T));
					}
				}
				else
				{
			...
			constexpr void swap(inplace_vector_base& x) noexcept(N == 0 || (std::is_nothrow_swappable_v<T> && std::is_nothrow_move_constructible_v<T>))
			{
				if (this == &x)
					return;
				...
				else
				{
					...
					for (size_type i = 0; i < min_size; ++i)
						std::swap(this->data_ptr()[i], x.data_ptr()[i]);
			...
			friend constexpr bool operator==(const inplace_vector_base& lhs, const inplace_vector_base& rhs)
			{
				...
				{
					if (lhs.storage_size() != rhs.storage_size())
						return false;
	...
	template <class T, size_t N> constexpr void swap(detail::inplace_vector::inplace_vector_base<T, N>& x, detail::inplace_vector::inplace_vector_base<T, N>& y) noexcept(noexcept(x.swap(y)))
	{
		x.swap(y);
	}

	template <class T, size_t N, class U> constexpr typename detail::inplace_vector::inplace_vector_base<T, N>::size_type erase(detail::inplace_vector::inplace_vector_base<T, N>& c, const U& value)
	{
	...
	template <class T, size_t N, class Predicate> constexpr typename detail::inplace_vector::inplace_vector_base<T, N>::size_type erase_if(detail::inplace_vector::inplace_vector_base<T, N>& c, Predicate pred)
	{
	...
	template <class T, size_t N> class inplace_vector : public detail::inplace_vector::inplace_vector_base<T, N>
	{
	public:
		// types
		using value_type			 = T;
	...
	public:
		...
		constexpr inplace_vector(size_type n, const T& value) // freestanding-deleted
		{
			...
			else
			{
				for (size_type i = 0; i < n; ++i)
					std::construct_at(this->data_ptr() + i, value);
			}

			this->set_storage_size(n);
		}

		template <class InputIterator> constexpr inplace_vector(InputIterator first, InputIterator last) // freestanding-deleted
		{
			assign(first, last);
		}
		...
		// Assign methods
		template <std::input_iterator InputIterator> constexpr void assign(InputIterator first, InputIterator last) // freestanding-deleted
		{
			this->clear();
			...
			{
				if (std::distance(first, last) > static_cast<difference_type>(N))
					GLZ_THROW_OR_ABORT(std::bad_alloc());
			}
		...
		constexpr void assign_range(R&& rg) // freestanding-deleted
		{
			...
			{
				if (std::ranges::size(rg) > N)
					GLZ_THROW_OR_ABORT(std::bad_alloc());
			}

			// Fast path for contiguous ranges of trivially copyable types
			if constexpr (std::is_trivially_copyable_v<T> && std::ranges::contiguous_range<R> && std::same_as<std::remove_cvref_t<std::ranges::range_reference_t<R>>, T>)
			{
		...
		constexpr void resize(size_type sz, const T& c) // freestanding-deleted
		{
			...
			if (sz > this->storage_size())
			{
				...
				else
				{
					...
					for (size_type i = this->storage_size(); i < sz; ++i)
						std::construct_at(this->data_ptr() + i, c);
				}
				this->set_storage_size(sz);
			}
			else if (sz < this->storage_size())
		...
		constexpr reference at(size_type n) // freestanding-deleted
		{
			if (n >= this->storage_size())
				GLZ_THROW_OR_ABORT(std::out_of_range("inplace_vector::at: index out of range"));
			return this->data_ptr()[n];
		}
		...
		// Standard modifiers
		template <class... Args> constexpr reference emplace_back(Args&&... args) // freestanding-deleted
		{
			if (this->storage_size() >= N)
				GLZ_THROW_OR_ABORT(std::bad_alloc());
		...
		constexpr void append_range(R&& rg) // freestanding-deleted
		{
			...
			if constexpr (std::ranges::sized_range<R>)
			{
				if (this->storage_size() + std::ranges::size(rg) > N)
					GLZ_THROW_OR_ABORT(std::bad_alloc());
			}
		...
		// Insert operations
		template <class... Args> constexpr iterator emplace(const_iterator position, Args&&... args) // freestanding-deleted
		{
			if (this->storage_size() >= N)
				GLZ_THROW_OR_ABORT(std::bad_alloc());
			...
			if constexpr (std::is_trivially_copyable_v<T> && std::is_trivially_destructible_v<T>)
			{
				...
				{
					std::memmove(this->data_ptr() + pos_idx + 1, this->data_ptr() + pos_idx, (this->storage_size() - pos_idx) * sizeof(T));
				}
			}
			else
			{
				// Construct a new element at the end
				std::construct_at(this->data_ptr() + this->storage_size(), std::move(this->data_ptr()[this->storage_size() - 1]));
		...
		constexpr iterator insert(const_iterator position, size_type n, const T& x) // freestanding-deleted
		{
			...
			if constexpr (std::is_trivially_copyable_v<T> && std::is_trivially_destructible_v<T>)
			{
				...
				if (pos_idx < this->storage_size())
				{
					std::memmove(this->data_ptr() + pos_idx + n, this->data_ptr() + pos_idx, (this->storage_size() - pos_idx) * sizeof(T));
				}
			...
			else
			{
				...
				{
					std::construct_at(this->data_ptr() + this->storage_size() + n - 1 - i, std::move(this->data_ptr()[this->storage_size() - 1 - i]));
				}

				// Move the remaining elements that need to be shifted but not constructed
				for (size_type i = this->storage_size() - std::min(n, this->storage_size() - pos_idx) - 1; i >= pos_idx && i < this->storage_size(); --i)
				{
		...
		template <class InputIterator> constexpr iterator insert(const_iterator position, InputIterator first,
																 InputIterator last) // freestanding-deleted
		{
			...
			{
				auto count = std::distance(first, last);
				if (this->storage_size() + count > N)
					GLZ_THROW_OR_ABORT(std::bad_alloc());

				if (count == 0)
					return this->begin() + pos_idx;

				if constexpr (std::is_trivially_copyable_v<T> && std::contiguous_iterator<InputIterator> && std::is_trivially_destructible_v<T> && std::same_as<std::remove_cvref_t<std::iter_reference_t<InputIterator>>, T>)
				{
					// Move existing elements to make space
					if (pos_idx < this->storage_size())
					{
						std::memmove(this->data_ptr() + pos_idx + count, this->data_ptr() + pos_idx, (this->storage_size() - pos_idx) * sizeof(T));
					}
				...
				else
				{
					...
					if (pos_idx < this->storage_size())
					{
						...
						{
							std::construct_at(this->data_ptr() + this->storage_size() + count - 1 - i, std::move(this->data_ptr()[this->storage_size() - 1 - i]));
						}

						// Move the remaining elements
						for (size_type i = this->storage_size() - std::min(count, this->storage_size() - pos_idx) - 1; i >= pos_idx && i < this->storage_size(); --i)
						{
			...
			else
			{
				...
				if (this->storage_size() + temp.size() > N)
					GLZ_THROW_OR_ABORT(std::bad_alloc());
		...
		constexpr iterator insert_range(const_iterator position, R&& rg) // freestanding-deleted
		{
			...
			if constexpr (std::ranges::sized_range<R>)
			{
				...
				if constexpr (std::is_trivially_copyable_v<T> && std::ranges::contiguous_range<R> && std::is_trivially_destructible_v<T> && std::same_as<std::remove_cvref_t<std::ranges::range_reference_t<R>>, T>)
				{
					// Move existing elements to make space
					if (pos_idx < this->storage_size())
					{
