...
namespace glz
{
	...
	template <class T>
	concept wide_char_t = std::same_as<std::remove_cvref_t<T>, char16_t> || std::same_as<std::remove_cvref_t<T>, char32_t> || std::same_as<std::remove_cvref_t<T>, wchar_t>;
	...
	template <class T>
	concept is_inplace_vector = has_try_emplace_back<T> && accessible<std::remove_cvref_t<T>> && has_data<std::remove_cvref_t<T>>;
...
namespace glz
{
	...
	// range like
	template <class T> using iterator_t = decltype(std::begin(std::declval<T&>()));

	template <range R> using range_value_t = std::iter_value_t<iterator_t<R>>;

	template <range R> [[nodiscard]] constexpr bool empty_range(R&& rng)
	{
...
namespace glz
{
	...
	template <class T> constexpr bool const_value_v = std::is_const_v<std::remove_pointer_t<std::remove_reference_t<T>>>;
}

namespace glz::detail
{
	template <class Container> using iterator_pair_type = typename std::iterator_traits<decltype(std::begin(std::declval<Container&>()))>::value_type;

	template <class Container, typename Iterator = iterator_pair_type<Container>> struct iterator_second_impl;
	...
	template <class Container> using iterator_second_type = typename iterator_second_impl<Container>::type;

	template <class Container, typename Iterator = iterator_pair_type<Container>> struct iterator_first_impl;
	...
	template <class Container> using iterator_first_type = typename iterator_first_impl<Container>::type;
}
