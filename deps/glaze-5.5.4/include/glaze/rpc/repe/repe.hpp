...
namespace glz
{
	...
	template <uint32_t P> struct protocol_storage
	{
	};
}
...
namespace glz::repe
{
	...
	template <auto Opts, class Value> void write_response(Value&& value, is_state auto&& state)
	{
		auto& in	  = state.in;
		auto& out	  = state.out;
	...
	template <auto Opts> void write_response(is_state auto&& state)
	{
		auto& in	  = state.in;
		auto& out	  = state.out;
		out.header.id = in.header.id;
	...
	// returns 0 on error
	template <auto Opts, class Value> size_t read_params(Value&& value, auto&& state)
	{
		glz::context ctx{};
	...
	namespace detail
	{
		template <auto Opts> struct request_impl
		{
			...
			template <class Value> message operator()(const user_header& h, Value&& value) const
			{
				message msg{};
				msg.header = encode(h);
				msg.query  = std::string{h.query};
			...
			template <class Value> void operator()(const user_header& h, message& msg, Value&& value) const
			{
				msg.header = encode(h);
				msg.query  = std::string{h.query};
				msg.header.write(true);
	...
	template <auto Opts> inline constexpr auto request = detail::request_impl<Opts>{};
