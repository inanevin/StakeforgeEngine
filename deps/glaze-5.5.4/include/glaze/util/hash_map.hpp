...
namespace glz
{
	...
	template <size_t I, pair_t T> constexpr decltype(auto) get(T&& p) noexcept
	{
		if constexpr (I == 0)
		{
			return p.first;
...
namespace glz
{
	...
	struct naive_hash final
	{
		...
		template <naive_map_desc D> constexpr uint64_t operator()(const std::string_view value) noexcept
		{
	...
	template <uint64_t N> consteval auto fit_unsigned_type() noexcept
	{
	...
	template <class Key, class Value, size_t N> struct normal_map
	{
		...
		constexpr void find_perfect_hash() noexcept
		{
			...
			else
			{
				...
				std::sort(buckets_index.begin(), buckets_index.end(), [&bucket_sizes](size_t i1, size_t i2) { return bucket_sizes[i1] > bucket_sizes[i2]; });
	...
	template <class T, const std::string_view& S> struct micro_map1
	{
	...
	template <const std::string_view& S, bool CheckSize = true> inline constexpr bool cx_string_cmp(const std::string_view key) noexcept
	{
		if (std::is_constant_evaluated())
		{
			return key == S;
	...
	template <class T, const std::string_view& S0, const std::string_view& S1> struct micro_map2
	{
