...
namespace glz
{
	template <class Char> inline bool compare(const Char* lhs, const Char* rhs, uint64_t count) noexcept
	{
		if (count > 7)
		{
			uint64_t v[2];
	...
	template <class Char> inline bool internal_compare(const Char* lhs, const Char* rhs, uint64_t count) noexcept
	{
		uint64_t v[2];
		while (count > 8)
		{
	...
	template <uint64_t Count, class Char> GLZ_ALWAYS_INLINE bool compare(const Char* lhs, const Char* rhs) noexcept
	{
	...
	template <const std::string_view& Str, size_t N = Str.size()> GLZ_ALWAYS_INLINE bool comparitor(const auto* other) noexcept
	{
		if constexpr (N == 8)
		{
	...
	template <const std::string_view& lhs> inline constexpr bool compare_sv(const std::string_view rhs) noexcept
	{
		if (std::is_constant_evaluated())
		{
			return lhs == rhs;
