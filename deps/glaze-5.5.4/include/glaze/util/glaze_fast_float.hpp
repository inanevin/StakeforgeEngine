...
namespace glz
{
	...
	template <bool null_terminated, class UC> GLZ_ALWAYS_INLINE constexpr glz::fast_float::parsed_number_string_t<UC> parse_number_string(UC const* p, UC const* pend) noexcept
	{
		...
		{ // C++17 20.19.3.(7.1) explicitly forbids '+' sign here
			...
			if (!glz::fast_float::is_integer(*p)) [[unlikely]]
	...
	template <bool null_terminated, class T, class UC> constexpr glz::fast_float::from_chars_result_t<UC> from_chars(UC const* first, UC const* last, T& value) noexcept
	{
		using namespace glz::fast_float;
