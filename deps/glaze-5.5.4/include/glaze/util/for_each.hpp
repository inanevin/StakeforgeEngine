...
namespace glz
{
	...
	template <size_t N> inline constexpr void for_each(auto&& lambda)
	{
		if constexpr (N > 0)
		{
			...
			{
				[&]<size_t... I>(std::index_sequence<I...>) { (void)(lambda.template operator()<I>(), ...); }(std::make_index_sequence<N>{});
			}
		}
	}

	// Runtime short circuiting if function returns true, return false to continue evaluation
	template <size_t N> constexpr void for_each_short_circuit(auto&& lambda)
	{
		if constexpr (N > 0)
		{
			[&]<size_t... I>(std::index_sequence<I...>) { (lambda.template operator()<I>() || ...); }(std::make_index_sequence<N>{});
		}
	}

	template <class Func, class Tuple> constexpr void for_each_apply(Func&& f, Tuple&& t)
	{
	...
	template <size_t I, class Lambda> constexpr auto make_jump_function()
	{
		return +[](Lambda& l) { l.template operator()<I>(); };
	}

	// Important: index must be less than N
	template <size_t N> inline constexpr void visit(auto&& lambda, const size_t index)
	{
		if constexpr (N > 0)
		{
			// Explicit sizes for small N to help the compiler and make debugging easier
			...
			else
			{
				...
				static const auto jump_table = []<size_t... I>(std::index_sequence<I...>) { return std::array{make_jump_function<I, Lambda>()...}; }(std::make_index_sequence<N>{});
#else
				static constexpr auto jump_table = []<size_t... I>(std::index_sequence<I...>) { return std::array{+[](std::decay_t<decltype(lambda)>& l) { l.template operator()<I>(); }...}; }(std::make_index_sequence<N>{});
#endif
