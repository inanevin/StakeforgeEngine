...
namespace glz
{
	...
	consteval uint64_t not_repeat_byte8(const uint8_t repeat)
	{
		return ~(0x0101010101010101ull * repeat);
	}
	...
	template <class Char> [[nodiscard]] GLZ_ALWAYS_INLINE uint32_t code_point_to_utf8(const uint32_t code_point, Char* c) noexcept
	{
		if (code_point <= 0x7F)
		{
			c[0] = Char(code_point);
	...
	template <class Char> [[nodiscard]] GLZ_ALWAYS_INLINE uint32_t handle_unicode_code_point(const Char*& it, Char*& dst, const Char* end) noexcept
	{
	...
	template <class Char> [[nodiscard]] GLZ_ALWAYS_INLINE bool skip_unicode_code_point(const Char*& it, const Char* end) noexcept
	{
		using namespace unicode;
		if (it + 4 >= end) [[unlikely]]
		{
	...
	template <char C, auto Opts> GLZ_ALWAYS_INLINE bool match_invalid_end(is_context auto& ctx, auto&& it, auto&& end) noexcept
	{
		if (*it != C) [[unlikely]]
		{
			if constexpr (C == '"')
	...
	template <char C> GLZ_ALWAYS_INLINE bool match(is_context auto& ctx, auto&& it) noexcept
	{
		if (*it != C) [[unlikely]]
		{
			if constexpr (C == '"')
	...
	template <char Char> GLZ_ALWAYS_INLINE constexpr auto has_char(const uint64_t chunk) noexcept
	{
...
{
	// skip whitespace
	template <auto Opts> GLZ_ALWAYS_INLINE bool skip_ws(is_context auto&& ctx, auto&& it, auto&& end) noexcept
	{
	...
	template <auto Opts> GLZ_ALWAYS_INLINE void skip_string_view(is_context auto&& ctx, auto&& it, auto&& end) noexcept
	{
		while (it < end) [[likely]]
		{
			const auto* pc = std::memchr(it, '"', size_t(end - it));
	...
	template <auto Opts> GLZ_ALWAYS_INLINE void skip_number(is_context auto&& ctx, auto&& it, auto&& end) noexcept
	{
		if constexpr (!check_validate_skipped(Opts))
		{
	...
	template <size_t multiple> GLZ_ALWAYS_INLINE constexpr auto round_up_to_multiple(const std::integral auto val) noexcept
	{
...
namespace glz
{
	...
	namespace detail
	{
		...
		template <class I> constexpr from_chars_result from_chars(const char* first, const char* last, I& value, int base = 10)
		{
