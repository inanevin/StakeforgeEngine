...
namespace glz
{
	...
	template <size_t N> struct string_literal
	{
		...
		[[nodiscard]] constexpr const std::string_view sv() const noexcept
		{
			return {value, length};
		}
	...
	template <size_t N> constexpr auto string_literal_from_view(sv str)
	{
	...
	template <string_literal Str> inline constexpr std::string_view chars = Str.sv();

	template <string_literal Str> inline constexpr std::string_view root = Str.sv();

	namespace detail
	{
		template <std::array V> struct make_static
		{
			static constexpr auto value = V;
		};

		template <const std::string_view&... Strs> inline constexpr std::string_view join()
		{
	...
	template <const std::string_view&... Strs> inline constexpr auto join_v = detail::join<Strs...>();
