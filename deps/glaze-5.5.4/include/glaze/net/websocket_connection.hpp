...
namespace glz
{
	...
	struct ws_frame_header
	{
		...
		void fin(bool v)
		{
			data[0] = (data[0] & ~0x80) | (v ? 0x80 : 0);
		}
		void opcode(ws_opcode v)
		{
			data[0] = (data[0] & ~0x0F) | (static_cast<uint8_t>(v) & 0x0F);
		}
		void mask(bool v)
		{
			data[1] = (data[1] & ~0x80) | (v ? 0x80 : 0);
		}
		void payload_len(uint8_t v)
		{
			data[1] = (data[1] & ~0x7F) | (v & 0x7F);
		}
		...
		ws_opcode opcode() const
		{
			return static_cast<ws_opcode>(data[0] & 0x0F);
		}
		bool mask() const
		{
	...
	namespace ws_util
	{
		...
		inline bool header_contains(std::string_view header, std::string_view value)
		{
			while (!header.empty())
			{
				...
				if (token.size() == value.size() && std::equal(token.begin(), token.end(), value.begin(), value.end(), [](char a, char b) { return std::tolower(a) == std::tolower(b); }))
				{
					return true;
				}
	...
	{
	public:
		inline websocket_connection(asio::ip::tcp::socket socket, websocket_server* server) : socket_(std::move(socket)), server_(server)
		{
		...
		// Set user data
		inline void set_user_data(std::shared_ptr<void> data)
		{
			user_data_ = data;
		}
		inline std::shared_ptr<void> get_user_data() const
		{
			return user_data_;
		}
	...
	private:
		inline void perform_handshake(const request& req)
		{
			...
			if (it == req.headers.end() || !std::equal(it->second.begin(), it->second.end(), websocket_str.begin(), websocket_str.end(), [](char a, char b) { return std::tolower(a) == std::tolower(b); }))
			{
				do_close();
				return;
			}
			...
			std::string response_str = "HTTP/1.1 101 Switching Protocols\r\n"
									   "Upgrade: websocket\r\n"
									   "Connection: Upgrade\r\n"
		...
		{
			auto self = shared_from_this();
			socket_.async_read_some(asio::buffer(read_buffer_), [self](std::error_code ec, std::size_t bytes_transferred) { self->on_read(ec, bytes_transferred); });
		}
		...
		inline void handle_frame(ws_opcode opcode, const uint8_t* payload, std::size_t length, bool fin)
		{
			...
			case ws_opcode::binary:
				...
				if (fin)
				{
					...
					if (server_)
					{
						server_->notify_message(shared_from_this(), std::string_view(reinterpret_cast<const char*>(message_buffer_.data()), message_buffer_.size()), current_opcode_);
					}
