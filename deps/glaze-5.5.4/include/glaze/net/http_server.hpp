...
namespace glz
{
	...
	struct streaming_connection : public std::enable_shared_from_this<streaming_connection>
	{
		...
		streaming_connection(std::shared_ptr<asio::ip::tcp::socket> socket) : socket_(socket), is_headers_sent_(false), is_closed_(false)
		{
		}

		// Send initial headers for streaming response
		void send_headers(int status_code, const std::unordered_map<std::string, std::string>& headers = {}, data_sent_handler handler = {})
		{
			if (is_headers_sent_)
				return;
		...
		void send_chunk(std::string_view data, data_sent_handler handler = {})
		{
			...
			if (chunked_encoding_)
			{
				...
				if (auto [ptr, ec] = std::to_chars(size_buf.data(), size_buf.data() + size_buf.size(), data.size(), 16); ec == std::errc())
				{
		...
		void send_event(std::string_view event_type, std::string_view data, std::string_view id = {}, data_sent_handler handler = {})
		{
			std::string sse_data;
		...
		template <class T> void send_json_event(const T& data, std::string_view event_type = "message", std::string_view id = {}, data_sent_handler handler = {})
		{
			std::string json_str;
		...
		bool is_open() const
		{
			return socket_ && socket_->is_open() && !is_closed_;
		}
	...
	private:
		...
		void start_disconnect_detection()
		{
			...
			socket_->async_receive(asio::buffer(*buffer), asio::socket_base::message_peek, [self, buffer](std::error_code ec, std::size_t) {
	...
	struct streaming_response
	{
		...
		streaming_response& start_stream(int status_code = 200, const std::unordered_map<std::string, std::string>& headers = {})
		{
			if (stream)
			{
		...
		// Send JSON data
		template <class T> streaming_response& send_json(const T& data)
		{
			if (stream)
			{
				std::string json_str;
		...
		{
			return start_stream(200, {{"Content-Type", "text/event-stream"}, {"Cache-Control", "no-cache"}, {"Access-Control-Allow-Origin", "*"}});
		}
	...
	template <bool EnableTLS = false> struct http_server
	{
		...
		{
			error_handler = [](std::error_code ec, std::source_location loc) { std::fprintf(stderr, "Error at %s:%d: %s\n", loc.file_name(), static_cast<int>(loc.line()), ec.message().c_str()); };
		...
		inline http_server& bind(uint16_t port)
		{
		 ...
		 */
		http_server& enable_openapi_spec(std::string_view path = "/openapi.json", std::string_view title = "API Specification", std::string_view version = "1.0.0")
		{
				...
				for (const auto& [route_path, method_handlers] : root_router.routes)
				{
					...
					for (const auto& [method, route_entry] : method_handlers)
					{
						...
						for (const auto& segment : segments)
						{
							...
							{
								if (!op.parameters)
									op.parameters.emplace();
								auto& param	   = op.parameters->emplace_back();
								param.name	   = segment.substr(1);
								param.in	   = "path";
								param.required = true;
								if (auto it = route_entry.spec.constraints.find(param.name); it != route_entry.spec.constraints.end())
								{
	...
	private:
		...
		inline void process_request(asio::ip::tcp::socket socket)
		{
			...
			asio::async_read_until(*socket_ptr, *buffer, "\r\n\r\n", [this, socket_ptr, buffer, remote_endpoint](std::error_code ec, std::size_t /*bytes_transferred*/) {
				...
				if (method_sv.empty() || !std::all_of(method_sv.begin(), method_sv.end(), [](char c) { return std::isalnum(c) || c == '_'; }))
				{
					send_error_response(socket_ptr, 400, "Bad Request");
					return;
				}
				...
				if (major_v.empty() || !std::all_of(major_v.begin(), major_v.end(), ::isdigit) || minor_v.empty() || !std::all_of(minor_v.begin(), minor_v.end(), ::isdigit))
				{
					send_error_response(socket_ptr, 400, "Bad Request");
					return;
				}
				...
				if (content_length > 0)
				{
					...
					{
						asio::async_read(*socket_ptr, *buffer, asio::transfer_exactly(content_length - body.length()), [this, socket_ptr, buffer, method_opt, target, headers, remote_endpoint, body = std::move(body)](std::error_code ec, size_t) mutable {
							if (ec)
							{
								error_handler(ec, std::source_location::current());
								return;
		...
		inline void handle_websocket_upgrade(std::shared_ptr<asio::ip::tcp::socket> socket, http_method method, const std::string& target, const std::unordered_map<std::string, std::string>& headers, asio::ip::tcp::endpoint remote_endpoint)
		{
		...
		inline void handle_streaming_request(
			std::shared_ptr<asio::ip::tcp::socket> socket, http_method method, const std::string& target, const std::unordered_map<std::string, std::string>& headers, std::string body, asio::ip::tcp::endpoint remote_endpoint, const streaming_handler& handler)
		{
		...
		inline void process_full_request(std::shared_ptr<asio::ip::tcp::socket> socket, http_method method, const std::string& target, const std::unordered_map<std::string, std::string>& headers, std::string body, asio::ip::tcp::endpoint remote_endpoint)
		{
			...
			if (handler_it != streaming_handlers_.end())
			{
				...
				if (method_it != handler_it->second.end())
				{
					...
					handle_streaming_request(socket, method, target, headers, std::move(body), remote_endpoint, method_it->second);
					return;
				}
			}
		...
		inline void send_response(std::shared_ptr<asio::ip::tcp::socket> socket, const response& response)
		{
			...
			asio::async_write(*socket, asio::buffer(*response_buffer), [socket, response_buffer](std::error_code /*ec*/, std::size_t /*bytes_transferred*/) {
				// response_buffer keeps the string alive for the duration of the async operation
				// Socket cleanup handled by shared_ptr
			});
		}

		inline void send_error_response(std::shared_ptr<asio::ip::tcp::socket> socket, int status_code, const std::string& message)
		{
			response response;
	...
	namespace streaming_utils
	{
		...
		template <typename T> void send_periodic_data(std::shared_ptr<streaming_connection> conn, std::function<T()> data_generator, std::chrono::milliseconds interval, size_t max_events = 0)
		{
		...
		template <typename Container> void stream_collection(std::shared_ptr<streaming_connection> conn, const Container& data, std::chrono::milliseconds delay_between_items = std::chrono::milliseconds(10))
		{
