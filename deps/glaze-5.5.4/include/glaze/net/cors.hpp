...
namespace glz
{
	...
	inline bool is_origin_allowed(const cors_config& config, const std::string& origin)
	{
		...
		if (config.allowed_origins.empty() || std::find(config.allowed_origins.begin(), config.allowed_origins.end(), "*") != config.allowed_origins.end())
		{
			return true;
		}
	...
	inline handler create_cors_middleware(const cors_config& config = {})
	{
		return [config](const request& req, response& res) {
			...
			bool is_preflight = (req.method == http_method::OPTIONS) && (req.headers.find("Access-Control-Request-Method") != req.headers.end());
			...
			if (!origin.empty() && is_origin_allowed(config, origin))
			{
				...
				if (config.allow_credentials || std::find(config.allowed_origins.begin(), config.allowed_origins.end(), "*") == config.allowed_origins.end())
				{
