...
namespace glz
{
	...
	struct response
	{
		...
		template <auto Opts, class T> response& body(T&& value)
		{
		...
		template <class T = json_t> response& json(T&& value)
		{
			content_type("application/json");
	...
	struct http_router
	{
		...
		static bool match_pattern(std::string_view value, std::string_view pattern)
		{
			...
			while (v_pos < value.size() || p_pos < pattern.size())
			{
				...
				if (v_pos >= value.size())
				{
					...
					if (p_pos < pattern.size() && pattern[p_pos] == '*' && p_pos == pattern.size() - 1)
						return true;
		 ...
		 */
		inline std::pair<handler, std::unordered_map<std::string, std::string>> match(http_method method, const std::string& target) const
		{
			std::unordered_map<std::string, std::string> params;
	...
	private:
		 ...
		 * @throws std::runtime_error if there's a route conflict
		 */
		void add_route(http_method method, std::string_view path, handler handle, const std::unordered_map<std::string, param_constraint>& constraints = {})
		{
			std::string path_str(path);
			...
			if (path_str.find(':') == std::string::npos && path_str.find('*') == std::string::npos)
			{
				...
				{
					throw std::runtime_error("Route conflict: handler already exists for " + std::string(to_string(method)) + " " + path_str);
				}
			...
			for (size_t i = 0; i < segments.size(); ++i)
			{
				...
				if (segment[0] == ':')
				{
					...
					else if (current->parameter_child->parameter_name != param_name)
					{
						...
						throw std::runtime_error("Route conflict: different parameter names at same position: :" + current->parameter_child->parameter_name + " vs :" + param_name);
					}
				...
				else if (segment[0] == '*')
				{
					...
					else if (current->wildcard_child->parameter_name != wildcard_name)
					{
						...
						throw std::runtime_error("Route conflict: different wildcard names at same position: *" + current->wildcard_child->parameter_name + " vs *" + wildcard_name);
					}
		 ...
		 */
		bool match_node(RadixNode* node, const std::vector<std::string>& segments, size_t index, http_method method, std::unordered_map<std::string, std::string>& params, handler& result) const
		{
			// End of path
