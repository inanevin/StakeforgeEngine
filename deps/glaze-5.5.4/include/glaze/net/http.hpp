...
namespace glz
{
	...
	inline std::expected<HttpStatusLine, std::error_code> parse_http_status_line(std::string_view status_line)
	{
		...
		std::string_view status_code_str = status_line.substr(first_space + 1, has_status_message ? second_space - first_space - 1 : std::string_view::npos);
		...
		if (has_status_message && second_space < status_line.length() - 1)
		{
			...
			const auto start = std::find_if(status_message.begin(), status_message.end(), [](unsigned char ch) { return !std::isspace(ch); });
			...
			{
				const auto end = std::find_if(status_message.rbegin(), status_message.rend(), [](unsigned char ch) { return !std::isspace(ch); }).base();
		...
		const bool valid_version = std::all_of(version.begin(), version.begin() + dot_pos, [](unsigned char ch) { return std::isdigit(ch); }) && std::all_of(version.begin() + dot_pos + 1, version.end(), [](unsigned char ch) { return std::isdigit(ch); });
		...
		if (status_code_str.empty() || status_code_str.length() > max_status_code_length || !std::all_of(status_code_str.begin(), status_code_str.end(), [](unsigned char ch) { return std::isdigit(ch); }))
		{
			return std::unexpected(std::make_error_code(std::errc::protocol_error));
		}
		...
		int status_code;
		const auto [ptr, ec] = std::from_chars(status_code_str.data(), status_code_str.data() + status_code_str.size(), status_code);
