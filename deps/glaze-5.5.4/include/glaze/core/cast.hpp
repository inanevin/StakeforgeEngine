...
namespace glz
{
	...
	template <class T, auto Target, class CastType> struct cast_t
	{
	...
	template <uint32_t Format, is_cast T> struct from<Format, T>
	{
	...
	template <uint32_t Format, is_cast T> struct to<Format, T>
	{
	...
	template <auto Target, class CastType> constexpr auto cast_impl() noexcept
	{
		return [](auto&& v) { return cast_t<std::remove_cvref_t<decltype(v)>, Target, CastType>{v}; };
	}

	template <auto Target, class CastType> constexpr auto cast = cast_impl<Target, CastType>();
}
