...
namespace glz
{
	...
	template <class T, auto Target, auto Constraint, string_literal Message> struct read_constraint_t
	{
	...
	template <uint32_t Format, is_read_constraint T> struct from<Format, T>
	{
	...
	template <uint32_t Format, is_read_constraint T> struct to<Format, T>
	{
		template <auto Opts> static void op(auto&& value, is_context auto&& ctx, auto&&... args)
		{
			...
			else
			{
				...
				{
					static_assert(false_v<Target>, "expected invocable function, perhaps you need const qualified input on your lambda");
				}
			}
		}
	};

	template <auto Target, auto Constraint, string_literal Message> constexpr auto read_constraint_impl() noexcept
	{
		return [](auto&& v) { return read_constraint_t<std::remove_cvref_t<decltype(v)>, Target, Constraint, Message>{v}; };
	}

	template <auto Target, auto Constraint, string_literal Message> constexpr auto read_constraint = read_constraint_impl<Target, Constraint, Message>();
}
