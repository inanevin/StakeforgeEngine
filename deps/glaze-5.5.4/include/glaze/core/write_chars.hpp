...
namespace glz
{
	template <class T> GLZ_ALWAYS_INLINE constexpr auto sized_integer_conversion() noexcept
	{
	...
	struct write_chars
	{
		template <auto Opts, class B> inline static void op(num_t auto&& value, is_context auto&& ctx, B&& b, auto&& ix) noexcept
		{
			...
			{
				if constexpr (uint8_t(check_float_max_write_precision(Opts)) > 0 && uint8_t(check_float_max_write_precision(Opts)) < sizeof(V))
				{
