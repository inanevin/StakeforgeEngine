...
namespace glz
{
	template <class T> struct seek_op;

	// Call a function on an value at the location of a json_ptr
	template <class F, class T> bool seek(F&& func, T&& value, sv json_ptr)
	{
	...
	template <class T> struct seek_op
	{
		template <class F> static bool op(F&& func, auto&& value, sv json_ptr)
		{
			if (json_ptr.empty())
			{
				func(value);
		...
		template <class F> static bool op(F&& func, auto&& value, sv json_ptr)
		{
			...
			if constexpr (glaze_array_t<T>)
			{
				...
				if (index >= member_array.size())
					return false;
				return std::visit([&](auto&& element) { return seek(std::forward<F>(func), get_member(value, element), json_ptr); }, member_array[index]);
			}
			else if constexpr (tuple_t<T> || is_std_tuple<T>)
			{
				if (index >= glz::tuple_size_v<T>)
					return false;
				auto tuple_element_ptr = detail::get_runtime(value, index);
				return std::visit([&](auto&& element_ptr) { return seek(std::forward<F>(func), *element_ptr, json_ptr); }, tuple_element_ptr);
			}
			else
			{
				if (index >= value.size())
	...
	struct seek_op<T>
	{
		template <class F> static bool op(F&& func, auto&& value, sv json_ptr)
		{
			...
			if constexpr (glaze_object_t<T> || reflectable<T>)
			{
				...
				const auto index = decode_hash_with_size<JSON_PTR, T, HashInfo, HashInfo.type>::op(key.data(), key.data() + key.size(), key.size());
	...
	using copy_cv_t = std::conditional_t<std::is_const_v<Source>, std::conditional_t<std::is_volatile_v<Source>, std::add_cv_t<Target>, std::add_const_t<Target>>, std::conditional_t<std::is_volatile_v<Source>, std::add_volatile_t<Target>, Target>>;

	// Helper to determine the correct pointer type based on value type
	template <class V, class T> using get_pointer_type = copy_cv_t<std::remove_reference_t<T>, V>*;

	// Helper to determine the correct reference wrapper type
	template <class V, class T> using get_reference_type = std::reference_wrapper<copy_cv_t<std::remove_reference_t<T>, V>>;

	// Get a reference to a value at the location of a json_ptr. Will error if
	// value doesnt exist or is wrong type
	template <class V, class T> expected<get_reference_type<V, T>, error_ctx> get(T&& root_value, sv json_ptr)
	{
	...
	template <class V, class T> get_pointer_type<V, T> get_if(T&& root_value, sv json_ptr) noexcept
	{
		get_pointer_type<V, T> result{};
		seek(
			[&](auto&& val) {
	...
	template <class V, class T> expected<V, error_code> get_value(T&& root_value, sv json_ptr) noexcept
	{
		V		   result{};
		bool	   found{};
		error_code ec{};
		seek(
			[&](auto&& val) {
				if constexpr (std::is_assignable_v<V&, decltype(val)> && non_narrowing_convertable<std::decay_t<decltype(val)>, V>)
				{
					found  = true;
					result = val;
				}
	...
	template <class T, class V> bool set(T&& root_value, const sv json_ptr, V&& value) noexcept
	{
		bool result{};
		seek(
			[&](auto&& val) {
				if constexpr (std::is_assignable_v<decltype(val), decltype(value)> && non_narrowing_convertable<std::decay_t<decltype(value)>, std::decay_t<decltype(val)>>)
				{
					result = true;
...
#pragma GCC diagnostic pop
	...
	template <class R> using call_result_t = std::conditional_t<std::is_reference_v<R> || std::is_pointer_v<R>, std::decay_t<R>*, R>;

	template <class R> using call_return_t = std::conditional_t<std::is_reference_v<R>, expected<std::reference_wrapper<std::decay_t<R>>, error_code>, expected<R, error_code>>;

	// call a member function
	template <class R, class T, class... Args> call_return_t<R> call(T&& root_value, sv json_ptr, Args&&... args)
	{
...
namespace glz
{
	...
	// TODO: handle ~ and / characters for full JSON pointer support
	template <auto& Str> constexpr auto split_json_ptr()
	{
	...
	namespace detail
	{
		...
		template <auto& Str> constexpr auto json_ptr_children()
		{
			constexpr auto	  str	= Str;
	...
	constexpr auto json_ptrs(auto&&... args)
	{
	...
	template <auto Arr, std::size_t... Is> constexpr auto make_arrays(std::index_sequence<Is...>)
	{
		return glz::tuplet::make_tuple(pair{sv{}, std::array<sv, Arr[Is]>{}}...);
	}

	template <size_t N, auto& Arr> constexpr auto sub_group(const auto start)
	{
		std::array<sv, N> ret;
	...
	template <auto& Arr> constexpr auto group_json_ptrs()
	{
	...
	template <class Root, string_literal ptr, class Expected = void> constexpr bool valid()
	{
		...
		else
		{
			...
			if constexpr (glz::glaze_object_t<V>)
			{
				...
				constexpr auto	I		 = decode_hash_with_size<JSON, V, HashInfo, HashInfo.type>::op(key_str.data(), key_str.data() + key_str.size(), key_str.size());
