...
namespace glz
{
	...
	template <> struct meta<std::string_view>
	{
		static constexpr std::string_view name = "std::string_view";
	};

	template <> struct meta<const std::string_view>
	{
		static constexpr std::string_view name = "const std::string_view";
	};

	template <class T> struct json_schema
	{
	};
	...
	namespace detail
	{
		...
		template <class T> Enum(T) -> Enum<T>;
	...
	template <class T> using meta_t = decay_keep_volatile_t<decltype(meta_v<T>)>;

	template <class T> using meta_wrapper_t = decay_keep_volatile_t<decltype(meta_wrapper_v<std::decay_t<T>>)>;
	...
	template <class T> using meta_keys_t = decay_keep_volatile_t<decltype(meta_keys_v<T>)>;

	template <class T> struct remove_meta_wrapper
	{
		using type = T;
	};
	template <glaze_t T> struct remove_meta_wrapper<T>
	{
		using type = std::remove_pointer_t<std::remove_const_t<meta_wrapper_t<T>>>;
	};
	template <class T> using remove_meta_wrapper_t = typename remove_meta_wrapper<T>::type;
	...
	template <class T> using meta_unknown_write_t = std::decay_t<decltype(meta_unknown_write_v<std::decay_t<T>>)>;
	...
	template <class T> using meta_unknown_read_t = std::decay_t<decltype(meta_unknown_read_v<std::decay_t<T>>)>;
	...
	concept meta_has_skip = requires(T t, const std::string_view s, const meta_context& mctx) {
		{
			glz::meta<std::remove_cvref_t<T>>::skip(s, mctx)
		} -> std::same_as<bool>;
	};
	...
	template <class T> using json_schema_type = std::decay_t<decltype(json_schema_v<T>)>;
