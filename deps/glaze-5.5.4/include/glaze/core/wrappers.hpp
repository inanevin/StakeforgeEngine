...
namespace glz
{
	...
	template <class T, auto OptsMemPtr> struct opts_wrapper_t
	{
		static constexpr bool glaze_wrapper = true;
	...
	template <auto MemPtr, auto OptsMemPtr> inline constexpr decltype(auto) opts_wrapper() noexcept
	{
		return [](auto&& val) {
	...
	template <class T, class From, class To> struct custom_t final
	{
		static constexpr auto glaze_reflect = false;
		using from_t						= From;
		using to_t							= To;
	...
	template <class T, class From, class To> custom_t(T&, From, To) -> custom_t<T, From, To>;

	template <auto From, auto To> constexpr auto custom_impl() noexcept
	{
	...
	template <auto MemPtr> constexpr auto append_arrays = opts_wrapper<MemPtr, &opts::append_arrays>();

	// Read and write booleans as numbers
	template <auto MemPtr> constexpr auto bools_as_numbers = opts_wrapper<MemPtr, &opts::bools_as_numbers>();

	// Read and write numbers as strings
	template <auto MemPtr> constexpr auto quoted_num = opts_wrapper<MemPtr, &opts::quoted_num>();

	// Treat types like std::string as numbers: read and write them quoted
	template <auto MemPtr> constexpr auto number = opts_wrapper<MemPtr, &opts::number>();

	// Write out string like types without quotes
	template <auto MemPtr> constexpr auto raw = opts_wrapper<MemPtr, &opts::raw>();

	// Reads into only existing fields and elements and then exits without parsing the rest of the input
	template <auto MemPtr> constexpr auto partial_read = opts_wrapper<MemPtr, &opts::partial_read>();

	// Customize reading and writing
	template <auto From, auto To> constexpr auto custom = custom_impl<From, To>();
}
