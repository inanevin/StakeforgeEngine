...
namespace glz
{
	...
	template <class V> struct shared_async_vector
	{
	private:
		struct shared_state
		{
	...
	public:
		...
		public:
			iterator(typename std::vector<std::unique_ptr<V>>::iterator	  item_it,
					 std::shared_ptr<shared_state>						  state,
		...
		public:
			const_iterator(typename std::vector<std::unique_ptr<V>>::const_iterator item_it, std::shared_ptr<shared_state> state, std::shared_ptr<std::shared_lock<std::shared_mutex>> existing_shared_lock = nullptr)
