...
namespace glz
{
	template <class T> struct async_vector
	{
	private:
		std::vector<T>			  items;
	...
	public:
		...
		public:
			...
			std::vector<T>& value() noexcept
			{
				return *ptr;
			}
			const std::vector<T>& value() const noexcept
			{
				return *ptr;
			}
		};
		...
		public:
			const_proxy(const std::vector<T>& p, std::shared_lock<std::shared_mutex>&& lock) noexcept : ptr{&p}, lock(std::move(lock))
			{
			}
		...
		proxy write()
		{
			return {items, std::unique_lock{mutex}};
		}
		const_proxy read() const
		{
			return {items, std::shared_lock{mutex}};
		}
		...
		friend void swap(async_vector& a, async_vector& b) noexcept
		{
			a.swap(b);
		}
