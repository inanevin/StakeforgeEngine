...
namespace glz
{
	...
	public:
		...
		public:
			...
			void insert(size_t pos, const std::string_view& sv)
			{
				ptr->insert(pos, sv);
			}
			...
			{
				return ptr->ends_with(sv);
			}
			bool ends_with(char c) const noexcept
			{
				return ptr->ends_with(c);
			}
			...
			{
				return ptr->find(str, pos);
			}
			size_t find(const std::string_view& sv, size_t pos = 0) const noexcept
			{
				return ptr->find(sv, pos);
			}
			...
			{
				return ptr->find_last_of(s, pos, count);
			}
			...
			{
				return ptr->find_first_not_of(s, pos);
			}
			...
			{
				return ptr->find_first_not_of(ch, pos);
			}
		...
		proxy write()
		{
			return {str, std::unique_lock{mutex}};
		}
		...
		const_proxy read() const
		{
			return {str, std::shared_lock{mutex}};
		}
		...
		friend void swap(async_string& lhs, async_string& rhs)
		{
			lhs.swap(rhs);
		}
	};
...
namespace glz
{
	template <uint32_t Format> struct from<Format, glz::async_string>
	{
	...
	template <uint32_t Format> struct to<Format, glz::async_string>
	{
		template <auto Opts> static void op(auto&& value, is_context auto&& ctx, auto&&... args) noexcept
		{
...
namespace std
{
	template <> struct formatter<glz::async_string>
	{
		...
		template <class FormatContext> auto format(const glz::async_string& s, FormatContext& ctx) const -> decltype(ctx.out())
		{
