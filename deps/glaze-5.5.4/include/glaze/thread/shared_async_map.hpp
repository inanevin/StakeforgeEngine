...
namespace glz
{
	template <class K, class V> struct shared_async_map
	{
	private:
		struct shared_state
		{
		...
		{
			auto it = std::lower_bound(state->items.cbegin(), state->items.cend(), key, [](const std::unique_ptr<std::pair<K, V>>& item_ptr, const K& key) { return item_ptr->first < key; });
			if (it != state->items.cend() && !(key < (*it)->first))
			{ // Equivalent to key == (*it)->first
		...
		{
			auto it = std::lower_bound(state->items.begin(), state->items.end(), key, [](const std::unique_ptr<std::pair<K, V>>& item_ptr, const K& key) { return item_ptr->first < key; });
			if (it != state->items.end() && !(key < (*it)->first))
			{ // Equivalent to key == (*it)->first
				return std::make_pair(it, true);
			}
			return std::make_pair(it, false);
	...
	public:
		...
		public:
			...
			// Copy Constructor
			iterator(const iterator& other) : item_it(other.item_it), state(other.state), shared_lock_ptr(other.shared_lock_ptr), unique_lock_ptr(other.unique_lock_ptr)
			{
			}

			// Move Constructor
			iterator(iterator&& other) noexcept : item_it(std::move(other.item_it)), state(std::move(other.state)), shared_lock_ptr(std::move(other.shared_lock_ptr)), unique_lock_ptr(std::move(other.unique_lock_ptr))
			{
			}
		...
		public:
			const_iterator(typename std::vector<std::unique_ptr<std::pair<K, V>>>::const_iterator item_it, std::shared_ptr<shared_state> state, std::shared_ptr<std::shared_lock<std::shared_mutex>> existing_shared_lock = nullptr)
			...
			// Copy Constructor
			const_iterator(const const_iterator& other) : item_it(other.item_it), state(other.state), shared_lock_ptr(other.shared_lock_ptr)
			{
			}

			// Move Constructor
			const_iterator(const_iterator&& other) noexcept : item_it(std::move(other.item_it)), state(std::move(other.state)), shared_lock_ptr(std::move(other.shared_lock_ptr))
			{
			}
		...
		public:
			value_proxy(V& value_ref, std::shared_ptr<std::shared_lock<std::shared_mutex>> existing_shared_lock = nullptr, std::shared_ptr<std::unique_lock<std::shared_mutex>> existing_unique_lock = nullptr)
			...
			template <class T> value_proxy& operator=(const T& other)
			{
				value_ref = other;
				return *this;
			}
		...
		public:
			const_value_proxy(const V& value_ref, std::shared_ptr<std::shared_lock<std::shared_mutex>> existing_shared_lock) : value_ref(value_ref), shared_lock_ptr(existing_shared_lock)
			{
				// Ensure that a lock is provided
		...
		value_proxy operator[](const K& key)
		{
			...
			else
			{
				...
				if (!found)
				{
					...
					it = state->items.insert(it, std::make_unique<std::pair<K, V>>(std::piecewise_construct, std::forward_as_tuple(key), std::forward_as_tuple()));
				}
		...
		template <class Key, class Value> std::pair<iterator, bool> emplace(Key&& key, Value&& value)
		{
			auto unique_lock_ptr = std::make_shared<std::unique_lock<std::shared_mutex>>(state->mutex);
			...
			{
				// Insert while maintaining sorted order
				it = state->items.insert(it, std::make_unique<std::pair<K, V>>(std::forward<Key>(key), std::forward<Value>(value)));
				return {iterator(it, state, nullptr, unique_lock_ptr), true};
			}
		}
		...
		template <typename... Args> std::pair<iterator, bool> emplace(const K& key, Args&&... args)
		{
			...
			else
			{
				...
				it = state->items.insert(it, std::make_unique<std::pair<K, V>>(std::piecewise_construct, std::forward_as_tuple(key), std::forward_as_tuple(std::forward<Args>(args)...)));
				return {iterator(it, state, nullptr, unique_lock_ptr), true};
			}
		}
