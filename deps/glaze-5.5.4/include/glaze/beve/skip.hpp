...
namespace glz
{
	template <> struct skip_value<BEVE>
	{
		template <auto Opts> inline static void op(is_context auto&& ctx, auto&& it, auto&& end) noexcept;
	};
	...
	template <auto Opts> inline void skip_object_beve(is_context auto&& ctx, auto&& it, auto&& end) noexcept
	{
	...
	template <auto Opts> inline void skip_typed_array_beve(is_context auto&& ctx, auto&& it, auto&& end) noexcept
	{
		const auto	  tag  = uint8_t(*it);
	...
	template <auto Opts> inline void skip_untyped_array_beve(is_context auto&& ctx, auto&& it, auto&& end) noexcept
	{
		++it;
		const auto n = int_from_compressed(ctx, it, end);
		if (bool(ctx.error)) [[unlikely]]
	...
	template <auto Opts> GLZ_ALWAYS_INLINE void skip_additional_beve(is_context auto&& ctx, auto&& it, auto&& end) noexcept
	{
		++it;
		skip_value<BEVE>::op<Opts>(ctx, it, end);
	}

	template <auto Opts> inline void skip_value<BEVE>::op(is_context auto&& ctx, auto&& it, auto&& end) noexcept
	{
