...
namespace glz
{
	template <> struct parse<BEVE>
	{
		...
		GLZ_ALWAYS_INLINE static void op(T&& value, Tag&& tag, Ctx&& ctx, It0&& it, It1&& end)
		{
			...
			else
			{
				...
				from<BEVE, V>::template op<Opts>(std::forward<T>(value), std::forward<Tag>(tag), std::forward<Ctx>(ctx), std::forward<It0>(it), std::forward<It1>(end));
			}
		}
		...
		GLZ_ALWAYS_INLINE static void op(T&& value, Ctx&& ctx, It0&& it, It1&& end)
		{
			...
			{
				using V = std::remove_cvref_t<T>;
				from<BEVE, V>::template op<Opts>(std::forward<T>(value), std::forward<Ctx>(ctx), std::forward<It0>(it), std::forward<It1>(end));
			}
		}
	};
	...
	struct from<BEVE, T>
	{
		template <auto Opts, class Value, is_context Ctx, class It0, class It1> GLZ_ALWAYS_INLINE static void op(Value&& value, Ctx&& ctx, It0&& it, It1&& end)
		{
			...
			from<BEVE, V>::template op<Opts>(get_member(std::forward<Value>(value), meta_wrapper_v<T>), std::forward<Ctx>(ctx), std::forward<It0>(it), std::forward<It1>(end));
		}
	};

	template <always_null_t T> struct from<BEVE, T>
	{
	...
	template <> struct from<BEVE, hidden>
	{
	...
	template <is_bitset T> struct from<BEVE, T>
	{
	...
	template <> struct from<BEVE, skip>
	{
	...
	template <glaze_flags_t T> struct from<BEVE, T>
	{
		template <auto Opts, is_context Ctx, class It0, class It1> static void op(auto&& value, Ctx&& ctx, It0&& it, It1&& end)
		{
			...
			for_each<N>([&]<size_t I>() { get_member(value, get<I>(reflect<T>::values)) = data[I / 8] & (uint8_t{1} << (7 - (I % 8))); });
		}
	};
	...
	struct from<BEVE, T>
	{
		...
		template <auto Opts>
			requires(check_no_header(Opts))
		GLZ_ALWAYS_INLINE static void op(auto&& value, const uint8_t tag, is_context auto&& ctx, auto&& it, auto&& end) noexcept
		{
			if (invalid_end(ctx, it, end))
			{
				return;
	...
	struct from<BEVE, T>
	{
		template <auto Opts> GLZ_ALWAYS_INLINE static void op(auto&& value, is_context auto&& ctx, auto&& it, auto&& end) noexcept
		{
	...
	template <boolean_like T> struct from<BEVE, T>
	{
		template <auto Opts> GLZ_ALWAYS_INLINE static void op(auto&& value, is_context auto&& ctx, auto&& it, auto&& end) noexcept
		{
			if (invalid_end(ctx, it, end))
	...
	template <is_member_function_pointer T> struct from<BEVE, T>
	{
		template <auto Opts> GLZ_ALWAYS_INLINE static void op(auto&& /*value*/, is_context auto&& /*ctx*/, auto&& /*it*/, auto&& /*end*/) noexcept
		{
		}
	...
	template <func_t T> struct from<BEVE, T>
	{
		template <auto Opts> GLZ_ALWAYS_INLINE static void op(auto&& /*value*/, is_context auto&& ctx, auto&& it, auto&& end) noexcept
		{
	...
	template <class T> struct from<BEVE, basic_raw_json<T>>
	{
	...
	template <class T> struct from<BEVE, basic_text<T>>
	{
		template <auto Opts> GLZ_ALWAYS_INLINE static void op(auto&& value, is_context auto&& ctx, auto&& it, auto&& end)
		{
			parse<BEVE>::op<Opts>(value.str, ctx, it, end);
	...
	template <is_variant T> struct from<BEVE, T>
	{
		template <auto Opts> GLZ_ALWAYS_INLINE static void op(auto&& value, is_context auto&& ctx, auto&& it, auto&& end) noexcept
		{
	...
	template <str_t T> struct from<BEVE, T> final
	{
	...
	template <readable_array_t T> struct from<BEVE, T> final
	{
		template <auto Opts> static void op(auto&& value, is_context auto&& ctx, auto&& it, auto&& end)
		{
			using V = range_value_t<std::decay_t<T>>;
	...
	template <pair_t T> struct from<BEVE, T> final
	{
		template <auto Opts> GLZ_ALWAYS_INLINE static void op(T& value, is_context auto&& ctx, auto&& it, auto&& end)
		{
	...
	template <readable_map_t T> struct from<BEVE, T> final
	{
		template <auto Opts> static void op(auto&& value, is_context auto&& ctx, auto&& it, auto&& end)
		{
	...
	template <is_includer T> struct from<BEVE, T>
	{
		template <auto Opts> GLZ_ALWAYS_INLINE static void op(auto&&, is_context auto&& ctx, auto&& it, auto&& end) noexcept
		{
			if constexpr (check_no_header(Opts))
	...
	struct from<BEVE, T> final
	{
		...
		static void op(auto&& value, is_context auto&& ctx, auto&& it, auto&& end)
		{
			...
			else
			{
				...
				for_each<N>([&]<size_t I>() { parse<BEVE>::op<Opts>(get_member(value, get<I>(reflect<V>::values)), ctx, it, end); });
			}
		}
	...
	struct from<BEVE, T> final
	{
		template <auto Opts> static void op(auto&& value, is_context auto&& ctx, auto&& it, auto&& end)
		{
			...
			for_each<N>([&]<size_t I>() { parse<BEVE>::op<Opts>(get_member(value, get<I>(reflect<T>::values)), ctx, it, end); });
		}
	};
	...
	template <filesystem_path T> struct from<BEVE, T>
	{
	...
	template <read_supported<BEVE> T, class Buffer> [[nodiscard]] inline error_ctx read_beve(T&& value, Buffer&& buffer)
	{
		return read<opts{.format = BEVE}>(value, std::forward<Buffer>(buffer));
	}

	template <read_supported<BEVE> T, class Buffer> [[nodiscard]] inline expected<T, error_ctx> read_beve(Buffer&& buffer)
	{
		T		   value{};
	...
	template <auto Opts = opts{}, read_supported<BEVE> T> [[nodiscard]] inline error_ctx read_file_beve(T& value, const sv file_name, auto&& buffer)
	{
		context ctx{};
	...
	template <read_supported<BEVE> T, class Buffer> [[nodiscard]] inline error_ctx read_binary_untagged(T&& value, Buffer&& buffer)
	{
		return read<opts{.format = BEVE, .structs_as_arrays = true}>(std::forward<T>(value), std::forward<Buffer>(buffer));
	}

	template <read_supported<BEVE> T, class Buffer> [[nodiscard]] inline expected<T, error_ctx> read_binary_untagged(Buffer&& buffer)
	{
		T		   value{};
	...
	template <auto Opts = opts{}, read_supported<BEVE> T> [[nodiscard]] inline error_ctx read_file_beve_untagged(T& value, const std::string& file_name, auto&& buffer)
	{
