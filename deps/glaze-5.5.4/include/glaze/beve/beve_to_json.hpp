...
namespace glz
{
	namespace detail
	{
		template <auto Opts> inline void beve_to_json_number(auto&& tag, auto&& ctx, auto&& it, auto&& end, auto& out, auto&& ix) noexcept
		{
		...
		template <auto Opts, class Buffer> inline void beve_to_json_value(auto&& ctx, auto&& it, auto&& end, Buffer& out, auto&& ix, uint32_t recursive_depth)
		{
	...
	template <auto Opts = glz::opts{}, class BEVEBuffer, class JSONBuffer> [[nodiscard]] inline error_ctx beve_to_json(const BEVEBuffer& beve, JSONBuffer& out)
	{
