...
namespace glz
{
	...
	template <typename T, std::size_t N> class volatile_array
	{
	public:
		...
		template <is_volatile_array Other> constexpr volatile volatile_array& operator=(const Other& other) volatile
		{
		...
		template <is_volatile_array Other> void swap(Other& other) volatile noexcept(std::is_nothrow_swappable_v<Other>)
		{
			for (size_type i = 0; i < N; ++i)
			{
