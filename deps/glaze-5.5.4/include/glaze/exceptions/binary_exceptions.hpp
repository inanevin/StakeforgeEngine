...
namespace glz::ex
{
	template <class T, class Buffer> void read_beve(T& value, Buffer&& buffer)
	{
	...
	template <class T, class Buffer> [[nodiscard]] T read_beve(Buffer&& buffer)
	{
	...
	template <auto Opts = opts{}, class T> void read_file_beve(T& value, const sv file_name, auto&& buffer)
	{
...
namespace glz::ex
{
	template <class T, class Buffer> void write_beve(T&& value, Buffer&& buffer)
	{
		glz::write_beve(std::forward<T>(value), std::forward<Buffer>(buffer));
	}

	template <class T> [[nodiscard]] auto write_beve(T&& value)
	{
		return glz::write_beve(std::forward<T>(value));
	}

	template <class T> void write_file_beve(T&& value, const std::string& file_name, auto&& buffer)
	{
