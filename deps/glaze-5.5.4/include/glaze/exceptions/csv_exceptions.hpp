...
namespace glz::ex
{
	template <uint32_t layout = rowwise, class T, class Buffer> inline void read_csv(T&& value, Buffer&& buffer)
	{
	...
	template <uint32_t layout = rowwise, class T, class Buffer> inline auto read_csv(Buffer&& buffer)
	{
	...
	template <uint32_t layout = rowwise, class T> inline void read_file_csv(T& value, const sv file_name, auto&& buffer)
	{
...
namespace glz::ex
{
	template <class T, class Buffer> inline auto write_csv(T&& value, Buffer&& buffer)
	{
	...
	template <class T> inline auto write_csv(T&& value)
	{
		return write_csv(std::forward<T>(value));
	}

	template <uint32_t layout = rowwise, class T> inline void write_file_csv(T&& value, const sv file_name, auto&& buffer)
	{
