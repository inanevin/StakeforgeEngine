...
namespace glz::ex
{
	template <auto Opts> void read(auto& value, auto&& buffer)
	{
	...
	template <auto Opts, class T, output_buffer Buffer> void write(T&& value, Buffer& buffer) noexcept
	{
		glz::write<Opts>(std::forward<T>(value), buffer);
	}

	template <auto Opts, class T, raw_buffer Buffer> size_t write(T&& value, Buffer&& buffer) noexcept
	{
