...
namespace glz
{
	template <auto Opts = opts{}, class Template, class T, resizable Buffer> [[nodiscard]] error_ctx stencilcount(Template&& layout, T&& value, Buffer& buffer)
	{
		context ctx{};
							...
							visit<N>(
								[&]<size_t I>() {
									...
									if ((Length == key.size()) && comparitor<TargetKey>(start)) [[likely]]
									{
										...
										{
											std::ignore = write<opt_true<Opts, &opts::raw>>(get_member(value, get<I>(to_tie(value))), temp, ctx);
										}
										else if constexpr (glaze_object_t<T> && N > 0)
										{
											std::ignore = write<opt_true<Opts, &opts::raw>>(get_member(value, get<I>(reflect<T>::values)), temp, ctx);
										}
									}
									else
									{
	...
	template <auto Opts = opts{}, class Template, class T> [[nodiscard]] expected<std::string, error_ctx> stencilcount(Template&& layout, T&& value)
	{
		std::string buffer{};
