...
namespace glz
{
	...
	template <auto Opts = opts{.format = STENCIL}, class Template, class T, resizable Buffer> [[nodiscard]] error_ctx stencil(Template&& layout, T&& value, Buffer& buffer)
	{
		context ctx{};
					...
					if (it != end && *it == '{')
					{
						...
						if (is_section || is_inverted_section)
						{
...

							{
								...
								const auto index = decode_hash_with_size<STENCIL, T, HashInfo, HashInfo.type>::op(start, end, key.size());
						...
						else [[likely]]
						{
							...
							static constexpr auto RawOpts = set_json<opt_true<Opts, &opts::raw>>(); // write out string like values without quotes
							...
							visit<N>(
								[&]<size_t I>() {
									...
									if ((TargetKey.size() == key.size()) && comparitor<TargetKey>(start)) [[likely]]
									{
										...
										if constexpr (reflectable<T>)
										{
											serialize<JSON>::template op<RawOpts>(get_member(value, get<I>(to_tie(value))), ctx, temp_buffer, ix);
										}
										else if constexpr (glaze_object_t<T>)
										{
											serialize<JSON>::template op<RawOpts>(get_member(value, get<I>(reflect<T>::values)), ctx, temp_buffer, ix);
										}
	...
	template <auto Opts = opts{.format = STENCIL}, class Template, class T> [[nodiscard]] expected<std::string, error_ctx> stencil(Template&& layout, T&& value)
	{
		std::string buffer{};
