...
#define CHECK_OFFSET(off)                                                                                                                                                                                                                                          \
	...
	namespace detail
	{
		template <class F, is_context Ctx, class It0, class It1> GLZ_ALWAYS_INLINE void decode_impl(F&& func, Ctx&& ctx, It0&& it, It1&& end)
		{
			int index{};
		...
		template <output_buffer B, class IX> [[nodiscard]] GLZ_ALWAYS_INLINE int resize_buffer(B&& b, IX&& ix, int index)
		{
		...
		template <class F, is_context Ctx, class B, class IX> GLZ_ALWAYS_INLINE void encode_impl(F&& func, Ctx&& ctx, B&& b, IX&& ix)
		{
	...
	template <class It> [[nodiscard]] GLZ_ALWAYS_INLINE int decode_version(is_context auto&& ctx, It&& it)
	{
		int index{};
		int version{};
	...
	template <class It> GLZ_ALWAYS_INLINE int get_type(is_context auto&& ctx, It&& it)
	{
		int type{};
		int size{};
		int index{};
	...
	template <class It0, class It1> auto skip_term(is_context auto&& ctx, It0&& it, It1&& end)
	{
		int index{};
	...
	template <num_t T, class... Args> GLZ_ALWAYS_INLINE void decode_number(T&& value, Args&&... args)
	{
	...
	template <class It0, class It1> GLZ_ALWAYS_INLINE void decode_token(auto&& value, is_context auto&& ctx, It0&& it, It1&& end)
	{
		using namespace std::placeholders;
	...
	template <class... Args> GLZ_ALWAYS_INLINE void decode_boolean(auto&& value, Args&&... args)
	{
		using namespace std::placeholders;
	...
	template <auto Opts, class T, class It0> void decode_binary(T&& value, std::size_t sz, is_context auto&& ctx, It0&& it, auto&& end)
	{
		using namespace std::placeholders;
	...
	template <is_context Ctx, class It> GLZ_ALWAYS_INLINE auto decode_list_header(Ctx&& ctx, It&& it)
	{
		int arity{};
		int index{};
	...
	template <auto Opts, class T> GLZ_ALWAYS_INLINE void decode_list(T&& value, is_context auto&& ctx, auto&& it, auto&& end)
	{
		using V = range_value_t<std::decay_t<T>>;
	...
	template <auto Opts, class T, is_context Ctx, class It0, class It1> GLZ_ALWAYS_INLINE void decode_sequence(T&& value, Ctx&& ctx, It0&& it, It1&& end)
	{
		int index{};
		int type{};
		int sz{};
		...
		{
			decode_binary<Opts>(std::forward<T>(value), static_cast<std::size_t>(sz), std::forward<Ctx>(ctx), std::forward<It0>(it), std::forward<It1>(end));
		}
		else if (eetf::is_list(type))
		{
			...
			{
				decode_list<Opts>(std::forward<T>(value), std::forward<Ctx>(ctx), std::forward<It0>(it), std::forward<It1>(end));
			}
		}
		// else if tuple?
		else
	...
	template <is_context Ctx, class It> GLZ_ALWAYS_INLINE auto decode_map_header(Ctx&& ctx, It&& it)
	{
		int arity{};
		int index{};
	...
	template <is_context Ctx, class It> GLZ_ALWAYS_INLINE auto decode_tuple_header(Ctx&& ctx, It&& it)
	{
		int arity{};
		int index{};
	...
	template <class B, class IX> GLZ_ALWAYS_INLINE void encode_version(is_context auto&& ctx, B&& b, IX&& ix)
	{
		int index{static_cast<int>(ix)};
	...
	template <class... Args> GLZ_ALWAYS_INLINE void encode_boolean(const bool value, Args&&... args)
	{
		using namespace std::placeholders;
		detail::encode_impl(std::bind(ei_encode_boolean, _1, _2, value), std::forward<Args>(args)...);
	}

	template <class T, class... Args> GLZ_ALWAYS_INLINE void encode_number(T&& value, Args&&... args)
	{
		using namespace std::placeholders;
	...
	template <class... Args> GLZ_ALWAYS_INLINE void encode_atom(auto&& value, Args&&... args)
	{
		using namespace std::placeholders;
		detail::encode_impl(std::bind(ei_encode_atom, _1, _2, value.data()), std::forward<Args>(args)...);
	}

	template <class... Args> GLZ_ALWAYS_INLINE void encode_atom_len(auto&& value, std::size_t sz, Args&&... args)
	{
		using namespace std::placeholders;
		detail::encode_impl(std::bind(ei_encode_atom_len, _1, _2, value.data(), static_cast<int>(sz)), std::forward<Args>(args)...);
	}

	template <class... Args> GLZ_ALWAYS_INLINE void encode_string(auto&& value, Args&&... args)
	{
		using namespace std::placeholders;
		detail::encode_impl(std::bind(ei_encode_string, _1, _2, value.data()), std::forward<Args>(args)...);
	}

	template <class... Args> GLZ_ALWAYS_INLINE void encode_tuple_header(std::size_t arity, Args&&... args)
	{
		using namespace std::placeholders;
		detail::encode_impl(std::bind(ei_encode_tuple_header, _1, _2, static_cast<int>(arity)), std::forward<Args>(args)...);
	}

	template <class... Args> GLZ_ALWAYS_INLINE void encode_list_header(std::size_t arity, Args&&... args)
	{
		using namespace std::placeholders;
		detail::encode_impl(std::bind(ei_encode_list_header, _1, _2, static_cast<int>(arity)), std::forward<Args>(args)...);
	}

	template <class... Args> GLZ_ALWAYS_INLINE void encode_list_tail(Args&&... args)
	{
		using namespace std::placeholders;
		detail::encode_impl(std::bind(ei_encode_list_header, _1, _2, 0), std::forward<Args>(args)...);
	}

	template <class... Args> GLZ_ALWAYS_INLINE void encode_map_header(std::size_t arity, Args&&... args)
	{
		using namespace std::placeholders;
