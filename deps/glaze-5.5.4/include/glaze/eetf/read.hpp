...
namespace glz
{
	...
	template <> struct skip_value<EETF>
	{
		template <auto Opts> GLZ_ALWAYS_INLINE static void op(is_context auto&& ctx, auto&& it, auto&& end) noexcept
		{
			skip_term(ctx, it, end);
		}
	};

	template <> struct parse<EETF>
	{
		...
		GLZ_ALWAYS_INLINE static void op(T&& value, Ctx&& ctx, It0&& it, It1&& end) noexcept
		{
			...
			parse<Opts.format>::template op<no_header_on<Opts>()>(std::forward<T>(value), std::forward<Ctx>(ctx), std::forward<It0>(it), std::forward<It1>(end));
		}
		...
		GLZ_ALWAYS_INLINE static void op(T&& value, Ctx&& ctx, It0&& it, It1&& end) noexcept
		{
			...
			{
				from<EETF, std::remove_cvref_t<T>>::template op<Opts>(std::forward<T>(value), std::forward<Ctx>(ctx), std::forward<It0>(it), std::forward<It1>(end));
			}
		}
	};

	template <readable_array_t T> struct from<EETF, T> final
	{
		template <auto Opts, is_context Ctx, class It0, class It1> GLZ_ALWAYS_INLINE static void op(auto&& value, Ctx&& ctx, It0&& it, It1&& end) noexcept
		{
			decode_sequence<Opts>(std::forward<T>(value), std::forward<Ctx>(ctx), std::forward<It0>(it), std::forward<It1>(end));
		}
	};

	template <boolean_like T> struct from<EETF, T>
	{
		template <auto Opts, is_context Ctx, class It0, class It1> GLZ_ALWAYS_INLINE static void op(auto&& value, Ctx&& ctx, It0&& it, It1&& end) noexcept
		{
	...
	template <num_t T> struct from<EETF, T> final
	{
		template <auto Opts, is_context Ctx, class It0, class It1> GLZ_ALWAYS_INLINE static void op(auto&& value, Ctx&& ctx, It0&& it, It1&& end) noexcept
		{
			if (bool(ctx.error)) [[unlikely]]
	...
	template <atom_t T> struct from<EETF, T> final
	{
		template <auto Opts, is_context Ctx, class It0, class It1> GLZ_ALWAYS_INLINE static void op(auto&& value, Ctx&& ctx, It0&& it, It1&& end) noexcept
		{
			if (bool(ctx.error)) [[unlikely]]
	...
	template <str_t T> struct from<EETF, T> final
	{
		template <auto Opts, is_context Ctx, class It0, class It1> GLZ_ALWAYS_INLINE static void op(auto&& value, Ctx&& ctx, It0&& it, It1&& end) noexcept
		{
			if (bool(ctx.error)) [[unlikely]]
	...
	struct from<EETF, T> final
	{
		template <is_context Ctx, class It0, class It1> class field_iterator
		{
		public:
			template <typename F> field_iterator(F&& f, Ctx&& ctx, It0&& it, It1&& end)
			{
			...
			field_iterator(error_code ec, Ctx&& ctx) : term_header{-1ull, -1ull}
			{
				ctx.error = ec;
			}

			template <auto Opts> bool next(Ctx&& ctx, It0&& it, It1&& end)
			{
		...
		template <auto Opts, is_context Ctx, class It0, class It1> static auto make_term_iterator(Ctx&& ctx, It0&& it, It1&& end)
		{
		...
		template <auto Opts, is_context Ctx, class It0, class It1> static void op(auto&& value, Ctx&& ctx, It0&& it, It1&& end) noexcept
		{
			if (bool(ctx.error)) [[unlikely]]
			{
				return;
			...
			while (term_it.template next<Opts>(ctx, it, end))
			{
				if constexpr (N > 0)
				{
					...
					const auto index = decode_hash_with_size<EETF, T, HashInfo, HashInfo.type>::op(mkey.data(), mkey.data() + n, n);
					if (index < N) [[likely]]
					{
	...
	template <uint8_t layout = glz::eetf::map_layout, read_supported<EETF> T, class Buffer> [[nodiscard]] inline error_ctx read_term(T&& value, Buffer&& buffer) noexcept
	{
		return read<eetf::eetf_opts{.format = EETF, .layout = layout}>(value, std::forward<Buffer>(buffer));
	}

	template <uint8_t layout = glz::eetf::map_layout, read_supported<EETF> T, is_buffer Buffer> [[nodiscard]] expected<T, error_ctx> read_term(Buffer&& buffer) noexcept
	{
		T				value{};
		context			ctx{};
		const error_ctx ec = read<eetf::eetf_opts{.format = EETF, .layout = layout}>(value, std::forward<Buffer>(buffer), ctx);
		if (ec)
		{
