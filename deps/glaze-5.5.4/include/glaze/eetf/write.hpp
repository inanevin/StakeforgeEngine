...
namespace glz
{
	...
	template <> struct serialize<EETF>
	{
	...
	template <boolean_like T> struct to<EETF, T>
	{
	...
	template <num_t T> struct to<EETF, T> final
	{
		template <auto Opts, class V, class... Args>
			requires(not check_no_header(Opts))
		GLZ_ALWAYS_INLINE static void op(V&& v, Args... args) noexcept
	...
	template <atom_t T> struct to<EETF, T> final
	{
		template <auto Opts, class V, class... Args>
			requires(not check_no_header(Opts))
		GLZ_ALWAYS_INLINE static void op(V&& v, Args... args) noexcept
	...
	template <str_t T> struct to<EETF, T> final
	{
		template <auto Opts, class V, class... Args>
			requires(not check_no_header(Opts))
		GLZ_ALWAYS_INLINE static void op(V&& v, Args... args) noexcept
	...
	template <string_t T> struct to<EETF, T> final
	{
		template <auto Opts, class V, class... Args>
			requires(not check_no_header(Opts))
		GLZ_ALWAYS_INLINE static void op(V&& v, Args... args) noexcept
	...
	struct to<EETF, T> final
	{
		...
		GLZ_ALWAYS_INLINE static void op(auto&& value, Ctx&& ctx, Args&&... args) noexcept
		{
			...
			{
				[&]<size_t... I>(std::index_sequence<I...>) { (serialize<EETF>::op<Opts>(std::get<I>(value), ctx, args...), ...); }(std::make_index_sequence<N>{});
			}
			else
			{
				[&]<size_t... I>(std::index_sequence<I...>) { (serialize<EETF>::op<Opts>(glz::get<I>(value), ctx, args...), ...); }(std::make_index_sequence<N>{});
			}
		}
	};

	template <writable_array_t T> struct to<EETF, T> final
	{
		template <auto Opts, class V, class... Args>
			requires(not check_no_header(Opts))
		GLZ_ALWAYS_INLINE static void op(V&& v, Args... args) noexcept
	...
	template <writable_map_t T> struct to<EETF, T> final
	{
		template <auto Opts, class V, class... Args>
			requires(not check_no_header(Opts))
		GLZ_ALWAYS_INLINE static void op(V&& v, Args... args) noexcept
	...
	template <class T> struct to<EETF, T> final
	{
		// empty for compilation error if use unsupported value type
	};

	template <uint8_t layout = glz::eetf::map_layout, write_supported<EETF> T, output_buffer Buffer> [[nodiscard]] error_ctx write_term(T&& value, Buffer&& buffer) noexcept
	{
		return write<eetf::eetf_opts{.format = EETF, .layout = layout}>(std::forward<T>(value), std::forward<Buffer>(buffer));
	}

	template <uint8_t layout = glz::eetf::map_layout, write_supported<EETF> T, raw_buffer Buffer> [[nodiscard]] expected<size_t, error_ctx> write_term(T&& value, Buffer&& buffer) noexcept
	{
		return write<eetf::eetf_opts{.format = EETF, .layout = layout}>(std::forward<T>(value), std::forward<Buffer>(buffer));
	}

	template <write_supported<EETF> T> [[nodiscard]] expected<std::string, error_ctx> write_term(T&& value) noexcept
	{
