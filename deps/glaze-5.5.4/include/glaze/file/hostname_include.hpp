...
namespace glz
{
	namespace detail
	{
		template <class T> struct hostname_includer
		{
			T& value;
	...
	template <class T> struct meta<detail::hostname_includer<T>>
	{
	...
	template <class T> struct from<JSON, detail::hostname_includer<T>>
	{
	...
	template <class T> struct to<JSON, detail::hostname_includer<T>>
	{
		template <auto Opts> static void op(auto&&...) noexcept
		{
		}
	};
}
