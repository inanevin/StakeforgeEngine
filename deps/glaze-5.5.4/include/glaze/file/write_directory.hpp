...
namespace glz
{
	[[nodiscard]] inline error_ctx buffers_to_directory(const std::unordered_map<std::filesystem::path, std::string>& buffers, const sv directory)
	{
	...
	template <auto Opts = opts{}, writable_map_t T> [[nodiscard]] error_ctx write_directory(T&& value, const sv directory_path)
	{
		namespace fs = std::filesystem;
