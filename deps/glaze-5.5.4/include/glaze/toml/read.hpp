...
namespace glz
{
	template <> struct parse<TOML>
	{
		...
		{
			from<TOML, std::decay_t<T>>::template op<Opts>(std::forward<T>(value), std::forward<Ctx>(ctx), std::forward<It0>(it), std::forward<It1>(end));
		}
	};

	// Skip whitespace and comments
	template <class It, class End> GLZ_ALWAYS_INLINE void skip_ws_and_comments(It&& it, End&& end) noexcept
	{
		while (it != end)
		{
			if (*it == ' ' || *it == '\t')
	...
	// Skip to next line
	template <class Ctx, class It, class End> GLZ_ALWAYS_INLINE bool skip_to_next_line(Ctx&, It&& it, End&& end) noexcept
	{
		while (it != end && *it != '\n' && *it != '\r')
		{
			++it;
	...
	template <class Ctx, class It, class End> GLZ_ALWAYS_INLINE bool parse_toml_key(std::string& key, Ctx& ctx, It&& it, End&& end) noexcept
	{
		key.clear();
	...
	template <glaze_value_t T> struct from<TOML, T>
	{
		template <auto Opts, is_context Ctx, class It0, class It1> static void op(auto&& value, Ctx&& ctx, It0&& it, It1&& end)
		{
			using V = decltype(get_member(std::declval<T>(), meta_wrapper_v<T>));
			from<TOML, V>::template op<Opts>(get_member(value, meta_wrapper_v<T>), std::forward<Ctx>(ctx), std::forward<It0>(it), std::forward<It1>(end));
		}
	};

	template <num_t T> struct from<TOML, T>
	{
		template <auto Opts, class It> static void op(auto&& value, is_context auto&& ctx, It&& it, auto&& end) noexcept
		{
	...
	template <string_t T> struct from<TOML, T>
	{
		template <auto Opts, class It> static void op(auto&& value, is_context auto&& ctx, It&& it, auto&& end)
		{
			if (bool(ctx.error)) [[unlikely]]
			{
				return;
			...
			else if (*it == '"')
			{
				...
				while (it != end && *it != '"')
				{
					...
					{ // Newlines not allowed in single-line basic strings
						ctx.error = error_code::syntax_error;
						return;
					}
					else
			...
			else if (*it == '\'')
			{
				...
				{
					if (*it == '\n' || *it == '\r')
					{ // Newlines not allowed in single-line literal strings
						ctx.error = error_code::syntax_error;
						return;
					}
					value.push_back(*it);
	...
	template <bool_t T> struct from<TOML, T>
	{
		template <auto Opts, class It> static void op(auto&& value, is_context auto&& ctx, It&& it, auto&& end) noexcept
		{
			if (bool(ctx.error)) [[unlikely]]
	...
	template <readable_array_t T> struct from<TOML, T>
	{
		template <auto Opts, class It> static void op(auto&& value, is_context auto&& ctx, It&& it, auto&& end)
		{
			if (bool(ctx.error)) [[unlikely]]
	...
	namespace detail
	{
		template <auto Opts, class T, class It, class End, class Ctx> GLZ_ALWAYS_INLINE void parse_toml_object_members(T&& value, It&& it, End&& end, Ctx&& ctx, bool is_inline_table)
		{
			...
			while (it != end)
			{
				...
				{
					if (is_inline_table)
						ctx.error = error_code::unexpected_end; // Inline table must end with '}'
					break;
				}
				...
				const auto index = decode_hash_with_size<TOML, std::decay_t<T>, HashInfo, HashInfo.type>::op(key_str.data(), key_str.data() + key_str.size(), key_str.size());
				...
				if (is_inline_table)
				{
					...
					else if (*it == ',')
					{
						...
						skip_ws_and_comments(it, end);
						if (it != end && *it == '}')
						{	// Trailing comma case like { key = "val", }
				...
				else
				{
					...
					{	// If not a comment, it's an error unless it's EOF
	...
	struct from<TOML, T>
	{
		template <auto Opts, class It> static void op(auto&& value, is_context auto&& ctx, It&& it, auto&& end)
		{
			skip_ws_and_comments(it, end);
			if (it == end)
			{ // Empty input for an object is valid (empty object)
				return;
			}
	...
	template <read_supported<TOML> T, class Buffer> [[nodiscard]] inline auto read_toml(T&& value, Buffer&& buffer)
	{
		return read<opts{.format = TOML}>(value, std::forward<Buffer>(buffer));
	}

	template <read_supported<TOML> T, class Buffer> [[nodiscard]] inline auto read_toml(Buffer&& buffer)
	{
		T value{};
	...
	template <read_supported<TOML> T, is_buffer Buffer> [[nodiscard]] inline error_ctx read_file_toml(T& value, const sv file_name, Buffer&& buffer)
	{
		context ctx{};
