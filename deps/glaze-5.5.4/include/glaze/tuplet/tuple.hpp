...
namespace glz
{
	...
	template <class... T> struct tuple : tuplet::tuple_base_t<T...>
	{
	...
	private:
		template <class U, class... B1, class... B2> constexpr void eq_impl(U&& u, tuplet::type_list<B1...>, tuplet::type_list<B2...>)
		{
			// See:
		...
		template <class... U, class... B> constexpr void assign_impl(tuplet::type_list<B...>, U&&... u)
		{
			(void(B::value = static_cast<U&&>(u)), ...);
		}

		template <class F, class... B> constexpr void for_each_impl(tuplet::type_list<B...>, F&& func) &
		{
			(void(func(B::value)), ...);
		}
		template <class F, class... B> constexpr void for_each_impl(tuplet::type_list<B...>, F&& func) const&
		{
			(void(func(B::value)), ...);
		}
		template <class F, class... B> constexpr void for_each_impl(tuplet::type_list<B...>, F&& func) &&
		...
		template <class F, class... B> constexpr bool any_impl(tuplet::type_list<B...>, F&& func) &
		{
			return (bool(func(B::value)) || ...);
		}
		template <class F, class... B> constexpr bool any_impl(tuplet::type_list<B...>, F&& func) const&
		{
			return (bool(func(B::value)) || ...);
		}
		template <class F, class... B> constexpr bool any_impl(tuplet::type_list<B...>, F&& func) &&
		...
		template <class F, class... B> constexpr bool all_impl(tuplet::type_list<B...>, F&& func) &
		{
			return (bool(func(B::value)) && ...);
		}
		template <class F, class... B> constexpr bool all_impl(tuplet::type_list<B...>, F&& func) const&
		{
			return (bool(func(B::value)) && ...);
		}
		template <class F, class... B> constexpr bool all_impl(tuplet::type_list<B...>, F&& func) &&
	...
	template <> struct tuple<> : tuplet::tuple_base_t<>
	{
	...
	namespace tuplet
	{
		...
		template <class Tuple> struct convert final
		{
		...
		private:
			template <class U, class... Bases> constexpr U convert_impl(type_list<Bases...>)
			{
				return U{static_cast<Tuple&&>(tuple).identity_t<Bases>::value...};
			}
		};
		template <class Tuple> convert(Tuple&) -> convert<Tuple&>;
		template <class Tuple> convert(const Tuple&) -> convert<const Tuple&>;
		template <class Tuple> convert(Tuple&&) -> convert<Tuple>;
	} // namespace tuplet
	...
	template <size_t I, tuplet::indexable Tup> GLZ_ALWAYS_INLINE constexpr decltype(auto) get(Tup&& tup) noexcept
	{
		return static_cast<Tup&&>(tup)[tuplet::tag<I>()];
	}

	template <class... T> constexpr tuple<T&...> tie(T&... t)
	{
		return {t...};
	}

	template <class F, tuplet::base_list_tuple Tup> constexpr decltype(auto) apply(F&& func, Tup&& tup)
	{
		return tuplet::apply_impl(static_cast<F&&>(func), static_cast<Tup&&>(tup), typename std::decay_t<Tup>::base_list());
	}
	...
	namespace tuplet
	{
		template <base_list_tuple... T> constexpr auto tuple_cat(T&&... ts)
		{
...
#define TUPLET_CAT_BY_FORWARDING_TUPLE 1
		...
		template <typename... T> constexpr auto forward_as_tuple(T&&... a) noexcept
		{
...
namespace glz
{
	...
	template <class T> constexpr size_t tuple_size_v = tuple_size<std::remove_const_t<T>>::value;

	template <class T, size_t N> struct tuple_size<std::array<T, N>>
	{
	...
	template <size_t I, class Tuple> using tuple_element_t = typename tuple_element<I, Tuple>::type;

	template <size_t I, class... T> struct tuple_element<I, std::tuple<T...>>
	{
		using type = typename std::tuple_element<I, std::tuple<T...>>::type;
	};

	template <std::size_t I, typename T1, typename T2> struct tuple_element<I, std::pair<T1, T2>>
	{
		using type = typename std::conditional<I == 0, T1, T2>::type;
	};

	template <class... T> struct tuple_size<glz::tuple<T...>> : std::integral_constant<size_t, sizeof...(T)>
	{
	};

	template <size_t I, class... T> struct tuple_element<I, glz::tuple<T...>>
	{
