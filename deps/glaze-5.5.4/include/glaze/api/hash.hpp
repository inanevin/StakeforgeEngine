...
namespace glz
{
	template <class T, T Value> consteval auto make_array()
	{
		return []<size_t... I>(std::index_sequence<I...>) { return std::array<char, sizeof(T)>{static_cast<char>(((Value >> (CHAR_BIT * I)) & 0xff))...}; }(std::make_index_sequence<sizeof(T)>{});
	}
	...
	namespace detail
	{
		...
		template <uint64_t x> struct numeric_string
		{
			...
			static constexpr const std::string_view get()
			{
				return {value.data, num_digits(x)};
			}
		};
	}

	template <class T, T Value> struct int_to_sv
	{
	...
	template <class T, T Value> inline constexpr std::string_view int_to_sv_v = int_to_sv<T, Value>{}.get();

	template <uint64_t I> inline consteval auto to_sv()
	{
		return detail::numeric_string<I>::get();
	}

	template <size_t I> struct hash128_i
	{
	...
	template <size_t I> inline constexpr std::string_view hash128_i_v = hash128_i<I>::value;

	template <const std::string_view& Str> struct hash128
	{
		static constexpr sv h0	  = int_to_sv_v<uint64_t, xxh64::hash(Str.data(), Str.size(), 0)>;
		static constexpr sv h1	  = int_to_sv_v<uint64_t, xxh64::hash(Str.data(), Str.size(), 1)>;
		static constexpr sv value = join_v<h0, h1>;
	...
	template <const std::string_view& Str> constexpr std::string_view hash128_v = hash128<Str>::value;
}
