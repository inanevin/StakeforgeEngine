...
namespace glz
{
	template <class UserType> struct impl : api
	{
		UserType user{};
		...
		template <class T> using ref_t = typename std::conditional_t<!std::is_const_v<std::remove_reference_t<T>> && std::is_lvalue_reference_v<T>, std::add_lvalue_reference_t<std::decay_t<T>>, std::add_rvalue_reference_t<std::decay_t<T>>>;

		template <class T> ref_t<T> to_ref(void* t)
		{
			using V = ref_t<T>;
								...
								if constexpr (std::same_as<P, Parent>)
								{
									...
									if (h == type_hash) [[likely]]
									{
										...
										else if constexpr (std::is_lvalue_reference_v<Ret>)
										{
											...
											ret = const_cast<std::decay_t<Ret>*>(&call_args<Tuple>(std::mem_fn(val), parent, args, std::make_index_sequence<N>{}));
										}
										else
										{
	...
	protected:
		template <class T> decltype(auto) unwrap(T&& val)
		{
		...
		template <class T> std::pair<void*, hash_t> get_void(T&& root_value, const sv json_ptr)
		{
			void* result{};
		...
		template <class T> auto get_void_fn(T& root_value, const sv json_ptr, const glz::hash_t type_hash)
		{
						...
						[&](auto&& val) {
							...
							if constexpr (std::is_member_function_pointer_v<V>)
							{
								...
								if constexpr (std::same_as<P, Parent>)
								{
									...
									{
										auto* f = new F{};
										*f		= [=, parent = &parent](auto&&... args) -> decltype(auto) { return ((*parent).*(val))(std::forward<decltype(args)>(args)...); };
										result	= std::unique_ptr<void, void (*)(void*)>{f, [](void* ptr) { delete static_cast<F*>(ptr); }};
									}
									else [[unlikely]]
									{
										error = "mismatching types";
	...
	template <class T> inline constexpr auto make_api()
	{
		return std::shared_ptr<impl<T>>{new impl<T>{}, [](impl<T>* ptr) { delete ptr; }};
	}

	template <class... Args> iface_fn make_iface()
	{
		return [] {
