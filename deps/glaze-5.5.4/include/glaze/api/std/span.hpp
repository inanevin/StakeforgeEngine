...
namespace glz
{
	template <class, template <class, size_t> class> inline constexpr bool							is_span_v						 = false;
	template <template <class, size_t> class T, class Element, size_t Extent> inline constexpr bool is_span_v<T<Element, Extent>, T> = true;

	template <class T>
	concept span = is_span_v<T, std::span>;

	template <span T> struct meta<T>
	{
