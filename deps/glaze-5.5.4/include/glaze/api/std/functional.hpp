...
namespace glz
{
	namespace detail
	{
		template <const std::string_view& Str, class Tuple, size_t I = 0> struct expander
		{
		...
		template <const std::string_view& Str, class Tuple> constexpr std::string_view expander_v = expander<Str, Tuple>::value;
	}

	template <class T>
	concept function = is_specialization_v<T, std::function>;

	template <function T> struct meta<T>
	{
		static constexpr auto impl() noexcept
		{
			...
			{
				return join_v<chars<"std::function<">, name_v<R>, chars<"(">, detail::expander_v<chars<"">, typename fun::arguments>, chars<")>">>;
			}
		}
