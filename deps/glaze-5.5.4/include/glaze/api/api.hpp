...
namespace glz
{
	...
	{
		template <class R> using func_return_t = std::conditional_t<std::is_lvalue_reference_v<R>, std::reference_wrapper<std::decay_t<R>>, R>;
		...
		struct api
		{
			...
			template <class T> [[nodiscard]] T* get(const sv path) noexcept;

			// Get a std::function from a member function or std::function across the API
			template <class T> [[nodiscard]] expected<T, error_code> get_fn(const sv path) noexcept;

			template <class Ret, class... Args> expected<func_return_t<Ret>, error_code> call(const sv path, Args&&... args) noexcept;
		...
		template <class T> T* api::get(const sv path) noexcept
		{
		...
		template <class T> glz::expected<T, error_code> api::get_fn(const sv path) noexcept
		{
			static constexpr auto hash = glz::hash<T>();
		...
		template <class Ret, class... Args> glz::expected<func_return_t<Ret>, error_code> api::call(const sv path, Args&&... args) noexcept
		{
