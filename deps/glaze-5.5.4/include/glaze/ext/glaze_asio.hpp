...
namespace glz
{
	namespace repe
	{
		...
		template <has_size ErrorMessage> inline void encode_error(const error_code& ec, message& msg, ErrorMessage&& error_message)
		{
			msg.header.ec = ec;
		...
		template <auto Opts = opts{}, class T> inline std::optional<std::string> decode_message(T&& value, message& msg)
		{
	...
	inline asio::awaitable<void> co_receive_buffer(asio::ip::tcp::socket& socket, repe::message& msg)
	{
		...
		co_await asio::async_read(socket, asio::buffer(&msg.header, sizeof(msg.header)), asio::transfer_exactly(sizeof(msg.header)), asio::use_awaitable);
		...
		msg.query.resize(msg.header.query_length);
		co_await asio::async_read(socket, asio::buffer(msg.query), asio::transfer_exactly(msg.header.query_length), asio::use_awaitable);
		...
		msg.body.resize(msg.header.body_length);
		co_await asio::async_read(socket, asio::buffer(msg.body), asio::transfer_exactly(msg.header.body_length), asio::use_awaitable);
	}
	...
	struct unique_socket
	{
		...
		std::shared_ptr<asio::ip::tcp::socket> value()
		{
			return ptr;
		}

		const std::shared_ptr<asio::ip::tcp::socket> value() const
		{
			return ptr;
		}
		...
		const asio::ip::tcp::socket& operator*() const
		{
			return *ptr;
		}

		unique_socket(socket_pool* input_pool) : pool(input_pool)
		{
			std::tie(ptr, index, ec) = pool->get();
		}
	...
	template <auto Opts = opts{}> struct asio_client
	{
		std::string host{"localhost"}; // host name
		std::string service{""};	   // often the port
		...
		template <class Header = repe::user_header, class... Params> void call(Header&& header, repe::message& response, Params&&... params)
		{
		...
		template <class... Params> void set(const std::string_view query, Params&&... params)
		{
			if (not connected())
			{
		...
		template <class Output> void get(const std::string_view query, Output& output)
		{
			if (not connected())
			{
				throw std::runtime_error("asio_client: NOT CONNECTED");
		...
		template <class Output> auto get(const std::string_view query)
		{
			Output out{};
			get(query, out);
			return out;
		...
		template <class Input, class Output> void inout(const std::string_view query, Input&& input, Output& output)
		{
			if (not connected())
			{
				throw std::runtime_error("asio_client: NOT CONNECTED");
	...
	template <auto Opts = opts{}> struct asio_server
	{
