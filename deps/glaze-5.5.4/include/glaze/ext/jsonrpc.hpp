...
namespace glz::rpc
{
	...
	struct error final
	{
		...
		{
			return {error_e::invalid_request, "Invalid version: " + std::string(presumed_version) + " only supported version is " + std::string(rpc::supported_version), std::string(code_as_sv(error_e::invalid_request))};
		}
	...
	template <class Params> struct request_t
	{
		id_t			 id{};
	...
	template <class Params> request_t(id_t&&, std::string_view, Params&&) -> request_t<std::decay_t<Params>>;

	using generic_request_t = request_t<glz::raw_json_view>;

	template <class Result> struct response_t
	{
	...
	template <string_literal Name, class Params, class Result> struct method
	{
	...
	namespace concepts
	{
		...
		concept call_return_type = requires { requires glz::is_any_of<call_return_t, std::string, std::vector<response_t<glz::raw_json>>>; };
	}

	template <concepts::method_type Method> struct server_method_t
	{
		...
		std::function<expected<result_t, rpc::error>(const params_t&)> callback{[](const auto&) { return glz::unexpected{rpc::error{rpc::error_e::internal, "Not implemented"}}; }};
	};

	template <concepts::method_type Method> struct client_method_t
	{
		static constexpr std::string_view name_v = Method::name_v;
		using params_t							 = typename Method::params_t;
		using result_t							 = typename Method::result_t;
	...
	namespace detail
	{
		template <string_literal name, class... Method> inline constexpr void set_callback(glz::tuple<Method...>& methods, const auto& callback)
		{
		...
		template <class T, T value, class... Ts> struct index_of_name;

		template <class T, T value, class... Ts> struct index_of_name<T, value, T, Ts...>
		{
			static constexpr std::size_t index = 0;
		};

		template <class T, T value, class U, class... Ts> struct index_of_name<T, value, U, Ts...>
		{
			static constexpr std::size_t index = (value == U::name_v) ? 0 : 1 + index_of_name<T, value, Ts...>::index;
		};

		template <class T, T value> struct index_of_name<T, value>
		{
			static constexpr std::size_t index = (std::numeric_limits<size_t>::max)();
		};

		template <class Map, string_literal Name, class... Method> auto get_request_map(glz::tuple<Method...>& methods) -> Map&
		{
			constexpr bool method_found = ((Method::name_v == Name) || ...);
	...
	template <concepts::method_type... Method> struct server
	{
		using raw_response_t = response_t<glz::raw_json>;

		glz::tuple<server_method_t<Method>...> methods{};

		template <string_literal name> constexpr void on(const auto& callback) // std::function<expected<result_t, rpc::error>(params_t const&)>
		{
		...
		template <concepts::call_return_type return_t = std::string> return_t call(std::string_view json_request)
		{
			...
			{
				return return_helper(raw_response_t{rpc::error{error_e::parse_error, format_error(parse_err, json_request)}});
			}
	...
	template <concepts::method_type... Method> struct client
	{
		...
		rpc::error call(std::string_view json_response)
		{
			...
			bool	   id_found = methods.any([&json_response, &res, &return_v](auto&& method) -> bool {
				  ...
				  if (auto it{method.pending_requests.find(res.id)}; it != std::end(method.pending_requests))
				  {
					  ...
					  expected<typename meth_t::response_t, glz::error_ctx> response{glz::read_json<typename meth_t::response_t>(json_response)};
		...
		template <string_literal Name> [[nodiscard]] std::pair<std::string, bool> request(id_t&& id, auto&& params, auto&& callback)
		{
			constexpr bool method_found = ((Method::name_v == Name) || ...);
			static_assert(method_found, "Method not declared in client.");
			...
			constexpr bool method_params_match = ((Method::name_v == Name && std::is_same_v<typename Method::params_t, Params>) || ...);
		...
		template <string_literal Name> [[nodiscard]] auto notify(auto&& params) -> std::string
		{
			auto placebo{[](auto&, auto&) {}};
			return request<Name>(glz::json_t::null_t{}, params, std::move(placebo)).first;
		}

		template <string_literal Name> [[nodiscard]] const auto& get_request_map() const
		{
		...
		template <string_literal Name> [[nodiscard]] auto& get_request_map()
		{
			constexpr auto idx	 = detail::index_of_name<decltype(Name), Name, Method...>::index;
			using method_element = glz::tuple_element_t<idx, std::tuple<client_method_t<Method>...>>;
			using request_map_t	 = decltype(method_element().pending_requests);
