...
struct my_functions
{
	...
	{
		std::printf("const Member Function!\n");
	}
...
template <> struct glz::meta<my_functions>
{
	using T						= my_functions;
	static constexpr auto value = object("hi", &T::hello, &T::member_function, &T::const_member_function, &T::dump, &T::input);
};

struct four_t
{
	four_t(glz::make_reflectable)
	{
	} // required for reflection since this struct has no members
