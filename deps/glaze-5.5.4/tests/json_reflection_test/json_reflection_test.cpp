...
template <> struct glz::meta<test_skip>
{
	static constexpr bool skip(const std::string_view, const meta_context&)
	{
		return true;
	}
};
...
namespace glz
{
	template <> struct from<JSON, std::chrono::seconds>
	{
		template <auto Opts> static void op(std::chrono::seconds& value, is_context auto&& ctx, auto&&... args)
		{
			int32_t sec_count{};
...
template <> struct glz::meta<custom_state>
{
...
template <> struct glz::meta<some_enum>
{
...
template <> struct glz::meta<renamed_t>
{
...
template <> struct glz::meta<suffixed_keys_t>
{
