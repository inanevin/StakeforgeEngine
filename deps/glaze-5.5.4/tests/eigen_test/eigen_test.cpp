...
void initialize_complex_struct()
{
	...
	complex_test_value.mcd << std::complex<double>(1, 2), std::complex<double>(3, 4), std::complex<double>(5, 6), std::complex<double>(7, 8);
}
...
suite additional_eigen_tests = [] {
	...
	"write_beve_complex_matrix"_test = [] {
		...
		m << std::complex<double>(1, 1), std::complex<double>(2, 2), std::complex<double>(3, 3), std::complex<double>(4, 4);
		std::string b;
		expect(not glz::write_beve(m, b));
		Eigen::MatrixXcd e;
