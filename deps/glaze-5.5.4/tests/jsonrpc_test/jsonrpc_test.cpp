...
ut::suite valid_vector_test_cases_server = [] {
	...
	const std::array valid_requests = {std::pair(R"({"jsonrpc": "2.0","method": "add", "params": [1, 2, 3],"id": 1})", R"({"jsonrpc": "2.0","result": 6,"id": 1})"),
									   std::pair(
										   // No id is valid
										   R"({"jsonrpc": "2.0","method": "add", "params": [1, 2, 3]})",
										   ""),
									   std::pair(R"({"jsonrpc": "2.0","method": "add", "params": [1, 2, 3],"id": null})", ""),
									   std::pair(R"({"jsonrpc": "2.0","method": "add", "params": [1, 2, 3],"id": 2})", R"({"jsonrpc": "2.0","result": 6, "id": 2})"),
									   std::pair(R"({"jsonrpc": "2.0","method": "add","params": [1, 2, 3],"id": "some_client_22"})", R"({"jsonrpc": "2.0","result": 6, "id": "some_client_22"})")};
	std::string		 raw_json;
	...
	for (const auto& pair : valid_requests)
	{
		...
		ut::test(test_name) = [&server, &raw_json, stripped]() {
			...
			if (stripped->empty())
			{ // if no id is supplied expected response should be none
...
ut::suite vector_test_cases = [] {
	...
	ut::test("sum_result = 6") = [&server, &client] {
		bool called{};
		auto request_str = client.request<"summer">(1, std::vector{1, 2, 3}, [&called](glz::expected<int, rpc::error> value, rpc::id_t id) -> void {
			called = true;
...
struct foo_result
{
	bool		foo_c{};
	std::string foo_d{};
	bool		operator==(const foo_result& rhs) const noexcept
	{
		return foo_c == rhs.foo_c && foo_d == rhs.foo_d;
	}
};
...
struct bar_result
{
	bool		bar_c{};
	std::string bar_d{};
	bool		operator==(const bar_result& rhs) const noexcept
	{
		return bar_c == rhs.bar_c && bar_d == rhs.bar_d;
	}
};
...
ut::suite struct_test_cases = [] {
	...
	ut::test("valid foo request") = [&server, &client] {
		bool called{};
		auto request_str{client.request<"foo">("42", foo_params{.foo_a = 1337, .foo_b = "hello world"}, [&called](glz::expected<foo_result, rpc::error> value, rpc::id_t id) -> void {
			called = true;
			ut::expect(value.has_value());
		...
		ut::expect(request_str.first == R"({"jsonrpc":"2.0","method":"foo","params":{"foo_a":1337,"foo_b":"hello world"},"id":"42"})");
	...
	ut::test("valid bar request") = [&server, &client] {
		bool called{};
		auto request_str{client.request<"bar">("bar-uuid", bar_params{.bar_a = 1337, .bar_b = "hello world"}, [&called](const glz::expected<bar_result, rpc::error>& value, const rpc::id_t& id) -> void {
			called = true;
			ut::expect(value.has_value());
		...
		ut::expect(request_str.first == R"({"jsonrpc":"2.0","method":"bar","params":{"bar_a":1337,"bar_b":"hello world"},"id":"bar-uuid"})");
	...
	ut::test("foo request error") = [&server, &client] {
		...
		std::string response = server.call(request_str.first);
		ut::expect(response == R"({"jsonrpc":"2.0","error":{"code":-32000,"message":"Server error","data":"my error"},"id":"42"})");
	...
	ut::test("server invalid version error") = [&server] {
		...
		auto response_vec = server.call<std::vector<rpc::response_t<glz::raw_json>>>(R"({"jsonrpc":"42.0","method":"foo","params":{},"id":"uuid"})");
		ut::expect(response_vec.size() == 1);
		ut::expect(glz::write_json(response_vec) == R"([{"jsonrpc":"2.0","error":{"code":-32600,"message":"Invalid request","data":"Invalid version: 42.0 only supported version is 2.0"},"id":"uuid"}])");
	...
	ut::test("server method not found") = [&server] {
		...
		// invalid method name
		auto response_vec = server.call<std::vector<rpc::response_t<glz::raw_json>>>(R"({"jsonrpc":"2.0","method":"invalid_method_name","params":{},"id":"uuid"})");
		ut::expect(response_vec.size() == 1);
		ut::expect(glz::write_json(response_vec) == R"([{"jsonrpc":"2.0","error":{"code":-32601,"message":"Method not found","data":"Method: 'invalid_method_name' not found"},"id":"uuid"}])");
		ut::expect(response_vec.at(0).error.has_value());
	...
	ut::test("server invalid json") = [&server] {
		...
		ut::expect(s == R"([{"jsonrpc":"2.0","error":{"code":-32700,"message":"Parse error","data":"1:66: expected_colon\n..._method_name\",\"params\":{},\"id:\"uuid\"}\"\n                                  ^"},"id":null}])") << s;
		ut::expect(response_vec.at(0).error.has_value());
	...
	ut::test("server invalid json batch") = [&server] {
		...
		auto response_vec = server.call<std::vector<rpc::response_t<glz::raw_json>>>(R"([{"jsonrpc":"2.0","method":"invalid_method_name","params":{},"id":"uuid"},{"jsonrpc":"2.0","method":"invalid_method_name","params":]")");
		ut::expect(response_vec.size() == 1);
		auto s = glz::write_json(response_vec).value_or("error");
		ut::expect(s == R"([{"jsonrpc":"2.0","error":{"code":-32700,"message":"Parse error","data":"1:132: syntax_error\n...\"invalid_method_name\",\"params\":]\"\n                                  ^"},"id":null}])") << s;
		ut::expect(response_vec.at(0).error.has_value());
		ut::expect(response_vec.at(0).error->code == rpc::error_e::parse_error);
	};
	...
	ut::test("server invalid json illformed batch one item") = [&server] {
		...
		ut::expect(response_vec.size() == 1);
		ut::expect(glz::write_json(response_vec) == R"([{"jsonrpc":"2.0","error":{"code":-32600,"message":"Invalid request","data":"1:1: expected_brace\n   1\n   ^"},"id":null}])");
		ut::expect(response_vec.at(0).error.has_value());
		ut::expect(response_vec.at(0).error->code == rpc::error_e::invalid_request);
	};
	...
	"client request map"_test = [&client] {
		...
		bool second_call{};
		std::ignore = client.request<"foo">("second_call", foo_params{}, [&second_call](auto, auto) { second_call = true; });
		bool third_call{};
	...
	"client request id needs to be unique"_test = [&client] {
		std::string id{"some id"};
		bool		first_called{};
		auto [unused, inserted] = client.request<"foo">(id, foo_params{}, [&first_called](auto, auto) { first_called = true; });
		ut::expect(inserted);
	...
	"client call erases id from queue"_test = [&client, &server] {
		...
		auto [request, inserted] = client.request<"foo">("next gen id", foo_params{}, [&call_cnt](auto, auto) { call_cnt++; });
