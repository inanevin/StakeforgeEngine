...
template <> struct glz::meta<MetaStruct>
{
	using T						= MetaStruct;
...
template <> struct glz::meta<VarA>
{
	using T						= VarA;
	static constexpr auto value = object("x", &T::x);
};

template <> struct glz::meta<VarB>
{
	using T						= VarB;
...
template <> struct glz::meta<VariantHolder>
{
...
suite pointer_access = [] {
	"json_pointer_get_set"_test = [] {
		...
		if (map_val)
			expect(map_val.value().get() == "value");
...
template <> struct glz::json_schema<SchemaDemo>
{
...
template <> struct glz::meta<UnknownFields>
{
