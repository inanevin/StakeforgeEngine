...
{
	// State
	std::unordered_map<int, User> users	  = {{1, {1, "Alice", "alice@example.com"}}, {2, {2, "Bob", "bob@example.com"}}, {3, {3, "Charlie", "charlie@example.com"}}};
	int							  next_id = 4;
...
template <> struct glz::meta<UserService>
{
	using T						= UserService;
...
int main()
{
	"openapi_test"_test = [] {
		...
		server.get("/health", [](const glz::request&, glz::response& res) { res.content_type("application/json").body(R"({"status": "healthy", "timestamp": "2025-01-01T00:00:00Z"})"); });

		server.get("/version", [](const glz::request&, glz::response& res) { res.content_type("application/json").body(R"({"version": "1.0.0", "service": "user-management", "build": "dev"})"); });

		// Add some custom PUT endpoints
		server.put("/settings", [](const glz::request&, glz::response& res) { res.content_type("application/json").body(R"({"message": "Settings updated successfully"})"); });

		server.put("/config/database", [](const glz::request&, glz::response& res) { res.content_type("application/json").body(R"({"message": "Database configuration updated", "applied": true})"); });

		// Add a custom POST endpoint as well
		server.post("/auth/login", [](const glz::request&, glz::response& res) { res.content_type("application/json").body(R"({"token": "abc123", "expires_in": 3600, "user_id": 1})"); });
