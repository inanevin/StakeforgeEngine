...
private:
	...
	static X509* create_certificate(EVP_PKEY* pkey, const std::string& subject, int days = 365)
	{
		...
		if (subject == "localhost")
		{
			...
			X509_EXTENSION* ext = X509V3_EXT_conf_nid(nullptr, &ctx, NID_subject_alt_name, const_cast<char*>("DNS:localhost,DNS:*.localhost,IP:127.0.0.1,IP:::1"));
			if (ext)
			{
...
public:
	static bool generate_test_certificates()
	{
		...
		std::unique_ptr<X509, decltype(&X509_free)> server_cert(create_certificate(server_key.get(), "localhost", 365), X509_free);
		...
		bool cert_written = write_pem_file("test_cert.pem", [&](BIO* bio) { return PEM_write_bio_X509(bio, server_cert.get()); });
		...
		bool key_written = write_pem_file("test_key.pem", [&](BIO* bio) { return PEM_write_bio_PrivateKey(bio, server_key.get(), nullptr, nullptr, 0, nullptr, nullptr); });
		...
		std::unique_ptr<X509, decltype(&X509_free)> client_cert(create_certificate(client_key.get(), "test-client", 365), X509_free);
		...
		bool client_cert_written = write_pem_file("test_client_cert.pem", [&](BIO* bio) { return PEM_write_bio_X509(bio, client_cert.get()); });

		// Write client private key
		bool client_key_written = write_pem_file("test_client_key.pem", [&](BIO* bio) { return PEM_write_bio_PrivateKey(bio, client_key.get(), nullptr, nullptr, 0, nullptr, nullptr); });
...
// Windows doesn't have chmod, but we can try to set file attributes
	...
	static bool verify_certificate_files()
	{
		...
		return cert_content.find("BEGIN CERTIFICATE") != std::string::npos && cert_content.find("END CERTIFICATE") != std::string::npos && key_content.find("BEGIN") != std::string::npos && key_content.find("END") != std::string::npos;
	}
};
...
bool should_suppress_error(const std::error_code& ec)
{
	...
	if (ec == asio::error::operation_aborted)
		return true; // Operation aborted (shutdown)
...
public:
	HTTPSTestServer() : start_time_(std::chrono::steady_clock::now()), next_user_id_(1)
	{
		...
		users_		  = {{1, "Alice Johnson", "alice@test.com"}, {2, "Bob Smith", "bob@test.com"}, {3, "Charlie Brown", "charlie@test.com"}};
		next_user_id_ = 4;
...
private:
	void setup_routes()
	{
		...
		server_.post("/echo", [](const glz::request& req, glz::response& res) {
			...
			echo_res.timestamp			= std::to_string(std::chrono::duration_cast<std::chrono::seconds>(std::chrono::system_clock::now().time_since_epoch()).count());
		...
		server_.get("/users/{id}", [this](const glz::request& req, glz::response& res) {
			...
			auto user_it = std::find_if(users_.begin(), users_.end(), [user_id](const TestUser& u) { return u.id == user_id; });
...
{
	return file_exists("test_cert.pem") && file_exists("test_key.pem");
}
...
suite certificate_tests = [] {
	"certificate_generation"_test = [] {
		...
		{
			expect(CertificateGenerator::generate_test_certificates()) << "Should generate SSL certificates programmatically\n";
		}
	...
	"certificate_content_valid"_test = [] {
		...
		expect(CertificateGenerator::verify_certificate_files()) << "Generated certificates should have valid PEM format\n";
	};
};
