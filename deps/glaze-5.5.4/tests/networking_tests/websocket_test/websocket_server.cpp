...
int main()
{
	...
	ws_server->on_open([&clients, &clients_mutex](std::shared_ptr<websocket_connection> conn, const request&) {
		...
		std::cout << "🔗 WebSocket opened: " << conn->remote_address() << " (Total clients: " << clients.size() << ")" << std::endl;
	...
	ws_server->on_message([&clients, &clients_mutex](std::shared_ptr<websocket_connection> conn, std::string_view message, ws_opcode opcode) {
	...
	ws_server->on_close([&clients, &clients_mutex](std::shared_ptr<websocket_connection> conn) {
		...
		std::cout << "❌ WebSocket closed: " << conn->remote_address() << " (Remaining clients: " << clients.size() << ")" << std::endl;
	...
	ws_server->on_error([](std::shared_ptr<websocket_connection> conn, std::error_code ec) { std::cout << "🚨 WebSocket error for " << conn->remote_address() << ": " << ec.message() << std::endl; });
