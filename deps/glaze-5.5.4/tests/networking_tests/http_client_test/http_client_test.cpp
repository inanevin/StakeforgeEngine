...
private:
	...
	void setup_routes()
	{
		...
		server_.get("/hello", [](const request&, response& res) { res.status(200).content_type("text/plain").body("Hello, World!"); });

		server_.post("/echo", [](const request& req, response& res) { res.status(200).content_type("text/plain").body(req.body); });

		server_.get("/json", [](const request&, response& res) { res.status(200).content_type("application/json").body(R"({"message": "test", "value": 42})"); });
		...
		server_.stream_get("/slow-stream", [](request&, streaming_response& res) {
			...
			send_data = [conn, timer, counter, send_data](const std::error_code& ec) {
				...
				conn->send_chunk("chunk" + std::to_string((*counter)++) + ";", [conn, timer, send_data](std::error_code write_ec) {
...
private:
	...
	std::expected<response, std::error_code> perform_request(const std::string& method, const url_parts& url, const std::string& body)
	{
		...
		asio::post(io_context_, [this, method, url, body, &promise]() {
			try
			{
				...
				std::string response_body{std::istreambuf_iterator<char>(&response_buffer), std::istreambuf_iterator<char>()};
...
suite glz_http_client_tests = [] {
	"basic_streaming_get"_test = [] {
		...
		auto conn = client.stream_request({.url = server.base_url() + "/stream-test", .on_data = on_data, .on_error = on_error, .method = "GET", .on_connect = on_connect, .on_disconnect = on_disconnect});
	...
	"client_disconnects_stream"_test = [] {
		...
		auto conn = client.stream_request({.url = server.base_url() + "/slow-stream", .on_data = on_data, .on_error = on_error, .method = "GET", .on_connect = on_connect, .on_disconnect = on_disconnect});
		expect(conn != nullptr) << "Connection handle should not be null\n";
	...
	"streaming_request_with_http_error"_test = [] {
		...
		auto conn = client.stream_request({.url = server.base_url() + "/stream-error", .on_data = on_data, .on_error = on_error, .method = "GET", .on_connect = on_connect, .on_disconnect = on_disconnect});
		expect(conn != nullptr);
