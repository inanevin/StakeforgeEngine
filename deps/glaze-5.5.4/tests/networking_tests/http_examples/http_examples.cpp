...
public:
	TaskService()
	{
		tasks_	 = {{1, "Learn Glaze", "Study the Glaze HTTP library", false, "2024-01-01T10:00:00Z", "2024-01-15T00:00:00Z"}, {2, "Write documentation", "Create API documentation", false, "2024-01-02T10:00:00Z", "2024-01-20T00:00:00Z"}};
		next_id_ = 3;
	}
	...
	std::vector<Task> searchTasks(const TaskSearchRequest& request)
	{
		...
		for (const auto& task : tasks_)
		{
			...
			{
				matches = task.title.find(request.query) != std::string::npos || task.description.find(request.query) != std::string::npos;
			}
	...
	std::vector<Task> getCompletedTasks()
	{
		...
		std::copy_if(tasks_.begin(), tasks_.end(), std::back_inserter(completed), [](const Task& task) { return task.completed; });
...
public:
	...
	{
		if ((request.username == "admin" && request.password == "admin123") || (request.username == "user" && request.password == "user123"))
		{
...
public:
	ProductService()
	{
		products_ = {{1, "Laptop", "High-performance laptop", 999.99, "Electronics", true}, {2, "Mouse", "Wireless mouse", 29.99, "Electronics", true}, {3, "Keyboard", "Mechanical keyboard", 129.99, "Electronics", true}};
		next_id_  = 4;
	}
	...
	{
		auto it = std::find_if(products_.begin(), products_.end(), [id](const Product& p) { return p.id == id; });
	...
	std::vector<Product> getProductsByCategory(const std::string& category)
	{
		...
		std::copy_if(products_.begin(), products_.end(), std::back_inserter(result), [&category](const Product& p) { return p.category == category; });
		return result;
	}
...
public:
	HealthStatus get_health_status(const MetricsData& metrics)
	{
		...
		status.uptime_seconds = std::chrono::duration_cast<std::chrono::seconds>(now - metrics.start_time).count();
		...
		status.metrics.error_rate		 = total_requests > 0 ? (metrics.error_count.load() * 100 / total_requests) : 0;
		status.metrics.avg_response_time = total_requests > 0 ? (metrics.total_response_time.load() / total_requests) : 0.0;
...
suite basic_rest_api_tests = [] {
	"user_api_initialization"_test = [] {
		...
		expect(users[0].name == "Alice Johnson" || users[1].name == "Alice Johnson") << "Should contain Alice Johnson\n";
	};
...
suite auto_generated_rest_tests = [] {
	"task_service_initialization"_test = [] {
		...
		expect(tasks[0].title == "Learn Glaze" || tasks[1].title == "Learn Glaze") << "Should contain 'Learn Glaze' task\n";
	};
	...
	"task_creation_validation"_test = [] {
		...
		expect(throws([&] { service.createTask(empty_request); })) << "Should throw exception for empty title\n";
	};
	...
	"task_retrieval"_test = [] {
		...
		expect(throws([&] { service.getTaskById(999); })) << "Should throw exception for missing task\n";
	};
	...
	"task_search"_test = [] {
		...
		expect(results[0].title.find("Glaze") != std::string::npos || results[0].description.find("Glaze") != std::string::npos) << "Results should contain search term\n";
	};
...
suite websocket_chat_tests = [] {
	...
	"user_connection"_test = [] {
		...
		expect(history[0].message.find("Alice") != std::string::npos) << "Join message should contain username\n";
	};
...
suite microservice_tests = [] {
	...
	"product_retrieval"_test = [] {
		...
		expect(throws([&] { service.getProductById(999); })) << "Should throw exception for missing product\n";
	};
