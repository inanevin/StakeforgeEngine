...
suite http_router_constraints_tests = [] {
	...
	"email_validation_with_regex"_test = [] {
		glz::http_router									   router;
		std::unordered_map<std::string, glz::param_constraint> constraints;
		constraints["email"] = glz::param_constraint{.description = "valid email address", .validation = [](std::string_view value) {
	...
	"four_digit_code_validation"_test = [] {
		glz::http_router									   router;
		std::unordered_map<std::string, glz::param_constraint> constraints;
		constraints["code"] = glz::param_constraint{.description = "4-digit code", .validation = [](std::string_view value) {
														if (value.size() != 4)
															return false;
														for (char c : value)
														{
...
suite http_router_functionality_tests = [] {
	...
	"wildcard_route_matching"_test = [] {
		...
		glz::request  req_wildcard{.method = glz::http_method::GET, .target = "/files/documents/report.pdf", .params = params_wildcard};
		...
		expect(res_wildcard.response_body == "File Path: documents/report.pdf") << "Response body should contain captured path\n";
		...
		expect(params_minimal.find("path") != params_minimal.end()) << "Path parameter should be present for minimal path\n";
