...
public:
	std::future<AsyncResult> process_async(const std::string& input)
	{
		return std::async(std::launch::async, [this, input]() {
			...
			return AsyncResult{"Processed: " + input + " (op #" + std::to_string(op_id) + ")", static_cast<int>(duration.count()), true};
		});
	}
	...
	{
		return operation_counter_.load();
	}
};
...
suite http_router_async_tests = [] {
	...
	"async_route_with_parameters"_test = [] {
		...
		router.get_async("/users/:id/async", [&captured_id](const glz::request& req, glz::response& res) -> std::future<void> {
	...
	"multiple_async_methods"_test = [] {
		...
		router.get_async("/resource", [](const glz::request&, glz::response& res) -> std::future<void> { return std::async(std::launch::async, [&res]() { res.json({{"method", "GET"}, {"async", true}}); }); });

		router.post_async("/resource", [](const glz::request&, glz::response& res) -> std::future<void> { return std::async(std::launch::async, [&res]() { res.json({{"method", "POST"}, {"async", true}}); }); });

		router.put_async("/resource", [](const glz::request&, glz::response& res) -> std::future<void> { return std::async(std::launch::async, [&res]() { res.json({{"method", "PUT"}, {"async", true}}); }); });

		router.del_async("/resource", [](const glz::request&, glz::response& res) -> std::future<void> { return std::async(std::launch::async, [&res]() { res.json({{"method", "DELETE"}, {"async", true}}); }); });

		router.patch_async("/resource", [](const glz::request&, glz::response& res) -> std::future<void> { return std::async(std::launch::async, [&res]() { res.json({{"method", "PATCH"}, {"async", true}}); }); });
	...
	"generic_async_route_method"_test = [] {
		...
		router.route_async(glz::http_method::HEAD, "/resource", [](const glz::request&, glz::response& res) -> std::future<void> {
...
suite http_server_api_tests = [] {
	...
	"mount_subrouter"_test = [] {
		...
		api_router.get("/users", [&api_handler_registered](const glz::request&, glz::response&) { api_handler_registered = true; });
	...
	"error_handler_configuration"_test = [] {
		...
		auto& error_ref = server.on_error([&error_handler_called](std::error_code, std::source_location) { error_handler_called = true; });
...
suite async_route_execution_tests = [] {
	...
	"async_route_error_handling"_test = [] {
		...
		router.get_async("/error", [](const glz::request&, glz::response&) -> std::future<void> { return std::async([]() { throw std::runtime_error("Async operation failed"); }); });
...
suite concurrent_async_tests = [] {
	"multiple_concurrent_async_routes"_test = [] {
		...
		router.get_async("/concurrent/:id", [&completed_requests](const glz::request& req, glz::response& res) -> std::future<void> {
...
suite advanced_async_scenarios = [] {
	"async_route_with_middleware"_test = [] {
		...
		// Add async route
		router.get_async("/with-middleware", [&execution_order](const glz::request&, glz::response& res) -> std::future<void> {
	...
	"async_route_chaining_operations"_test = [] {
		...
		router.post_async("/chain", [&service](const glz::request& req, glz::response& res) -> std::future<void> {
			return std::async(std::launch::async, [&service, &req, &res]() {
				...
				res.json({{"first_operation", first_result.message}, {"second_operation", second_result.message}, {"total_operations", service.get_operation_count()}, {"chained", true}});
			});
		});
...
suite response_building_tests = [] {
	"response_method_chaining"_test = [] {
		...
		auto& chained_res = res.status(201).header("X-Custom", "value").content_type("application/json").body("test body");
