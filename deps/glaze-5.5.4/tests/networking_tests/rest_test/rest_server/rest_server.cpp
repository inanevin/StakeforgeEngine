...
struct UserService
{
	std::unordered_map<int, User> users	  = {{1, {1, "Alice Johnson", "alice@example.com", "👩‍💼"}}, {2, {2, "Bob Smith", "bob@example.com", "👨‍💻"}}, {3, {3, "Carol Davis", "carol@example.com", "👩‍🎨"}}};
	int							  next_id = 4;
...
struct PostService
{
	std::unordered_map<int, Post> posts	  = {{1, {1, "Welcome to Glaze", "This is a demo of Mithril with a Glaze C++ backend.", "Alice Johnson", "2025-05-27T10:00:00Z"}},
											 {2, {2, "Building REST APIs", "Learn how to build REST APIs with Glaze library.", "Bob Smith", "2025-05-27T11:00:00Z"}}};
	int							  next_id = 3;
...
{
	using T						= UserService;
	static constexpr auto value = object(&T::getAllUsers, &T::getUserById, &T::createUser, &T::updateUser, &T::deleteUser);
};

template <> struct glz::meta<PostService>
{
	using T						= PostService;
...
int main()
{
	...
	server.get("/test-cors", [](const glz::request& req, glz::response& res) {
		...
		res.json({{"message", "CORS test endpoint"}, {"origin", req.headers.count("Origin") ? req.headers.at("Origin") : "none"}, {"method", glz::to_string(req.method)}});
	});
