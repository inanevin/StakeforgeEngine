...
<body>
    ...
    <div class="content">
		...
		html_content blog_post{"My <Amazing> Blog Post", "This post discusses \"HTML\" & safety", "<p>This is <strong>formatted content</strong> that should render as HTML.</p>", ""};
...
suite mustache_container_iteration_tests = [] {
	"basic_container_iteration"_test = [] {
		...
		TodoList list{"My Tasks", {{"Task 1", false, "high", "work", 1, 0}, {"Task 2", true, "low", "personal", 2, 1}}, true, 2};
	...
	"container_with_multiple_properties"_test = [] {
		...
		TodoList list{"Tasks", {{"Buy milk", false, "normal", "shopping", 1, 0}, {"Call mom", true, "high", "personal", 2, 1}}, true, 2};
	...
	"container_with_nested_boolean_sections"_test = [] {
		...
		TodoList list{"Mixed Tasks", {{"Task 1", false, "high", "work", 1, 0}, {"Task 2", true, "low", "personal", 2, 1}, {"Task 3", false, "normal", "home", 3, 2}}, true, 3};
	...
	"mixed_boolean_and_container_sections"_test = [] {
		std::string_view layout = R"({{title}} {{#has_items}}({{total_count}} items): {{#items}}{{text}}{{#completed}} ✓{{/completed}} | {{/items}}{{/has_items}}{{^has_items}}No items yet{{/has_items}})";

		TodoList list_with_items{"Active List", {{"Task 1", true, "high", "work", 1, 0}, {"Task 2", false, "low", "personal", 2, 1}}, true, 2};
...
<div class="todo-list" data-component-id="{{component_id}}">
  ...
  {{/has_items}}
		...
		TodoListData data{
			"todo-123",
			{{"Buy groceries", false, "pending", 1, 0, "high", "shopping", "priority-high"}, {"Finish report", true, "completed", 2, 1, "normal", "work", "priority-normal"}, {"Call dentist", false, "pending", 3, 2, "low", "health", "priority-low"}},
			true,
			3,
			1,
			2};
		...
		expect(result.find("class=\"todo-item pending priority-high\"") != std::string::npos) << "First item classes should be present";
		expect(result.find("class=\"todo-item completed priority-normal\"") != std::string::npos) << "Second item classes should be present";
...
<div class="todo-list" data-component-id="{{component_id}}">
  ...
  {{/has_items}}
		...
		TodoListData data{"htmx-todo", {{"Test HTMX", false, "testing", 1, 0, "normal", "dev", "priority-normal"}}, true, 1, 0, 1};
		...
		expect(result.find("hx-target=\"closest .todo-list\"") != std::string::npos) << "HTMX target attribute should be preserved";
