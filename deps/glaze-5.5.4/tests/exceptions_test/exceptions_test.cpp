...
suite basic_types = [] {
	...
	"bool read invalid"_test = [] { expect(throws([] { [[maybe_unused]] bool val = glz::ex::read_json<bool>("tru"); })); };
};
...
template <> struct glz::meta<times>
{
	using T						= times;
	static constexpr auto value = object("time", glz::custom<&T::read_time, nullptr>, "time1", glz::custom<&T::read_time1, nullptr>);
};
