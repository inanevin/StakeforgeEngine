...
template <> struct glz::meta<sub_thing>
{
	...
	static constexpr auto			  value = object("a",
										 &sub_thing::a, //
										 "b",
										 [](auto&& v) -> auto& { return v.b; } //
	);
};
...
template <> struct glz::meta<sub_thing2>
{
	using T									= sub_thing2;
...
template <> struct glz::meta<enum_array_t>
{
...
template <> struct glz::meta<UnscopedEnum>
{
	static constexpr auto value = enumerate(ENUM_VALUE_0, ENUM_VALUE_1);
};

template <> struct glz::meta<MyArrayStruct>
{
	using T = MyArrayStruct;
	// Mapping enum parsing to a uint8_t array
	static constexpr auto value = object("my_array", [](auto& s) { return std::span{reinterpret_cast<UnscopedEnum*>(s.my_array), MY_ARRAY_MAX}; });
};
...
template <> struct glz::meta<Vehicle>
{
	using enum Vehicle;
...
template <> struct glz::meta<Shapes>
{
	using enum Shapes;
...
template <> struct glz::meta<TestData>
{
	using enum TestData;
	static constexpr auto value = enumerate(None, A, B, C, D, ERROR_E);
};

template <> struct glz::meta<DummyData>
{
	using T									= DummyData;
...
template <> struct glz::meta<Escaped>
{
	...
	using T								   = Escaped;
	static constexpr auto value			   = object(R"(escaped\"key)",
											&T::escaped_key, //
											R"(escaped\"\"key2)",
											&T::escaped_key2,
											R"(escape_chars)",
...
template <std::floating_point T> T generate_uniform()
{
...
template <std::floating_point T> bool equal_within_ulps(T x, T y, size_t n)
{
...
template <std::floating_point T> bool equal(T x, T y)
{
	if (std::abs(x) < (std::numeric_limits<T>::min)())
	{
		// a denormalized value - check that it is close enough to zero.
...
/**/, "a":322})") == glz::error_code::none);
   ...
   "thing_ptr": {
	...
	"complex user obect member names"_test = [] {
		expect(glz::name_v<glz::detail::member_tuple_t<Thing>> == "glz::tuple<sub_thing,std::array<sub_thing2,1>,V3,std::list<int32_t>,std::deque<double>,std::"
...
template <> struct glz::meta<oob>
{
	static constexpr std::string_view name	= "oob";
	static constexpr auto			  value = object("v", &oob::v, "n", &oob::n);
};

template <typename Pair_key, typename Pair_value> struct Read_pair_test_case
{
	Pair_key		 expected_key{};
	Pair_value		 expected_value{};
	std::string_view input_json{};
};
template <typename Pair_key, typename Pair_value> Read_pair_test_case(Pair_key, Pair_value, std::string_view) -> Read_pair_test_case<Pair_key, Pair_value>;
...
template <> struct glz::meta<ThreeODetic>
{
...
template <> struct glz::meta<NineODetic>
{
...
template <> struct glz::meta<Named>
{
...
template <> struct glz::meta<EmptyArray>
{
...
template <> struct glz::meta<EmptyObject>
{
	static constexpr std::string_view name	= "EmptyObject";
	static constexpr auto			  value = object();
};

template <typename Pair_key, typename Pair_value> struct Write_pair_test_case
{
	Pair_key		 key{};
	Pair_value		 value{};
	std::string_view expected_json{};
};
template <typename Pair_key, typename Pair_value> Write_pair_test_case(Pair_key, Pair_value, std::string_view) -> Write_pair_test_case<Pair_key, Pair_value>;
...
suite write_tests = [] {
	...
	"Write map"_test = [] {
		...
		const std::map<std::string, std::optional<double>> nullable{{"a", std::nullopt}, {"b", 13.4}, {"c", std::nullopt}, {"d", 211.2}, {"e", std::nullopt}};
	...
	"Write map-like input range"_test = [] {
		"input range of pairs"_test = [] {
			...
			auto str_view = std::views::iota(-2, 3) | std::views::transform([](const auto i) { return std::pair(i, std::to_string(i * i)); });
		...
		"unsized range of pairs"_test = [] {
			...
			auto str_view = base_rng | std::views::transform([](const auto i) { return std::pair(i, std::to_string(i * i)); });
		...
		"initializer list w/ ranges"_test = [] {
			...
			auto user_ports		 = {std::pair("tcp", std::views::iota(80, 83) | std::views::transform(remap_user_port)), std::pair("udp", std::views::iota(21, 25) | std::views::transform(remap_user_port))};
...
template <> struct glz::meta<study_obj>
{
...
template <> struct glz::meta<put_action>
{
	using T									= put_action;
...
template <> struct glz::meta<delete_action>
{
...
template <> struct glz::meta<tagged_variant2>
{
...
template <> struct glz::meta<holds_some_num>
{
...
template <> struct glz::meta<TaggedObject>
{
...
template <> struct glz::meta<variant_obj>
{
...
template <> struct glz::meta<holder0_t>
{
...
template <> struct glz::meta<holder1_t>
{
...
template <> struct glz::meta<holder2_t>
{
...
template <> struct glz::meta<nested0>
{
...
template <> struct glz::meta<hide_struct>
{
	static constexpr std::string_view name = "hide_struct";
	using T								   = hide_struct;
	static constexpr auto value			   = object("i",
											&T::i, //
											"d",
											&T::d, //
											"hello",
...
template <> struct glz::meta<mem_f_struct>
{
...
template <> struct glz::meta<complex_function_call_t>
{
...
template <> struct glz::meta<string_t>
{
...
template <> struct glz::meta<another_object_t>
{
	...
	static constexpr auto value			   = object("string", &T::string, "another_string", &T::another_string, "boolean", &T::boolean, "nested_object", &T::nested_object);
};
...
template <> struct glz::meta<obj_t>
{
	...
	using T								   = obj_t;
	static constexpr auto value			   = object(
		   "fixed_object", &T::fixed_object, "fixed_name_object", &T::fixed_name_object, "another_object", &T::another_object, "string_array", &T::string_array, "string", &T::string, "number", &T::number, "boolean", &T::boolean, "another_bool", &T::another_bool);
};
...
namespace glz
{
	template <> struct from<JSON, date>
	{
		template <auto Opts> static void op(date& value, auto&&... args)
		{
	...
	template <> struct to<JSON, date>
	{
		template <auto Opts> static void op(date& value, auto&&... args) noexcept
		{
...
namespace glz
{
	...
	struct from<JSON, T>
	{
		template <auto Opts> static void op(date_base& value, auto&&... args)
		{
			parse<JSON>::op<Opts>(value.human_readable, args...);
			value.data = std::stoi(value.human_readable);
		}
	...
	struct to<JSON, T>
	{
		template <auto Opts> static void op(date_base& value, auto&&... args) noexcept
		{
			value.human_readable = std::to_string(value.data);
			serialize<JSON>::op<Opts>(value.human_readable, args...);
		}
...
template <> struct glz::meta<unicode_keys_t>
{
...
template <> struct glz::meta<question_t>
{
	using T						= question_t;
...
template <> struct glz::meta<question_escaped_t>
{
...
template <> struct glz::meta<control_char_escaped_t>
{
...
suite control_character_tests = [] {
	...
	"multiple_control_chars"_test = [] {
		...
		expect(buffer.find("\\u0001\\u0002\\u0003\\u0004\\u0005") != std::string::npos) << "Multiple consecutive control chars should be escaped";
	...
	"all_ascii_control_chars"_test = [] {
		...
		std::string buffer{};
		expect(not glz::write<opts_escape_control_characters{}>(str, buffer)) << "Should successfully write all control characters";
...
template <> struct glz::meta<value_t>
{
	using T = value_t;
...
template <> struct glz::meta<lambda_value_t>
{
...
template <> struct glz::meta<TestMsg>
{
...
template <class T> struct custom_unique
{
	std::unique_ptr<T> x{};
...
template <class T, class... Args> inline auto make_custom_unique(Args&&... args)
{
	return custom_unique{std::make_unique<T>(std::forward<Args>(args)...)};
}

template <class T> struct glz::meta<custom_unique<T>>
{
...
template <> struct glz::meta<flags_t>
{
	using T						= flags_t;
...
template <> struct glz::meta<OKX_OrderBook>
{
...
template <> struct glz::meta<A>
{
	static constexpr auto value = object("x", glz::quoted_num<&A::x>, "y", glz::quoted_num<&A::y>, "z", glz::quoted_num<&A::z>);
};
...
template <> struct glz::meta<map_quoted_num>
{
...
template <> struct glz::meta<bool_map>
{
...
template <> struct glz::meta<array_map>
{
...
template <> struct glz::meta<custom_key_map>
{
	static constexpr auto value = object("x", &custom_key_map::x);
};

template <typename Map> struct Arbitrary_key_test_case
{
...
suite arbitrary_key_maps = [] {
	...
	"arbitrary_key_maps"_test = [] {
		...
		std::tuple tests{Arbitrary_key_test_case<array_map>{.name = "array_map", .input = {.x = {{{1, 2, 3}, "hello"}, {{4, 5, 6}, "goodbye"}}}, .serialized = R"({"x":{"[1,2,3]":"hello","[4,5,6]":"goodbye"}})"},
...
template <> struct glz::meta<nullable_keys>
{
...
suite required_keys = [] {
	...
	"required_keys_long_object"_test = [] {
		...
		OKX_OrderBook order_book{};
		auto		  ec = glz::read<glz::opts{.error_on_unknown_keys = false, .error_on_missing_keys = true}>(order_book, order_book_str);
		expect(ec == glz::error_code::none);
		...
		ec										= glz::read<glz::opts{.error_on_unknown_keys = false, .error_on_missing_keys = true}>(order_book, order_book_str_missing);
...
template <> struct glz::meta<numbers_as_strings>
{
...
template <> struct glz::meta<numbers_as_strings2>
{
...
suite json_logging = [] {
	"json_logging"_test = [] {
		...
		expect(s == R"({"pi":3.141,"happy":true,"name":"Stephen","map":{"aa":1,"b":2,"c":3},"arr":["Hello","World",2],"vec":[1,2,3]})") << s;
	};

	"json_custom_logging"_test = [] {
		std::vector						vec = {1, 2, 3};
		std::map<std::string_view, int> map = {{"a", 1}, {"b", 2}, {"c", 3}};
		auto							obj = glz::obj{"pi", 3.141, "happy", true, "name", "Stephen", "map", map, "vec", vec, "my_struct", my_struct{}};
		...
		expect(s == R"({"pi":3.141,"happy":true,"name":"Stephen","map":{"a":0,"b":2,"c":3},"vec":[1,2,3],"my_struct":{"i":287,"d":3.14,"hello":"Hello World","arr":[1,2,3]}})") << s;
	};
	...
	"merged potentiallyContainsUnknownJSON"_test = [] {
		...
		data.hello = "goodbye";
		std::string buffer{};
		expect(not glz::write_json(glz::merge{glz::obj{"my_arr", my_arr, "data", data}, potentiallyContainsUnknownJSON}, buffer));
...
template <> struct glz::meta<cx_values>
{
	using T = cx_values;
...
template <typename... Ts> struct variant_to_tuple<std::variant<Ts...>>
{
...
suite constexpr_values_test = [] {
	...
	using const_only_variant = std::
		variant<direct_cx_value_conversion_different_value, direct_cx_value_conversion, string_direct_cx_value_conversion, string_two_direct_cx_value_conversion, array_direct_cx_value_conversion, array_two_direct_cx_value_conversion, const_red, const_green>;
...
template <> struct glz::meta<test_enum_struct>
{
...
template <> struct glz::meta<invoke_struct>
{
...
suite enum_map = [] {
	...
	"enum map key vector pair concatenate"_test = [] {
		std::vector<std::pair<Color, std::string>> colors{{Color::Red, "red"}, {Color::Green, "green"}, {Color::Blue, "blue"}};
...
suite obj_nested_merge = [] {
	...
	"obj_json_t_merge"_test = [] {
		glz::json_t json;
		expect(!glz::read_json(json, "{\"key1\":42,\"key2\":\"hello world\",\"v\":[1,2,3],\"m\":{\"a\":1,\"b\":2,\"c\":3}}"));
...
{
	using T						= Person;
	static constexpr auto value = glz::object("name", &T::name, "full_name", &T::name, "age", &T::age, "years_old", &T::age, "date_of_birth", invoke<&T::getAge>, "city", &T::city, "residence", &T::residence);
};
...
template <> struct glz::meta<named_always_null>
{
...
template <> struct glz::meta<custom_encoding>
{
	using T						= custom_encoding;
	static constexpr auto value = object("x",
										 custom<&T::read_x, &T::write_x>, //
										 "y",
										 custom<&T::read_y, &T::y>, //
										 "z",
										 custom<&T::z, &T::write_z>);
};
...
template <> struct glz::meta<custom_buffer_input>
{
...
template <> struct glz::meta<age_custom_error_obj>
{
...
template <> struct glz::meta<class_with_const_mem_func>
{
...
template <> struct glz::meta<constrained_object>
{
	...
	static constexpr auto value = object("age",
										 read_constraint<&T::age, limit_age, "Age out of range">, //
										 "name",
										 read_constraint<&T::name, limit_name, "Name is too long">);
};
...
template <> struct glz::meta<client_state>
{
...
template <> struct glz::meta<manage_x>
{
	using T						= manage_x;
...
template <> struct glz::meta<manage_x_lambda>
{
...
template <> struct glz::meta<manage_test_struct>
{
...
template <typename PARAMS> struct request_t
{
	int					id = -1;
	...
	struct glaze
	{
		...
		static constexpr auto value = glz::object("id", &T::id, "proxy", &T::proxy, "method", &T::method, "params", &T::params);
	};
};
...
template <> struct glz::meta<updater>
{
	using T						= updater;
...
template <> struct glz::meta<unknown_fields_member>
{
...
template <> struct glz::meta<unknown_fields_2>
{
...
template <> struct glz::meta<my_unknown_struct>
{
...
template <> struct glz::meta<unknown_fields_method>
{
...
template <> struct glz::meta<unknown_fields_known_type>
{
...
template <> struct glz::meta<key_reflection>
{
...
template <some_enum E, class DataType> struct wrapper_struct
{
	some_enum type{E};
	DataType  data{};
...
template <> struct glz::meta<unicode_keys>
{
	using T						= unicode_keys;
	static constexpr auto value = object("alpha", &T::field1, "bravo", &T::field2, "charlie", &T::field3, "♥️", &T::field4, "delta", &T::field5, "echo", &T::field6, "😄", &T::field7);
};
...
template <> struct glz::meta<unicode_keys2>
{
...
template <> struct glz::meta<unicode_keys3>
{
	using T						= unicode_keys3;
	static constexpr auto value = object("简体汉字",
										 &T::field0, // simplified chinese characters
										 "漢字寿限無寿限無五劫",
										 &T::field1, // traditional chinese characters / kanji
										 "こんにちはむところやぶら",
										 &T::field2, // katakana
										 "한국인",
										 &T::field3, // korean
...
template <> struct glz::meta<string_tester>
{
	using T						= string_tester;
	static constexpr auto value = object("val1", &T::val1);
};

template <size_t N> std::vector<char> make_vector(const char (&literal)[N])
{
	// include the null
...
suite address_sanitizer_test = [] {
	...
	"invalid json_t 3"_test = [] {
		...
		std::vector<unsigned char> udata{0x22, 0x5c, 0x75, 0x22, 0x65, 0x65, 0x65, 0x65, 0x65, 0x65, 0x65, 0x65, 0x65, 0x65, 0x65, 0x65, 0x65, 0x65, 0x65, 0x65, 0x65, 0x65, 0x65, 0x65, 0x65, 0x65, 0x65, 0x65, 0x65, 0x65, 0x65, 0x65,
										 ...
										 0x65, 0x65, 0x65, 0x65, 0x65, 0x65, 0x65, 0x65, 0x65, 0x65, 0x65, 0x65, 0x65, 0x65, 0x65, 0x65, 0x65, 0x65, 0x65, 0x65, 0x65, 0x65, 0x65, 0x65, 0x65, 0x65, 0x65, 0x65, 0x65, 0x65, 0x4b, 0x00};
...
suite nested_partial_read_tests = [] {
	...
	"nested object partial read, don't read garbage"_test = [] {
		NestedPartialRead n{};
		std::string		  buf = R"({"method":"m1","header":{"id":"51e2affb","type":"message_type","unknown key":"value",garbage},"number":51})";
...
template <> struct glz::meta<AccountUpdateInner>
{
...
template <> struct glz::meta<meta_schema_t>
{
	using T						= meta_schema_t;
	static constexpr auto value = object(&T::x, &T::file_name, &T::is_valid);
};

template <> struct glz::json_schema<meta_schema_t>
{
...
namespace glz
{
	template <> struct from<JSON, custom_struct>
	{
		template <auto Opts> static void op(custom_struct& value, auto&&... args)
		{
	...
	template <> struct to<JSON, custom_struct>
	{
		template <auto Opts> static void op(custom_struct& value, auto&&... args) noexcept
		{
			value.str += "write";
		...
		template <auto Opts> static void op(const custom_struct& value, auto&&... args) noexcept
		{
			serialize<JSON>::op<Opts>(value.str, args...);
		}
	};
...
template <> struct glz::meta<struct_for_volatile>
{
...
template <> struct glz::meta<path_test_struct>
{
...
template <> struct glz::meta<struct_c_arrays_meta>
{
...
template <> struct glz::meta<sum_hash_obj_t>
{
...
template <> struct glz::meta<specify_only_skip_obj>
{
...
template <> struct glz::meta<skip_hidden_elements>
{
	static constexpr bool skip(const std::string_view key, const meta_context&)
	{
...
template <> struct glz::meta<skip_first_and_last>
{
	static constexpr bool skip(const std::string_view key, const meta_context&)
	{
		return key == "i" || key == "l";
...
template <size_t N> struct FixedName
{
...
template <> struct glz::meta<raw_struct>
{
	using T						= raw_struct;
...
template <> struct glz::meta<large_struct_t>
{
	using T						= large_struct_t;
	static constexpr auto value = object(
		&T::a, &T::b, &T::c, &T::d, &T::e, &T::f, &T::g, &T::h, &T::i, &T::j, &T::k, &T::l, &T::m, &T::n, &T::o, &T::p, &T::q, &T::r, &T::s, &T::t, &T::u, &T::v, &T::w, &T::x, &T::y, &T::z, &T::one, &T::two, &T::three, &T::four, &T::five, &T::six, &T::seven);
};
...
suite msvc_ice_tests = [] {
	"WorkshopModConfig"_test = [] {
		...
		auto ec = glz::write<glz::opts{.comments = true, .error_on_unknown_keys = true, .skip_null_members = true, .prettify = false}>(settings, buffer);
		expect(not ec) << glz::format_error(ec, buffer);
	};
};
...
template <> struct glz::meta<custom_errors_t>
{
...
namespace glz
{
	template <> struct from<JSON, custom_errors_t>
	{
		template <auto Opts> static void op(auto&, is_context auto& ctx, auto&&...)
		{
			ctx.custom_error_message = "custom_errors_t read error";
		}
	};

	template <> struct to<JSON, custom_errors_t>
	{
		template <auto Opts> static void op(auto&, is_context auto& ctx, auto&&...) noexcept
		{
...
suite TestSettingsData_test = [] {
	...
	static constexpr opts_allow_conversions read_options{{glz::opts{.comments = true, .error_on_unknown_keys = false, .skip_null_members = true, .error_on_missing_keys = false}}};
...
template <> struct glz::meta<shark_t>
{
...
template <> struct glz::meta<shark_ptr_t>
{
	static constexpr std::string_view tag = "name";
	static constexpr auto			  ids = std::array{"hammerhead", "mako"};
};
...
template <size_t N> struct naive_static_string_t
{
	using value_type = char;
	using size_type	 = size_t;

	naive_static_string_t() = default;
	naive_static_string_t(std::string_view sv)
	{
...
template <size_t N> struct glz::meta<naive_static_string_t<N>>
{
...
template <class T> struct response_t
{
	T&						   result;
	uint32_t				   id{};
...
template <class T> response_t(T) -> response_t<T>;

template <> struct response_t<void>
{
	uint32_t				   id{};
	std::optional<std::string> error{};
};

template <class V> struct glz::meta<response_t<V>>
{
...
template <> struct glz::meta<response_t<void>>
{
...
template <> struct glz::meta<C_empty>
{
...
template <> struct glz::meta<fishes>
{
	using enum fishes;
...
suite ndjson_options = [] {
	"ndjson_options"_test = [] {
		...
		const auto ec = glz::read<opts{{glz::opts{.format = glz::NDJSON, .error_on_unknown_keys = false}}}>(assets, "{\"x\":1}\n{\"x\":2}");
		expect(not ec);
	};
};
...
template <class T> struct custom_nullable_t
{
	std::optional<T> val{};
...
template <> struct glz::meta<A1>
{
	static constexpr auto value = object("p", &A1::p);
};

template <> struct glz::meta<B1>
{
	static constexpr auto value = object("p", &B1::p);
};

template <> struct glz::meta<X1>
{
	static constexpr std::string_view tag = "tag";
};

template <> struct glz::meta<Y1>
{
	static constexpr std::string_view tag = "tag";
};
...
template <> struct glz::meta<birds>
{
	using T = birds;
...
template <> struct glz::meta<append_obj>
{
	using T						= append_obj;
...
template <> struct glz::meta<Number>
{
...
template <> struct glz::meta<Boolean>
{
	static constexpr auto value = glz::object();
};
...
template <> struct glz::meta<Integer>
{
...
template <> struct glz::meta<Array>
{
...
template <> struct glz::meta<Data2>
{
	static constexpr std::string_view tag = "type";
...
template <> struct glz::meta<Array2>
{
...
template <> struct glz::meta<private_fields_t>
{
...
template <auto Member> constexpr auto ImmutableStructInserter = [] { return [](auto&& v) { return ImmutableStructInserter_t{v.*Member}; }; }();

template <> struct glz::meta<ImmutableStructInserter_t>
{
...
template <> struct glz::meta<cast_obj>
{
	using T						= cast_obj;
	static constexpr auto value = object("integer",
										 cast<&T::integer, double>, //
										 "indirect",
										 cast<[](T& s) -> auto& { return s.integer; }, double>);
};
...
template <> struct glz::meta<CommandVariant>
{
...
template <> struct glz::meta<versioned_data_t>
{
...
suite glaze_error_category_tests = [] {
	...
	"error_code_message_consistency"_test = [] {
		...
		for (auto err : {none, parse_error, expected_brace, expected_bracket, expected_quote, syntax_error, unknown_key, missing_key, constraint_violated})
		{
