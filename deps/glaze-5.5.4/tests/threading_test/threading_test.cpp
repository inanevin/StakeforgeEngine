...
suite async_vector_tests = [] {
	...
	"iterators"_test = [] {
		...
		expect(vec.read()[0] == 0 && vec.read()[1] == 2 && vec.read()[2] == 4 && vec.read()[3] == 6 && vec.read()[4] == 8) << "Elements should be modifiable through iterators";
	...
	"complex_types"_test = [] {
		...
		expect(vec_copy.read()[0].id == 1 && vec_copy.read()[0].name == "one") << "Copied vector should not be affected by changes to original";
	};
...
struct ComplexObject
{
	...
	ComplexObject(int id, std::string data, std::vector<double> values) : id(id), data(std::move(data)), values(std::move(values)), ptr(std::make_unique<int>(id))
	{
	}
	...
	// Copy constructor
	ComplexObject(const ComplexObject& other) : id(other.id), data(other.data), values(other.values), ptr(other.ptr ? std::make_unique<int>(*other.ptr) : nullptr), access_count(other.access_count.load())
	{
	}

	// Move constructor
	ComplexObject(ComplexObject&& other) noexcept : id(other.id), data(std::move(other.data)), values(std::move(other.values)), ptr(std::move(other.ptr)), was_moved(true), access_count(other.access_count.load())
	{
	}
	...
	{
		return id == other.id && data == other.data && values == other.values && ((!ptr && !other.ptr) || (ptr && other.ptr && *ptr == *other.ptr));
	}
};
