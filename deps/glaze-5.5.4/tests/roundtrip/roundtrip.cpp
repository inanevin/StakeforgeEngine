...
template <auto Opts, class T, class Buffer> decltype(auto) read(T&& value, Buffer&& buffer)
{
	return glz::read<Opts>(std::forward<T>(value), std::forward<Buffer>(buffer));
}

template <auto Opts, class T, class Buffer> decltype(auto) write(T&& value, Buffer&& buffer)
{
...
template <auto Opts, class T> void roundtrip(T& v)
{
	std::string buffer{};
