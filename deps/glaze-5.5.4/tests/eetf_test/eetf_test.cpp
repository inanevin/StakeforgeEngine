...
std::array<std::uint8_t, 81> term_map_001{131, 116, 0,	0,	0,	 5,	 100, 0, 1, 97,	 100, 0,   9,	97,	 116, 111, 109, 95, 116, 101, 114, 109, 100, 0,	 3,	  97,  114, 114, 107, 0,  3,  9,   8,	7,	 100, 0, 1, 100, 70, 64, 9,
										  33,  251, 77, 18, 216, 74, 100, 0, 5, 104, 101, 108, 108, 111, 107, 0,   17,	72, 101, 108, 108, 111, 32,	 69, 114, 108, 97,	110, 103, 32, 84, 101, 114, 109, 100, 0, 1, 105, 97, 1};

std::array<std::uint8_t, 92> term_proplist_001{131, 108, 0,	 0,	 0,	  5,  104, 2, 100, 0, 1, 97,  100, 0,	9,	 97,  116, 111, 109, 95, 116, 101, 114, 109, 104, 2,  100, 0,	3,	97,	 114, 114, 107, 0,	 3,	  9,   8,	7, 104, 2, 100, 0,	 1,	 100, 70, 64, 9,
											   33,	251, 77, 18, 216, 74, 104, 2, 100, 0, 5, 104, 101, 108, 108, 111, 107, 0,	17,	 72, 101, 108, 108, 111, 32,  69, 114, 108, 97, 110, 103, 32,  84,	101, 114, 109, 104, 2, 100, 0, 1,	105, 97, 1,	  106};
...
struct my_struct_meta
{
	...
	my_struct_meta(int i, double d, std::string s, std::vector<uint64_t> v) : val_i{i}, val_d{d}, val_str{s}, val_arr{std::move(v)}
	{
	}
...
template <> struct glz::meta<my_struct_meta>
{
...
template <> struct glz::meta<atom_rw>
{
	using T						= atom_rw;
...
suite etf_tests = [] {
	...
	"read_proplist_term_meta"_test = [] {
		...
		my_struct_meta s{};
		auto		   ec = glz::read<glz::eetf::eetf_opts{.format = glz::EETF, .layout = glz::eetf::proplist_layout, .error_on_unknown_keys = false}>(s, term_proplist_001);
		expect(not ec) << glz::format_error(ec, "can't read");
		expect(s.val_d == 3.1415926);
		expect(s.val_i == 1);
		expect(s.val_arr == decltype(s.val_arr){9, 8, 7});
