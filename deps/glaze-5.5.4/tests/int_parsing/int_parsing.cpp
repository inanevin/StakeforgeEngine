...
template <class T> bool test_samples()
{
...
template <class T> bool test_to_max()
{
	std::string buffer{};
	bool		valid = true;
...
template <class T> bool test_performance()
{
#ifdef NDEBUG
...
template <class T> struct vector_object
{
	std::vector<T> vec{};
};

template <class T> void test_struct_with_array_minified()
{
	std::mt19937					 gen{};
	std::uniform_int_distribution<T> dist{std::numeric_limits<T>::lowest(), (std::numeric_limits<T>::max)()};
...
template <class T> bool test_single_char_performance()
{
#ifdef NDEBUG
	constexpr size_t n = 10000000;
#else
...
template <class T> bool test_lengths()
{
