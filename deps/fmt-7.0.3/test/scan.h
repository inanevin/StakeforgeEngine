...
namespace detail
{
	...
	private:
		template <typename T> static void scan_custom_arg(void* arg, scan_parse_context& parse_ctx, scan_context& ctx)
		{
			scanner<T> s;
...
struct scan_args
{
	...
	template <size_t N> scan_args(const std::array<detail::scan_arg, N>& store) : size(N), data(store.data())
	{
...
namespace detail
{
	...
	public:
		scan_handler(string_view format, string_view input, scan_args args) : parse_ctx_(format), scan_ctx_(input), args_(args), next_arg_id_(0)
		{
		}
		...
		void on_text(const char* begin, const char* end)
		{
			...
			if (it + size > scan_ctx_.end() || !std::equal(begin, end, make_checked(it, size))) { on_error("invalid input"); }
		...
		int on_arg_id(string_view)
		{
			return on_error("invalid format"), 0;
		}
...
template <typename... Args> std::array<detail::scan_arg, sizeof...(Args)> make_scan_args(Args&... args)
{
	return {{args...}};
}
...
template <typename... Args> string_view::iterator scan(string_view input, string_view format_str, Args&... args)
{
