...
#define EXPECT_THROW_MSG(statement, expected_exception, expected_message) FMT_TEST_THROW_(statement, expected_exception, expected_message, GTEST_NONFATAL_FAILURE_)

std::string format_system_error(int error_code, fmt::string_view message);

#define EXPECT_SYSTEM_ERROR(statement, error_code, message) EXPECT_THROW_MSG(statement, fmt::system_error, format_system_error(error_code, message))
...
#define EXPECT_WRITE(file, statement, expected_output) FMT_TEST_WRITE_(statement, expected_output, file, GTEST_NONFATAL_FAILURE_)
...
public:
	SuppressAssert() : original_handler_(_set_invalid_parameter_handler(handle_invalid_parameter)), original_report_mode_(_CrtSetReportMode(_CRT_ASSERT, 0))
	{
	}
	~SuppressAssert()
	{
...
#define EXPECT_SYSTEM_ERROR_NOASSERT(statement, error_code, message) EXPECT_SYSTEM_ERROR(SUPPRESS_ASSERT(statement), error_code, message)

// Attempts to read count characters from a file.
std::string read(fmt::file& f, size_t count);

#define EXPECT_READ(file, expected_content) EXPECT_EQ(expected_content, read(file, fmt::string_view(expected_content).size()))
