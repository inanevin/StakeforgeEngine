...
template <typename matType> static void test_mat_transpose(std::vector<matType> const& I, std::vector<matType>& O)
{
	for (std::size_t i = 0, n = I.size(); i < n; ++i)
		O[i] = glm::transpose(I[i]);
}

template <typename matType> static int launch_mat_transpose(std::vector<matType>& O, matType const& Scale, std::size_t Samples)
{
...
template <typename packedMatType, typename alignedMatType> static int comp_mat2_transpose(std::size_t Samples)
{
...
template <typename packedMatType, typename alignedMatType> static int comp_mat3_transpose(std::size_t Samples)
{
	typedef typename packedMatType::value_type T;
...
template <typename packedMatType, typename alignedMatType> static int comp_mat4_transpose(std::size_t Samples)
{
	typedef typename packedMatType::value_type T;
