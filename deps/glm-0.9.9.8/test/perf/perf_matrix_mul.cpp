...
template <typename matType> static void test_mat_mul_mat(matType const& M, std::vector<matType> const& I, std::vector<matType>& O)
{
	for (std::size_t i = 0, n = I.size(); i < n; ++i)
		O[i] = M * I[i];
}

template <typename matType> static int launch_mat_mul_mat(std::vector<matType>& O, matType const& Transform, matType const& Scale, std::size_t Samples)
{
...
template <typename packedMatType, typename alignedMatType> static int comp_mat2_mul_mat2(std::size_t Samples)
{
...
template <typename packedMatType, typename alignedMatType> static int comp_mat3_mul_mat3(std::size_t Samples)
{
	typedef typename packedMatType::value_type T;
...
template <typename packedMatType, typename alignedMatType> static int comp_mat4_mul_mat4(std::size_t Samples)
{
	typedef typename packedMatType::value_type T;
