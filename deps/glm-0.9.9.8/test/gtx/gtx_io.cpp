...
namespace
{
	template <typename CTy, typename CTr> std::basic_ostream<CTy, CTr>& operator<<(std::basic_ostream<CTy, CTr>& os, glm::qualifier const& a)
	{
	...
	template <typename U, glm::qualifier P, typename T, typename CTy, typename CTr> std::basic_string<CTy> type_name(std::basic_ostream<CTy, CTr>&, T const&)
	{
...
template <typename T, glm::qualifier P, typename OS> int test_io_quat(OS& os)
{
	os << '\n' << typeid(OS).name() << '\n';
...
template <typename T, glm::qualifier P, typename OS> int test_io_vec(OS& os)
{
	os << '\n' << typeid(OS).name() << '\n';
...
template <typename T, glm::qualifier P, typename OS> int test_io_mat(OS& os, glm::io::order_type otype)
{
	os << '\n' << typeid(OS).name() << '\n';
	...
	os << glm::io::precision(2) << glm::io::width(1 + 2 + 1 + 2) << glm::io::order(otype) << "mat2x2<" << typeid(T).name() << ',' << P << ">: " << glm::mat<2, 2, T, P>(v2_1, v2_2) << '\n'
