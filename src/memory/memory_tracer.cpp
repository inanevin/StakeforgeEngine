...
namespace Game
{
	...
	void memory_tracer::on_allocation(void* ptr, size_t sz)
	{
		...
		if (_categories.empty()) return;

		memory_category& cat   = _categories.back();
	...
	void memory_tracer::on_free(void* ptr)
	{
		...
		for (memory_category& cat : _categories)
		{
	...
	void memory_tracer::push_category(const char* name)
	{
		...
		if (cat.name) GAME_MEMCPY((void*)cat.name, (void*)name, sz);
	...
	void memory_tracer::pop_category()
	{
		...
		if (!cat.allocations.empty())
		{
			for (auto& [ptr, track] : cat.allocations)
				_leaks[ptr] = track;
		}

		_categories.pop_back();
	}
	...
	void memory_tracer::check_leaks()
	{
		...
		for (auto& [ptr, alloc] : _leaks)
		{
			...
			for (int i = 0; i < alloc.stack_size; ++i)
			{
				...
				if (::SymGetModuleInfo64(process, symbol->ModBase, &moduleInfo)) ss << "Module:" << moduleInfo.ModuleName << "\n";
			}
