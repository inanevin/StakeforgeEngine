...
namespace Game
{
	...
	void scene_context::add_gltf_from_file(const char* file)
	{
		...
		{
			SFG_ERR("scene_context::add_gltf_from_file() -> file not found! {0}", file);
			return;
		}

		const string str = file_system::read_file_as_string(file);
		SFG_PROG("scene_context::add_gtlf_from_file() -> Loading gltf file: {0}", file);
		...
		if (pe)
		{
			...
			SFG_ERR("scene_context::add_gtlf_from_file() -> Failed reading gltf json! {0}", err);
			return;
		}
	}
}